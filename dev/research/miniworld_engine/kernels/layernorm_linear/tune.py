...
Run on a GPU compute node via srun (NEVER the login node):
    ...
    pixi run --frozen bash -c \
      "export LD_LIBRARY_PATH=\$CONDA_PREFIX/lib:\$LD_LIBRARY_PATH QUACK_CACHE_ENABLED=0; \
       python -m miniworld_engine.kernels.layernorm_linear.tune"
"""
...
from miniworld_engine.kernels.layernorm_linear.cute import fold_for_gemm, layernorm_linear_cute
from miniworld_engine.kernels.layernorm_linear.cute.gemm_layernorm_linear_fused import (
    layernorm_linear_cute_fused,
)
from miniworld_engine.kernels.layernorm_linear.reference import LayerNormLinearRef
