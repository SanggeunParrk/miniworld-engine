...
if __name__ == "__main__":
    ...
    mm = len(sys.argv) > 3 and sys.argv[3] == "mm"
    if mm:
        out_nm = gate_gemm(A, Bp, Bg, mmajor=True).float()  # (N, M)
        ref_nm = ref.t().contiguous()  # (N, M)
        err = (out_nm - ref_nm).abs()
        cos = torch.nn.functional.cosine_similarity(out_nm.flatten(), ref_nm.flatten(), dim=0).item()
        print(f"[M-major] M={M} N={N} K={K}: maxabs={err.max().item():.3e} meanabs={err.mean().item():.3e} cos={cos:.6f}", flush=True)
    else:
