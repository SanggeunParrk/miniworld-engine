...
if __name__ == "__main__":
    ...
    Ws = [torch.randn(N, K, device="cuda", dtype=torch.bfloat16) * 0.3 for _ in range(4)]
    print(f"PRE M={M} N={N} K={K}", flush=True)
    WLp, WLg, WRp, WRg = Ws
    lref = (torch.sigmoid(A.float() @ WLg.float().T) * (A.float() @ WLp.float().T)).t().contiguous()
    rref = (torch.sigmoid(A.float() @ WRg.float().T) * (A.float() @ WRp.float().T)).t().contiguous()
    Cl, Cr = fused_gate_gemm(A, WLp, WLg, WRp, WRg)
    for nm, out, ref in (("left", Cl.float(), lref), ("right", Cr.float(), rref)):
        cos = torch.nn.functional.cosine_similarity(out.flatten(), ref.flatten(), dim=0).item()
        print(f"  {nm}: maxabs={(out-ref).abs().max().item():.3e} cos={cos:.6f}", flush=True)
