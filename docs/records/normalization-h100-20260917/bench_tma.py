...
for bwd in (True,False):
 ...
 configs=itertools.product((32,64,128),(4,8,16),(1,2,3)) if not a.quick else [(64,4,1),(64,8,2),(128,8,2)]
 for bm,nw,ns in configs:
  ...
  rows.append(record);report['results'][label]={'baseline_ms':base,'rows':rows};(root/f'tma-L{a.length}.json').write_text(json.dumps(report,indent=2)+'\n');print('RESULT',label,json.dumps(record),flush=True)
 del tg,ref
