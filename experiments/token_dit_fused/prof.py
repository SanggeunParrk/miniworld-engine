...
p = argparse.ArgumentParser(); p.add_argument("--length", type=int, default=384); p.add_argument("--blocks", type=int, default=4); p.add_argument("--dtype", choices=("bf16", "fp32"), default="bf16")
a = p.parse_args(); L, S, NB, dev = a.length, 5, a.blocks, "cuda"
bf = torch.float32 if a.dtype == "fp32" else torch.bfloat16
torch.backends.cuda.matmul.allow_tf32 = a.dtype == "fp32"
torch.manual_seed(0)
...
print(f"{a.dtype} L={L} S={S}: {tot:.1f} us per block (kernel time), launches per step {sum(v[1] for v in per.values())}")
