...
#ifndef DYPRE
#define DYPRE 0                                           // 1: dy prefetched into registers at the tile start, like x
#endif
#ifndef ABL_NOEPI
#define ABL_NOEPI 0
#endif
...
#endif
#if DYPRE
    uint32_t dyr0[NG], dyr1[NG];
#pragma unroll
    for (int g = 0; g < NG; ++g) {
      const int c = cbase + 8 * g + 2 * q;
      dyr0[g] = ldg32(dy + (size_t)r0 * D + c); dyr1[g] = ldg32(dy + (size_t)r1 * D + c);
    }
#endif
