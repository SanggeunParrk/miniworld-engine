...
#ifndef EPI_DIRECT
#define EPI_DIRECT 0                                      // 1: h / dA / dB stored straight from the fragments (no staging smem)
#endif
constexpr int F_STG = NSTAGE * F_STAGE, F_STGW = EPI_DIRECT ? 0 : 3 * CPP * 8192;   // per consumer WG: {h, dA, dB} x CPP blocks of [64][64]
...
gate_gemm(const __grid_constant__ CUtensorMap mXN, const __grid_constant__ CUtensorMap mDY, const __grid_constant__ CUtensorMap mW1,
          ...
          int M, int K, int H, __nv_bfloat16* __restrict__ hout, __nv_bfloat16* __restrict__ dabout) {
...
#if EPI_DIRECT
    {
      // C fragment: group g (8 columns), row r0 = 16 warp + lane / 4 (+ 8 for rb = 1), columns 8 g + 2 (lane % 4) + {0, 1}
      const int r0 = m0 + 64 * cw + 16 * warp + (lane >> 2), cc = n * THB + 2 * (lane & 3);
      __nv_bfloat16* hrow = hout + (size_t)r0 * H + cc;
      __nv_bfloat16* arow = dabout + (size_t)r0 * (2 * H) + cc;
#pragma unroll
      for (int g = 0; g < 16; ++g) {
#pragma unroll
        for (int rb = 0; rb < 2; ++rb) {
          const int ia = 4 * g + 2 * rb, ib = 4 * (g + 16) + 2 * rb, id = 4 * g + 2 * rb;
          ...
          const size_t ro = (size_t)(8 * rb);
          stg32(hrow + ro * H + 8 * g, pack_bf16(l0 * b0, l1 * b1));
          stg32(arow + ro * 2 * H + 8 * g, pack_bf16((g0 * b0) * (s0 + l0 * (1.f - s0)), (g1 * b1) * (s1 + l1 * (1.f - s1))));
          stg32(arow + ro * 2 * H + H + 8 * g, pack_bf16(g0 * l0, g1 * l1));
        }
      }
    }
#else
