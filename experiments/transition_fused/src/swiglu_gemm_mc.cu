// swiglu_gemm_mc.cu -- swiglu_gemm on a 2-CTA cluster: the two CTAs take adjacent 128-row M tiles of the same N tile and
// each loads HALF of every weight slab with TMA multicast into both, halving the per-SM weight traffic from L2.
...
TMN_DEVI void mbar_arrive_peer(uint64_t* bar, uint32_t peer) {       // arrive on the barrier at the same offset in CTA `peer`
...
swiglu_gemm(const __grid_constant__ CUtensorMap mA, const __grid_constant__ CUtensorMap mB,
  ...
  const int ptiles = (M / (2 * TBM)) * NT, cid = blockIdx.x >> 1, ncl = gridDim.x >> 1;   // tile pairs: M rows 256 x one N tile
  if (tid == 0) {
    for (int s = 0; s < NSTAGE; ++s) { mbar_init(full + s, 1); mbar_init(empty + s, 4); }   // empty: 2 warpgroups x 2 CTAs
    fence_barrier_init();
  }
  __syncthreads();
  cluster_sync_all();                                     // the peer's barriers exist before any multicast or remote arrive
  if (wg == 0) {                                          // ------------------------------------------------ producer
    setmaxnreg_dec<40>();
    if (tid == 0) {
      uint32_t it = 0;
      for (int p = cid; p < ptiles; p += ncl) {
        const int m0 = (2 * (p / NT) + (int)rank) * TBM, r0 = (p % NT) * TBN;
        for (int kb = 0; kb < KB; ++kb, ++it) {
          const int s = (int)(it % NSTAGE);
          if (it >= NSTAGE) mbar_wait(empty + s, ((it / NSTAGE) - 1) & 1u);
          uint8_t* st = sm + s * F_STAGE;
          mbar_arrive_expect_tx(full + s, F_STAGE);        // own A (16 KB) + both halves of B (16 KB each, one from the peer)
          tma_load_2d(st, &mA, full + s, kb * TBK, m0);
          tma_load_2d(st + 8192, &mA, full + s, kb * TBK, m0 + 64);
          tma_load_2d_mc(st + F_A + rank * 16384, &mB, full + s, kb * TBK, r0 + 128 * (int)rank, (uint16_t)0x3);
        }
      }
    }
  } else {
  ...
  for (int p = cid; p < ptiles; p += ncl) {
    const int m0 = (2 * (p / NT) + (int)rank) * TBM, h0 = (p % NT) * THB;
    float acc[128];
#pragma unroll
...
#pragma unroll
      ...
      wgmma_commit();
      if (prev >= 0) { wgmma_wait<1>(); fence_regs(acc); if (wtid == 0) { mbar_arrive(empty + prev); mbar_arrive_peer(empty + prev, peer); } }
      prev = s;
      if (STAGGER > 0 && cw == 0 && it == (uint32_t)(STAGGER - 1)) named_bar_arrive(BAR_GO, 256);
    }
    wgmma_wait<0>(); fence_regs(acc);
    if (wtid == 0) { mbar_arrive(empty + prev); mbar_arrive_peer(empty + prev, peer); }
...
#pragma unroll
  ...
  __syncwarp();
  cluster_sync_all();                                     // neither CTA leaves while the other may still write to it
}
