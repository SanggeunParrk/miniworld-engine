...
//
// p11 = p10 with wait<1> every chunk (G1(j) and G2(j-1) retire together) and the ring as [Wa;Wb] x 3 + Ws^T x 2.
...
constexpr int NAB = 3, F_ABS = 32768, F_AB = 0;                // [Wa_j; Wb_j] packed, [128 n][128 d]
constexpr int NWS = 2, F_WSS = 16384, F_WS = F_AB + NAB * F_ABS;   // Ws^T_j, [64 hs][128 d]
...
#pragma unroll
    ...
    // Commit order per tile: G1(0) | G1(1) G2(0) | G1(2) G2(1) | ... | G1(7) G2(6) | G2(7).  Chunk j waits until only G1(j+1)
    // is in flight (wait<1>; wait<0> at j = 7): that retires G1(j) AND G2(j-1), so the [Wa;Wb] slot of j and the Ws^T slot of
    // j-1 are released one chunk earlier than a wait<2> schedule would, and each refill gets about a chunk of slack.  The
    // tensor pipe still has G1(j+1) queued for the whole of SwiGLU(j), which is shorter than it.
    float acc[64];
#pragma unroll
    ...
    auto chunk = [&](float (&ABc)[64], float (&ABn)[64], int j) __attribute__((always_inline)) {
      ...
      if (j + 1 < NCH) wgmma_wait<1>(); else wgmma_wait<0>();
      fence_regs(ABc); fence_regs(acc);
      if (wtid == 0) {
        mbar_arrive(ab_free + (int)(c % NAB));           // G1(c) has retired: its [Wa;Wb] slot may be refilled
        if (c >= 1) mbar_arrive(ws_free + (int)((c - 1) % NWS));   // so has G2(c-1): its Ws^T slot too
      }
...
#pragma unroll
      ...
      if (tid == 0) {                                     // refills: both warpgroups had this chunk's SwiGLU to reach their release
        ...
        if (c >= 1 && c + 1 < wmax) { mbar_wait(ws_free + (int)((c - 1) % NWS), ((c - 1) / NWS) & 1u); issue_ws(c + 1); }
      }
