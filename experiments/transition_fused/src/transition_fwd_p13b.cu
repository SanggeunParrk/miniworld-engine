...
//
// p13b: p13 with the refills back at the base kernel's single point per chunk (after wait<1>), refilling seq w-1's two slots
// with w-1+NAB and w-1+NWS.  NAB = NWS = 2 is then the base kernel's timing; NAB = 3 adds one chunk of slack to [Wa;Wb] only.
...
#pragma unroll
      ...
      if (wseq > 0) {
        const uint32_t pw = wseq - 1;
        if (wtid == 0) mbar_arrive(ws_free + (int)(pw % NWS));
        if (tid == 0) {                                   // ONE refill point per chunk, where the base kernel has it: seq pw's two slots
          if (pw + NAB < wmax) { mbar_wait(ab_free + (int)(pw % NAB), (pw / NAB) & 1u); issue_ab(pw + NAB); }
          if (pw + NWS < wmax) { mbar_wait(ws_free + (int)(pw % NWS), (pw / NWS) & 1u); issue_ws(pw + NWS); }
        }
      }
