// p15 = the base kernel with the output store waiting for .read instead of global completion.
...
#pragma unroll
      ...
      tma_store_commit();
      tma_store_wait_read<0>();                            // p15: the READ of the staged tile is all the refill needs; waiting for
                                                           // global completion held warp 0 -- and through the next LayerNorm's barrier, the whole warpgroup                                // the store has read the tile: the buffer may be refilled
