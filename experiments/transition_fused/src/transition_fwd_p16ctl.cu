// p16ctl = the base kernel launched with 320 threads (two idle warps): isolates p16's register-cap / warp-count effect.
...
#pragma unroll
  ...
  if (tid >= 256) return;                               // p16ctl: two idle warps, the register cap of a 320-thread block, nothing else
