// p17b: sigmoid with ex2 on the MUFU and the reciprocal as 3 Newton steps on the FMA pipe.
...
// p17b: ex2 stays on the MUFU; the reciprocal moves to the FMA pipe (8x wider): a bit-trick seed and three Newton steps,
// r <- r (2 - d r), which converge to ~fp32 accuracy (the seed carries ~4 bits; each step doubles them).  d = 1 + 2^(-a log2e)
// lies in [1, inf); for d >= 2^126 the seed is 0 and so is the result, which is the sigmoid's limit.
TMN_DEVI float sigmoid_fast(float a) {
  const float d = __fadd_rn(1.f, ex2f(__fmul_rn(-1.4426950408889634f, a)));
  float r = __int_as_float(0x7EF311C7 - __float_as_int(d));
  r = r * fmaf(-d, r, 2.f); r = r * fmaf(-d, r, 2.f); r = r * fmaf(-d, r, 2.f);
  return r;
}
...
#pragma unroll
        for (int r = 0; r < 2; ++r) {
          ...
          fh[ks][o + r] = pack_bf16(a0 * sigmoid_fast(a0) * b0, a1 * sigmoid_fast(a1) * b1);
        }
      }
