// transition_fwd_p5.cu DIAGNOSTIC (wrong results): the weight ring always fetches chunk 0, so the stream is one 48 KB working set
...
transition_fwd_fused(const __grid_constant__ CUtensorMap mx, const __grid_constant__ CUtensorMap mwa,
  ...
  auto issue_w = [&](uint32_t seq) {                      // chunk seq % NCH into slot seq & 1
    const int s = seq & 1, j = 0; uint8_t* b = sm + F_RING + s * F_SLOT;   // p5: always chunk 0
