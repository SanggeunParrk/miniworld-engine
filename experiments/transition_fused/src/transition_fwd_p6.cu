// transition_fwd_p6.cu DIAGNOSTIC (wrong results): no weight TMA at all
...
transition_fwd_fused(const __grid_constant__ CUtensorMap mx, const __grid_constant__ CUtensorMap mwa,
  ...
  auto issue_w = [&](uint32_t seq) {                      // chunk seq % NCH into slot seq & 1
    ...
    mbar_arrive_expect_tx(w_full + s, 8192);   // p6: one box instead of six
    tma_load_2d(b, p.wa, w_full + s, 0, 0);
  };
  auto issue_x = [&](int i) {
