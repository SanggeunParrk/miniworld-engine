...
if __name__=='__main__':
 ap=argparse.ArgumentParser();ap.add_argument('--source',default='front_prefetch_lnpair_storepipe');ap.add_argument('--lengths',type=int,nargs='+',default=[64,384,768]);ap.add_argument('--counts',type=int,nargs='+',default=[264]);ap.add_argument('--parts',type=int,nargs='+',default=[2]);ap.add_argument('--dropouts',type=float,nargs='+',default=[0,.25]);ap.add_argument('--splits',type=int,default=13);ap.add_argument('--output',required=True);args=ap.parse_args();records=[]
 with torch.no_grad():
