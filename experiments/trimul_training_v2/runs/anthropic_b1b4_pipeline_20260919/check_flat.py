...
with torch.no_grad():
 for n in (64,384,768):
  ...
  for part in (1,):
   p=Unified(d,dy,s,min(66,n*n//64),part);o=p();torch.cuda.synchronize();er=[rel(x,y) for x,y in zip(o,ref)];print('CHECK',n,part,er,flush=True);assert max(er)<.01,er;plans[str(part)]=p
