...
with torch.no_grad():
 for n in (64,72,384,768):
  ...
  cfgs=[(64,64,1,0,1,w,0,0,2,f,ss) for w,f,ss in ((2,0,3),)];plans={}
  for cfg in cfgs:
  ...
  if n>=384:
   tt=paired({k:graph(p) for k,p in plans.items()},reps=80,rounds=10);print('RESULT',n,{k:round(v['median_us'],2) for k,v in tt.items()},flush=True);(R/f'ws-{n}.json').write_text(json.dumps(tt,indent=2))
