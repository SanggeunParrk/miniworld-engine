...
#pragma unroll
 ...
 static_for<4>([&](auto ni){constexpr int n=decltype(ni)::value;
  float acc[32]={};uint64_t de=smem_desc(smem_u32(sw+n*16384),16,1024,1);uint32_t lo[1]={(uint32_t)de},hi[1]={(uint32_t)(de>>32)};
  fence_regs(acc);wgmma_fence();mma_chain<2,8,1>(acc,a,lo,hi);wgmma_commit();wgmma_wait<0>();fence_regs(acc);
#pragma unroll
  for(int j=0;j<16;++j)dn[n*4+j/4][j%4]=pack_bf16(acc[j*2],acc[j*2+1]);
 });
 ...
 static_for<16>([&](auto ki){constexpr int k=decltype(ki)::value;uint32_t fx[4];
  ...
  float gd[4]={},bd[4]={};
#pragma unroll
  for(int q=0;q<4;++q){int rr=q&1,cc=k*16+2*(lane%4)+8*(q>>1);float da=bf16lo(dn[k][q]),db=bf16hi(dn[k][q]);
...
#pragma unroll
 ...
 static_for<16>([&](auto ki){constexpr int k=decltype(ki)::value;uint32_t fx[4],fo[4];
  ldsm_x4_t(fx,smem_u32(sx)+swz128(k*16+r8+8*(mat>>1),(w*16+8*(mat&1))*2));
#pragma unroll
  for(int q=0;q<4;++q){int rr=q&1,cc=k*16+2*(lane%4)+8*(q>>1);float xa=__fmul_rn(__fsub_rn(bf16lo(fx[q]),mu[rr]),rs[rr]),xb=__fmul_rn(__fsub_rn(bf16hi(fx[q]),mu[rr]),rs[rr]);fo[q]=pack_bf16(rs[rr]*((bf16lo(dn[k][q])*p.gamma[cc]-s2[rr])-xa*s1[rr]),rs[rr]*((bf16hi(dn[k][q])*p.gamma[cc+1]-s2[rr])-xb*s1[rr]));}
