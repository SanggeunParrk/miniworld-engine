// One variable on bankfree: planar LN row statistics.
...
#pragma unroll
 ...
 if(lane%4==0){stats[wi*128+ra]=s1[0];stats[wi*128+64+ra]=s2[0];stats[wi*128+rb]=s1[1];stats[wi*128+64+rb]=s2[1];}allsync();
...
#pragma unroll
  for(int k=0;k<4;++k){int r=2*(4*(lane%8)+(k^(c&3)));uint32_t x=pair_get(sx,c,r),dn=pair_get(sn,c,r);float mua=mus[r],mub=mus[r+1],rsa=rss[r],rsb=rss[r+1];float xa=__fmul_rn(__fsub_rn(bf16lo(x),mua),rsa),xb=__fmul_rn(__fsub_rn(bf16hi(x),mub),rsb),da=bf16lo(dn),dd=bf16hi(dn);dg+=da*xa+dd*xb;db+=da+dd;pair_put(sx,c,r,pack_bf16(rsa*((da*gamma-(stats[64+r]+stats[192+r]))-xa*(stats[r]+stats[128+r])),rsb*((dd*gamma-(stats[64+r+1]+stats[192+r+1]))-xb*(stats[r+1]+stats[128+r+1]))));}
#pragma unroll
