// One additional redundant-barrier removal from preceding variant.
...
#pragma unroll
  ...
  // dnorm stores are consumed only after the CTA row-stat barrier below.
 }
...
#pragma unroll
 ...
 // Every writer fences its own stores, then the WG joins before TMA issue.
 fence_proxy_async();sync_group();if(tid==0){for(int c=wi*128;c<(wi+1)*128;c+=16)tma_store_3d(&p.dtri,sx+c*128,m0,c,0);tma_store_commit();tma_store_wait_all();}
 // Caller CTA barrier joins the TMA issuer and all shared-buffer writers.
}
...
#pragma unroll
 ...
 // Initialized mbarrier parity wait itself acquires TMA completion.
 mbar_wait(bar,phase);int j0=row%p.L;
