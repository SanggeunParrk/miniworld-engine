...
extern "C" __global__ __launch_bounds__(256,1) void dual_b1b4(__grid_constant__ const Params p){
 ...
 float acc[3][64]={};
 if(threadIdx.x==0){for(int i=1;i<11;++i)clocks[i]=0;clocks[0]=detail_now();clocks[11]=clocks[0];}
 for(int it=first;it<end;++it){dual_load(p,sm,&bar,it*64,phase,it==first);detail_mark(clocks,1);phase^=1;
