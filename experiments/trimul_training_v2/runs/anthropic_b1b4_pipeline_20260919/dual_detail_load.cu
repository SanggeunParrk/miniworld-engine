...
// Diagnostic only: thread zero timestamps successive phases with a common ns clock.
TMN_DEVI unsigned long long detail_now(){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t)::"memory");return t;}
TMN_DEVI void detail_mark(unsigned long long* ts,int i){if(threadIdx.x==0){auto now=detail_now();ts[i]+=now-ts[0];ts[0]=now;}}
// LN row reductions in the GEMM register layout; channel-contiguous epilogue.
TMN_DEVI void dual_dgrad(const Params& p,uint8_t* sm,uint64_t* bar,int m0,bool next,unsigned long long* clocks){
 ...
 if(threadIdx.x<64){mus[tid]=p.mean[m0+tid];rss[tid]=p.rs[m0+tid];}gam[threadIdx.x]=p.gamma[threadIdx.x];allsync();detail_mark(clocks,3);
...
TMN_DEVI void dual_load(const Params& p,uint8_t* sm,uint64_t* bar,int row,int phase,bool first,unsigned long long* clocks){
...
#pragma unroll
 ...
 allsync();mbar_wait(bar,phase);detail_mark(clocks,11);int j0=row%p.L;
...
extern "C" __global__ __launch_bounds__(256,1) void dual_b1b4(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar;__shared__ unsigned long long clocks[12];
 ...
 float acc[3][64]={};
 if(threadIdx.x==0){for(int i=1;i<12;++i)clocks[i]=0;clocks[0]=detail_now();}
 for(int it=first;it<end;++it){dual_load(p,sm,&bar,it*64,phase,it==first,clocks);detail_mark(clocks,1);phase^=1;
  ...
  detail_mark(clocks,2);dual_dgrad(p,sm,&bar,it*64,it+1<end,clocks);allsync();detail_mark(clocks,6);
 }
...
#pragma unroll
 ...
 __threadfence();allsync();detail_mark(clocks,9);
 if(threadIdx.x==0&&atomicAdd(p.counts+1,1u)==UCOUNT-1){atomicExch(p.counts,0u);atomicExch(p.counts+1,0u);}
#endif
 detail_mark(clocks,10);
 if(threadIdx.x==0){auto* out=reinterpret_cast<unsigned long long*>(p.groupln)+split*12;for(int i=0;i<12;++i)out[i]=clocks[i];}
}
