// Experiment: dual_ds_tma
// One variable on dual_ln_prefetch: the BF16 dropout-scale rows ds[row%L] are
// TMA-loaded into shared memory (DW: 16KiB per input slot at196608+slot*16384;
// DX: one 16KiB buffer at81920 refilled right after each B1), replacing the
// per-element register mask decode and the mask prologue. Tiles whose64 rows
// wrap around L keep the generic global ds loads. Same y=dy*ds arithmetic.
...
// ds rows arrive by TMA (see ds_buf/issue_ds); wrapping tiles use generic ds loads.
...
// SPDX-License-Identifier: Apache-2.0
// Inlined copy of dual_primitives.cuh (so the hash covers it) with one extra tensor map.
...
struct Params {
 ...
 CUtensorMap dsm; // dual_ds_tma: [L,128] BF16 dropout scale rows; appended (64B aligned) after the original fields.
};
...
// ds rows for a 64-row tile are j0..j0+63 with j0=(tile*64)%L. A TMA box cannot
// wrap around L, so tiles crossing L keep the generic per-element global loads.
TMN_DEVI bool ds_tma_ok(const Params& p,int tile){int j0=(tile*64)%p.L;return j0+64<=p.L;}
TMN_DEVI uint32_t ds_bytes(const Params& p,int tile){return ds_tma_ok(p,tile)?16384u:0u;}
// DW: one 16KiB ds buffer per input slot (196608+slot*16384). DX: a single
// 16KiB buffer at81920 with its own mbarrier, refilled after each B1.
TMN_DEVI uint8_t* ds_buf(uint8_t* sm,bool dw,int slot){return dw?sm+196608+slot*16384:sm+81920;}
TMN_DEVI void issue_ds(const Params& p,uint8_t* dst,uint64_t* bar,int tile){
 if(!ds_tma_ok(p,tile))return;int j0=(tile*64)%p.L;
#pragma unroll
 for(int c=0;c<2;++c)tma_load_2d(dst+c*8192,&p.dsm,bar,c*64,j0);
}
...
#pragma unroll
  ...
  issue_ds(p,ds_buf(sm,true,slot),full+slot,tile);
 }else{
#pragma unroll
...
// sds: TMA-loaded ds rows in the dy layout (two 8KiB column blocks, 128B swizzle), or nullptr for the generic path.
template<bool DW> TMN_DEVI void gate_backward(const Params& p,uint8_t* s,int row,const uint8_t* sds){
 int j0=row%p.L;
 for(int i=threadIdx.x;i<1024;i+=256){int cb=i/512,r=(i%512)/8,c=(i%8)*8,jr=j0+r;if(jr>=p.L)jr-=p.L;
  ...
  if(sds)ds=lds128(smem_u32(sds+cb*8192)+swz128(r,c*2));else ds=ldg128(p.ds+jr*128+cb*64+c);
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 for(int it=split;it<p.tiles;it+=DWCOUNT,++round){int slot=round&1;
  ...
  gate_backward<true>(p,s,it*64,ds_tma_ok(p,it)?ds_buf(sm,true,slot):nullptr);
  ...
  if(next&&threadIdx.x==0)mbar_arrive_expect_tx(full+slot,98304u+ds_bytes(p,it+2*DWCOUNT));
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* full,uint64_t* dsbar){
 int split=blockIdx.x-DWCOUNT;int round=0;
 for(int it=split;it<p.tiles;it+=DXCOUNT,++round){int slot=round&1;
  mbar_wait(full+slot,(round/2)&1);
  prefetch_stats(p,sm,it,slot);
  mbar_wait(dsbar,round&1); // ds rows of this tile (or a zero-byte phase for wrapping tiles).
  gate_backward<false>(p,sm+slot*98304,it*64,ds_tma_ok(p,it)?ds_buf(sm,false,0):nullptr);
  // B1's CTA barrier: every thread finished reading the single ds buffer; refill it for the next tile.
  if(threadIdx.x==0&&it+DXCOUNT<p.tiles){mbar_arrive_expect_tx(dsbar,ds_bytes(p,it+DXCOUNT));issue_ds(p,ds_buf(sm,false,0),dsbar,it+DXCOUNT);}
...
void dual_b1b4(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t full[2],dsbar;
 const bool dw=blockIdx.x<DWCOUNT;int split=dw?blockIdx.x:blockIdx.x-DWCOUNT;int stride=dw?DWCOUNT:DXCOUNT;
 if(threadIdx.x==0){
  mbar_init(full,1);mbar_init(full+1,1);mbar_init(&dsbar,1);fence_barrier_init();
  if(split<p.tiles)mbar_arrive_expect_tx(full,(dw?98304u:131072u)+(dw?ds_bytes(p,split):0u));
  if(split+stride<p.tiles)mbar_arrive_expect_tx(full+1,(dw?98304u:65536u)+(dw?ds_bytes(p,split+stride):0u));
  if(!dw&&split<p.tiles)mbar_arrive_expect_tx(&dsbar,ds_bytes(p,split));
 }
 ...
 if(dw){
  ...
  if(split<p.tiles){issue_slot<false>(p,sm,full,0,split,true);if(threadIdx.x==0)issue_ds(p,ds_buf(sm,false,0),&dsbar,split);}
  if(split+stride<p.tiles)issue_slot<false>(p,sm,full,1,split+stride,false);
  input_role(p,sm,full,&dsbar);
 }
#if PART_ONLY == 2
