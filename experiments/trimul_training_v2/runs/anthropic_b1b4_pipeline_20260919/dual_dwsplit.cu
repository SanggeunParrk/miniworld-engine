// Experiment: dual_dwsplit
// One variable on dual_ln_prefetch: the DW role writes dp/dg into a shared 32KiB buffer (196608..229376)
// instead of over dy/gate, so the slot's dy/gate/proj (48KiB) are free right after B1 and are refilled
// with tile t+2 while the WGMMAs of tile t run (fullA); xn/norm (48KiB) are refilled after the MMAs
// (fullB). Up to 144KiB per SM is in flight instead of 96KiB; barrier count per tile is unchanged.
// Same B1 arithmetic/rounding, WGMMA operands, accumulators, partials. DX is unchanged.
...
// DW part A (B1 inputs dy/gate/proj, 49152B, barrier fullA=full[slot]) and part B (MMA inputs
// xn/norm, 49152B, barrier fullB=full[2+slot]). Called by thread 0 only.
TMN_DEVI void issue_dw_a(const Params& p,uint8_t* sm,uint64_t* full,int slot,int tile){
 uint8_t* s=sm+slot*98304;int row=tile*64;
#pragma unroll
 for(int c=0;c<2;++c){tma_load_2d(s+c*8192,&p.dy,full+slot,c*64,row);tma_load_2d(s+16384+c*8192,&p.gate,full+slot,c*64,row);tma_load_2d(s+32768+c*8192,&p.proj,full+slot,c*64,row);}
}
TMN_DEVI void issue_dw_b(const Params& p,uint8_t* sm,uint64_t* full,int slot,int tile){
 uint8_t* s=sm+slot*98304;int row=tile*64;
#pragma unroll
 for(int c=0;c<2;++c)tma_load_2d(s+49152+c*8192,&p.xn,full+2+slot,c*64,row);
#pragma unroll
 for(int c=0;c<4;++c)tma_load_2d(s+65536+c*8192,&p.norm,full+2+slot,c*64,row);
}
template<bool DW> TMN_DEVI void issue_slot(const Params& p,uint8_t* sm,uint64_t* full,int slot,int tile,bool initial){
 if(threadIdx.x!=0)return;
 uint8_t* s=sm+slot*98304;int row=tile*64;
 if constexpr(DW){issue_dw_a(p,sm,full,slot,tile);issue_dw_b(p,sm,full,slot,tile);
 }else{
#pragma unroll
...
// DW: dp -> sm+196608 (two 8KiB column blocks), dg -> sm+212992; DX: dp over dy in the slot.
template<bool DW> TMN_DEVI void gate_backward(const Params& p,uint8_t* sm,uint8_t* s,int row,const MaskCycle& mask,int mi){
...
#pragma unroll
  ...
  if constexpr(DW){uint32_t spy=smem_u32(sm+196608+cb*8192)+swz128(r,c*2),spg=spy+16384;
   asm volatile("st.shared.v4.b32 [%0],{%1,%2,%3,%4};"::"r"(spy),"r"(dp.x),"r"(dp.y),"r"(dp.z),"r"(dp.w):"memory");
   asm volatile("st.shared.v4.b32 [%0],{%1,%2,%3,%4};"::"r"(spg),"r"(dg.x),"r"(dg.y),"r"(dg.z),"r"(dg.w):"memory");stg128(p.dg+(size_t)(row+r)*128+cb*64+c,dg);
  }else asm volatile("st.shared.v4.b32 [%0],{%1,%2,%3,%4};"::"r"(sy),"r"(dp.x),"r"(dp.y),"r"(dp.z),"r"(dp.w):"memory");
 }
...
// DW: two 96 KiB input slots + one shared 32 KiB dp/dg buffer; 192 FP32 dW accumulators per thread.
// Part A of tile t+2 is issued right after B1(t) (its inputs are dead); part B after the MMAs of t.
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 uint8_t* dp=sm+196608;uint8_t* dg=sm+212992;
 for(int it=split;it<p.tiles;it+=DWCOUNT,++round){int slot=round&1;uint32_t par=(round/2)&1;
  mbar_wait(full+slot,par);uint8_t* s=sm+slot*98304; // part A: dy/gate/proj
  gate_backward<true>(p,sm,s,it*64,mask,mi); // ends with fence.proxy.async + CTA barrier
  bool next=it+2*DWCOUNT<p.tiles;
  if(next&&threadIdx.x==0){mbar_arrive_expect_tx(full+slot,49152);issue_dw_a(p,sm,full,slot,it+2*DWCOUNT);} // early refill of dead B1 inputs
  mbar_wait(full+2+slot,par); // part B: xn/norm
  // A = xn block (dWg) or dp block (dWp), MN-major; B = dg (shared buffer) or norm, two 8KiB column blocks.
  static_for<3>([&](auto ni){constexpr int n=decltype(ni)::value;int t=wi*3+n;uint8_t* sa=t<2?s+49152+t*8192:dp+((t-2)/2)*8192;uint8_t* sb=t<2?dg:s+65536+((t-2)%2)*16384;
  ...
  if(next&&threadIdx.x==0)mbar_arrive_expect_tx(full+2+slot,49152);
  allsync(); // Both WGs finished reading xn/norm and the dp/dg buffer.
  if(next&&threadIdx.x==0)issue_dw_b(p,sm,full,slot,it+2*DWCOUNT);
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 for(int it=split;it<p.tiles;it+=DXCOUNT,++round){int slot=round&1;
  ...
  gate_backward<false>(p,sm,sm+slot*98304,it*64,mask,mi);
...
void dual_b1b4(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t full[4]; // DW: [slot]=part A, [2+slot]=part B; DX: [slot]
 ...
 if(threadIdx.x==0){
  ...
  if(dw){if(split<p.tiles){mbar_arrive_expect_tx(full,49152);mbar_arrive_expect_tx(full+2,49152);}
   if(split+stride<p.tiles){mbar_arrive_expect_tx(full+1,49152);mbar_arrive_expect_tx(full+3,49152);}}
  else{if(split<p.tiles)mbar_arrive_expect_tx(full,131072);if(split+stride<p.tiles)mbar_arrive_expect_tx(full+1,65536);}
 }
