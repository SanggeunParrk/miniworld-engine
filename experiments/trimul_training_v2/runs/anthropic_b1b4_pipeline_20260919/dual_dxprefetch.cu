// Independent32K DX output allows48K input prefetch during LN epilogue.
...
TMN_DEVI void dual_dgrad(const Params& p,uint8_t* sm,int m0,int slot,uint64_t* full,bool next){
 const int wi=threadIdx.x/128,tid=threadIdx.x%128,lane=tid%32,w=tid/32,mat=lane/8,r8=lane%8;
 uint8_t* sx=sm+slot*98304+32768;uint8_t* so=sm+65536;
 ...
 // All raw tri has been consumed into registers, as proved by published row
 // sums depending on every fragment. Output uses a separate32K scratch tile.
 // Refill dy+tri48K now; gate16K still holds LN metadata/partials until return.
 if(next&&threadIdx.x==0){
  mbar_arrive_expect_tx(full+slot,65536);int nr=m0+128*DXCOUNT;
#pragma unroll
  for(int c=0;c<2;++c)tma_load_2d(sm+slot*98304+c*8192,&p.dy,full+slot,c*64,nr);
  tma_load_2d(sx,&p.tri,full+slot,nr,0);
 }
 ...
 float* tmp=reinterpret_cast<float*>(sm+slot*98304+16384+4096);
...
#pragma unroll
   ...
   int chq=8*(mat>>1)+r8;
   stsm_x4_t(smem_u32(so)+swz128(n*64+q*16+chq,(w*16+8*(mat&1))*2),out[0],out[1],out[2],out[3]);
  });
 });
 ...
 if(tid==0){for(int ch=wi*128;ch<(wi+1)*128;ch+=16)tma_store_3d(&p.dtri,so+ch*128,m0,ch,0);tma_store_commit();tma_store_wait_all();}
...
TMN_DEVI void cluster_dx(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 for(int it=split;it<p.tiles;it+=DXCOUNT,++round){int slot=round&1;
  ...
  dual_dgrad(p,sm,it*64,slot,full,next);
  // DX store completion protects tri reuse within this independent CTA.
  allsync();
  // All generic gate-scratch readers finished at the CTA barrier above.
  // Complete the same TMA phase with its final16K; early48K already issued.
  if(next&&threadIdx.x==0){
#pragma unroll
   for(int c=0;c<2;++c)tma_load_2d(sm+slot*98304+16384+c*8192,&p.gate,full+slot,c*64,(it+2*DXCOUNT)*64);
  }
  if(++mi==mask.period)mi=0;
 }
