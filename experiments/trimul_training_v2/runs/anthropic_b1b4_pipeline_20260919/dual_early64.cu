// One variable on dual_maskbits: issue 64 KiB of next inputs before dnorm.
...
TMN_DEVI void dual_dgrad(const Params& p,uint8_t* sm,uint64_t* bar,int m0,bool next){
 ...
 // The caller waited for both WGs' dW chains; the CTA barrier above joins
 // them. gate/dg, xn and norm are now dead. Refill these 64 KiB while the
 // current dnorm + LN row-stat calculation still consumes only dp/tri/Wp.
 // This starts the next phase; the later prefetch adds dy (16 KiB), and
 // dual_load supplies proj/tri (48 KiB). Total: 128 KiB, exactly one arrive.
 if(next&&threadIdx.x==0){mbar_arrive_expect_tx(bar,131072);
#pragma unroll
  for(int c=0;c<2;++c){tma_load_2d(sm+16384+c*8192,&p.gate,bar,c*64,m0+64*UCOUNT);tma_load_2d(sm+49152+c*8192,&p.xn,bar,c*64,m0+64*UCOUNT);}
#pragma unroll
...
#pragma unroll
 ...
 // dnorm has consumed dp in both WGs (preceding CTA barrier). Refill dy
 // into that 16 KiB slot, adding to the phase begun by the early 64 KiB load.
 if(next&&threadIdx.x==0){
#pragma unroll
  for(int c=0;c<2;++c)tma_load_2d(sm+c*8192,&p.dy,bar,c*64,m0+64*UCOUNT);
 }
