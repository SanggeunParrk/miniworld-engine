// One variable on dual_vec2: LN epilogue loop order.
...
#pragma unroll
 ...
 // Loop interchange: load row-only statistics once for eight channel groups.
 // Keep each channel's k=0..3 accumulation order and the BF16 dtri rounding.
 float dga[8]={},dbe[8]={};
#pragma unroll 1
 for(int k=0;k<4;++k){
  int r=2*(lane%8)+16*k;
  float mua=mus[r],mub=mus[r+1],rsa=rss[r],rsb=rss[r+1];
  float ha=stats[r*2+1]+stats[128+r*2+1],hb=stats[(r+1)*2+1]+stats[128+(r+1)*2+1];
  float hxa=stats[r*2]+stats[128+r*2],hxb=stats[(r+1)*2]+stats[128+(r+1)*2];
  static_for<8>([&](auto bi){constexpr int b=decltype(bi)::value;
   int c=wi*128+b*16+w*4+lane/8;float gamma=gam[c];
   uint32_t x=pair_get(sx,c,r),dn=pair_get(sn,c,r);
   float xa=__fmul_rn(__fsub_rn(bf16lo(x),mua),rsa),xb=__fmul_rn(__fsub_rn(bf16hi(x),mub),rsb);
   float da=bf16lo(dn),dd=bf16hi(dn);
   dga[b]+=da*xa+dd*xb;dbe[b]+=da+dd;
   pair_put(sx,c,r,pack_bf16(rsa*((da*gamma-ha)-xa*hxa),rsb*((dd*gamma-hb)-xb*hxb)));
  });
 }
 static_for<8>([&](auto bi){constexpr int b=decltype(bi)::value;
  int c=wi*128+b*16+w*4+lane/8;float dg=dga[b],db=dbe[b];
#pragma unroll
