// Experiment: accumulate LN parameter gradients across tiles.
...
TMN_DEVI void dual_dgrad(const Params& p,uint8_t* sm,int m0,int slot,float (&pg)[32],float (&pb)[32]){
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 for(int it=split;it<p.tiles;it+=DXCOUNT,++round){int slot=round&1;
  ...
  dual_dgrad(p,sm,it*64,slot,pg,pb);
 ...
 // All input tiles and dtri TMA stores are complete. Reuse dead slot0
