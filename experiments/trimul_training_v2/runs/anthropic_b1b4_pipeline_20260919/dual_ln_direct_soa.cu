...
TMN_DEVI void dual_dgrad(const Params& p,uint8_t* sm,int m0,int slot){
 ...
 // Read saved row statistics directly; gamma is published once per CTA.
 ...
 if(lane%4==0){stats[wi*128+ra]=s1[0];stats[wi*128+64+ra]=s2[0];stats[wi*128+rb]=s1[1];stats[wi*128+64+rb]=s2[1];}
 allsync(); // Both channel halves of each row must be published before LN epilogue.
 float c1a=stats[ra]+stats[128+ra],c1b=stats[rb]+stats[128+rb];
 float c2a=stats[64+ra]+stats[192+ra],c2b=stats[64+rb]+stats[192+rb];
...
void dual_b1b4(__grid_constant__ const Params p){
 ...
 // DX-only persistent gamma: 73728..74752 never overlaps either input slot.
