// Experiment: dual_ln_prefetch
// Diagnostic only; reject32-bit timer wraps.
...
#define UCOUNT 132
#endif
TMN_DEVI unsigned int phase_ns(){unsigned int t;asm volatile("mov.u32 %0, %%globaltimer_lo;":"=r"(t)::"memory");return t;}
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* full,unsigned int* stamps){
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* full,unsigned int* stamps){
...
void dual_b1b4(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t full[2];__shared__ unsigned int stamps[6];
...
#else
 if(threadIdx.x==0){stamps[2]=phase_ns();stamps[3]=stamps[2];stamps[4]=stamps[2];}
#endif
 if(threadIdx.x==0){stamps[5]=phase_ns();auto* dst=reinterpret_cast<unsigned long long*>(p.groupln)+blockIdx.x*6;for(int i=0;i<6;++i)dst[i]=stamps[i];}
}
