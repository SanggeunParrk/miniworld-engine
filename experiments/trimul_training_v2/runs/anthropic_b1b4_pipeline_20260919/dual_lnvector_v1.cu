...
#pragma unroll
 ...
 if(threadIdx.x<128)stats[threadIdx.x]+=stats[128+threadIdx.x];allsync();
 float* red=reinterpret_cast<float*>(sm+229376);
#pragma unroll 1
 for(int b=0;b<8;++b){int c=wi*128+b*16+w*4+lane/8;float dg=0,db=0,gamma=gam[c];int rr=(lane%8)*8;
  uint4 xv=lds128(smem_u32(sx)+swz128(c,rr*2)),nv=lds128(smem_u32(sn)+swz128(c,rr*2)),ov;
  uint32_t *xx=reinterpret_cast<uint32_t*>(&xv),*nn=reinterpret_cast<uint32_t*>(&nv),*oo=reinterpret_cast<uint32_t*>(&ov);
#pragma unroll
  for(int k=0;k<4;++k){int r=rr+2*k;float2 mm=lds64f(smem_u32(mus+r)),rv=lds64f(smem_u32(rss+r));uint4 sv=lds128(smem_u32(stats+r*2));float xa=__fmul_rn(__fsub_rn(bf16lo(xx[k]),mm.x),rv.x),xb=__fmul_rn(__fsub_rn(bf16hi(xx[k]),mm.y),rv.y),da=bf16lo(nn[k]),dd=bf16hi(nn[k]);dg+=da*xa+dd*xb;db+=da+dd;oo[k]=pack_bf16(rv.x*((da*gamma-__uint_as_float(sv.y))-xa*__uint_as_float(sv.x)),rv.y*((dd*gamma-__uint_as_float(sv.w))-xb*__uint_as_float(sv.z)));}
  uint32_t adr=smem_u32(sx)+swz128(c,rr*2);asm volatile("st.shared.v4.b32 [%0],{%1,%2,%3,%4};"::"r"(adr),"r"(ov.x),"r"(ov.y),"r"(ov.z),"r"(ov.w):"memory");
