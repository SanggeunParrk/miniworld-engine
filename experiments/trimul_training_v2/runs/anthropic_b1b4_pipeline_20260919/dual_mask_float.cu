// Experiment: no BF16 repack for cached mask.
...
template<bool DW> TMN_DEVI void gate_backward(const Params& p,uint8_t* s,int row,const MaskCycle& mask,int mi){
 ...
 for(int i=threadIdx.x;i<1024;i+=256){int cb=i/512,r=(i%512)/8,c=(i%8)*8,jr=j0+r;if(jr>=p.L)jr-=p.L;
  ...
  if(!mask.cached)ds=ldg128(p.ds+jr*128+cb*64+c);
...
#pragma unroll
  for(int q=0;q<4;++q){int bit=8*(i/256)+2*q;
   ...
   float ya=bf16lo(yy[q])*da,yb=bf16hi(yy[q])*db,ga=bf16lo(gg[q]),gb=bf16hi(gg[q]);oo[q]=pack_bf16(ya*ga,yb*gb);
