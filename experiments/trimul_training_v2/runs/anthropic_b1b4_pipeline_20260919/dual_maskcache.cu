// One variable on dual_maskbits: keep compressed masks in coherent cache scratch.
...
TMN_DEVI void dual_load(const Params& p,uint8_t* sm,uint64_t* bar,int row,int phase,bool first,bool mask_reuse){
...
#pragma unroll
 ...
 // Scratch belongs only to this CTA/thread. Reload a coherent L1-cached pair
 // here so the mask registers are not live across the dnorm/LN computation.
 uint32_t mask_bits=0,mask_scale=0;
 if(mask_reuse){const void* cache=p.partln+blockIdx.x*512+threadIdx.x*2;
 asm volatile("ld.global.ca.v2.u32 {%0,%1}, [%2];":"=r"(mask_bits),"=r"(mask_scale):"l"(cache):"memory");}
...
#pragma unroll
 ...
 // Before publication, this per-CTA workspace is unused by LN. Store the
 // lossless mask there; final LN partials overwrite it only after the loop.
 if(mask_reuse){void* cache=p.partln+split*512+threadIdx.x*2;
 asm volatile("st.global.v2.u32 [%0], {%1,%2};"::"l"(cache),"r"(mask_bits),"r"(mask_scale):"memory");}
 float acc[3][64]={};
 for(int it=first;it<end;it+=UCOUNT){dual_load(p,sm,&bar,it*64,phase,it==first,mask_reuse);phase^=1;
