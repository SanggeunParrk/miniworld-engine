// One variable on dual_maskread: overlap dW and dnorm issue/metadata setup.
...
#pragma unroll
 ...
 for(int it=first;it<end;it+=UCOUNT){dual_load(p,sm,&bar,it*64,phase,it==first,mask_bits,mask_scale,mask_reuse);phase^=1;
  ...
  // First dnorm wait<0> also waits for all preceding dW groups, before any
  // of their operands are overwritten. Keep dW registers live until then.
  dual_dgrad(p,sm,&bar,it*64,it+UCOUNT<end);fence_regs(acc[0]);fence_regs(acc[1]);fence_regs(acc[2]);allsync();
 }
...
#endif
 // Finish global TMA stores before returning; in-loop wait only protects source reuse.
