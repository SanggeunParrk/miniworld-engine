// One variable on dual_maskbits: one shared scale instead of a live per-thread scale.
...
TMN_DEVI void dual_load(const Params& p,uint8_t* sm,uint64_t* bar,int row,int phase,bool first,uint32_t mask_bits,uint32_t* shared_scale,bool mask_reuse){
...
#pragma unroll
 ...
 uint32_t mask_scale=0;
 if(mask_reuse)asm volatile("ld.shared.u32 %0, [%1];":"=r"(mask_scale):"r"(smem_u32(shared_scale)):"memory");
...
extern "C" __global__ __launch_bounds__(256,1) void dual_b1b4(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar;__shared__ uint32_t shared_scale;
 const int wi=threadIdx.x/128,tid=threadIdx.x%128,lane=tid%32,w=tid/32,split=blockIdx.x;
 if(threadIdx.x==0){shared_scale=0;mbar_init(&bar,1);fence_barrier_init();}
...
#pragma unroll
 ...
 // One common positive BF16 dropout scale; warp max handles all-zero lanes.
 if(mask_reuse){uint32_t v=__reduce_max_sync(0xffffffff,mask_scale);if(lane==0)atomicMax(&shared_scale,v);}
 allsync(); // Publish the CTA dropout scale before any thread loads it.
 float acc[3][64]={};
 for(int it=first;it<end;it+=UCOUNT){dual_load(p,sm,&bar,it*64,phase,it==first,mask_bits,&shared_scale,mask_reuse);phase^=1;
