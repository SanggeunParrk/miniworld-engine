// One variable on dual_maskbits: cache a generic mask period of up to three.
...
extern "C" __global__ __launch_bounds__(256,1) void dual_b1b4(__grid_constant__ const Params p){
 ...
 // Cache up to three distinct masks in a periodic cyclic CTA schedule.
 // The period is derived from L and the CTA row stride, never from benchmark L.
 int ga=64*UCOUNT,gb=p.L;while(gb){int rem=ga%gb;ga=gb;gb=rem;}
 const int mask_period=p.L/ga;const bool mask_reuse=mask_period<=3;
 uint32_t mask0=0,mask1=0,mask2=0,mask_scale=0;int mask_index=0;
 if(mask_reuse&&first<end){
  for(int phase=0;phase<mask_period;++phase){uint32_t bits=0;int j0=((first+phase*UCOUNT)*64)%p.L;
   for(int i=threadIdx.x;i<1024;i+=256){int cb=i/512,r=(i%512)/8,c=(i%8)*8,jr=j0+r;if(jr>=p.L)jr-=p.L;
...
#pragma unroll
    for(int q=0;q<4;++q){uint32_t lo=dd[q]&65535u,hi=dd[q]>>16;int bit=8*(i/256)+2*q;
     if(lo){bits|=1u<<bit;mask_scale=lo;}if(hi){bits|=1u<<(bit+1);mask_scale=hi;}
    }
   }
   if(phase==0)mask0=bits;else if(phase==1)mask1=bits;else mask2=bits;
  }
 }
 float acc[3][64]={};
 for(int it=first;it<end;it+=UCOUNT){dual_load(p,sm,&bar,it*64,phase,it==first,mask_index==0?mask0:mask_index==1?mask1:mask2,mask_scale,mask_reuse);phase^=1;
  ...
  dual_dgrad(p,sm,&bar,it*64,it+UCOUNT<end);allsync();if(++mask_index==mask_period)mask_index=0;
 }
