...
__device__ __forceinline__ int logical_block(){return (blockIdx.x*13)%UCOUNT;}
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* full){
 int split=logical_block()-DWCOUNT;int mi=0,round=0;
...
void dual_b1b4(__grid_constant__ const Params p){
 ...
 const int logical=logical_block();const bool dw=logical<DWCOUNT;int split=dw?logical:logical-DWCOUNT;int stride=dw?DWCOUNT:DXCOUNT;
 if(threadIdx.x==0){
