// Independent CTA roles, DW:DX=1:1. B1-B4 remains one cooperative launch.
#define DW_RATIO 1
#define DX_RATIO 1
...
void dual_b1b4(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t full[2];__shared__ unsigned int started;
 if(threadIdx.x==0){unsigned int t;asm volatile("mov.u32 %0, %%globaltimer_lo;":"=r"(t));started=t;}
...
if(threadIdx.x==0){unsigned int ended;asm volatile("mov.u32 %0, %%globaltimer_lo;":"=r"(ended));
  auto* t=reinterpret_cast<unsigned long long*>(p.groupln)+blockIdx.x*12;t[0]=started;t[1]=ended;t[2]=dw;
 }
#if PART_ONLY == 2
