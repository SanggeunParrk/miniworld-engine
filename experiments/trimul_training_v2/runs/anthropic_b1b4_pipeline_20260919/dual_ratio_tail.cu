#define COUNTER_WORDS 4
// Independent CTA roles, DW:DX=10:23. B1-B4 remains one cooperative launch.
#define DW_RATIO 10
#define DX_RATIO 23
...
TMN_DEVI void cluster_dw(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 for(int it=split;it<p.tiles;it+=DWCOUNT,++round){int slot=round&1;
  ...
  allsync();
  if(next)cluster_issue<true>(p,sm,full,slot,it+2*DWCOUNT,false);
...
TMN_DEVI void cluster_dx(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 for(int it=split;it<p.tiles;it+=DXCOUNT,++round){int slot=round&1;
  ...
  allsync();
  if(next)cluster_issue<false>(p,sm,full,slot,it+2*DXCOUNT,false);
  if(++mi==mask.period)mi=0;
 }
...
void dual_b1b4(__grid_constant__ const Params p){
 ...
 if(dw){
  ...
  if(split+stride<p.tiles)cluster_issue<true>(p,sm,full,1,split+stride,false);
  cluster_dw(p,sm,full);
 }else{
  if(split<p.tiles)cluster_issue<false>(p,sm,full,0,split,true);
  if(split+stride<p.tiles)cluster_issue<false>(p,sm,full,1,split+stride,false);
  cluster_dx(p,sm,full);
 }

#if PART_ONLY == 2
 // Publish role partials before its CTA leader increments that role's counter.
 __threadfence();allsync();
 if(threadIdx.x==0)atomicAdd(p.counts+(dw?0:1),1u);
 // DX CTAs with one extra ragged tile skip dW reduction. The other resident
 // CTAs reduce dW as soon as DW finishes, overlapping the final DX tile.
 int tail=p.tiles%DXCOUNT,workers=UCOUNT-tail;
 if(dw||split>=tail){
  if(threadIdx.x==0)while(atomicAdd(p.counts,0u)!=DWCOUNT)__nanosleep(32);
  allsync(); // The role-ready ticket releases all globally fenced dW partials.
  int worker=dw?split:DWCOUNT+split-tail;
  for(int i=worker*256+threadIdx.x;i<49152;i+=workers*256){
   int tile=i/16384,j=i%16384;float v=0;
   for(int b=0;b<DWCOUNT;++b)v+=reinterpret_cast<volatile float*>(p.partw)[(tile*DWCOUNT+b)*16384+j];
   (tile==0?p.dwg:p.dwp+(tile-1)*16384)[j]=__float2bfloat16_rn(v);
  }
 }
 // Exactly two whole CTAs own the512 LN parameter outputs. These CTA-wide
 // branches are uniform; all other CTAs may proceed to completion.
 if(blockIdx.x<2){
  if(threadIdx.x==0)while(atomicAdd(p.counts+1,0u)!=DXCOUNT)__nanosleep(32);
  allsync(); // All DX partials were fenced before publishing their tickets.
  int j=blockIdx.x*256+threadIdx.x;float v=0;
  for(int b=0;b<DXCOUNT;++b)v+=reinterpret_cast<volatile float*>(p.partln)[b*512+j];
  (j<256?p.dgam:p.dbeta)[j%256]=v;
 }
 // Complete every reader/store before the last CTA resets ready and done
 // counters. The fourth allocated counter is reserved and remains zero.
 __threadfence();allsync();
 if(threadIdx.x==0&&atomicAdd(p.counts+2,1u)==UCOUNT-1){atomicExch(p.counts,0u);atomicExch(p.counts+1,0u);atomicExch(p.counts+2,0u);}
#endif
