// One variable on dual_maskbits: retain LN parameter partials in two registers.
...
TMN_DEVI void dual_dgrad(const Params& p,uint8_t* sm,uint64_t* bar,int m0,bool next,float& pdg,float& pdb){
...
#pragma unroll
 ...
 // Each of eight lanes in a channel group retains one of its eight channels.
...
extern "C" __global__ __launch_bounds__(256,1) void dual_b1b4(__grid_constant__ const Params p){
 ...
 allsync(); // Publish mbarrier initialization before the first TMA issue.
 float pdg=0,pdb=0;
...
#pragma unroll
 ...
 for(int it=first;it<end;it+=UCOUNT){dual_load(p,sm,&bar,it*64,phase,it==first,mask_bits,mask_scale,mask_reuse);phase^=1;
  ...
  dual_dgrad(p,sm,&bar,it*64,it+UCOUNT<end,pdg,pdb);allsync();
 }
...
#pragma unroll
 ...
 // Each channel is owned by exactly one thread, including empty CTAs.
 int lc=wi*128+(lane%8)*16+w*4+lane/8;
 p.partln[split*512+lc]=pdg;p.partln[split*512+256+lc]=pdb;
#if PART_ONLY == 2
