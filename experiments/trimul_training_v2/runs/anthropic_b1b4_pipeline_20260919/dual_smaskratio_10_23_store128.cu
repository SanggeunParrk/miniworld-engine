...
template<bool DW> TMN_DEVI void cluster_b1(const Params& p,uint8_t* s,int row,const MaskCycle& mask,int mi){
 int j0=row%p.L;uint32_t bits=0;if(mask.cached)asm volatile("ld.shared.b32 %0,[%1];":"=r"(bits):"r"(mask.cache+mi*1024+threadIdx.x*4):"memory");
 for(int i=threadIdx.x;i<1024;i+=256){int cb=i/512,r=(i%512)/8,c=(i%8)*8,jr=j0+r;if(jr>=p.L)jr-=p.L;
...
#pragma unroll
 ...
 if(tid==0){for(int ch=wi*128;ch<(wi+1)*128;ch+=DTRI_CHANNELS)tma_store_3d(&p.dtri,sx+ch*128,m0,ch,0);tma_store_commit();tma_store_wait_all();}
...
TMN_DEVI void cluster_dw(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 MaskCycle mask=mask_cycle<DWCOUNT,true>(p,split,sm);int mi=0,round=0;float acc[3][64]={};
...
TMN_DEVI void cluster_dx(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 MaskCycle mask=mask_cycle<DXCOUNT,false>(p,split,sm);
