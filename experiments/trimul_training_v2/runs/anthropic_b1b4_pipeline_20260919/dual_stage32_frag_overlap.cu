...
#pragma unroll
  for(int q=0;q<4;++q){int c=n*64+w*16+lane/4,r=q*8+2*(lane%4);
   uint32_t da=pack_bf16(acc[4*q],acc[4*q+1]),db=pack_bf16(acc[4*q+2],acc[4*q+3]);
   put32(sn,c,r,da);put32(sn,c+8,r,db);
   uint32_t xa=get32(sx,c,r),xb=get32(sx,c+8,r);
   float mu0=mus[r],mu1=mus[r+1],rs0=rss[r],rs1=rss[r+1],ga=gam[c],gb=gam[c+8];
   float ha0=bf16lo(da)*ga,ha1=bf16hi(da)*ga,hb0=bf16lo(db)*gb,hb1=bf16hi(db)*gb;
   float a=ha0*__fmul_rn(__fsub_rn(bf16lo(xa),mu0),rs0)+hb0*__fmul_rn(__fsub_rn(bf16lo(xb),mu0),rs0);
   float b=ha1*__fmul_rn(__fsub_rn(bf16hi(xa),mu1),rs1)+hb1*__fmul_rn(__fsub_rn(bf16hi(xb),mu1),rs1);
   float aa=ha0+hb0,bb=ha1+hb1;
   // Same token pairs, eight channel groups per warp, two channels each.
#pragma unroll
   for(int sh=4;sh<32;sh*=2){a+=__shfl_xor_sync(0xffffffff,a,sh);b+=__shfl_xor_sync(0xffffffff,b,sh);aa+=__shfl_xor_sync(0xffffffff,aa,sh);bb+=__shfl_xor_sync(0xffffffff,bb,sh);}
   if(lane<4){int off=(wi*4+w)*64+r*2;
    // Same owner updates its warp/token partial across the two channel tiles.
    stats[off]=qn?stats[off]+a:a;stats[off+1]=qn?stats[off+1]+aa:aa;
    stats[off+2]=qn?stats[off+2]+b:b;stats[off+3]=qn?stats[off+3]+bb:bb;
   }
  }
 }
 allsync(); // Publish all eight warp partials and dnorm before row reduction.
 if(threadIdx.x<32){int rr=threadIdx.x;float a=0,b=0;
#pragma unroll
  for(int k=0;k<8;++k){a+=stats[k*64+rr*2];b+=stats[k*64+rr*2+1];}
  // Each owner overwrites only its own consumed row partials.
  stats[rr*2]=a/256.f;stats[rr*2+1]=b/256.f;stats[128+rr*2]=0;stats[128+rr*2+1]=0;
 }
 allsync(); // Publish full-channel row sums before B4 reads.
...
#pragma unroll
 ...
 for(int it=first;it<end;it+=UCOUNT,++round){int slot=round&1;uint8_t* s=sm+slot*65536;
  ...
  // First dnorm wait completes the three dW groups too; operands stay live.
  stage_dgrad(p,sm,slot,it*32);
  fence_regs(acc[0]);fence_regs(acc[1]);fence_regs(acc[2]);allsync();
