...
#pragma unroll
 for(int qn=0;qn<4;++qn){int n=wi*4+qn;uint8_t* sw=sm+131072+(n/2)*16384+(n%2)*4096;
  ...
  if(w<2){
#pragma unroll
   for(int q=0;q<2;++q){uint32_t fx[4],dn[4];ldsm_x4_t(fx,smem_u32(sx)+sw32(n*32+q*16+r8+8*(mat>>1),w*16+8*(mat&1)));
#pragma unroll
    ...
    int chq=8*(mat>>1)+r8;stsm_x4_t(smem_u32(sn)+sw32(n*32+q*16+chq,w*16+8*(mat&1)),dn[0],dn[1],dn[2],dn[3]);
   }
  }
