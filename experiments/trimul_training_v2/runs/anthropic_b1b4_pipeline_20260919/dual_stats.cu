// One variable on dual_vec2: combine WG row statistics once.
...
#pragma unroll
 ...
 // Both WGs published row stats at the preceding CTA barrier.
 // Combine once per row instead of adding the same WG sums per output channel.
 if(threadIdx.x<128)stats[threadIdx.x]+=stats[128+threadIdx.x];
 allsync(); // Publish combined stats to both WGs before LN consumes them.
