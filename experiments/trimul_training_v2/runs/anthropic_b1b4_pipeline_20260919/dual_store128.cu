// One variable on dual_maskbits: TMA dtri store channel-box width.
...
#pragma unroll
 ...
 sync_group();fence_proxy_async();sync_group();if(tid==0){for(int c=wi*128;c<(wi+1)*128;c+=DTRI_CHANNELS)tma_store_3d(&p.dtri,sx+c*128,m0,c,0);tma_store_commit();tma_store_wait_all();}sync_group();
}
