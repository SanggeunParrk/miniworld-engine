// Experiment: dual_tma_read_stream
...
// Streaming-only inputs are not reused by the other CTA role.
...
template<bool DW> TMN_DEVI void issue_slot(const Params& p,uint8_t* sm,uint64_t* full,int slot,int tile,bool initial){
 ...
 if constexpr(DW){
#pragma unroll
  for(int c=0;c<2;++c){tma_load_2d(s+c*8192,&p.dy,full+slot,c*64,row);tma_load_2d(s+16384+c*8192,&p.gate,full+slot,c*64,row);tma_load_stream(s+32768+c*8192,&p.proj,full+slot,c*64,row);tma_load_stream(s+49152+c*8192,&p.xn,full+slot,c*64,row);}
#pragma unroll
  for(int c=0;c<4;++c)tma_load_stream(s+65536+c*8192,&p.norm,full+slot,c*64,row);
 }else{
#pragma unroll
  for(int c=0;c<2;++c){tma_load_2d(s+c*8192,&p.dy,full+slot,c*64,row);tma_load_2d(s+16384+c*8192,&p.gate,full+slot,c*64,row);}
  tma_load_stream(s+32768,&p.tri,full+slot,row,0);
...
#pragma unroll
 ...
 if(tid==0){for(int ch=wi*128;ch<(wi+1)*128;ch+=16)tma_store_3d(&p.dtri,sx+ch*128,m0,ch,0);tma_store_commit();tma_store_wait_read<0>();}
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 // Complete outstanding dtri writes before the global publication protocol.
