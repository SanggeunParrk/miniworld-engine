// One-variable experiment on dual_vec4: four-lane split reduction.
// One-variable experiment: vector-friendly FP32 partial workspace layout.
...
// These partials are written by other CTAs during this very launch.
// Use coherent L2-only reads, NOT Anthropic's read-only/noncoherent ldg128.
// Publication fence + CTA ticket precedes these loads. Inline asm memory
// clobber prevents compiler hoisting across the publication barrier.
TMN_DEVI float4 partial_load(const float* p){
 float4 v;asm volatile("ld.global.cg.v4.f32 {%0,%1,%2,%3}, [%4];"
 :"=f"(v.x),"=f"(v.y),"=f"(v.z),"=f"(v.w):"l"(p):"memory");return v;
}
TMN_DEVI void reduce_fragment(const Params& p,int i){
 const int rank=threadIdx.x%4;
 if(i<12288){
  int t=i/2048,j=i%2048;float4 v={0,0,0,0};
  // Partition the split dimension over a 4-lane tree; BF16 rounding is still only at the final output.
  for(int b=rank;b<UCOUNT;b+=4){float4 a=partial_load(p.partw+(t*UCOUNT+b)*8192+j*4);v.x+=a.x;v.y+=a.y;v.z+=a.z;v.w+=a.w;}
  // Four adjacent lanes reduce different split residues. The whole warp
  // reaches both shuffles; every output group contains exactly four lanes.
#pragma unroll
  for(int sh=2;sh;sh/=2){v.x+=__shfl_xor_sync(0xffffffff,v.x,sh);v.y+=__shfl_xor_sync(0xffffffff,v.y,sh);v.z+=__shfl_xor_sync(0xffffffff,v.z,sh);v.w+=__shfl_xor_sync(0xffffffff,v.w,sh);}
  int q=j/128,tid=j%128,lane=tid%32,w=tid/32;
  int rr=w*16+lane/4+(t<2?t*64:((t-2)/2)*64);
  int c=q*8+2*(lane%4)+(t<2?0:((t-2)%2)*128),stride=t<2?128:256;
  auto* out=t<2?p.dwg:p.dwp;
  if(rank==0){stg32(out+rr*stride+c,pack_bf16(v.x,v.y));
  stg32(out+(rr+8)*stride+c,pack_bf16(v.z,v.w));}
 }else if(i<12800){int j=i-12288;float v=0;
  for(int b=rank;b<UCOUNT;b+=4)v+=reinterpret_cast<volatile float*>(p.partln)[b*512+j];
  v+=__shfl_xor_sync(0xffffffff,v,2);v+=__shfl_xor_sync(0xffffffff,v,1);
  if(rank==0)(j<256?p.dgam:p.dbeta)[j%256]=v;
 }
}
extern "C" __global__ __launch_bounds__(256,1) void dual_b1b4(__grid_constant__ const Params p){
 ...
 // Fragment-major layout: [6 output tiles, UCOUNT, 16 quads, 128 lanes, 4].
 // Each aligned vector holds (rowA,c:c+2), (rowB,c:c+2). Four-lane fragment
 // permutation is decoded only once by the final reducer, not by 192 stores.
 static_for<3>([&](auto ni){constexpr int n=decltype(ni)::value;int t=wi*3+n;
#pragma unroll
 for(int q=0;q<16;++q){uint4 v={__float_as_uint(acc[n][4*q]),__float_as_uint(acc[n][4*q+1]),__float_as_uint(acc[n][4*q+2]),__float_as_uint(acc[n][4*q+3])};
 stg128(p.partw+(t*UCOUNT+split)*8192+q*512+tid*4,v);}});
 ...
 allsync();
 for(int i=(blockIdx.x*256+threadIdx.x)/4;i<12800;i+=UCOUNT*64)reduce_fragment(p,i);
 __threadfence();allsync();
...
extern "C" __global__ void unified_reduce(__grid_constant__ const Params p){
 for(int i=(blockIdx.x*256+threadIdx.x)/4;i<12800;i+=gridDim.x*64)reduce_fragment(p,i);
}
