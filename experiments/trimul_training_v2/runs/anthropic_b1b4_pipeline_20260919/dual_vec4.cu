...
TMN_DEVI void reduce_fragment(const Params& p,int i){
 if(i<12288){
  ...
  // Keep the previous split summation order; only vector layout changes.
  for(int b=0;b<UCOUNT;++b){float4 a=partial_load(p.partw+(t*UCOUNT+b)*8192+j*4);v.x+=a.x;v.y+=a.y;v.z+=a.z;v.w+=a.w;}
  ...
  stg32(out+rr*stride+c,pack_bf16(v.x,v.y));
  stg32(out+(rr+8)*stride+c,pack_bf16(v.z,v.w));
 }else if(i<12800){int j=i-12288;float v=0;
  for(int b=0;b<UCOUNT;++b)v+=reinterpret_cast<volatile float*>(p.partln)[b*512+j];
...
#pragma unroll
 ...
 allsync();
 for(int i=blockIdx.x*256+threadIdx.x;i<12800;i+=UCOUNT*256)reduce_fragment(p,i);
 __threadfence();allsync();
...
extern "C" __global__ void unified_reduce(__grid_constant__ const Params p){
 for(int i=blockIdx.x*256+threadIdx.x;i<12800;i+=gridDim.x*256)reduce_fragment(p,i);
}
