...
#pragma unroll
 ...
 for(int i=threadIdx.x;i<1024;i+=384){int cb=i/512,r=(i%512)/8,c=(i%8)*8,jr=j0+r;if(jr>=p.L)jr-=p.L;
...
#pragma unroll
  for(int q=0;q<4;++q){int bit=8*(i/384)+2*q;dd[q]=((mask_bits>>bit)&1?mask_scale:0)|((mask_bits>>(bit+1))&1?mask_scale<<16:0);}
...
#pragma unroll
   for(int q=0;q<4;++q){uint32_t lo=dd[q]&65535u,hi=dd[q]>>16;int bit=8*(i/384)+2*q;
