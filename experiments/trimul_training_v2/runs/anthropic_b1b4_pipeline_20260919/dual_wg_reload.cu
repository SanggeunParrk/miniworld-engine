// Experiment: independent WG with bounded raw-tri lifetime.
// Experiment: independent full-channel DX warpgroups.
...
struct MaskCycleDX{uint64_t b0,b1,b2;uint32_t scale;int period;bool cached;};
template<int STRIDE> TMN_DEVI MaskCycleDX mask_cycle_dx(const Params& p,int first){
 MaskCycleDX v={0,0,0,0,0,false};int a=64*STRIDE,b=p.L;while(b){int r=a%b;a=b;b=r;}
 v.period=p.L/a;v.cached=v.period<=3;
 if(v.cached&&first<p.tiles)for(int z=0;z<v.period;++z){uint64_t bits=0;int j0=((first+z*STRIDE)*64)%p.L;
  for(int i=threadIdx.x%128;i<1024;i+=128){int cb=i/512,r=(i%512)/8,c=(i%8)*8,jr=j0+r;if(jr>=p.L)jr-=p.L;
   uint4 ds=ldg128(p.ds+jr*128+cb*64+c);uint32_t* dd=reinterpret_cast<uint32_t*>(&ds);
#pragma unroll
   for(int q=0;q<4;++q){uint32_t lo=dd[q]&65535u,hi=dd[q]>>16;int bit=8*(i/128)+2*q;
    bits|=(uint64_t(lo!=0)<<bit)|(uint64_t(hi!=0)<<(bit+1));v.scale|=lo|hi;}
  }
  if(z==0)v.b0=bits;else if(z==1)v.b1=bits;else v.b2=bits;
 }
 return v;
}
template<bool DW> TMN_DEVI void gate_backward_dx(const Params& p,uint8_t* s,int row,const MaskCycleDX& mask,int mi){
 int j0=row%p.L;uint64_t bits=mi==0?mask.b0:mi==1?mask.b1:mask.b2;
 for(int i=threadIdx.x%128;i<1024;i+=128){int cb=i/512,r=(i%512)/8,c=(i%8)*8,jr=j0+r;if(jr>=p.L)jr-=p.L;
  uint32_t sy=smem_u32(s+cb*8192)+swz128(r,c*2),sg=smem_u32(s+16384+cb*8192)+swz128(r,c*2);
  uint4 y=lds128(sy),g=lds128(sg),v,ds,dp,dg;
  if constexpr(DW)v=lds128(smem_u32(s+32768+cb*8192)+swz128(r,c*2));
  if(mask.cached){uint32_t* dd=reinterpret_cast<uint32_t*>(&ds);
#pragma unroll
   for(int q=0;q<4;++q){int bit=8*(i/128)+2*q;dd[q]=((bits>>bit)&1?mask.scale:0)|((bits>>(bit+1))&1?mask.scale<<16:0);}
  }else ds=ldg128(p.ds+jr*128+cb*64+c);
  uint32_t *yy=reinterpret_cast<uint32_t*>(&y),*gg=reinterpret_cast<uint32_t*>(&g),*vv=reinterpret_cast<uint32_t*>(&v),*dd=reinterpret_cast<uint32_t*>(&ds),*oo=reinterpret_cast<uint32_t*>(&dp),*zz=reinterpret_cast<uint32_t*>(&dg);
#pragma unroll
  for(int q=0;q<4;++q){float ya=bf16lo(yy[q])*bf16lo(dd[q]),yb=bf16hi(yy[q])*bf16hi(dd[q]),ga=bf16lo(gg[q]),gb=bf16hi(gg[q]);oo[q]=pack_bf16(ya*ga,yb*gb);
...
TMN_DEVI void dual_dgrad(const Params& p,uint8_t* sm,int m0,int slot){
 ...
 if(tid<64){mus[tid]=p.mean[m0+tid];rss[tid]=p.rs[m0+tid];}
 ...
 float mu[2]={mus[ra],mus[rb]},rs[2]={rss[ra],rss[rb]},v1[2][2]={},v2[2][2]={};
 uint32_t dn[4][4][4];
 static_for<4>([&](auto ni){constexpr int nlocal=decltype(ni)::value;int n=nlocal;
  ...
  static_for<4>([&](auto qi){constexpr int q=decltype(qi)::value;
   ...
   static_for<4>([&](auto ji){constexpr int j=decltype(ji)::value;
    ...
    float xa=__fmul_rn(__fsub_rn(bf16lo(fx[j]),mu[rr]),rs[rr]);
    float xb=__fmul_rn(__fsub_rn(bf16hi(fx[j]),mu[rr]),rs[rr]);
    float ha=bf16lo(dn[nlocal][q][j])*gam[c],hb=bf16hi(dn[nlocal][q][j])*gam[c+1];
    v1[nlocal/2][rr]+=ha*xa+hb*xb;v2[nlocal/2][rr]+=ha+hb;
   });
  });
 });
 // Preserve the two channel-half sum trees and BF16 dnorm rounding.
 ...
 float* tmp=reinterpret_cast<float*>(sm+65536+wi*8192);
 static_for<4>([&](auto ni){constexpr int nl=decltype(ni)::value;int n=nl;
  static_for<4>([&](auto qi){constexpr int q=decltype(qi)::value;uint32_t out[4],fx[4];
   ...
   static_for<2>([&](auto pi){constexpr int pair=decltype(pi)::value;constexpr int j=pair*2;
    ...
    uint32_t xa=fx[j],xb=fx[j+1],da=dn[nl][q][j],db=dn[nl][q][j+1];
...
#pragma unroll
 ...
 if(tid==0){for(int ch=0;ch<256;ch+=16)tma_store_3d(&p.dtri,sx+ch*128,m0,ch,0);tma_store_commit();tma_store_wait_all();}
...
// Independent WG slots. Each slot is reused only after its own TMA store
