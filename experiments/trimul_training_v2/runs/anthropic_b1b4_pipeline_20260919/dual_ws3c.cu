// Experiment: dual_ws3c
// One variable on dual_ln_prefetch: DX warp specialization only. DX CTAs: WG0 producer (56 regs:
// TMA, mean/rstd prefetch, B1) + two LN consumer WGs (224 regs). DW CTAs keep the parent fused
// schedule on the two consumer WGs (240 regs); their producer WG idles at24 registers.
...
//   DX: WG0 producer (TMA, stats prefetch, B1) + WG1/WG2 LN consumers; B1(t+1) overlaps MMA/LN(t).
//   DW: fused B1+WGMMA on WG1/WG2 exactly as dual_ln_prefetch (2-slot depth suits its short MMA phase). Same forward saves, BF16 dp/dg/dnorm
...
template<bool DW> TMN_DEVI void gate_backward_fused(const Params& p,uint8_t* s,int row,const MaskCycle256& mask,int mi){
 const int ct=threadIdx.x-128; // consumer-local thread index (threads128..383)
...
TMN_DEVI void weight_fused(const Params& p,uint8_t* sm,uint64_t* full){
 ...
 MaskCycle256 mask=mask_cycle256<DWCOUNT>(p,split);int mi=0,round=0;float acc[3][64]={};
 for(int it=split;it<p.tiles;it+=DWCOUNT,++round){int slot=round&1;
  ...
  gate_backward_fused<true>(p,s,it*64,mask,mi);
