// Experiment: dual_ws3d
// Warp specialization with consumer-issued refills: the slot for round r+2 is re-armed and its TMA
// issued by consumer thread 0 as soon as both consumer WGs finished round r (the parent kernel's
// issue point), so no empty barrier exists and the producer's B1(r+1) fully overlaps MMA/LN(r).
...
// Slot handshake: full[s] (TMA -> producer), ready[s] (producer -> consumers); consumers re-issue TMA into s.
...
// Producer loop shared by both roles: rounds 0 and 1 are issued here; later refills come from the consumers.
template<bool DW> TMN_DEVI void producer(const Params& p,uint8_t* sm,uint64_t* full,uint64_t* ready,int split,int stride){
 ...
 for(int it=split;it<p.tiles;it+=stride,++round){int slot=round&1;
  mbar_wait(full+slot,(round>>1)&1); // Round r data: issued by the prologue (r<2) or by consumers after round r-2.
...
TMN_DEVI void weight_consumer(const Params& p,uint8_t* sm,uint64_t* full,uint64_t* ready,int split){
 ...
 for(int it=split;it<p.tiles;it+=DWCOUNT,++round){int slot=round&1;
  ...
  consync(); // Both consumer WGs' MMAs finished reading this slot.
  if(ct==0&&it+2*DWCOUNT<p.tiles){mbar_arrive_expect_tx(full+slot,98304u);issue_slot<true>(p,sm,full,slot,it+2*DWCOUNT,false);}
 }
...
TMN_DEVI void input_consumer(const Params& p,uint8_t* sm,uint64_t* full,uint64_t* ready,int split){
 int round=0;const int ct=threadIdx.x-128;
 for(int it=split;it<p.tiles;it+=DXCOUNT,++round){int slot=round&1;
  mbar_wait(ready+slot,(round>>1)&1);
  dual_dgrad(p,sm,it*64,slot);
  consync(); // Both WGs' dtri TMA stores finished reading this slot.
  if(ct==0&&it+2*DXCOUNT<p.tiles){mbar_arrive_expect_tx(full+slot,65536u);issue_slot<false>(p,sm,full,slot,it+2*DXCOUNT,false);}
 }
}
extern "C" __global__ __launch_bounds__(384,1)
void dual_b1b4(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t full[2],ready[2];
 ...
 if(threadIdx.x==0){
  mbar_init(full,1);mbar_init(full+1,1);mbar_init(ready,128);mbar_init(ready+1,128);fence_barrier_init();
 }
 ...
 if(prod){
  ...
  if(dw)producer<true>(p,sm,full,ready,split,stride);else producer<false>(p,sm,full,ready,split,stride);
 }else{
  setmaxnreg_inc<WS_CONS_REGS>();
  if(dw)weight_consumer(p,sm,full,ready,split);else input_consumer(p,sm,full,ready,split);
 }
