// Experiment: dual_ws3e_probe (diagnostic only: per-CTA globaltimer stamps into p.groupln[cta*12+i])
// Warp specialization, CUTLASS/Anthropic style. Producer WG0: warp0 = TMA issuer (waits empty[slot],
// re-arms full[slot], issues round r+2 without blocking B1), warps1-3 (96 threads) = B1 + stats prefetch.
// Consumer WG1/WG2 release a slot by arriving on empty[slot] after their MMAs (DW) or TMA stores (DX):
// no consumer CTA barrier per tile. B1(r+1) overlaps MMA/LN(r); refills are issued as early as possible.
...
TMN_DEVI void st_issuer_done(const Params& p){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t)::"memory");reinterpret_cast<unsigned long long*>(p.groupln)[blockIdx.x*12+2]=t;}
TMN_DEVI void st_b1_done(const Params& p){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t)::"memory");reinterpret_cast<unsigned long long*>(p.groupln)[blockIdx.x*12+1]=t;}
TMN_DEVI void st_cons_done(const Params& p){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t)::"memory");reinterpret_cast<unsigned long long*>(p.groupln)[blockIdx.x*12+3]=t;}
template<bool DW> TMN_DEVI void producer(const Params& p,uint8_t* sm,uint64_t* full,uint64_t* ready,uint64_t* empty,int split,int stride){
 ...
 if(threadIdx.x<32){
  ...
  if(threadIdx.x==0)st_issuer_done(p);
  return;
 }
 ...
 if(threadIdx.x==32)st_b1_done(p);
}
...
TMN_DEVI void weight_consumer(const Params& p,uint8_t* sm,uint64_t* ready,uint64_t* empty,int split){
 ...
 for(int it=split;it<p.tiles;it+=DWCOUNT,++round){int slot=round&1;
  ...
  mbar_arrive(empty+slot); // This thread's WG finished its MMAs on the slot; empty counts 256 arrivals.
 }
 if(ct==0)st_cons_done(p);
...
TMN_DEVI void input_consumer(const Params& p,uint8_t* sm,uint64_t* ready,uint64_t* empty,int split){
 ...
 if(threadIdx.x==128)st_cons_done(p);
}
TMN_DEVI unsigned long long phase_ns(){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t)::"memory");return t;}
extern "C" __global__ __launch_bounds__(384,1)
void dual_b1b4(__grid_constant__ const Params p){
 unsigned long long* st=reinterpret_cast<unsigned long long*>(p.groupln)+blockIdx.x*12;
 if(threadIdx.x==0)st[0]=phase_ns();
 ...
 if(threadIdx.x==0)st[4]=phase_ns();
