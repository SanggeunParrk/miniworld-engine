// Experiment: dual_ws3f_probe (diagnostic only: per-CTA globaltimer stamps into p.groupln[cta*12+i]; ratio4:7)
...
TMN_DEVI void stamp(const Params& p,int i){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t)::"memory");reinterpret_cast<unsigned long long*>(p.groupln)[blockIdx.x*12+i]=t;}
...
void dual_b1b4(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0)st[5]=phase_ns();
 ...
 if(threadIdx.x==0)st[6]=phase_ns();
