...
extern "C" __global__ __launch_bounds__(256,1) void flat_b1b4(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar;__shared__ uint64_t wp_bars[2];
 if(blockIdx.x%2){flat_wp(p,sm,wp_bars);return;}
 const int split=blockIdx.x/2,role=0,wi=threadIdx.x/128,tid=threadIdx.x%128,lane=tid%32,w=tid/32;
