"""Two-CTA cluster experiment. Original single-CTA sources remain unchanged."""
from pathlib import Path

R=Path(__file__).resolve().parent
old=(R/'dual.cu').read_text()
head=old[:old.index('// LN row reductions')]
head+='''#include <cooperative_groups.h>
...
'''
dx=old[old.index('TMN_DEVI void dual_dgrad'):old.index('TMN_DEVI void dual_load')]
dx=dx.replace('uint64_t* bar,int m0,bool next','int m0,int slot')
dx=dx.replace('sm+196608,*sx=sm+98304','sm+65536,*sx=sm+slot*98304+32768')
dx=dx.replace('sm+32768','sm+slot*98304+16384',1)
dx=dx.replace('sm+131072+n*16384','sm+163840+n*16384')
dx=dx.replace('sm+(k/4)*8192','sm+slot*98304+(k/4)*8192')
lo=dx.index(' if(next&&threadIdx.x==0)');hi=dx.index(' float* red=',lo)
dx=dx[:lo]+dx[hi:]
head+=dx
head+='''
...
'''
(R/'dual_cluster.cu').write_text('// Experimental two-CTA TMA multicast + double-buffer pipeline.\n'+head)
(R/'dual_cluster.py').write_text('from dual_experiment import *\nclass Unified(Experiment):\n def __init__(self,d,dy,s,count=128,part=2):\n  super().__init__(d,dy,s,count,part,"dual_cluster")\n')
print('Generated dual_cluster.cu and dual_cluster.py')
