...
old=(r/'dual_optimized.cu').read_text()
head=old[old.index('#include'):old.index('// LN row reductions')]
tail=old[old.index(' static_for<3>([&](auto ni){constexpr int n=decltype(ni)::value;int t=wi*3+n,tile='):]
body=r'''
#define ROW_TILE 32
...
'''
(r/'dual_stage32.cu').write_text('// SPDX-License-Identifier: Apache-2.0\n// Anthropic primitives, MiniWorld 32-row two-stage B1-B4 experiment.\n'+head+body+tail)
(r/'dual_stage32.launch.json').write_text(json.dumps({'row_tile':32}))
(r/'dual_stage32.py').write_text('from dual_experiment import Experiment\nclass Plan(Experiment):\n    def __init__(self,*args,**kwargs):\n        super().__init__(*args,source="dual_stage32",**kwargs)\n')
