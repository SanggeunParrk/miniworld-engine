...
with torch.no_grad():
 for n in (384,768):
  ...
  plans={k:Experiment(d,dy,s,132,2,k) for k in ['dual_optimized','dual_balanced']}
  ...
  records.append(record);(R/'balanced-paired-results.json').write_text(json.dumps(records,indent=2))
