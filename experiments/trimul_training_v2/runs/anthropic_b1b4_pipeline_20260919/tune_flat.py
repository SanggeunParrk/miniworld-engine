...
with torch.no_grad():
 for n in (384,768):
  ...
  tt=paired({k:graph(p) for k,p in plans.items()},reps=80,rounds=10);print('RESULT',n,{k:round(v['median_us'],2) for k,v in tt.items()},flush=True);(R/f'flat-tune-{n}.json').write_text(json.dumps(tt,indent=2))
