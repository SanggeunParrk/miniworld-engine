...
def ukernel(count,part=0):
 inc=T._upstream()/'csrc';source=R/'unified.cu';flags=['-std=c++17','-O3','-arch=sm_90a','--cubin','-lineinfo','-Xptxas=-v','-I'+str(inc),'-DROLE=0',f'-DUCOUNT={count}',f'-DPART_ONLY={part}']
 ...
 L=T._launch_module();drv=L.BlockDriver(device=0);mod=drv.load(path.read_bytes());unit=L.Unit('unified','sm_90a',0,drv.drv,mod,{},str(path));k=unit.kernel('unified_b1b4');k.set_max_dynamic_smem(188416);print('CUBIN',str(path),flush=True);return k,unit.kernel('unified_reduce')
class Unified(Plan):
