...
if __name__=='__main__':
 ap=argparse.ArgumentParser();ap.add_argument('--source',default='front_selected');ap.add_argument('--lengths',type=int,nargs='+',default=[64,384,768]);ap.add_argument('--counts',type=int,nargs='+',default=[66,132]);ap.add_argument('--parts',type=int,nargs='+',default=[1,2]);ap.add_argument('--dropouts',type=float,nargs='+',default=[0,.25]);ap.add_argument('--splits',type=int,default=15);ap.add_argument('--output',required=True);args=ap.parse_args();records=[]
 with torch.no_grad():
   ...
   for drop in args.dropouts:
    ...
    for count in args.counts:
     for part in args.parts:
      sp=max(1,args.splits*count//132);p=Plan(a,count,sp,part,source=args.source)
