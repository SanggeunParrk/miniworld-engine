...
TMN_DEVI void dump_cached(const Params& p,const uint32_t (&pw)[2][12][4],int tile,int tid){uint64_t base=(uint64_t)p.debugdc+((size_t)tile*256+tid)*96*4;
 static_for<2>([&](auto si){constexpr int side=decltype(si)::value;static_for<12>([&](auto ki){constexpr int k=decltype(ki)::value;asm volatile("{.reg .b64 a; add.u64 a,%0,%5; st.global.v4.u32 [a],{%1,%2,%3,%4};}"::"l"(base),"r"(pw[side][k][0]),"r"(pw[side][k][1]),"r"(pw[side][k][2]),"r"(pw[side][k][3]),"n"((side*12+k)*16):"memory");});});}
TMN_DEVI void consumer(const Params& p,uint8_t* sm,Bars* b,const float* gamma,float* mus,float* rss){
 ...
 for(int tile=blockIdx.x;tile<p.tiles;tile+=UCOUNT,++round){static_for<2>([&](auto si){constexpr int side=decltype(si)::value;static_for<12>([&](auto ki){constexpr int k=decltype(ki)::value;fence_regs(pw[side][k]);});});int row=tile*64,ph=round&1;uint32_t gatepacked[16];mbar_wait(b->tx+2,ph);
  ...
  static_for<2>([&](auto si){constexpr int side=decltype(si)::value;
   ...
   static_for<2>([&](auto hi){constexpr int half=decltype(hi)::value;if constexpr(side==0&&half==0)dump_cached(p,pw,tile,tid);uint32_t tail[4][4];if constexpr(half==1){mbar_wait(b->tx+5,side);static_for<4>([&](auto ti){constexpr int q=decltype(ti)::value;int mat=lane/8,rr=w*16+(lane%8)+((mat&1)?8:0),kk=(12+q-12)*16+((mat&2)?8:0);ldsm_x4(tail[q],smem_u32(sm+65536+wi*8192)+swz128(rr,kk*2));});}uint64_t bd=smem_desc(smem_u32(sm+half*49152),16,1024,1);fence_regs(acc);wgmma_fence();
  ...
  fence_proxy_async();csync();if(tid==0){store2d(&p.dx,sm+16384,0,row);store2d(&p.dx,sm+24576,64,row);tma_store_commit();tma_store_wait_all();}release(b,3);static_for<2>([&](auto si){constexpr int side=decltype(si)::value;static_for<12>([&](auto ki){constexpr int k=decltype(ki)::value;fence_regs(pw[side][k]);});});
 }
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 int wg=__shfl_sync(0xffffffff,threadIdx.x/128,0);if(wg<2){setmaxnreg_dec<32>();producer(p,sm,&b);}else{setmaxnreg_inc<224>();consumer(p,sm,&b,gamma,mus,rss);}fullsync();
