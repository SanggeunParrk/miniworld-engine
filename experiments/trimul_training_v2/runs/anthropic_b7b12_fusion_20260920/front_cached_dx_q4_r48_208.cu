...
TMN_DEVI void glu_cached(const Params& p,uint8_t* sm,int group,int row){
 ...
 static_for<4>([&](auto hi){constexpr int half=decltype(hi)::value;uint32_t pp[4];
  static_for<4>([&](auto qi){constexpr int q=decltype(qi)::value;unsigned i=tid+(q+half*4)*256,c=i/32,r=(i%32)*2;
  ...
  static_for<4>([&](auto qi){constexpr int q=decltype(qi)::value;unsigned i=tid+(q+half*4)*256,c=i/32,r=(i%32)*2;*reinterpret_cast<uint32_t*>(s+swz128(c,r*2))=pp[q];});
 });
}
