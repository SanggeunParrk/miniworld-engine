...
TMN_DEVI void consumer(const Params& p,uint8_t* sm,Bars* b,const float* gamma,float* mus,float* rss){
 ...
 for(int tile=blockIdx.x;tile<p.tiles;tile+=UCOUNT,++round){static_for<2>([&](auto si){constexpr int side=decltype(si)::value;static_for<12>([&](auto ki){constexpr int k=decltype(ki)::value;fence_regs(pw[side][k]);});});uint32_t ck=0;static_for<2>([&](auto si){constexpr int side=decltype(si)::value;static_for<12>([&](auto ki){constexpr int k=decltype(ki)::value;static_for<4>([&](auto qi){constexpr int q=decltype(qi)::value;ck^=pw[side][k][q];});});});reinterpret_cast<uint32_t*>(p.partw)[tile*256+tid]=ck;int row=tile*64,ph=round&1;uint32_t gatepacked[16];mbar_wait(b->tx+2,ph);
