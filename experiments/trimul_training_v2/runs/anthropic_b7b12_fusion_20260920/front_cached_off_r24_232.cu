...
void front_b7b12(__grid_constant__ const Params p){
 ...
 int wg=__shfl_sync(0xffffffff,threadIdx.x/128,0);if(wg<2){setmaxnreg_dec<24>();producer(p,sm,&b);}else{setmaxnreg_inc<232>();consumer(p,sm,&b,gamma,mus,rss);}fullsync();
