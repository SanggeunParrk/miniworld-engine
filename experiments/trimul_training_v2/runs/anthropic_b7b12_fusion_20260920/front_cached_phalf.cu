 ...
 for(int tile=blockIdx.x;tile<p.tiles;tile+=UCOUNT,++round){int row=tile*64,ph=round&1;uint32_t gatepacked[16];mbar_wait(b->tx+2,ph);
  ...
  static_for<2>([&](auto si){constexpr int side=decltype(si)::value;
   ...
   static_for<2>([&](auto hi){constexpr int half=decltype(hi)::value;uint32_t tail[4][4];if constexpr(half==1){mbar_wait(b->tx+5,side);static_for<4>([&](auto ti){constexpr int q=decltype(ti)::value;int mat=lane/8,rr=w*16+(lane%8)+((mat&1)?8:0),kk=(12+q-12)*16+((mat&2)?8:0);ldsm_x4(tail[q],smem_u32(sm+65536+wi*8192)+swz128(rr,kk*2));});}uint64_t bd=smem_desc(smem_u32(sm+half*49152),16,1024,1);fence_regs(acc);wgmma_fence();
    static_for<8>([&](auto ki){constexpr int k=decltype(ki)::value;if constexpr(half*8+k<12)mma_projection_rs<k*2048>(acc,pw[side][half*8+k],uint32_t(bd),uint32_t(bd>>32),1);else mma_projection_rs<k*2048>(acc,tail[half*8+k-12],uint32_t(bd),uint32_t(bd>>32),1);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);if(tid%128==0)p.partw[tile*8+side*4+half*2+wi]=acc[0];release(b,half);
   });
  });
