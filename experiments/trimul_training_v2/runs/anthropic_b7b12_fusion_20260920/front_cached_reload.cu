...
TMN_DEVI void producer(const Params& p,uint8_t* sm,Bars* b){
 load_resident(p,sm,b);
 int round=0;for(int tile=blockIdx.x;tile<p.tiles;tile+=UCOUNT,++round){int row=tile*64,ph=round&1;
  fullsync();load_projection(p,sm,b,0);if(threadIdx.x==0)mbar_wait(b->empty+2,ph);load_projection(p,sm,b,1);if(threadIdx.x==0)mbar_wait(b->empty+2,ph^1);psync();load_gate(p,sm,b,row);
  ...
  if(threadIdx.x==0){mbar_wait(b->empty,1);load_ln(p,sm,b,row);mbar_wait(b->empty+1,1);}
 }
}
TMN_DEVI void consumer(const Params& p,uint8_t* sm,Bars* b,const float* gamma,float* mus,float* rss){
 ...
 for(int tile=blockIdx.x;tile<p.tiles;tile+=UCOUNT,++round){int row=tile*64,ph=round&1;fullsync();static_for<2>([&](auto si){constexpr int side=decltype(si)::value;mbar_wait(b->tx+2,side^ph);load_frag_bf16<12,8192>(pw[side],smem_u32(sm+wi*32768),w*16,lane);release(b,2);});uint32_t gatepacked[16];mbar_wait(b->tx+2,ph);
