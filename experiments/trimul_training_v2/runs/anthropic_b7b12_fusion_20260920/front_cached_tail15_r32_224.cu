...
TMN_DEVI void consumer(const Params& p,uint8_t* sm,Bars* b,const float* gamma,float* mus,float* rss){
 int tid=cid(),wi=tid/128,lane=tid%32,w=(tid/32)%4,cr=w*16+lane/4;uint32_t pw[2][15][4];
 static_for<2>([&](auto si){constexpr int side=decltype(si)::value;mbar_wait(b->tx+2,side);load_frag_bf16<15,8192>(pw[side],smem_u32(sm+wi*32768),w*16,lane);release(b,2);});mbar_wait(b->tx+4,0);
 ...
 for(int tile=blockIdx.x;tile<p.tiles;tile+=UCOUNT,++round){int row=tile*64,ph=round&1;uint32_t gatepacked[16];mbar_wait(b->tx+2,ph);
  ...
  static_for<2>([&](auto si){constexpr int side=decltype(si)::value;
   ...
   static_for<2>([&](auto hi){constexpr int half=decltype(hi)::value;uint32_t tail[1][4];if constexpr(half==1){mbar_wait(b->tx+5,side);static_for<1>([&](auto ti){constexpr int q=decltype(ti)::value;int mat=lane/8,rr=w*16+(lane%8)+((mat&1)?8:0),kk=(15+q-12)*16+((mat&2)?8:0);ldsm_x4(tail[q],smem_u32(sm+65536+wi*8192)+swz128(rr,kk*2));});}uint64_t bd=smem_desc(smem_u32(sm+half*49152),16,1024,1);fence_regs(acc);wgmma_fence();
    static_for<8>([&](auto ki){constexpr int k=decltype(ki)::value;if constexpr(half*8+k<15)mma_projection_rs<k*2048>(acc,pw[side][half*8+k],uint32_t(bd),uint32_t(bd>>32),1);else mma_projection_rs<k*2048>(acc,tail[half*8+k-15],uint32_t(bd),uint32_t(bd>>32),1);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);release(b,half);
   });
  });
 ...
 static_for<2>([&](auto si){constexpr int side=decltype(si)::value;static_for<15>([&](auto ki){constexpr int k=decltype(ki)::value;fence_regs(pw[side][k]);});});
}
