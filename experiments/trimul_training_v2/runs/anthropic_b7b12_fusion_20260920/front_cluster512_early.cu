...
TMN_DEVI void producer(const Params& p,uint8_t* sm,Bars* b){
 ...
 for(int r=0;r<total;++r){int q=r%4,round=r/4,row=(cid*4+q+round*CLUSTERS*4)*64;uint8_t* out=sm+(q%2)*32768;uint8_t* inp=sm+65536+(r%2)*65536;
  if(threadIdx.x==0){if(r>=2)mbar_wait(b->empty+q%2,(r/2-1)&1);if(round)mbar_wait(b->free+q,(round-1)&1);}mbar_wait(b->io+(r%2),(r/2)&1);allsync();
   ...
   int half=wi%2;fence_regs(acc);wgmma_fence();static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_weight128(acc,smem_desc(smem_u32(out+(wi/2)*16384+half*8192+k*32),16,1024,1),smem_desc(smem_u32(inp+49152+k*2048),8192,1024,1),r%segment>0||k>0);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);allsync();
   if((r+1)%segment==0||r+1==total){float* dest=p.partw+((group*CLUSTERS+cid)*2+r/segment)*32768+(wi/2)*16384;
...
TMN_DEVI void consumer(const Params& p,uint8_t* sm,Bars* b,const float* gamma){
 ...
 for(int tile=split;tile<p.tiles;tile+=CLUSTERS*4,++round){int row=tile*64;uint32_t gate_packed[8];float acc[16]={};
  ...
  if(threadIdx.x==0){if(tile+CLUSTERS*4<p.tiles)mbar_arrive_expect_tx(&b->copy,131072);for(int group=0;group<4;++group)remote_arrive(b->free+slot,group);}uint8_t* lnsm=sm+131072;
  if(threadIdx.x==0){mbar_arrive_expect_tx(bar,32768);
   for(int c=0;c<2;++c){tma_load_2d(lnsm+c*8192,&p.x,bar,c*64,row);tma_load_2d(lnsm+16384+c*8192,&p.res,bar,c*64,row);}}
  mbar_wait(bar,1);
  ...
  static_for<2>([&](auto qi){constexpr int q=decltype(qi)::value;
   static_for<4>([&](auto ji){constexpr int j=decltype(ji)::value;int rr=j&1,c=q*16+2*(lane%4)+8*(j>>1),r=rr?rb:ra;
    float xa=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,r,c+(wi%2)*32),mu[rr]),rs[rr]),xb=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,r,c+1+(wi%2)*32),mu[rr]),rs[rr]);
  ...
  static_for<2>([&](auto qi){constexpr int q=decltype(qi)::value;
   static_for<2>([&](auto pi){constexpr int pair=decltype(pi)::value;int j=pair*2,c=q*16+2*(lane%4)+8*pair,gc=wi*32+c;
    float xaa=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,ra,c+(wi%2)*32),mu[0]),rs[0]),xab=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,ra,c+1+(wi%2)*32),mu[0]),rs[0]);
    float xba=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,rb,c+(wi%2)*32),mu[1]),rs[1]),xbb=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,rb,c+1+(wi%2)*32),mu[1]),rs[1]);
    ...
    put(lnsm+(wi/2)*8192,ra,c+(wi%2)*32,bf16lo(outa)+get(lnsm+16384+(wi/2)*8192,ra,c+(wi%2)*32));put(lnsm+(wi/2)*8192,ra,c+1+(wi%2)*32,bf16hi(outa)+get(lnsm+16384+(wi/2)*8192,ra,c+1+(wi%2)*32));
    put(lnsm+(wi/2)*8192,rb,c+(wi%2)*32,bf16lo(outb)+get(lnsm+16384+(wi/2)*8192,rb,c+(wi%2)*32));put(lnsm+(wi/2)*8192,rb,c+1+(wi%2)*32,bf16hi(outb)+get(lnsm+16384+(wi/2)*8192,rb,c+1+(wi%2)*32));
