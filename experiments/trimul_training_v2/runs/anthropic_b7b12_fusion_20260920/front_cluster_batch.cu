...
TMN_DEVI void producer(const Params& p,uint8_t* sm,Bars* b){
 ...
 for(int r=0;r<total;++r){int q=r%4,round=r/4,row=(cid*4+q+round*CLUSTERS*4)*64;uint8_t* out=sm+(q%2)*32768;uint8_t* inp=sm+65536+(r%2)*65536;
  if(threadIdx.x==0){if(r>=2)mbar_wait(b->empty+q%2,(r/2-1)&1);if(round)mbar_wait(b->free+q,(round-1)&1);}mbar_wait(b->io+(r%2),(r/2)&1);allsync();
   ...
   fence_regs(acc[0]);fence_regs(acc[1]);wgmma_fence();static_for<2>([&](auto hi){constexpr int half=decltype(hi)::value;
 ...
 for(int tile=split;tile<p.tiles;tile+=CLUSTERS*4,++round){int row=tile*64;uint32_t gate_packed[16];float acc[32]={};
  ...
  for(int side=0;side<2;++side){
   for(int kind=0;kind<2;++kind){load_weight(p,sm,b,side,kind,0);load_weight(p,sm,b,side,kind,1);
    mbar_wait(b->weight,kind);mbar_wait(b->weight+1,kind);fence_regs(acc);wgmma_fence();for(int half=0;half<2;++half){uint8_t* as=sm+(side*2+half)*32768+kind*16384;uint8_t* ws=sm+131072+half*32768+wi*16384;
     static_for<8>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input64(acc,smem_desc(smem_u32(as+k*2048),16,1024,1),smem_desc(smem_u32(ws+(k/4)*8192+(k%4)*32),16,1024,1),side>0||kind>0||half>0||k>0);});
