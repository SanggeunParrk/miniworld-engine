...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t));reinterpret_cast<unsigned long long*>(p.counts+2)[blockIdx.x*2]=t;}if(blockIdx.x%8<4)producer(p,sm,&b);else consumer(p,sm,&b,gamma);if(threadIdx.x==0){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t));reinterpret_cast<unsigned long long*>(p.counts+2)[blockIdx.x*2+1]=t;}
