...
TMN_DEVI float fast_sigmoid(float g){return math::rcp_approx_ftz(__fadd_rn(1.f,math::ex2_approx(__fmul_rn(-g,1.4426950408889634f))));}
...
TMN_DEVI void glu_pair(const Params& p,uint8_t* s,int i,int row,float ma,float mb,uint32_t& dg,uint32_t& dp){
 ...
 float ga=fast_sigmoid(bf16lo(gl)),gb=fast_sigmoid(bf16hi(gl));
