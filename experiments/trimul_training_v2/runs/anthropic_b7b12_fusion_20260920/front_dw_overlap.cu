...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 if(split<p.tiles){mbar_wait(bar,0);glu<false>(p,sm,split*64,group);}
 int round=0;
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++round){
  int slot=round&1;uint8_t* s=sm+slot*DW_SLOT;
  ...
  // GLU for the next stage overlaps current WGMMA; stages are disjoint.
  if(tile+DW_SPLITS<p.tiles){int next=slot^1;mbar_wait(bar+next,((round+1)/2)&1);glu<false>(p,sm+next*DW_SLOT,(tile+DW_SPLITS)*64,group);}
