...
TMN_DEVI void glu_small(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row){
 ...
 #pragma unroll 4
 for(unsigned q=0;q<4;++q){unsigned i=tid+q*512,c=i/32,r=(i%32)*2;
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 int rounds=(p.tiles-split+DW_SPLITS-1)/DW_SPLITS,segmentRounds=(rounds+1)/2,r=0;float acc[32]={};
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(b+slot,(r/2)&1);glu_small(p,s,s+40960,s+49152,tile*64);
  fence_regs(acc);wgmma_fence();static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;
   mma_weight64(acc,smem_desc(smem_u32(s+40960+(wi/2)*8192+k*32),16,1024,1),smem_desc(smem_u32(s+24576+(wi%2)*8192+k*2048),16,1024,1),r%segmentRounds>0||k>0);
  ...
  if((r+1)%segmentRounds==0||tile+DW_SPLITS>=p.tiles){float* out=p.partw+((group*DW_SPLITS+split)*2+r/segmentRounds)*16384+(wi/2)*8192+(wi%2)*64;
   static_for<8>([&](auto qi){constexpr int q=decltype(qi)::value;int rr=w*16+lane/4,c=q*8+2*(lane%4);stg64f(out+rr*128+c,acc[q*4],acc[q*4+1]);stg64f(out+(rr+8)*128+c,acc[q*4+2],acc[q*4+3]);});}
 }
}
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  int row=tile*64;uint32_t gate_packed[8];float acc[16]={};
  ...
  static_for<8>([&](auto ki){constexpr int k=decltype(ki)::value;
   mma_gate32(gate,smem_desc(smem_u32(sm+(k/4)*8192+(k%4)*32),16,1024,1),smem_desc(smem_u32(sm+16384+(wi/2)*16384+(wi%2)*4096+(k/4)*8192+(k%4)*32),16,1024,1),k>0);
  ...
  for(int side=0;side<2;++side){
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);glu_small(p,s,s+16384,sm+81920+h*8192,row);
    fence_regs(acc);wgmma_fence();static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input32(acc,smem_desc(smem_u32(s+16384+k*2048),16,1024,1),smem_desc(smem_u32(s+24576+wi*4096+k*32),16,1024,1),side>0||h>0||k>0);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);allsync();if(h<2)load_g(p,sm,bar,row,side,h+2);
   }
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);fence_regs(acc);wgmma_fence();
    static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input32(acc,smem_desc(smem_u32(sm+81920+h*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+wi*4096+k*32),16,1024,1),1);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);allsync();if(h<2)load_p(p,sm,bar,side,h+2);if(side==1&&h==1)load_ln_next(p,sm+65536,bar+3,row);
   }
  }
  ...
  static_for<2>([&](auto qi){constexpr int q=decltype(qi)::value;
   static_for<4>([&](auto ji){constexpr int j=decltype(ji)::value;int rr=j&1,c=q*16+2*(lane%4)+8*(j>>1),r=rr?rb:ra;
    float xa=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,r,(wi%2)*32+c),mu[rr]),rs[rr]),xb=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,r,(wi%2)*32+c+1),mu[rr]),rs[rr]);
  ...
  static_for<2>([&](auto qi){constexpr int q=decltype(qi)::value;
   static_for<2>([&](auto pi){constexpr int pair=decltype(pi)::value;int j=pair*2,c=q*16+2*(lane%4)+8*pair,gc=wi*32+c;
    float xaa=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,ra,(wi%2)*32+c),mu[0]),rs[0]),xab=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,ra,(wi%2)*32+c+1),mu[0]),rs[0]);
    float xba=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,rb,(wi%2)*32+c),mu[1]),rs[1]),xbb=__fmul_rn(__fsub_rn(get(lnsm+(wi/2)*8192,rb,(wi%2)*32+c+1),mu[1]),rs[1]);
    ...
    uint32_t resa=pair_get(lnsm+16384+(wi/2)*8192,ra,(wi%2)*32+c),resb=pair_get(lnsm+16384+(wi/2)*8192,rb,(wi%2)*32+c);
    *reinterpret_cast<uint32_t*>(lnsm+(wi/2)*8192+swz128(ra,((wi%2)*32+c)*2))=pack_bf16(bf16lo(outa)+bf16lo(resa),bf16hi(outa)+bf16hi(resa));
    *reinterpret_cast<uint32_t*>(lnsm+(wi/2)*8192+swz128(rb,((wi%2)*32+c)*2))=pack_bf16(bf16lo(outb)+bf16lo(resb),bf16hi(outb)+bf16hi(resb));
