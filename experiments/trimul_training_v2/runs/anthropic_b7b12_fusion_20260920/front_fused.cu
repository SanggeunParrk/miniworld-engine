...
struct Params{
 CUtensorMap dl,dr,pre,xn,dg,wfront,wgate,x,res,dx;
...
TMN_DEVI void glu_pair(const Params& p,uint8_t* s,int i,int row,uint32_t& dg,uint32_t& dp){
 int c=i/32,r=(i%32)*2;
 uint32_t dy=pair_get(s+32768,c,r),gl=pair_get(s,c*2,r),pr=pair_get(s,c*2+1,r);
 float ma=__bfloat162float(p.mask[row+r]),mb=__bfloat162float(p.mask[row+r+1]);
...
TMN_DEVI void write_glu(uint8_t* out,int i,uint32_t g,uint32_t p){
 int c=i/32,r=(i%32)*2,cb=c/64,cc=c%64;
 put(out+cb*8192,r,cc,bf16lo(g));put(out+cb*8192,r+1,cc,bf16hi(g));
 put(out+16384+cb*8192,r,cc,bf16lo(p));put(out+16384+cb*8192,r+1,cc,bf16hi(p));
}
template<bool INPLACE> TMN_DEVI void glu(const Params& p,uint8_t* s,int row,int group){
 if constexpr(INPLACE){
  uint32_t gg[16],pp[16];
  static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;glu_pair(p,s,threadIdx.x+q*256,row,gg[q],pp[q]);});
  ...
  for(int i=threadIdx.x;i<4096;i+=256){uint32_t g,pr;glu_pair(p,s,i,row,g,pr);write_glu(s+65536,i,g,pr);}
 }
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 if(split+DW_SPLITS<p.tiles)issue_dw(p,sm,bar,1,split+DW_SPLITS,group);
 int round=0;
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++round){
  ...
  static_for<2>([&](auto ni){constexpr int n=decltype(ni)::value;fence_regs(acc[n]);wgmma_fence();
   static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;
    mma_ss128(acc[n],smem_desc(smem_u32(s+49152+wi*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+65536+n*16384+k*2048),8192,1024,1),round>0||k>0);
   });wgmma_commit();
  });wgmma_wait<0>();fence_regs(acc[0]);fence_regs(acc[1]);
  // WGMMA consumers completed before a producer refills this stage.
  allsync();if(tile+2*DW_SPLITS<p.tiles)issue_dw(p,sm,bar,slot,tile+2*DW_SPLITS,group);
 }
 static_for<2>([&](auto ni){constexpr int n=decltype(ni)::value;float* out=p.partw+(group*DW_SPLITS+split)*32768+n*16384;
...
TMN_DEVI void issue_dx(const Params& p,uint8_t* sm,uint64_t* bar,int slot,int row,int group){
 ...
 for(int kind=0;kind<2;++kind)for(int n=0;n<2;++n)for(int k=0;k<2;++k)
  tma_load_2d(s+49152+kind*32768+n*16384+k*8192,&p.wfront,bar+slot,side*512+kind*256+h+k*64,n*64);
}
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 for(int tile=split;tile<p.tiles;tile+=DXCOUNT){
  int row=tile*64;uint32_t gate_packed[16];float acc[32]={};int phase[2]={0,0};
  ...
  issue_gate(p,sm,bar,row);mbar_wait(bar,phase[0]);phase[0]^=1;
  {float gate[32]={};
  ...
  for(int group=0;group<4;++group){
   int slot=group&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,phase[slot]);phase[slot]^=1;
   glu<true>(p,s,row,group);
   static_for<2>([&](auto ni){constexpr int n=decltype(ni)::value;fence_regs(acc);wgmma_fence();
    static_for<8>([&](auto ki){constexpr int k=decltype(ki)::value;
     mma_dgrad(acc,smem_desc(smem_u32(s+n*16384+(k/4)*8192+(k%4)*32),16,1024,1),smem_desc(smem_u32(s+49152+n*32768+wi*16384+(k/4)*8192+(k%4)*32),16,1024,1),group>0||n>0||k>0);
    });wgmma_commit();wgmma_wait<0>();fence_regs(acc);
   });
   allsync();if(group+2<4)issue_dx(p,sm,bar,slot,row,group+2);
  }
  ...
  mbar_wait(bar,phase[0]);phase[0]^=1;
...
TMN_DEVI void reduce_at(const Params& p,int i){
 if(i<131072){int group=i/32768,j=i%32768,kind=j/16384,z=j%16384;float v=0;
  for(int b=0;b<DW_SPLITS;++b)v+=reinterpret_cast<volatile float*>(p.partw)[(group*DW_SPLITS+b)*32768+j];
