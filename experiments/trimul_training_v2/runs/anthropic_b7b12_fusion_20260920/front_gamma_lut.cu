...
__device__ float sigmoid_lut[65536];
extern "C" __global__ void init_sigmoid_lut(){unsigned int i=blockIdx.x*blockDim.x+threadIdx.x;if(i<65536)sigmoid_lut[i]=math::sigmoid_div(__uint_as_float(i<<16));}
#ifndef UCOUNT
#define UCOUNT 132
#endif
#ifndef DW_SPLITS
...
TMN_DEVI void glu_pair(const Params& p,uint8_t* s,int i,int row,float ma,float mb,uint32_t& dg,uint32_t& dp){
 ...
 float ga=__ldg(sigmoid_lut+(gl&65535)),gb=__ldg(sigmoid_lut+(gl>>16));
