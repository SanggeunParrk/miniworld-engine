...
template<bool INPLACE> TMN_DEVI void glu(const Params& p,uint8_t* s,int row,int group){
 ...
 if constexpr(INPLACE){
  ...
  uint32_t gg[16],pp[16];static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;glu_pair(p,s,threadIdx.x+q*256,row,ma,mb,gg[q],pp[q]);});static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;write_glu(s+65536,threadIdx.x+q*256,gg[q],pp[q]);});
 }
