...
TMN_DEVI void tma_last(void* dst,const CUtensorMap* map,uint64_t* bar,int c0,int c1){
 asm volatile("{ .reg .b64 policy; createpolicy.fractional.L2::evict_last.b64 policy, 1.0; cp.async.bulk.tensor.2d.shared::cluster.global.mbarrier::complete_tx::bytes.L2::cache_hint [%0], [%1, {%3,%4}], [%2], policy; }"::"r"(smem_u32(dst)),"l"(map),"r"(smem_u32(bar)),"r"(c0),"r"(c1):"memory");
}
TMN_DEVI void tma_first(void* dst,const CUtensorMap* map,uint64_t* bar,int c0,int c1){
 asm volatile("{ .reg .b64 policy; createpolicy.fractional.L2::evict_first.b64 policy, 1.0; cp.async.bulk.tensor.2d.shared::cluster.global.mbarrier::complete_tx::bytes.L2::cache_hint [%0], [%1, {%3,%4}], [%2], policy; }"::"r"(smem_u32(dst)),"l"(map),"r"(smem_u32(bar)),"r"(c0),"r"(c1):"memory");
}
...
TMN_DEVI void load_dw(const Params& p,uint8_t* sm,uint64_t* b,int slot,int row,int group){
 ...
 tma_last(s,&p.pre,b+slot,row,side*512+h*2);tma_last(s+16384,side?&p.dr:&p.dl,b+slot,row,h);
 for(int n=0;n<2;++n)tma_last(s+24576+n*8192,&p.xn,b+slot,n*64,row);
}
...
TMN_DEVI void load_g(const Params& p,uint8_t* sm,uint64_t* b,int row,int side,int h){
 ...
 tma_last(s,&p.pre,b+slot,row,side*512+h*128);tma_last(s+16384,side?&p.dr:&p.dl,b+slot,row,h*64);
 for(int n=0;n<2;++n)tma_last(s+24576+n*8192,side?&p.wrg:&p.wlg,b+slot,h*64,n*64);
}
TMN_DEVI void load_p(const Params& p,uint8_t* sm,uint64_t* b,int side,int h){
 ...
 for(int n=0;n<2;++n)tma_last(s+n*8192,side?&p.wr:&p.wl,b+slot,h*64,n*64);
}
TMN_DEVI void issue_gate(const Params& p,uint8_t* sm,uint64_t* bar,int row){
 ...
 for(int k=0;k<2;++k){tma_first(sm+k*8192,&p.dg,bar,k*64,row);
  for(int n=0;n<2;++n)tma_last(sm+16384+n*16384+k*8192,&p.wgate,bar,k*64,n*64);}
}
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 for(int tile=split;tile<p.tiles;tile+=DXCOUNT){
  ...
  if(threadIdx.x==0){mbar_arrive_expect_tx(bar,32768);
   for(int c=0;c<2;++c){tma_first(sm+c*8192,&p.x,bar,c*64,row);tma_first(sm+16384+c*8192,&p.res,bar,c*64,row);}}
  mbar_wait(bar,1);
