...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 int group=((unsigned(blockIdx.x)*131)%UCOUNT)/DW_SPLITS,split=((unsigned(blockIdx.x)*131)%UCOUNT)%DW_SPLITS,wi=threadIdx.x/128,lane=threadIdx.x%32,w=(threadIdx.x/32)%4;
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 int split=((unsigned(blockIdx.x)*131)%UCOUNT)-DWCOUNT,wi=threadIdx.x/128,lane=threadIdx.x%32,w=(threadIdx.x/32)%4;
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(((unsigned(blockIdx.x)*131)%UCOUNT)<DWCOUNT)weight_role(p,sm,bar);else input_role(p,sm,bar,gamma);
#if PART_ONLY == 2
 ...
 for(int i=((unsigned(blockIdx.x)*131)%UCOUNT)*256+threadIdx.x;i<131328;i+=UCOUNT*256)reduce_at(p,i);
 __threadfence();allsync();
 if(threadIdx.x==0&&atomicAdd(p.counts+1,1u)==UCOUNT-1){atomicExch(p.counts,0u);atomicExch(p.counts+1,0u);}
#endif
}
extern "C" __global__ void front_reduce(__grid_constant__ const Params p){reduce_at(p,((unsigned(blockIdx.x)*131)%UCOUNT)*256+threadIdx.x);}
