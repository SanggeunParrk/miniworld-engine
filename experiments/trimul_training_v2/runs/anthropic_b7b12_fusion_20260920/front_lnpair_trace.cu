...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){unsigned long long t;unsigned smid;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t));asm volatile("mov.u32 %0, %%smid;":"=r"(smid));reinterpret_cast<unsigned long long*>(p.counts+2)[blockIdx.x*2]=t;p.counts[2+4*UCOUNT+blockIdx.x]=smid;}allsync();
 if(blockIdx.x<DWCOUNT)weight_role(p,sm,bar);else input_role(p,sm,bar,gamma);
 allsync();if(threadIdx.x==0){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t));reinterpret_cast<unsigned long long*>(p.counts+2)[blockIdx.x*2+1]=t;}
