...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){unsigned long long tick;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(tick));reinterpret_cast<unsigned long long*>(p.counts+2)[blockIdx.x*2]=tick;}
 ...
 if(threadIdx.x==0){unsigned long long tick;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(tick));reinterpret_cast<unsigned long long*>(p.counts+2)[blockIdx.x*2+1]=tick;}
#if PART_ONLY == 2
