...
TMN_DEVI void glu_small(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row){
 ...
 #pragma unroll 1
 for(unsigned q=0;q<16;++q){unsigned i=tid+q*128,c=i/32,r=(i%32)*2;
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){load_dw(p,sm,b,0,tile*64,group);mbar_wait(b,r&1);glu_small(p,sm,sm+40960,sm+49152,tile*64);
  static_for<2>([&](auto qi){constexpr int kind=decltype(qi)::value;uint64_t ad=smem_desc(smem_u32(sm+40960+kind*8192),16,1024,1),bd=smem_desc(smem_u32(sm+24576),8192,1024,1);fence_regs(acc[kind]);wgmma_fence();static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_weight_off<k*32,k*2048>(acc[kind],uint32_t(ad),uint32_t(ad>>32),uint32_t(bd),uint32_t(bd>>32),r%segmentRounds>0||k>0);});wgmma_commit();wgmma_wait<0>();fence_regs(acc[kind]);});allsync();
  if((r+1)%segmentRounds==0||tile+DW_SPLITS>=p.tiles){static_for<2>([&](auto ki){constexpr int kind=decltype(ki)::value;float* out=p.partw+((group*DW_SPLITS+split)*2+r/segmentRounds)*16384+kind*8192;float* dst=out+(w*16+lane/4)*128+2*(lane%4);static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;store_off<q*8*4>(dst,acc[kind][q*4],acc[kind][q*4+1]);store_off<(8*128+q*8)*4>(dst,acc[kind][q*4+2],acc[kind][q*4+3]);});});}
 }
}
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){int row=tile*64,ph=round&1;uint32_t packed[32];float acc[64]={};
  ...
  if(tid==0){mbar_arrive_expect_tx(bar+1,32768);for(int c=0;c<2;++c){tma_load_2d(sm+c*8192,&p.x,bar+1,c*64,row);tma_load_2d(sm+16384+c*8192,&p.res,bar+1,c*64,row);}}mbar_wait(bar+1,1);
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 for(int i=blockIdx.x*128+threadIdx.x;i<131328;i+=UCOUNT*128)reduce_at(p,i);
