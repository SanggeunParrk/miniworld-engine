 ...
 for(int tile=split;tile<p.tiles;tile+=DXCOUNT){
  ...
  for(int side=0;side<2;++side){
   for(int half=0;half<2;++half){
    ...
    mbar_wait(bar+slot,phase[slot]);phase[slot]^=1;glu<true>(p,s,row,group);
    fence_regs(acc);wgmma_fence();
    static_for<8>([&](auto ki){constexpr int k=decltype(ki)::value;
     mma_dgrad(acc,smem_desc(smem_u32(s+(k/4)*8192+(k%4)*32),16,1024,1),smem_desc(smem_u32(s+49152+wi*16384+(k/4)*8192+(k%4)*32),16,1024,1),side>0||half>0||k>0);
    });wgmma_commit();wgmma_wait<0>();fence_regs(acc);allsync();
   }
   for(int half=0;half<2;++half){
    uint8_t* s=sm+half*DX_SLOT;fence_regs(acc);wgmma_fence();
    static_for<8>([&](auto ki){constexpr int k=decltype(ki)::value;
     mma_dgrad(acc,smem_desc(smem_u32(s+16384+(k/4)*8192+(k%4)*32),16,1024,1),smem_desc(smem_u32(s+81920+wi*16384+(k/4)*8192+(k%4)*32),16,1024,1),1);
    });wgmma_commit();wgmma_wait<0>();fence_regs(acc);allsync();
