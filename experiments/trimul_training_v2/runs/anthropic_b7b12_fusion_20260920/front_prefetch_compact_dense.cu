...
extern "C" __global__ void init_sigmoid_lut(){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i<3328){int ab=i%1664+0x3b80,sign=i/1664;sigmoid_lut_compact[i]=math::sigmoid(bf16lo(ab|(sign<<15)));}}
TMN_DEVI float gate_bits(unsigned raw){unsigned ab=raw&0x7fff;if(ab>=0x3b80&&ab<0x4200){unsigned i=ab-0x3b80+(raw>>15)*1664;return __ldg(sigmoid_lut_compact+i);}return math::sigmoid(bf16lo(raw));}
TMN_DEVI void glu_small(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row){
 ...
 for(unsigned q=0;q<8;++q){unsigned i=tid+q*256,c=i/32,r=(i%32)*2;
  ...
  uint32_t masked;asm("mul.rn.bf16x2 %0,%1,%2;":"=r"(masked):"r"(dy),"r"(mask));float ga=gate_bits(gl&0xffff),gb=gate_bits(gl>>16);
