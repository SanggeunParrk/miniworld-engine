...
extern "C" __global__ void init_sigmoid_lut(){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i<3328){int ab=i/2+0x3b80,sign=i%2;sigmoid_lut_compact[i]=math::sigmoid(bf16lo(ab|(sign<<15)));}}
TMN_DEVI float gate_bits(unsigned raw){unsigned ab=raw&0x7fff;if(ab>=0x3b80&&ab<0x4200){unsigned i=(ab-0x3b80)*2+(raw>>15);return __ldg(sigmoid_lut_compact+i);}return math::sigmoid(bf16lo(raw));}
