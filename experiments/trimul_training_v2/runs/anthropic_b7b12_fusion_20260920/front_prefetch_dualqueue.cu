...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 __shared__ int queue_next;int split=blockIdx.x-DWCOUNT,wi=threadIdx.x/128,lane=threadIdx.x%32,w=(threadIdx.x/32)%4;
 int ra=w*16+lane/4,rb=ra+8;float running=0;
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile=queue_next,++round){
  ...
  if(threadIdx.x==0)queue_next=DXCOUNT+atomicAdd(p.counts+10,1u);allsync();if(queue_next<p.tiles)issue_gate(p,sm,bar+2,queue_next*64);
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0&&atomicAdd(p.counts+1,1u)==UCOUNT-1){for(int i=0;i<EXTRA_COUNTS;++i)atomicExch(p.counts+2+i,0u);atomicExch(p.counts,0u);atomicExch(p.counts+1,0u);}
#endif
}
