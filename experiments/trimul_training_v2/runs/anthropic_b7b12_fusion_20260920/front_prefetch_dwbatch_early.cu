...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 for(int tile=split;tile<p.tiles;tile+=2*DW_SPLITS,++round){
  ...
  for(int half=0;half<2;++half)if(tile+(half+2)*DW_SPLITS<p.tiles)load_dw_data(p,sm+half*DW_SLOT,b+half,(tile+(half+2)*DW_SPLITS)*64,group);
  ...
  for(int half=0;half<2;++half)if(tile+(half+2)*DW_SPLITS<p.tiles)load_dw_xn(p,sm+half*DW_SLOT,b+half,(tile+(half+2)*DW_SPLITS)*64);
