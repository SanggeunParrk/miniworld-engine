...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile=queue_next,++round){
  ...
  if(threadIdx.x==0)queue_next=DXCOUNT+atomicAdd(p.counts+2,1u);allsync();if(queue_next<p.tiles)issue_gate(p,sm,bar+2,queue_next*64);
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0&&atomicAdd(p.counts+1,1u)==UCOUNT-1){atomicExch(p.counts+2,0u);atomicExch(p.counts,0u);atomicExch(p.counts+1,0u);}
#endif
}
