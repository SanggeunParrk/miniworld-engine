...
TMN_DEVI void load_dw_data(const Params& p,uint8_t* s,uint64_t* b,int row,int group){if(threadIdx.x)return;int side=group/4,h=(group%4)*64;mbar_arrive_expect_tx(b,40960);tma_load_2d(s,&p.pre,b,row,side*512+h*2);tma_load_2d(s+16384,side?&p.dr:&p.dl,b,row,h);}
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(b+slot,(r/2)&1);glu_small(p,s,s+40960,s+49152,tile*64);if(tile+2*DW_SPLITS<p.tiles)load_dw_data(p,s,b+slot,(tile+2*DW_SPLITS)*64,group);
  ...
  if(tile+2*DW_SPLITS<p.tiles)load_dw_xn(p,s,b+slot,(tile+2*DW_SPLITS)*64);
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  ...
  for(int side=0;side<2;++side){
   if(side==0)load_g(p,sm,bar,row,side,0);load_g(p,sm,bar,row,side,1);
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);glu_small(p,s,s+16384,sm+81920+h*8192,row);
    fence_regs(acc);wgmma_fence();static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input64(acc,smem_desc(smem_u32(s+16384+k*2048),16,1024,1),smem_desc(smem_u32(s+24576+wi*8192+k*32),16,1024,1),side>0||h>0||k>0);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);allsync();if(h<2)load_g(p,sm,bar,row,side,h+2);if(h==2)load_p(p,sm,bar,side,0);
   }
   load_p(p,sm,bar,side,1);
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);fence_regs(acc);wgmma_fence();
    static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input64(acc,smem_desc(smem_u32(sm+81920+h*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+wi*8192+k*32),16,1024,1),1);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);allsync();if(h<2)load_p(p,sm,bar,side,h+2);if(side==1&&h==1)load_ln_next(p,sm+65536,bar+3,row);if(side==0&&h==2)load_g(p,sm,bar,row,1,0);
   }
  }
