...
TMN_DEVI void glu_input(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row,uint32_t mask){
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  int row=tile*64;uint32_t tilemask=*reinterpret_cast<const uint32_t*>(p.mask+row+(threadIdx.x%32)*2);uint32_t gate_packed[16];float acc[32]={};
  ...
  for(int side=0;side<2;++side){
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);glu_input(p,s,s+16384,sm+81920+h*8192,row,tilemask);
