...
TMN_DEVI void load_dw(const Params& p,uint8_t* sm,uint64_t* b,int slot,int row,int group){
 if(threadIdx.x)return;asm volatile("prefetch.global.L2 [%0];"::"l"(p.mask+row):"memory");uint8_t* s=sm+slot*DW_SLOT;int side=group/4,h=(group%4)*64;mbar_arrive_expect_tx(b+slot,40960);
...
TMN_DEVI void issue_gate(const Params& p,uint8_t* sm,uint64_t* bar,int row){
 if(threadIdx.x)return;asm volatile("prefetch.global.L2 [%0];"::"l"(p.mask+row):"memory");mbar_arrive_expect_tx(bar,49152);
