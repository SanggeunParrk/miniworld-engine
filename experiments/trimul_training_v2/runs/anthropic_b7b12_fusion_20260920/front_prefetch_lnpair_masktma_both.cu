...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b,uint8_t* masksm){
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(b+slot,(r/2)&1);glu_shared(p,s,s+40960,s+49152,tile*64,masksm+slot*128);
...
TMN_DEVI void issue_gate(const Params& p,uint8_t* sm,uint64_t* bar,int row,uint8_t* masksm){
 if(threadIdx.x)return;mbar_arrive_expect_tx(bar,49280);mask_load(p,masksm,bar,row);
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,uint8_t* masksm){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64,masksm);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  ...
  for(int side=0;side<2;++side){
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);glu_shared(p,s,s+16384,sm+81920+h*8192,row,masksm);
  ...
  if(tile+DXCOUNT<p.tiles)issue_gate(p,sm,bar+2,(tile+DXCOUNT)*64,masksm);
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(blockIdx.x<DWCOUNT)weight_role(p,sm,bar,masksm);else input_role(p,sm,bar,gamma,masksm);
#if PART_ONLY == 2
