...
TMN_DEVI float fast_gate(float x){float e,r;asm("ex2.approx.ftz.f32 %0,%1;":"=f"(e):"f"(__fmul_rn(x,-1.4426950408889634f)));e=__fadd_rn(1.f,e);asm("rcp.approx.ftz.f32 %0,%1;":"=f"(r):"f"(e));return r;}
TMN_DEVI void glu_small(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row){
 ...
 for(unsigned q=0;q<8;++q){unsigned i=tid+q*256,c=i/32,r=(i%32)*2;
  ...
  uint32_t masked;asm("mul.rn.bf16x2 %0,%1,%2;":"=r"(masked):"r"(dy),"r"(mask));float ga=fast_gate(bf16lo(gl)),gb=fast_gate(bf16hi(gl));
