...
TMN_DEVI void glu_small_dw(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row){
 unsigned tid=threadIdx.x;uint32_t mask=*reinterpret_cast<const uint32_t*>(p.mask+row+(tid%32)*2);
 #pragma unroll 4
 for(unsigned q=0;q<8;++q){unsigned i=tid+q*256,c=i/32,r=(i%32)*2;
  uint32_t dy=pair_get(s+16384,c,r),gl=pair_get(s,c*2,r),pr=pair_get(s,c*2+1,r);
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 if(split<p.tiles){mbar_wait(b,0);glu_small_dw(p,sm,sm+40960,sm+49152,split*64);}
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;
  ...
  if(tile+DW_SPLITS<p.tiles){int next=slot^1;uint8_t* ns=sm+next*DW_SLOT;
   mbar_wait(b+next,((r+1)/2)&1);glu_small_dw(p,ns,ns+40960,ns+49152,(tile+DW_SPLITS)*64);
  }
