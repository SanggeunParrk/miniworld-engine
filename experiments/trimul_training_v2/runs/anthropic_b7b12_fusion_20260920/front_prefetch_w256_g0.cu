...
TMN_DEVI void window_sync(const Params& p,int completed){
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 int tile=split;for(int window=0;window*WINDOW<p.tiles;++window){int limit=min(p.tiles,(window+1)*WINDOW);for(;tile<limit;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(b+slot,(r/2)&1);glu_small(p,s,s+40960,s+49152,tile*64);
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;int tile=split;for(int window=0;window*WINDOW<p.tiles;++window){int limit=min(p.tiles,(window+1)*WINDOW);for(;tile<limit;tile+=DXCOUNT,++round){
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0&&atomicAdd(p.counts+1,1u)==UCOUNT-1){for(int i=0;i<2*((p.tiles+WINDOW-1)/WINDOW);++i)atomicExch(p.counts+2+i,0u);atomicExch(p.counts,0u);atomicExch(p.counts+1,0u);}
#endif
}
