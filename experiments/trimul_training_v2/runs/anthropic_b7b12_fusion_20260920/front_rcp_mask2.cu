...
TMN_DEVI void glu_pair(const Params& p,uint8_t* s,int i,int row,uint32_t mask2,uint32_t& dg,uint32_t& dp){
 int c=i/32,r=(i%32)*2;
 ...
 uint32_t masked;asm("mul.bf16x2 %0,%1,%2;":"=r"(masked):"r"(dy),"r"(mask2));
...
template<bool INPLACE> TMN_DEVI void glu(const Params& p,uint8_t* s,int row,int group){
 int mr=row+(threadIdx.x%32)*2;uint32_t mask2=*reinterpret_cast<const uint32_t*>(p.mask+mr);
 if constexpr(INPLACE){
  uint32_t gg[16],pp[16];
  static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;glu_pair(p,s,threadIdx.x+q*256,row,mask2,gg[q],pp[q]);});
  ...
  uint32_t gg[16],pp[16];static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;glu_pair(p,s,threadIdx.x+q*256,row,mask2,gg[q],pp[q]);});static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;write_glu(s+65536,threadIdx.x+q*256,gg[q],pp[q]);});
 }
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* cached_gamma){
...
void front_b7b12(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar[2];__shared__ float cached_gamma[128];
 if(blockIdx.x>=DWCOUNT&&threadIdx.x<128)cached_gamma[threadIdx.x]=p.gamma[threadIdx.x];
 if(threadIdx.x==0){mbar_init(bar,1);mbar_init(bar+1,1);fence_barrier_init();}allsync();
 if(blockIdx.x<DWCOUNT)weight_role(p,sm,bar);else input_role(p,sm,bar,cached_gamma);
#if PART_ONLY == 2
