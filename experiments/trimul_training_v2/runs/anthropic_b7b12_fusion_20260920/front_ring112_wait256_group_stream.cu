...
TMN_DEVI void load_g(const Params& p,uint8_t* sm,uint64_t* b,int row,int side,int h){
 if(threadIdx.x)return;ring_wait(p.counts+2+((row/64)%RING_TILES)*8+side*4+h,row/64+1);int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_arrive_expect_tx(b+slot,32768);
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  ...
  for(int side=0;side<2;++side){
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(threadIdx.x==0)ring_publish(p.counts+2+8*RING_TILES+(tile%RING_TILES)*8+side*4+h,tile+1);
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 for(int i=blockIdx.x*256+threadIdx.x;i<16*RING_TILES;i+=UCOUNT*256)p.counts[2+i]=0;
