...
TMN_DEVI void glu_shared(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row,const uint8_t* masksm){
 unsigned tid=threadIdx.x;uint32_t mask=*reinterpret_cast<const uint32_t*>(masksm+(tid%32)*4);
 #pragma unroll 8
...
TMN_DEVI void mask_load(const Params& p,uint8_t* dst,uint64_t* bar,int row){asm volatile("cp.async.bulk.shared::cluster.global.mbarrier::complete_tx::bytes [%0], [%1], 128, [%2];"::"r"(smem_u32(dst)),"l"(p.mask+row),"r"(smem_u32(bar)):"memory");}
TMN_DEVI void load_dw(const Params& p,uint8_t* sm,uint64_t* b,int slot,int row,int group,uint8_t* masksm){
 if(threadIdx.x)return;uint8_t* s=sm+slot*DW_SLOT;int side=group/4,h=(group%4)*64;mbar_arrive_expect_tx(b+slot,41088);mask_load(p,masksm+slot*128,b+slot,row);
...
TMN_DEVI void ring_wait(const unsigned* ptr,unsigned want){unsigned got;do{asm volatile("ld.acquire.gpu.global.u32 %0,[%1];":"=r"(got):"l"(ptr):"memory");if(got<want)__nanosleep(256);}while(got<want);}
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b,uint8_t* masksm){
 ...
 if(split<p.tiles)load_dw(p,sm,b,0,split*64,group,masksm);if(split+DW_SPLITS<p.tiles)load_dw(p,sm,b,1,(split+DW_SPLITS)*64,group,masksm);
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(b+slot,(r/2)&1);glu_shared(p,s,s+40960,s+49152,tile*64,masksm+slot*128);if(r>0)ring_finish(p,tile-DW_SPLITS,group);ring_begin(p,s,tile,group);
  ...
  if(tile+2*DW_SPLITS<p.tiles)load_dw(p,sm,b,slot,(tile+2*DW_SPLITS)*64,group,masksm);
...
TMN_DEVI void load_g(const Params& p,uint8_t* sm,uint64_t* b,int row,int side,int h){
 ...
 tma_first(s+16384,&p.ring,b+slot,row%(RING_TILES*64),side*512+h*64);tma_first(sm+81920+h*8192,&p.ring,b+slot,row%(RING_TILES*64),side*512+256+h*64);
...
void front_b7b12(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ __align__(128) uint8_t masksm[256];__shared__ uint64_t bar[4];__shared__ float gamma[128];if(threadIdx.x<128)gamma[threadIdx.x]=p.gamma[threadIdx.x];
 if(threadIdx.x==0){for(int i=0;i<4;++i)mbar_init(bar+i,1);fence_barrier_init();}allsync();
 if(blockIdx.x<DWCOUNT)weight_role(p,sm,bar,masksm);else input_role(p,sm,bar,gamma);
#if PART_ONLY == 2
