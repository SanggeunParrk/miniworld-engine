...
TMN_DEVI void ring_wait(const unsigned* ptr,unsigned want){unsigned got;
asm volatile("ld.acquire.gpu.global.u32 %0,[%1];":"=r"(got):"l"(ptr):"memory");if(got>=want)return;
 do{asm volatile("ld.relaxed.gpu.global.u32 %0,[%1];":"=r"(got):"l"(ptr):"memory");
  if(got<want)__nanosleep(256);
 }while(got<want);
