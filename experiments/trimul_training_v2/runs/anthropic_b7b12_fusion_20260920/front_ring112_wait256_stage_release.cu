...
// Both consumers retire a WGMMA stage before its next TMA overwrite.
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  ...
  for(int side=0;side<2;++side){
   drain_stages(bar);load_g(p,sm,bar,row,side,0);load_g(p,sm,bar,row,side,1);
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(side==1&&h==3&&threadIdx.x==0)ring_publish(p.counts+2+8*RING_TILES+tile%RING_TILES,tile+1);
    fence_regs(acc);wgmma_fence();static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input64(acc,smem_desc(smem_u32(s+16384+k*2048),16,1024,1),smem_desc(smem_u32(s+24576+wi*8192+k*32),16,1024,1),side>0||h>0||k>0);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);retire_stage(bar,slot);if(h<2){if(threadIdx.x==0)mbar_wait(bar+4+slot,h/2);load_g(p,sm,bar,row,side,h+2);}
   }
   drain_stages(bar);load_p(p,sm,bar,side,0);load_p(p,sm,bar,side,1);
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);fence_regs(acc);wgmma_fence();
    static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input64(acc,smem_desc(smem_u32(sm+81920+h*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+wi*8192+k*32),16,1024,1),1);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);retire_stage(bar,slot);if(h<2){if(threadIdx.x==0)mbar_wait(bar+4+slot,h/2);load_p(p,sm,bar,side,h+2);}if(side==1&&h==1)load_ln_next(p,sm+65536,bar+3,row);
   }
  }
  allsync(); // LN scratch overlaps the retired P2/P3 cache.
...
void front_b7b12(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar[6];__shared__ float gamma[128];if(threadIdx.x<128)gamma[threadIdx.x]=p.gamma[threadIdx.x];
 if(threadIdx.x==0){for(int i=0;i<4;++i)mbar_init(bar+i,1);mbar_init(bar+4,2);mbar_init(bar+5,2);fence_barrier_init();}allsync();
