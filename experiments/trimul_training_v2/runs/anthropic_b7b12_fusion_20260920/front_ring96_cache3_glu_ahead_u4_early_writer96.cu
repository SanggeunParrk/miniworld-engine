...
TMN_DEVI void ring_begin(const Params& p,uint8_t* s,int tile,int group){if(threadIdx.x!=96)return;int slot=tile%RING_TILES;if(tile>=RING_TILES)ring_wait(p.counts+2+8*RING_TILES+slot,tile-RING_TILES+1);int h=(group/4)*512+(group%4)*64;ring_store_last(&p.ring,s+40960,slot*64,h);ring_store_last(&p.ring,s+49152,slot*64,h+256);tma_store_commit();}
TMN_DEVI void ring_finish(const Params& p,int tile,int group){if(threadIdx.x!=96)return;tma_store_wait_all();ring_publish(p.counts+2+(tile%RING_TILES)*8+group,tile+1);}
