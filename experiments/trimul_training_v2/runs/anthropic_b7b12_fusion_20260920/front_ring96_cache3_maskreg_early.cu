...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;uint32_t premask=*reinterpret_cast<const uint32_t*>(p.mask+tile*64+(threadIdx.x%32)*2);asm volatile(""::"r"(premask):"memory");mbar_wait(b+slot,(r/2)&1);glu_premask(p,s,s+40960,s+49152,tile*64,premask);if(r>0)ring_finish(p,tile-DW_SPLITS,group);ring_begin(p,s,tile,group);
