...
TMN_DEVI void glu_premask(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row,uint32_t mask){
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 uint32_t mask0=split<p.tiles?*reinterpret_cast<const uint32_t*>(p.mask+split*64+(threadIdx.x%32)*2):0;
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(b+slot,(r/2)&1);glu_premask(p,s,s+40960,s+49152,tile*64,slot?mask1:mask0);
 ...
 if(r>0)ring_finish(p,tile-DW_SPLITS,group);ring_begin(p,s,tile,group);
