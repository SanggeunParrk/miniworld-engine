...
TMN_DEVI void load_dw(const Params& p,uint8_t* sm,uint64_t* b,int slot,int row,int group){
 uint8_t* s=sm+slot*DW_SLOT;int side=group/4,h=(group%4)*64;if(threadIdx.x==0)mbar_arrive_expect_tx(b+slot,40960);cluster_sync_pair();if(threadIdx.x)return;
...
TMN_DEVI void load_g(const Params& p,uint8_t* sm,uint64_t* b,int row,int side,int h){
 int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;if(threadIdx.x==0)mbar_arrive_expect_tx(b+slot,32768);cluster_sync_pair();if(threadIdx.x)return;
...
TMN_DEVI void load_p(const Params& p,uint8_t* sm,uint64_t* b,int side,int h){
 int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;if(threadIdx.x==0)mbar_arrive_expect_tx(b+slot,16384);cluster_sync_pair();if(threadIdx.x)return;
...
TMN_DEVI void issue_gate(const Params& p,uint8_t* sm,uint64_t* bar,int row){
 if(threadIdx.x==0)mbar_arrive_expect_tx(bar,49152);cluster_sync_pair();if(threadIdx.x)return;
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){for(int i=0;i<4;++i)mbar_init(bar+i,1);fence_barrier_init();}allsync();cluster_sync_pair();
