...
TMN_DEVI void load_dw(const Params& p,uint8_t* sm,uint64_t* b,int slot,int row,int group){
 if(threadIdx.x)return; uint8_t* s=sm+slot*DW_SLOT;int side=group/4,h=(group%4)*64;mbar_arrive_expect_tx(b+slot,40960);pair_ready(b+slot+4,(row/(64*DW_SPLITS)/2)&1);
 tma_first(s,&p.pre,b+slot,row,side*512+h*2);tma_first(s+16384,side?&p.dr:&p.dl,b+slot,row,h);
 for(int n=0;n<2;++n)tma_pair(s+24576+n*8192,&p.xn,b+slot,n*64,row);
}
...
TMN_DEVI void load_g(const Params& p,uint8_t* sm,uint64_t* b,int row,int side,int h){
 if(threadIdx.x)return; int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_arrive_expect_tx(b+slot,32768);pair_ready(b+slot+4,h/2);
 ...
 for(int n=0;n<2;++n)tma_pair(s+24576+n*8192,side?&p.wrg:&p.wlg,b+slot,h*64,n*64);
}
TMN_DEVI void load_p(const Params& p,uint8_t* sm,uint64_t* b,int side,int h){
 if(threadIdx.x)return; int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_arrive_expect_tx(b+slot,16384);pair_ready(b+slot+4,h/2);
 for(int n=0;n<2;++n)tma_pair(s+n*8192,side?&p.wr:&p.wl,b+slot,h*64,n*64);
}
TMN_DEVI void issue_gate(const Params& p,uint8_t* sm,uint64_t* bar,int row){
 if(threadIdx.x)return; mbar_arrive_expect_tx(bar,49152);pair_ready(bar+4,(row/64/DXCOUNT)&1);
 for(int k=0;k<2;++k){tma_load_2d(sm+k*8192,&p.dg,bar,k*64,row);
  for(int n=0;n<2;++n)tma_pair(sm+16384+n*16384+k*8192,&p.wgate,bar,k*64,n*64);}
}
...
extern "C" __global__ __cluster_dims__(2,1,1) __launch_bounds__(256,2)
void front_b7b12(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar[8];__shared__ float gamma[128];if(threadIdx.x<128)gamma[threadIdx.x]=p.gamma[threadIdx.x];
 if(threadIdx.x==0){for(int i=0;i<4;++i){mbar_init(bar+i,1);mbar_init(bar+i+4,2);}fence_barrier_init();}allsync();cluster_sync_pair();
