...
TMN_DEVI void tma_pair(void* dst,const CUtensorMap* map,uint64_t* bar,int c0,int c1){
 ...
 asm volatile("cp.async.bulk.tensor.2d.shared::cluster.global.mbarrier::complete_tx::bytes.multicast::cluster [%0],[%1,{%3,%4}],[%2],%5;"::"r"(smem_u32(dst)),"l"(map),"r"(smem_u32(bar)),"r"(c0),"r"(c1),"h"((unsigned short)15):"memory");
}
TMN_DEVI void pair_ready(uint64_t* bar,int phase){
 ...
 if(!(blockIdx.x&3))mbar_wait(bar,phase);
}
...
extern "C" __global__ __cluster_dims__(4,1,1) __launch_bounds__(256,2)
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){for(int i=0;i<4;++i){mbar_init(bar+i,1);mbar_init(bar+i+4,4);}for(int i=8;i<10;++i)mbar_init(bar+i,1);fence_barrier_init();}allsync();cluster_sync_pair();
