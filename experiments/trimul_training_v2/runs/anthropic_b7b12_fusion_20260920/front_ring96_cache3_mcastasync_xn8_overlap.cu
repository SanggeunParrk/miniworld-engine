...
TMN_DEVI void cluster_sync_pair(){asm volatile("barrier.cluster.arrive.aligned; barrier.cluster.wait.aligned;":::"memory");}
TMN_DEVI void tma_pair(void* dst,const CUtensorMap* map,uint64_t* bar,int c0,int c1){
 if(blockIdx.x&7)return;
 asm volatile("cp.async.bulk.tensor.2d.shared::cluster.global.mbarrier::complete_tx::bytes.multicast::cluster [%0],[%1,{%3,%4}],[%2],%5;"::"r"(smem_u32(dst)),"l"(map),"r"(smem_u32(bar)),"r"(c0),"r"(c1),"h"((unsigned short)255):"memory");
}
TMN_DEVI void pair_ready(uint64_t* bar,int phase){
 ...
 if(!(blockIdx.x&7))mbar_wait(bar,phase);
}
...
TMN_DEVI void load_dw(const Params& p,uint8_t* sm,uint64_t* b,int slot,int row,int group){
 if(threadIdx.x)return; uint8_t* s=sm+slot*DW_SLOT;int side=group/4,h=(group%4)*64;mbar_arrive_expect_tx(b+slot,24576);mbar_arrive_expect_tx(b+slot+8,16384);
 tma_first(s,&p.pre,b+slot,row,side*512+h*2);tma_first(s+16384,side?&p.dr:&p.dl,b+slot,row,h);
 pair_ready(b+slot+4,(row/(64*DW_SPLITS)/2)&1);
 for(int n=0;n<2;++n)tma_pair(s+24576+n*8192,&p.xn,b+slot+8,n*64,row);
}
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(b+slot,(r/2)&1);glu_small(p,s,s+40960,s+49152,tile*64);if(r>0)ring_finish(p,tile-DW_SPLITS,group);ring_begin(p,s,tile,group);mbar_wait(b+slot+8,(r/2)&1);
...
extern "C" __global__ __cluster_dims__(8,1,1) __launch_bounds__(256,2)
void front_b7b12(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar[10];__shared__ float gamma[128];if(threadIdx.x<128)gamma[threadIdx.x]=p.gamma[threadIdx.x];
 if(threadIdx.x==0){for(int i=0;i<4;++i){mbar_init(bar+i,1);mbar_init(bar+i+4,8);}for(int i=8;i<10;++i)mbar_init(bar+i,1);fence_barrier_init();}allsync();cluster_sync_pair();
