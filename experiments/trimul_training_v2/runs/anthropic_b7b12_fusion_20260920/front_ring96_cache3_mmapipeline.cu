 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  ...
  for(int side=0;side<2;++side){
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(side==1&&h==3&&threadIdx.x==0)ring_publish(p.counts+2+8*RING_TILES+tile%RING_TILES,tile+1);
    if(h==0){fence_regs(acc);wgmma_fence();}static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input64(acc,smem_desc(smem_u32(s+16384+k*2048),16,1024,1),smem_desc(smem_u32(s+24576+wi*8192+k*32),16,1024,1),side>0||h>0||k>0);});wgmma_commit();if(h>0){if(h==3){wgmma_wait<0>();fence_regs(acc);}else wgmma_wait<1>();allsync();if(h<3)load_g(p,sm,bar,row,side,h+1);}
   }
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(h==0){fence_regs(acc);wgmma_fence();}
    static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input64(acc,smem_desc(smem_u32(sm+81920+h*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+wi*8192+k*32),16,1024,1),1);});wgmma_commit();if(h>0){if(h==3){wgmma_wait<0>();fence_regs(acc);}else wgmma_wait<1>();allsync();if(h<3)load_p(p,sm,bar,side,h+1);}if(side==1&&h==2)load_ln_next(p,sm+65536,bar+3,row);
   }
  }
