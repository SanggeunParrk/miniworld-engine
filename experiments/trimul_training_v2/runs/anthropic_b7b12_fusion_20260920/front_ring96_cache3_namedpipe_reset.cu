...
TMN_DEVI void load_g(const Params& p,uint8_t* sm,uint64_t* b,int row,int side,int h){
 if(threadIdx.x)return;int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_arrive_expect_tx(b+slot,32768);
 tma_last(s+16384,&p.ring,b+slot,row%(RING_TILES*64),side*512+h*64);tma_last(sm+81920+h*8192,&p.ring,b+slot,row%(RING_TILES*64),side*512+256+h*64);
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 asm volatile("mov.f32 pipeline_acc31,0f00000000;":::"memory");
  for(int side=0;side<2;++side){
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(side==1&&h==3&&threadIdx.x==0)ring_publish(p.counts+2+8*RING_TILES+tile%RING_TILES,tile+1);
    if(h==0)wgmma_fence();static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_pipeline(smem_desc(smem_u32(s+16384+k*2048),16,1024,1),smem_desc(smem_u32(s+24576+wi*8192+k*32),16,1024,1),side>0||h>0||k>0);});wgmma_commit();if(h>0){if(h==3){wgmma_wait<0>();}else wgmma_wait<1>();allsync();if(h<3)load_g(p,sm,bar,row,side,h+1);}
   }
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(h==0)wgmma_fence();
    static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_pipeline(smem_desc(smem_u32(sm+81920+h*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+wi*8192+k*32),16,1024,1),1);});wgmma_commit();if(h>0){if(h==3){wgmma_wait<0>();}else wgmma_wait<1>();allsync();if(h<3)load_p(p,sm,bar,side,h+1);}if(side==1&&h==2)load_ln_next(p,sm+65536,bar+3,row);
   }
  }
 ...
 asm volatile("mov.f32 %0,pipeline_acc31;":"=f"(acc[31])::"memory");
  ...
  fence_proxy_async();allsync();
  if(threadIdx.x==0){store2d(&p.dx,lnsm,0,row);store2d(&p.dx,lnsm+8192,64,row);tma_store_commit();tma_store_wait_all();}
