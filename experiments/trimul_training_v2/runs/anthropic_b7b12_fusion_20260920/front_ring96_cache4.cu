...
TMN_DEVI void load_ln_next(const Params& p,uint8_t* sm,uint64_t* bar,int row){if(threadIdx.x)return;mbar_arrive_expect_tx(bar,32768);for(int c=0;c<2;++c){tma_first(sm+c*8192,&p.x,bar,c*64,row);tma_first(sm+16384+c*8192,&p.res,bar,c*64,row);}}
