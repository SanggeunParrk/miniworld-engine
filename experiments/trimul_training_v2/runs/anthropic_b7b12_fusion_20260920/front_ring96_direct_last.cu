...
struct Params{
 ...
 __nv_bfloat16* ring_data;const __nv_bfloat16* mask;const float *mean,*rs,*gamma;
...
TMN_DEVI void ring_word(__nv_bfloat16* dst,uint32_t v){asm volatile("{.reg .b64 policy;createpolicy.fractional.L2::evict_last.b64 policy,1.0;st.global.L2::cache_hint.u32 [%0],%1,policy;}"::"l"(dst),"r"(v):"memory");}
TMN_DEVI void glu_small(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row){
  ...
  *reinterpret_cast<uint32_t*>(gout+swz128(c,r*2))=g;*reinterpret_cast<uint32_t*>(pout+swz128(c,r*2))=pp;
  int group=blockIdx.x%8;size_t off=((group/4)*512+(group%4)*64+c)*(RING_TILES*64)+(row%(RING_TILES*64))+r;ring_word(p.ring_data+off,g);ring_word(p.ring_data+off+256*(RING_TILES*64),pp);
...
TMN_DEVI void ring_begin(const Params& p,uint8_t* s,int tile,int group){if(threadIdx.x==0&&tile>=RING_TILES)ring_wait(p.counts+2+8*RING_TILES+tile%RING_TILES,tile-RING_TILES+1);allsync();}
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(b+slot,(r/2)&1);ring_begin(p,s,tile,group);glu_small(p,s,s+40960,s+49152,tile*64);
  ...
  });wgmma_commit();wgmma_wait<0>();fence_regs(acc);allsync();ring_finish(p,tile,group);
