...
TMN_DEVI void issue_gate(const Params& p,uint8_t* sm,uint64_t* bar,int row){
 ...
 for(int k=0;k<2;++k){tma_load_2d(sm+k*8192,&p.dg,bar,k*64,row);
  for(int n=0;n<2;++n)tma_load_2d(sm+16384+k*16384+n*8192,&p.wgate,bar,k*64,n*64);}
}
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 __shared__ volatile int done;int tid=threadIdx.x;if(tid==0)done=0;allsync();if(tid>=128){setmaxnreg_dec<32>();while(!done)__nanosleep(32);setmaxnreg_inc<128>();return;}setmaxnreg_inc<192>();
 int split=blockIdx.x-DWCOUNT,wi=threadIdx.x/128,lane=threadIdx.x%32,w=(threadIdx.x/32)%4;
 int ra=w*16+lane/4,rb=ra+8;float running_g=0,running_b=0;
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  int row=tile*64;uint32_t gate_packed[32];float acc[64]={};
  ...
  static_for<8>([&](auto ki){constexpr int k=decltype(ki)::value;
   mma_gate128(gate,smem_desc(smem_u32(sm+(k/4)*8192+(k%4)*32),16,1024,1),smem_desc(smem_u32(sm+16384+(k/4)*16384+(k%4)*32),16,1024,1),k>0);
  });wgmma_commit();wgmma_wait<0>();fence_regs(gate);
  // B9 rounds separately before B10's accumulation/add.
  static_for<32>([&](auto ji){constexpr int j=decltype(ji)::value;gate_packed[j]=pack_bf16(gate[j*2],gate[j*2+1]);});}

  named_bar_sync(1,128);if(tid<8)ring_wait(p.counts+2+(tile%RING_TILES)*8+tid,tile+1);named_bar_sync(1,128);
  for(int side=0;side<2;++side){
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(side==1&&h==3&&threadIdx.x==0)ring_publish(p.counts+2+8*RING_TILES+tile%RING_TILES,tile+1);
    fence_regs(acc);wgmma_fence();static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_front128(acc,smem_desc(smem_u32(s+16384+k*2048),16,1024,1),smem_desc(smem_u32(s+24576+k*32),16,1024,1),side>0||h>0||k>0);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);named_bar_sync(1,128);if(h<2)load_g(p,sm,bar,row,side,h+2);
   }
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);fence_regs(acc);wgmma_fence();
    static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_front128(acc,smem_desc(smem_u32(sm+81920+h*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+k*32),16,1024,1),1);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);named_bar_sync(1,128);if(h<2)load_p(p,sm,bar,side,h+2);if(side==1&&h==1)load_ln_next(p,sm+65536,bar+3,row);
   }
  }
  if(tile+DXCOUNT<p.tiles)issue_gate(p,sm,bar+2,(tile+DXCOUNT)*64);
  static_for<32>([&](auto qi){constexpr int q=decltype(qi)::value;acc[q*2]=__bfloat162float(__float2bfloat16_rn(acc[q*2]+bf16lo(gate_packed[q])));acc[q*2+1]=__bfloat162float(__float2bfloat16_rn(acc[q*2+1]+bf16hi(gate_packed[q])));});
  ...
  static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;int c=q*8+2*(lane%4),base=(c/64)*8192,cc=c%64;
   static_for<2>([&](auto ri){constexpr int r=decltype(ri)::value;int rr=r?rb:ra;
    float xa=__fmul_rn(__fsub_rn(get(lnsm+base,rr,cc),mu[r]),rs[r]),xb=__fmul_rn(__fsub_rn(get(lnsm+base,rr,cc+1),mu[r]),rs[r]);
    float ha=acc[q*4+r*2]*gamma[c],hb=acc[q*4+r*2+1]*gamma[c+1];s1[r]+=ha*xa+hb*xb;s2[r]+=ha+hb;
   });
  });
  float c1[2]={quad_sum(s1[0])/128.f,quad_sum(s1[1])/128.f},c2[2]={quad_sum(s2[0])/128.f,quad_sum(s2[1])/128.f};float* tmp=reinterpret_cast<float*>(lnsm+32768);
  static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;int c=q*8+2*(lane%4),base=(c/64)*8192,cc=c%64;
   float xaa=__fmul_rn(__fsub_rn(get(lnsm+base,ra,cc),mu[0]),rs[0]),xab=__fmul_rn(__fsub_rn(get(lnsm+base,ra,cc+1),mu[0]),rs[0]);
   float xba=__fmul_rn(__fsub_rn(get(lnsm+base,rb,cc),mu[1]),rs[1]),xbb=__fmul_rn(__fsub_rn(get(lnsm+base,rb,cc+1),mu[1]),rs[1]);
   ...
   put(lnsm+base,ra,cc,bf16lo(outa)+get(lnsm+16384+base,ra,cc));put(lnsm+base,ra,cc+1,bf16hi(outa)+get(lnsm+16384+base,ra,cc+1));
   put(lnsm+base,rb,cc,bf16lo(outb)+get(lnsm+16384+base,rb,cc));put(lnsm+base,rb,cc+1,bf16hi(outb)+get(lnsm+16384+base,rb,cc+1));
  ...
  named_bar_sync(1,128);running_g+=(tmp[tid]+tmp[256+tid])+(tmp[512+tid]+tmp[768+tid]);running_b+=(tmp[128+tid]+tmp[384+tid])+(tmp[640+tid]+tmp[896+tid]);
  fence_proxy_async();named_bar_sync(1,128);if(tid==0){store2d(&p.dx,lnsm,0,row);store2d(&p.dx,lnsm+8192,64,row);tma_store_commit();tma_store_wait_all();}named_bar_sync(1,128);
 }
