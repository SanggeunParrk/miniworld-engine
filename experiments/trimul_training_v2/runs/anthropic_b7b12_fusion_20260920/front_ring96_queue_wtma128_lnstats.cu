...
TMN_DEVI void load_stats(float* dst,const float* src,uint64_t* bar){asm volatile("cp.async.bulk.shared::cluster.global.mbarrier::complete_tx::bytes [%0],[%1],256,[%2];"::"r"(smem_u32(dst)),"l"(src),"r"(smem_u32(bar)):"memory");}
...
TMN_DEVI void load_g(const Params& p,uint8_t* sm,uint64_t* b,int row,int side,int h){
 ...
 tma_load_2d(s+24576,side?&p.wrg:&p.wlg,b+slot,h*64,0);
}
TMN_DEVI void load_p(const Params& p,uint8_t* sm,uint64_t* b,int side,int h){
 ...
 tma_load_2d(s,side?&p.wr:&p.wl,b+slot,h*64,0);
}
...
TMN_DEVI void load_ln_next(const Params& p,uint8_t* sm,uint64_t* bar,int row){if(threadIdx.x)return;mbar_arrive_expect_tx(bar,33280);load_stats(reinterpret_cast<float*>(sm+49152),p.mean+row,bar);load_stats(reinterpret_cast<float*>(sm+49408),p.rs+row,bar);for(int c=0;c<2;++c){tma_load_2d(sm+c*8192,&p.x,bar,c*64,row);tma_load_2d(sm+16384+c*8192,&p.res,bar,c*64,row);}}
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 if(first<p.tiles)issue_gate(p,sm,bar+2,first*64);int round=0;for(int tile=first;tile<p.tiles;tile=queued_tile,++round){
  ...
  if(threadIdx.x==0)queued_tile=atomicAdd(p.counts+2+9*RING_TILES,1u);allsync();if(queued_tile<p.tiles)issue_gate(p,sm,bar+2,queued_tile*64);
  ...
  float mu[2]={reinterpret_cast<float*>(sm+114688)[ra],reinterpret_cast<float*>(sm+114688)[rb]},rs[2]={reinterpret_cast<float*>(sm+114944)[ra],reinterpret_cast<float*>(sm+114944)[rb]},s1[2]={},s2[2]={};
...
TMN_DEVI void wait_total(const unsigned* ptr){unsigned got;do{asm volatile("ld.acquire.gpu.global.u32 %0,[%1];":"=r"(got):"l"(ptr):"memory");if(got!=UCOUNT)__nanosleep(32);}while(got!=UCOUNT);}
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){atomicAdd(p.counts,1u);wait_total(p.counts);}allsync();
 for(int i=blockIdx.x*256+threadIdx.x;i<9*RING_TILES+1;i+=UCOUNT*256)p.counts[2+i]=0;
