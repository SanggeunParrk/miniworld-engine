...
TMN_DEVI void load_ln_next(const Params& p,uint8_t* sm,uint64_t* bar,int row){if(threadIdx.x)return;mbar_arrive_expect_tx(bar,33280);load_stats(reinterpret_cast<float*>(sm-8192),p.mean+row,bar);load_stats(reinterpret_cast<float*>(sm-7936),p.rs+row,bar);for(int c=0;c<2;++c){tma_load_2d(sm+c*8192,&p.x,bar,c*64,row);tma_load_2d(sm+16384+c*8192,&p.res,bar,c*64,row);}}
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 ...
 if(first<p.tiles)issue_gate(p,sm,bar+2,first*64);int round=0;for(int tile=first;tile<p.tiles;tile=queued_tile,++round){
  ...
  float mu[2]={reinterpret_cast<float*>(sm+57344)[ra],reinterpret_cast<float*>(sm+57344)[rb]},rs[2]={reinterpret_cast<float*>(sm+57600)[ra],reinterpret_cast<float*>(sm+57600)[rb]},s1[2]={},s2[2]={};
