...
TMN_DEVI int logical_block(){int t=blockIdx.x;int d=DWCOUNT,x=DXCOUNT,m=d<x?d:x;if(t<2*m)return (t&1)?d+t/2:t/2;return d>x?t-m:t;}
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 int group=logical_block()%8,split=logical_block()/8,wi=threadIdx.x/128,lane=threadIdx.x%32,w=(threadIdx.x/32)%4;
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 int split=logical_block()-DWCOUNT,wi=threadIdx.x/128,lane=threadIdx.x%32,w=(threadIdx.x/32)%4;
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(logical_block()<DWCOUNT)weight_role(p,sm,bar);else input_role(p,sm,bar,gamma);
#if PART_ONLY == 2
 ...
 for(int i=logical_block()*256+threadIdx.x;i<9*RING_TILES;i+=UCOUNT*256)p.counts[2+i]=0;
 for(int i=logical_block()*256+threadIdx.x;i<131328;i+=UCOUNT*256)reduce_at(p,i);
 __threadfence();allsync();
 if(threadIdx.x==0&&atomicAdd(p.counts+1,1u)==UCOUNT-1){atomicExch(p.counts,0u);atomicExch(p.counts+1,0u);}
#endif
}
extern "C" __global__ void front_reduce(__grid_constant__ const Params p){reduce_at(p,logical_block()*256+threadIdx.x);}
