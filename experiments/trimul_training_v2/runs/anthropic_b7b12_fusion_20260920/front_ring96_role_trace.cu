...
TMN_DEVI void stamp(const Params& p,int slot){if(threadIdx.x)return;unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t));reinterpret_cast<unsigned long long*>(p.counts+2+9*RING_TILES)[blockIdx.x*4+slot]=t;if(slot==0){unsigned id;asm volatile("mov.u32 %0, %%smid;":"=r"(id));p.counts[2+9*RING_TILES+8*UCOUNT+blockIdx.x]=id;}}
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 stamp(p,0);if(blockIdx.x<DWCOUNT)weight_role(p,sm,bar);else input_role(p,sm,bar,gamma);
#if PART_ONLY == 2
 __threadfence();allsync();stamp(p,1); // Publish all partial writers before completion ticket.
 if(threadIdx.x==0){atomicAdd(p.counts,1u);while(atomicAdd(p.counts,0u)!=UCOUNT)__nanosleep(32);}allsync();stamp(p,2);
 ...
 __threadfence();allsync();stamp(p,3);
