...
TMN_DEVI unsigned long long now_ns(){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t));return t;}
...
TMN_DEVI void ring_begin(const Params& p,uint8_t* s,int tile,int group){if(threadIdx.x)return;int slot=tile%RING_TILES;unsigned long long start=now_ns();if(tile>=RING_TILES)ring_wait(p.counts+2+8*RING_TILES+slot,tile-RING_TILES+1);p.counts[2+9*RING_TILES+8*tile+group]=unsigned(now_ns()-start);int h=(group/4)*512+(group%4)*64;ring_store_last(&p.ring,s+40960,slot*64,h);ring_store_last(&p.ring,s+49152,slot*64,h+256);tma_store_commit();}
...
TMN_DEVI void ring_ready(const Params& p,int tile){unsigned long long start=0;if(threadIdx.x==0)start=now_ns();if(threadIdx.x<8)ring_wait(p.counts+2+(tile%RING_TILES)*8+threadIdx.x,tile+1);allsync();if(threadIdx.x==0)p.counts[2+9*RING_TILES+8*p.tiles+tile]=unsigned(now_ns()-start);}
