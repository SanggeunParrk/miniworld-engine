...
TMN_DEVI void pair_ln64(const Params& p,uint8_t* sm,const float* gamma,int base_row,float& running){
 ...
 for(int sub=0;sub<2;++sub){
  ...
  float* stats=reinterpret_cast<float*>(sm+69632);float* tmp=reinterpret_cast<float*>(sm+65536);
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* b,const float* gamma){
 ...
 for(int tile=split*2;tile<p.tiles;tile+=DXCOUNT*2,++round){int row=tile*64;
  ...
  mbar_wait(b+3,round&1);allsync();pair_ln64(p,sm,gamma,row,running);
 }
