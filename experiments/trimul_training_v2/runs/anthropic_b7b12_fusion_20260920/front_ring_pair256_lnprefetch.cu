...
TMN_DEVI void pair_ln_load(const Params& p,uint8_t* sm,uint64_t* b,int row,int sub){
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* b,const float* gamma){
 ...
 for(int tile=split*2;tile<p.tiles;tile+=DXCOUNT*2,++round){int row=tile*64;
  pair_gate_load(p,sm,b+2,row);mbar_wait(b+2,0);
  ...
  mbar_wait(b+3,round&1);mbar_wait(b+2,1);allsync();pair_ln(p,sm,gamma,row,run_g,run_b);
 }
