 ...
 if(split*2<p.tiles)issue_gate(p,sm,bar+2,split*128);int round=0;for(int tile=split*2;tile<p.tiles;tile+=DXCOUNT*2,++round){
  ...
  for(int side=0;side<2;++side){
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(side==1&&h==3&&threadIdx.x==0){ring_publish(p.counts+2+8*RING_TILES+tile%RING_TILES,tile+1);ring_publish(p.counts+2+8*RING_TILES+(tile+1)%RING_TILES,tile+2);}
    if(h==0){fence_regs(acc);wgmma_fence();}static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input64(acc,smem_desc(smem_u32(s+mg*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+16384+wi*8192+k*32),16,1024,1),side>0||h>0||k>0);});wgmma_commit();if(h>0){if(h==3){wgmma_wait<0>();fence_regs(acc);}else wgmma_wait<1>();allsync();if(h<3)load_g(p,sm,bar,tile*64,side,h+1);}
   }
   load_p(p,sm,bar,side,0);load_p(p,sm,bar,side,1);
   #pragma unroll 4
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(h==0){fence_regs(acc);wgmma_fence();}
    static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_input64(acc,smem_desc(smem_u32(sm+65536+h*16384+mg*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+wi*8192+k*32),16,1024,1),1);});wgmma_commit();if(h>0){if(h==3){wgmma_wait<0>();fence_regs(acc);}else wgmma_wait<1>();allsync();if(h<3)load_p(p,sm,bar,side,h+1);}if(side==1&&h==1)load_ln_next(p,sm+131072,bar+3,tile*64);
   }
  }
