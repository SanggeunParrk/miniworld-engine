...
constexpr int DW_SLOT=98304,DX_SLOT=32768;
...
TMN_DEVI void glu_small(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row){
 ...
 #pragma unroll 8
 for(unsigned q=0;q<8;++q){unsigned i=tid+q*512,c=i/32,r=(i%32)*2;
...
TMN_DEVI void ring_begin(const Params& p,uint8_t* s,int tile,int group){
...
TMN_DEVI void mma_pipeline(uint64_t a,uint64_t b,int accumulate){asm volatile("{.reg .pred p;setp.ne.b32 p,%2,0;wgmma.mma_async.sync.aligned.m64n64k16.f32.bf16.bf16 {pipeline_acc0,pipeline_acc1,pipeline_acc2,pipeline_acc3,pipeline_acc4,pipeline_acc5,pipeline_acc6,pipeline_acc7,pipeline_acc8,pipeline_acc9,pipeline_acc10,pipeline_acc11,pipeline_acc12,pipeline_acc13,pipeline_acc14,pipeline_acc15,pipeline_acc16,pipeline_acc17,pipeline_acc18,pipeline_acc19,pipeline_acc20,pipeline_acc21,pipeline_acc22,pipeline_acc23,pipeline_acc24,pipeline_acc25,pipeline_acc26,pipeline_acc27,pipeline_acc28,pipeline_acc29,pipeline_acc30,pipeline_acc31},%0,%1,p,1,1,1,0;}"::"l"(a),"l"(b),"r"(accumulate):"memory");}
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma){
 asm volatile(".reg .f32 pipeline_acc<32>;");
 int split=blockIdx.x-DWCOUNT,wi=(threadIdx.x/128)%2,mg=threadIdx.x/256,lane=threadIdx.x%32,w=(threadIdx.x/32)%4;
 int ra=w*16+lane/4,rb=ra+8;float running=0;
 if(split*2<p.tiles)issue_gate(p,sm,bar+2,split*128);int round=0;for(int tile=split*2;tile<p.tiles;tile+=DXCOUNT*2,++round){
  int row=(tile+mg)*64;uint32_t gate_packed[16];float acc[32];
  ...
  static_for<8>([&](auto ki){constexpr int k=decltype(ki)::value;
   mma_dgrad(gate,smem_desc(smem_u32(sm+mg*16384+(k/4)*8192+(k%4)*32),16,1024,1),smem_desc(smem_u32(sm+32768+wi*16384+(k/4)*8192+(k%4)*32),16,1024,1),k>0);
  ...
  allsync();ring_ready(p,tile);
 asm volatile("mov.f32 pipeline_acc0,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc1,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc2,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc3,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc4,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc5,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc6,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc7,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc8,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc9,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc10,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc11,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc12,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc13,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc14,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc15,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc16,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc17,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc18,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc19,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc20,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc21,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc22,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc23,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc24,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc25,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc26,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc27,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc28,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc29,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc30,0f00000000;":::"memory");
 asm volatile("mov.f32 pipeline_acc31,0f00000000;":::"memory");
  for(int side=0;side<2;++side){
   load_g(p,sm,bar,tile*64,side,0);load_g(p,sm,bar,tile*64,side,1);
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(side==1&&h==3&&threadIdx.x==0){ring_publish(p.counts+2+8*RING_TILES+tile%RING_TILES,tile+1);ring_publish(p.counts+2+8*RING_TILES+(tile+1)%RING_TILES,tile+2);}
    if(h==0)wgmma_fence();static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_pipeline(smem_desc(smem_u32(s+mg*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+16384+wi*8192+k*32),16,1024,1),side>0||h>0||k>0);});wgmma_commit();if(h>0){if(h==3){wgmma_wait<0>();}else wgmma_wait<1>();allsync();if(h<3)load_g(p,sm,bar,tile*64,side,h+1);}
   }
   load_p(p,sm,bar,side,0);load_p(p,sm,bar,side,1);
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);if(h==0)wgmma_fence();
    static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_pipeline(smem_desc(smem_u32(sm+65536+h*16384+mg*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+wi*8192+k*32),16,1024,1),1);});wgmma_commit();if(h>0){if(h==3){wgmma_wait<0>();}else wgmma_wait<1>();allsync();if(h<3)load_p(p,sm,bar,side,h+1);}if(side==1&&h==1)load_ln_next(p,sm+131072,bar+3,tile*64);
   }
  }
  if(tile+DXCOUNT*2<p.tiles)issue_gate(p,sm,bar+2,(tile+DXCOUNT*2)*64);
 asm volatile("mov.f32 %0,pipeline_acc0;":"=f"(acc[0])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc1;":"=f"(acc[1])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc2;":"=f"(acc[2])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc3;":"=f"(acc[3])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc4;":"=f"(acc[4])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc5;":"=f"(acc[5])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc6;":"=f"(acc[6])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc7;":"=f"(acc[7])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc8;":"=f"(acc[8])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc9;":"=f"(acc[9])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc10;":"=f"(acc[10])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc11;":"=f"(acc[11])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc12;":"=f"(acc[12])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc13;":"=f"(acc[13])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc14;":"=f"(acc[14])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc15;":"=f"(acc[15])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc16;":"=f"(acc[16])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc17;":"=f"(acc[17])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc18;":"=f"(acc[18])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc19;":"=f"(acc[19])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc20;":"=f"(acc[20])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc21;":"=f"(acc[21])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc22;":"=f"(acc[22])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc23;":"=f"(acc[23])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc24;":"=f"(acc[24])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc25;":"=f"(acc[25])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc26;":"=f"(acc[26])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc27;":"=f"(acc[27])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc28;":"=f"(acc[28])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc29;":"=f"(acc[29])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc30;":"=f"(acc[30])::"memory");
 asm volatile("mov.f32 %0,pipeline_acc31;":"=f"(acc[31])::"memory");
  ...
  uint8_t* lnsm=sm+131072+mg*32768;mbar_wait(bar+3,round&1);
  ...
  float* stats=reinterpret_cast<float*>(sm+73728+mg*1024);float* tmp=reinterpret_cast<float*>(sm+65536+mg*4096);
  ...
  running+=(tmp[threadIdx.x%256]+tmp[256+threadIdx.x%256])+(tmp[512+threadIdx.x%256]+tmp[768+threadIdx.x%256]);
  fence_proxy_async();allsync();
  if(threadIdx.x==0){for(int m=0;m<2;++m){store2d(&p.dx,sm+131072+m*32768,0,(tile+m)*64);store2d(&p.dx,sm+131072+m*32768+8192,64,(tile+m)*64);}tma_store_commit();tma_store_wait_all();}
  allsync(); // TMA store cannot read a slot overwritten by next tile's gate loads.
 }
 p.partln[split*512+threadIdx.x]=running;
}
TMN_DEVI void reduce_at(const Params& p,int i){
 ...
 }else if(i<131328){int c=i-131072;float v=0;for(int b=0;b<DXCOUNT*2;++b)v+=reinterpret_cast<volatile float*>(p.partln)[b*256+c];(c<128?p.dgam:p.dbeta)[c%128]=v;}
}
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 for(int i=blockIdx.x*512+threadIdx.x;i<9*RING_TILES;i+=UCOUNT*512)p.counts[2+i]=0;
 for(int i=blockIdx.x*512+threadIdx.x;i<131328;i+=UCOUNT*512)reduce_at(p,i);
 __threadfence();allsync();
 if(threadIdx.x==0&&atomicAdd(p.counts+1,1u)==UCOUNT-1){atomicExch(p.counts,0u);atomicExch(p.counts+1,0u);}
#endif
}
extern "C" __global__ void front_reduce(__grid_constant__ const Params p){reduce_at(p,blockIdx.x*512+threadIdx.x);}
