...
TMN_DEVI void ring_ready(const Params& p,int tile){if(threadIdx.x<8)ring_wait(p.counts+2+(tile%RING_TILES)*8+threadIdx.x,tile+1);named_bar_sync(1,256);}
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(b+slot,(r/2)&1);glu_small(p,s,s+40960,s+49152,tile*64);if(r>0)ring_finish(p,tile-DW_SPLITS,group);ring_begin(p,s,tile,group);
  ...
  });wgmma_commit();wgmma_wait<0>();fence_regs(acc);named_bar_sync(1,256);
...
TMN_DEVI void ws_consumer(const Params& p,uint8_t* sm,WsBarriers* b,const float* gamma){
 ...
 for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  ...
  uint8_t* lnsm=sm;mbar_wait(b->tx+5,round&1);
  ...
  named_bar_sync(1,256); // Publish both64-channel halves before full128-channel LN reduction.
  ...
  named_bar_sync(1,256); // All warp parameter partials and dx stores published.
  running+=(tmp[tid]+tmp[256+tid])+(tmp[512+tid]+tmp[768+tid]);
  fence_proxy_async();named_bar_sync(1,256);
  if(tid==0){store2d(&p.dx,lnsm,0,row);store2d(&p.dx,lnsm+8192,64,row);tma_store_commit();tma_store_wait_all();}
  named_bar_sync(1,256); // TMA store cannot read a slot overwritten by next tile's gate loads.
 }
