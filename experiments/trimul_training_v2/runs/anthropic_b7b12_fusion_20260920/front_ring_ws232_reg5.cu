...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(blockIdx.x<DWCOUNT)weight_role(p,sm,bar);else {
  int wg=__shfl_sync(0xffffffff,threadIdx.x/128,0);if(wg==0){setmaxnreg_dec<24>();ws_producer(p,sm,&bars);setmaxnreg_inc<128>();}
  else{setmaxnreg_inc<232>();ws_consumer(p,sm,&bars,gamma);setmaxnreg_dec<128>();}
 }
#if PART_ONLY == 2
