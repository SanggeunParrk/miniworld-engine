...
TMN_DEVI float sigmoid_shared(uint32_t bits,const float* lut){
 unsigned idx=(bits&0x7fffu)-0x3b80u;
 if(idx<2048u)return lut[idx+((bits&0x8000u)?2048:0)];
...
TMN_DEVI void glu64(const Params& p,uint8_t* s,int row){
 ...
 static_for<8>([&](auto qi){constexpr int q=decltype(qi)::value;int i=tid+q*256,c=i/32,r=(i%32)*2;
  ...
  extern __shared__ __align__(1024) uint8_t sm[];const float* lut=reinterpret_cast<float*>(sm+204800);float ga=sigmoid_shared(gl&65535,lut),gb=sigmoid_shared(gl>>16,lut);
...
TMN_DEVI int dxid(){return int(threadIdx.x)-256;}
...
TMN_DEVI void glu_rows128(const Params& p,uint8_t* sm,int h,int row){
 int tid=threadIdx.x;uint8_t* s=sm+(h&1)*FRONT_SLOT;
 ...
 #pragma unroll 1
 for(int q=0;q<16;++q){int i=tid+q*256,m=i/2048,c=(i%2048)/32,r=(i%32)*2;
  ...
  *reinterpret_cast<uint32_t*>(s+32768+m*8192+swz128(c,r*2))=gg;*reinterpret_cast<uint32_t*>(sm+PGRAD+h*16384+m*8192+swz128(c,r*2))=pp;
 }fence_proxy_async();psync();
}
...
TMN_DEVI void dx_consumer(const Params& p,uint8_t* sm,Barriers* b,const float* gamma){
 ...
 for(int tile=split;tile<p.tiles/2;tile+=DXCOUNT,++round){
  ...
  for(int side=0;side<2;++side){
   for(int h=0;h<4;++h){int slot=h&1;mbar_wait(b->ready+slot,h/2);uint8_t* s=sm+slot*FRONT_SLOT;
    fence_regs(acc);wgmma_fence();static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;mma_front128(acc,smem_desc(smem_u32(s+32768+wi*8192+k*2048),16,1024,1),smem_desc(smem_u32(s+49152+k*32),16,1024,1),side>0||h>0||k>0);});wgmma_commit();wgmma_wait<0>();fence_regs(acc);dxrelease(b,slot);
   }
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){for(int i=0;i<5;++i){mbar_init(bars.tx+i,1);mbar_init(bars.ready+i,1);mbar_init(bars.empty+i,1);}fence_barrier_init();}named_bar_sync(0,512);
 if(blockIdx.x<DWCOUNT){float* lut=reinterpret_cast<float*>(sm+204800);for(int i=threadIdx.x;i<4096;i+=512){unsigned bits=0x3b80+(i%2048)+((i/2048)*32768);lut[i]=math::sigmoid_div(__uint_as_float(bits<<16));}}named_bar_sync(0,512);
 int wg=__shfl_sync(0xffffffffu,threadIdx.x>>7,0);
 if(blockIdx.x<DWCOUNT){
  if(wg<2){setmaxnreg_dec<120>();dw_producer(p,sm,&bars);}else if(wg==2){setmaxnreg_inc<240>();dw_consumer(p,sm,&bars);}else{setmaxnreg_dec<24>();}
 }else{
  if(wg<2){setmaxnreg_dec<32>();dx_producer(p,sm,&bars);}else{setmaxnreg_inc<224>();dx_consumer(p,sm,&bars,gamma);}
 }
 named_bar_sync(0,512);
#if PART_ONLY == 2
 __threadfence();named_bar_sync(0,512);
 if(threadIdx.x==0){atomicAdd(p.counts,1u);while(atomicAdd(p.counts,0u)!=UCOUNT)__nanosleep(32);}named_bar_sync(0,512);
