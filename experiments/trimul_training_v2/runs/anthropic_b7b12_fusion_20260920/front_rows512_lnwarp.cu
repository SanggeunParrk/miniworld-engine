 ...
 for(int tile=split;tile<p.tiles/2;tile+=DXCOUNT,++round){
  ...
  static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;int c=q*8+2*(lane%4),base=(c/64)*8192,cc=c%64;
   static_for<2>([&](auto ri){constexpr int r=decltype(ri)::value;int rr=r?rb:ra;
    ...
    float ha=acc[q*4+r*2]*gamma[c],hb=acc[q*4+r*2+1]*gamma[c+1];s1[r]+=ha*xa+hb*xb;s2[r]+=ha+hb;__syncwarp();
   });
  });
  ...
  static_for<16>([&](auto qi){constexpr int q=decltype(qi)::value;int c=q*8+2*(lane%4),base=(c/64)*8192,cc=c%64;
   ...
   if(lane<4){tmp[(wi*4+w)*256+c]=dga;tmp[(wi*4+w)*256+c+1]=dgb;tmp[(wi*4+w)*256+128+c]=dba;tmp[(wi*4+w)*256+128+c+1]=dbb;}__syncwarp();
  });
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(blockIdx.x<DWCOUNT){
  ...
  if(wg<2){setmaxnreg_dec<64>();dx_producer(p,sm,&bars);}else{setmaxnreg_inc<192>();dx_consumer(p,sm,&bars,gamma);}
 }
