...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t));reinterpret_cast<unsigned long long*>(p.counts+2)[blockIdx.x*2]=t;}named_bar_sync(0,512);
 ...
 named_bar_sync(0,512);if(threadIdx.x==0){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t));reinterpret_cast<unsigned long long*>(p.counts+2)[blockIdx.x*2+1]=t;}
#if PART_ONLY == 2
