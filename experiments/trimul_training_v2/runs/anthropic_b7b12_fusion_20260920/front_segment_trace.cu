...
TMN_DEVI void mark(const Params& p,int i){if(threadIdx.x==0){unsigned long long t;asm volatile("mov.u64 %0, %%globaltimer;":"=l"(t));reinterpret_cast<unsigned long long*>(p.counts+2)[blockIdx.x*16+i]=t;}}
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++round){
  if(round==4)mark(p,2);int slot=round&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(bar+slot,(round/2)&1);if(round==4)mark(p,3);
  glu<false>(p,s,tile*64,group);if(round==4)mark(p,4);
  ...
  });wgmma_wait<0>();fence_regs(acc[0]);fence_regs(acc[1]);if(round==4)mark(p,5);
  // WGMMA consumers completed before a producer refills this stage.
  allsync();if(tile+2*DW_SPLITS<p.tiles)issue_dw(p,sm,bar,slot,tile+2*DW_SPLITS,group);if(round==4)mark(p,6);
 }
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 for(int tile=split;tile<p.tiles;tile+=DXCOUNT){
  bool sample=tile==split+4*DXCOUNT;if(sample)mark(p,2);int row=tile*64;uint32_t gate_packed[16];float acc[32]={};
  ...
  issue_gate(p,sm,bar,row);mbar_wait(bar,0);if(sample)mark(p,3);
  {float gate[32]={};
  ...
  allsync();if(sample)mark(p,4);issue_dx(p,sm,bar,0,row,0);issue_dx(p,sm,bar,1,row,1);
  // Preserve baseline K order: Lg[0:256], Lp[0:256], Rg, Rp.
  for(int side=0;side<2;++side){
   for(int half=0;half<2;++half){
    int slot=half,group=side*2+half;uint8_t* s=sm+slot*DX_SLOT;
    mbar_wait(bar+slot,(side+1-half)&1);if(sample&&group==0)mark(p,10);glu<true>(p,s,row,group);if(sample&&group==0)mark(p,11);
    ...
    });wgmma_commit();wgmma_wait<0>();fence_regs(acc);allsync();if(sample&&side==0&&half==0)mark(p,12);
   }
   for(int half=0;half<2;++half){
  ...
  mbar_wait(bar,1);if(sample)mark(p,6);
  ...
  allsync();if(sample)mark(p,7); // Publish both64-channel halves before full128-channel LN reduction.
  ...
  allsync();if(sample)mark(p,8); // All warp parameter partials and dx stores published.
  ...
  allsync();if(sample)mark(p,9); // TMA store cannot read a slot overwritten by next tile's gate loads.
 }
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 mark(p,0);if(blockIdx.x<DWCOUNT)weight_role(p,sm,bar);else input_role(p,sm,bar);mark(p,1);
#if PART_ONLY == 2
