...
TMN_DEVI void glu_pair(const Params& p,uint8_t* s,int i,int row,float ma,float mb,uint32_t& dg,uint32_t& dp){
 int c=i/32,r=(i%32)*2;
 uint32_t dy=pair_get(s+32768,c,r),gl=pair_get(s,c,r),pr=pair_get(s+16384,c,r);
...
// TMA gathers saved interleaved logits into separate g/p planes. Each
// thread reads and overwrites its own row pair, so no register transpose
// buffer or intermediate CTA barrier is necessary.
template<bool INPLACE> TMN_DEVI void glu(const Params& p,uint8_t* s,int row,int group){
 ...
 for(int i=threadIdx.x;i<4096;i+=256){
  uint32_t g,pr;glu_pair(p,s,i,row,ma,mb,g,pr);write_glu(s,i,g,pr);
#if DEBUG_SAVE
  if constexpr(INPLACE){int c=i/32,r=(i%32)*2,side=group/2,h=(group%2)*128+c;
 ...
 fence_proxy_async();allsync(); // In-place GLU stores precede async MMA reads.
}
TMN_DEVI void issue_dw(const Params& p,uint8_t* sm,uint64_t* bar,int slot,int tile,int group){
 ...
 tma_load_3d(s,&p.pre,bar+slot,row,0,side*256+h);
 tma_load_3d(s+16384,&p.pre,bar+slot,row,1,side*256+h);
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++round){
  ...
  static_for<2>([&](auto ni){constexpr int n=decltype(ni)::value;fence_regs(acc[n]);wgmma_fence();
   static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;
    mma_weight128(acc[n],smem_desc(smem_u32(s+n*16384+wi*8192+k*32),16,1024,1),smem_desc(smem_u32(s+49152+k*2048),8192,1024,1),round>0||k>0);
   });wgmma_commit();
