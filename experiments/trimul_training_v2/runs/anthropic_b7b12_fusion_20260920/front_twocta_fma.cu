...
TMN_DEVI void glu_small(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row){
 ...
 for(unsigned q=0;q<8;++q){unsigned i=tid+q*256,c=i/32,r=(i%32)*2;
  ...
  float ya=(bf16lo(masked)*bf16lo(pr))*ga,yb=(bf16hi(masked)*bf16hi(pr))*gb;uint32_t g=pack_bf16(fmaf(-ya,ga,ya),fmaf(-yb,gb,yb)),pp=pack_bf16(bf16lo(masked)*ga,bf16hi(masked)*gb);
