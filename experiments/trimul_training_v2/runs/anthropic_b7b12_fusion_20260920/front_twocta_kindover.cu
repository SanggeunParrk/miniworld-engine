...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b){
 ...
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;if(r==0){mbar_wait(b,(r/2)&1);glu_small(p,s,s+40960,s+49152,tile*64);}
