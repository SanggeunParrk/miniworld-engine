...
ap=argparse.ArgumentParser();ap.add_argument('--part',type=int,default=2);ap.add_argument('--source',default='front_ring96_cache3');args=ap.parse_args()
with torch.no_grad():
 ...
 a['wg']=init((128,128));p=RingPlan(a,count=264,splits=20,source=args.source,part=args.part)
 for _ in range(2):p();torch.cuda.synchronize()
 assert all(x==0 for x in p.counts.cpu().tolist())
