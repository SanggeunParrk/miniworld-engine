...
old=(p/'front_roundonly.cu').read_text()
start=old.index('TMN_DEVI void store2d');end=old.index('// Each pair')
store=old[start:end]
prim=(p/'front_mn_primitives.cuh').read_text()
w=prim[prim.index('TMN_DEVI void mma_weight128'):prim.index('TMN_DEVI void mma_input64')]
helpers=w.replace('mma_weight128','mma_gate128').replace('p, 1, 1, 0, 1;','p, 1, 1, 0, 0;')+w.replace('mma_weight128','mma_front128').replace('p, 1, 1, 0, 1;','p, 1, 1, 1, 0;')
(p/'warp_primitives.cuh').write_text('#pragma once\n#include "front_mn_primitives.cuh"\n'+helpers)
