...
with torch.no_grad():
 ...
 for src,split in [('front_twocta_kindwg',13)]+[(s,k) for s in ['front_kindunroll8_inter','front_window2048_inter'] for k in [11,12,13,14,15]]:
 ...
 (R/'kindwg-combined-sweep-L768.json').write_text(json.dumps(rec,indent=2));print('RESULT',{k:(v.get('time') or {}).get('median_us',v['status']) for k,v in rec.items()},flush=True)
