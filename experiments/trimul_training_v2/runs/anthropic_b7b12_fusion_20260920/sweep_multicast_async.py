...
with torch.no_grad():
 ...
 for mode in ['plain','weights','xn','both']:
  src=base if mode=='plain' else base+'_mcastasync_'+mode
  try:
 ...
 (R/f'multicast-async-sweep-L{args.length}.json').write_text(json.dumps(dict(records=records,times=ts),indent=2));print('MULTICAST',{k:v['median_us'] for k,v in ts.items()},flush=True)
