...
with torch.no_grad():
 for n in [384,768]:
  a=setup(n);p=WarpPlan(a,count=264,splits=13,source='front_lnpair_trace');g=capture(p)
  ...
  (R/f'lnpair-role-trace-L{n}.json').write_text(json.dumps(d,indent=2));print('ROLES',n,{k:v for k,v in d.items() if not isinstance(v,list)},flush=True)
