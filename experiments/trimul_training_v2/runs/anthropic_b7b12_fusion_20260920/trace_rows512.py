...
with torch.no_grad():
 for n in [384,768]:
  a=setup(n);p=WarpPlan(a,source='front_rows512_trace');g=capture(p)
  ...
  (R/f'rows512-role-trace-L{n}.json').write_text(json.dumps(d,indent=2));print('ROLES',n,{k:v for k,v in d.items() if not isinstance(v,list)},flush=True)
