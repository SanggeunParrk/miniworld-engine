...
ap=argparse.ArgumentParser();ap.add_argument('--length',type=int,default=64);ap.add_argument('--splits',type=int,default=8);ap.add_argument('--sources',nargs='+',required=True);ap.add_argument('--output',required=True);args=ap.parse_args()
with torch.no_grad():
 a=setup(args.length);ref=baseline(a);plans={};rec={}
 for src in args.sources:
  try:
   p=WarpPlan(a,splits=args.splits,source=src);p();torch.cuda.synchronize();es=errors(p.outputs,ref)
