...
with torch.no_grad():
 ...
 ts=paired({k:capture(p) for k,p in plans.items()});(R/f'{args.source}-more-tune.json').write_text(json.dumps(dict(records=records,times=ts),indent=2));print('RING_TUNE',{k:v['median_us'] for k,v in ts.items()},flush=True)
