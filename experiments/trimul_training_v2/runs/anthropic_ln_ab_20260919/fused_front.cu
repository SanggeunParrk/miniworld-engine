...
// Experiment: restore original ln_fragment; save normalized input + mean/rstd.
...
TMN_DEVI void saved_front_body(const SavedFrontParams& tp) {
  ...
  uint8_t* sGate = sStage + G::SMEM_STAGE;
...
#pragma unroll
    ...
    if (SAVE) {                                           // dormant save-intermediates: LN_in statistics per pair row (row owner lane of each quad)
      if (p.stats != nullptr && (lane & 3) == 0) {
        if (vA) { p.stats[(size_t)iA * p.N + jA] = st.mA; p.stats[(size_t)p.N*p.N + (size_t)iA * p.N + jA] = st.rA; }
        if (vB) { p.stats[(size_t)iB * p.N + jB] = st.mB; p.stats[(size_t)p.N*p.N + (size_t)iB * p.N + jB] = st.rB; }
      }
    }
...
#pragma unroll
      ...
      if (TMAST && p.vec) {
        ...
        if (st_elect) { tma_store_3d(&p.tm_ab, sStage + cw * 8192 + (b & 1) * 4096, jw, iw, 32 * b); 
...
void mw_saved_front(__grid_constant__ const tmn::sm90::SavedFrontParams p) {
 tmn::sm90::saved_front_body<tmn::sm90::SavedFrontCfg,true,1,true,true>(p);
}
