...
TMN_DEVI void infer_k3_body(const K3Params& p) {
...
#pragma unroll
          for (int kb = 0; kb < 2; ++kb) {
            uint32_t rz[4];
            if (MW_FUSED) {ldsm_x4(rz, sZ_u + (uint32_t)(b0 >> 1) * (uint32_t)CHB + swz128((uint32_t)(rho0 + lrow), (uint32_t)(((2 * h + kb) * 16 + ((mat & 2) ? 8 : 0)) * 2)));} else {
