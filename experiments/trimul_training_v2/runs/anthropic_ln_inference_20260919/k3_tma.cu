// Inference only: no training saves/dropout; upstream inference rounding.
#define MWK3_CZ 128
#define MWK3_CH 256
...
struct TrainParams { K3Params base; CUtensorMap tm_residual; };
struct TrainCfg : K3Cfg<MWK3_CZ, MWK3_CH, 0, MWK3_BI, MWK3_BJ, MWK3_NSLOT, MWK3_NACC> {
  static constexpr bool PRENORM = true;
  static constexpr int SMEM = K3Cfg<MWK3_CZ, MWK3_CH, 0, MWK3_BI, MWK3_BJ, MWK3_NSLOT, MWK3_NACC>::SMEM;
};
template <class G>
TMN_DEVI void k3_training_body(const TrainParams& tp) {
  ...
  if (tid == 0) {
    ...
    tma_prefetch_desc(&tp.tm_residual);
  }
  __syncthreads();
...
#pragma unroll                                           // dependency); the proxy fence orders these generic-proxy reads before the async-proxy refill
    ...
    ln_fragment<KSP,MW_SERIAL>(fx,sGout,sBout,lane,p.eps);
    PF(3);                                               // 3: LN_out
...
#pragma unroll
        for (int kc = 0; kc < NKCZ; ++kc) {
          ...
          const bool now = true;                                // residual word 0: every chunk at once
...
void infer_k3_tma(__grid_constant__ const tmn::sm90::TrainParams p) {
