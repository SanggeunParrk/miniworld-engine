...
# Original Transition remains the baseline; latest_all uses snapshotted native CUDA forward/backward.
...
old_trans=torch.compile(transition,fullgraph=True,dynamic=False,options={'triton.cudagraphs':False})
# Load the current Transition implementation without replacing the retained TriMul engine.
import miniworld_engine.kernels.transition.cuda
native=load('miniworld_engine.kernels.transition.cuda.block_native_sm90a',R/'transition_snapshot/fused_sm90a.py')
def native_transition(x):
 return native.transition_fused_sm90a(x,transition.ln_in.weight,transition.ln_in.bias,transition.expand_a.weight,transition.expand_b.weight,transition.squeeze.weight,transition.ln_in.eps)
new_trans=torch.compile(native_transition,fullgraph=True,dynamic=False,options={'triton.cudagraphs':False})
trans=old_trans
with torch.no_grad():
...
def set_backend(name):
 global trans
 trans=new_trans if name=='latest_all' else old_trans
...
def cueq_forward(x, wl, wlg, wr, wrg, wg, wp, gi, bi, go, bo, mask, ds):
    ...
    return (update if ds is None else update * ds) + x

def torch_trimul(x,wl,wlg,wr,wrg,wg,wp,gi,bi,go,bo,mask,ds):
 xn=F.layer_norm(x.float(),(128,),gi,bi,1e-5).to(x.dtype)
 left=torch.sigmoid(F.linear(xn,wlg))*F.linear(xn,wl)
 right=torch.sigmoid(F.linear(xn,wrg))*F.linear(xn,wr)
 left=left*mask[...,None];right=right*mask[...,None]
 outgoing=torch.einsum('bikd,bjkd->bijd',left[...,:128],right[...,:128])
 incoming=torch.einsum('bkid,bkjd->bijd',left[...,128:],right[...,128:])
 tri=torch.cat((outgoing,incoming),dim=-1)
 norm=F.layer_norm(tri.float(),(256,),go,bo,1e-5).to(x.dtype)
 update=torch.sigmoid(F.linear(xn,wg))*F.linear(norm,wp)
 return (update if ds is None else update*ds)+x

def torch_transition(x):
 xn=F.layer_norm(x.float(),(128,),transition.ln_in.weight,transition.ln_in.bias,transition.ln_in.eps).to(x.dtype)
 h=F.silu(F.linear(xn,transition.expand_a.weight))*F.linear(xn,transition.expand_b.weight)
 return F.linear(h,transition.squeeze.weight)+x

def torch_block(*args):return torch_transition(torch_trimul(*args))
def cueq_torch_block(*args):return torch_transition(cueq_forward(*args))
def cueq_cuda_block(*args):return native_transition(cueq_forward(*args))
opts=dict(fullgraph=True,dynamic=False,options={'triton.cudagraphs':False})
base_blocks={'pytorch_eager':torch_block,'pytorch_compile':torch.compile(torch_block,**opts),'cueq_torch':torch.compile(cueq_torch_block,**opts),'cueq_cuda':torch.compile(cueq_cuda_block,**opts)}
def baseline_step(scope,name):
 block=base_blocks[name]
 def run():
  with torch.set_grad_enabled(scope!='inference'):
   y=block(*leaves,mask,None if scope=='inference' else ds)
   return (y,torch.autograd.grad(y,(*leaves,*tp),block_dy)) if scope=='training' else y
 return run

names=('pytorch_eager','pytorch_compile','cueq_torch','cueq_cuda','latest_all')
record=dict(L=384,B=1,C=128,H_trimul=256,transition_expansion=4,blocks=1,use_single=False,host=platform.node(),job=os.environ.get('SLURM_JOB_ID'),gpu=torch.cuda.get_device_name(),dropout_training=.25,dropout_inference=0,transition='latest_all and cueq_cuda: native D128 CUDA; other arms: PyTorch Transition; whole reference block statically compiled except pytorch_eager',baseline='PyTorch reference and matched bidirectional cuEquivariance primitive composition (not two public TMU calls)',includes=['bidirectional TriMul','Transition','both residuals','pair mask','fixed training dropout','live weight packing','all block parameter gradients'],excludes=['optimizer','dropout RNG generation','compile','CPU dispatch'],latest_training_diagnostic=True,known_issue='latest B7 input LN gradient exceeds strict same-algorithm threshold in prior full TriMul test',times={},rounds={},checks={},traces={},transition_parameters=trans_names,engine=str(Path(PIN.B.__file__)),source_sha256={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),R/'pin_previous.py',ROOT/'src/miniworld/modules/mini_pairformer.py']})
...
for scope in ('training','training_forward','inference'):
 ...
 for name in names:
  set_backend(name);fn=(latest_train if scope=='training' else latest_training_forward if scope=='training_forward' else latest_inference) if name=='latest_all' else baseline_step(scope,name)
 ...
 ref=clone(outs[scope]['pytorch_compile']);record['checks'][scope]={}
 for name in names:
...
assert record['traces']['training']['latest_all'].get('b7_joint')==1
assert record['traces']['inference']['latest_all'].get('infer_k3')==1
assert 'b7_joint' not in record['traces']['inference']['latest_all']
...
record['transition_source_manifest']=json.loads((R/'transition_sources.json').read_text())
for scope in ('training','inference'):
 native_names=list(record['traces'][scope]['latest_all'])
 assert any('transition_fwd_fused' in n for n in native_names),native_names
 if scope=='training':assert any('transition_bwd_fused' in n for n in native_names),native_names
record['versions']={k:importlib.metadata.version(k) for k in ('torch','triton','cuequivariance-torch','cuequivariance-ops-torch-cu12')}
record['cueq_tuning']=os.environ.get('CUEQ_TRITON_TUNING','default')
