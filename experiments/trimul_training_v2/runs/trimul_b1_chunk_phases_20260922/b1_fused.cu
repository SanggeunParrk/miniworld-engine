...
#include <cuda_fp16.h>
#ifndef B1_WG_PARAM_SYNC
#define B1_WG_PARAM_SYNC 0
#endif
#ifndef B1_SKIP_FINAL_GROUP_SYNC
#define B1_SKIP_FINAL_GROUP_SYNC 0
#endif
#ifndef B1_ASYNC_WP_DN
#define B1_ASYNC_WP_DN 0
#endif
#ifndef B1_DEFER_DG_STORE
#define B1_DEFER_DG_STORE 0
#endif
#ifndef B1_LOCAL_GATE
#define B1_LOCAL_GATE 0
#endif
#ifndef B1_DTRI_ASYNC_EPI
#define B1_DTRI_ASYNC_EPI 0
#endif
#ifndef B1_PARAM_SHARED
#define B1_PARAM_SHARED 0
#endif
#ifndef B1_PARAM_UNROLL
#define B1_PARAM_UNROLL 1
#endif
#ifndef B1_REG_EARLY_RAW
#define B1_REG_EARLY_RAW 0
#endif
#ifndef B1_DN_REG
#define B1_DN_REG 0
#endif
...
#if XHAT_FP32 == -1
TMN_DEVI void load_raw(const Params&,uint8_t*,uint64_t*,int,int);
...
TMN_DEVI void load_raw(const Params& p,uint8_t* sm,uint64_t* bar,int slot,int row){
 if(threadIdx.x)return;uint8_t* dst=sm+16384+slot*49152;mbar_arrive_expect_tx(bar+slot,49152+(XHAT_FP32==-1?512:0));
...
#if XHAT_FP32 == -1
 tma_load_2d(sm+B1_STATS_BASE(slot),&p.statsmap,bar+slot,row,1);tma_load_2d(sm+B1_STATS_BASE(slot)+256,&p.statsmap,bar+slot,row,0);
#endif
#endif
...
#if XHAT_FP32 == -1
 if(B1_AFFINE_BOTH || wi==1){
  int ra=w*16+lane/4,rb=ra+8;
#if XHAT_FP32 != -1
  if(lane%4==0){float* rs=reinterpret_cast<float*>(sm+B1_STATS_BASE(slot))+64;rs[ra]=p.saved_rs[row+ra];rs[rb]=p.saved_rs[row+rb];
...
#endif
#if XHAT_FP32 == -1
 int r8=lane%8;float* mus=reinterpret_cast<float*>(sm+B1_STATS_BASE(slot));float ma=mus[ra],mb=mus[rb],rA=mus[64+ra],rB=mus[64+rb];
 constexpr int affine_unroll = B1_STREAM_AFFINE_UNROLL;
 #pragma unroll affine_unroll
 for(int k=(B1_AFFINE_BOTH?wi*8:0);k<(B1_AFFINE_BOTH?(wi+1)*8:16);++k){uint32_t f[4];ldsm_x4_t(f,smem_u32(x+16384)+swz128(k*16+r8+8*(mat>>1),(w*16+8*(mat&1))*2));
  int c=16*k+2*(lane&3);float2 g0=lds64f(smem_u32((ps+256))+4*c),b0=lds64f(smem_u32((ps+512))+4*c),g1=lds64f(smem_u32((ps+256))+4*c+32),b1=lds64f(smem_u32((ps+512))+4*c+32);
  ...
  stsm_x4(smem_u32(norm+(k/4)*8192)+swz128(w*16+r8+8*(mat&1),(2*(k%4)+(mat>>1))*16),f[0],f[1],f[2],f[3]);
 }
#else
  #pragma unroll 4
...
#if !B1_ASYNC_WP_DN
 wgmma_wait<0>();fence_regs(a);fence_regs(b);
#endif
}
...
// Inverse of store_weight. Preserve FP32 accumulation across phase chunks.
...
TMN_DEVI void shared_role(const Params& p,uint8_t* sm,uint64_t* bars,int tile_begin,int tile_end){
 int wi=threadIdx.x/128,round=0,mi=0;
 FragmentMask mask=fragment_mask<UCOUNT>(p,tile_begin+blockIdx.x);
 float wp0[64]={},wp1[64]={};
...
#endif
#if !PRODUCER
 if(tile_begin+blockIdx.x<tile_end)load_raw(p,sm,bars,0,(tile_begin+blockIdx.x)*64);
#if B1_PREFETCH_DY
 if(tile_begin+blockIdx.x<tile_end)load_dy(p,sm,bars+2,(tile_begin+blockIdx.x)*64);
#endif
#endif
 for(int tile=tile_begin+blockIdx.x;tile<tile_end;tile+=UCOUNT,++round){int slot=round&1;
  ...
  uint8_t* norm=sm+16384+(1-slot)*49152;
#if !GATE_PHASE
  weight_gemm(wg,x+wi*8192,sm,(round>0||tile_begin>0));
#endif
#if PAIR_WP
  weight_pair(wp0,wp1,norm,(round>0||tile_begin>0));
#else
  weight_gemm(wp0,norm+32768,norm+wi*16384,(round>0||tile_begin>0));
  weight_gemm(wp1,norm+40960,norm+wi*16384,(round>0||tile_begin>0));
#endif
#if !B1_DEFER_DG_STORE
...
#endif
#if B1_PREFETCH_DY
  if(tile+UCOUNT<tile_end)load_dy(p,sm,bars+2,(tile+UCOUNT)*64);
#endif
  // All consumers have released the opposite slot; overlap its next TMA load
  // with dTri/LN backward on the current raw tri tile.
#if LOWREG
  lowreg_dgrad(p,sm,bars,tile*64,slot,wp0,wp1,tile_end);allsync();
  if(!B1_EARLY_RAW && tile+UCOUNT<tile_end){
#if PRODUCER
...
#endif
#if PRODUCER
  if(tile+UCOUNT<tile_end && threadIdx.x==0)mbar_arrive(bars+5);
#endif
...
TMN_DEVI void gate_phase(const Params& p,uint8_t* sm,uint64_t* bars,int tile_begin,int tile_end){
 ...
 if(tile_begin+blockIdx.x<tile_end)load_gate_operands(p,sm,bars,0,(tile_begin+blockIdx.x)*64);
 for(int tile=tile_begin+blockIdx.x;tile<tile_end;tile+=UCOUNT,++round){int slot=round&1;
  mbar_wait(bars+slot,(round/2)&1);
  if(tile+UCOUNT<tile_end)load_gate_operands(p,sm,bars,1-slot,(tile+UCOUNT)*64);
  weight_gemm(acc,sm+slot*32768+16384+wi*8192,sm+slot*32768,(round>0||tile_begin>0));allsync();
 }
