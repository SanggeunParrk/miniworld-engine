...
Q=B.BASE.OLD.Q
ap=argparse.ArgumentParser();ap.add_argument('--length',type=int,required=True);ap.add_argument('--levels',default='0,2,4,8,16');ap.add_argument('--rounds',type=int,default=3);ap.add_argument('--iterations',type=int,default=150);args=ap.parse_args();n=args.length
records=[]
with torch.no_grad():
 ...
 for level in map(int,args.levels.split(',')):
  cfg=json.loads(json.dumps(base));cfg['defines']['B1_PHASE_CHUNK']=level
