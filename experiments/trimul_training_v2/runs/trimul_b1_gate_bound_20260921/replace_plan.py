...
def build(count,length,part,defines=()):
 ...
 L=T._launch_module();drv=L.BlockDriver(device=0);mod=drv.load(out.read_bytes());u=L.Unit('shared_b1','sm_90a',0,drv.drv,mod,{},str(out));k=u.kernel('b1_gate_only');smem=227840+512*int(dict(defines).get('B1_SPLIT_DN',0));k.set_max_dynamic_smem(smem)
