...
#include <cuda_fp16.h>
// NVIDIA PTX cp.async.bulk.tensor cache-policy hints affect replacement
...
#if B1_TMA_PRIORITY & 2
TMN_DEVI void dg_store(const CUtensorMap* map,const void* src,int c,int r){
...
TMN_DEVI void load_raw(const Params& p,uint8_t* sm,uint64_t* bar,int slot,int row){
 ...
 cached_load<(B1_TMA_PRIORITY&1)?1:0>(dst,&p.x,bar+slot,0,row);cached_load<(B1_TMA_PRIORITY&1)?1:0>(dst+8192,&p.x,bar+slot,64,row);
#if XHAT_FP32 == 1
 for(int c=0;c<2;++c)for(int k=0;k<2;++k)tma_load_2d(sm+114688+k*16384+c*8192,&p.wg,bar+slot,c*64,k*64);
#else
 cached_load<(B1_TMA_PRIORITY&4)?-1:0>(dst+16384,&p.xhat,bar+slot,row,0);
#if XHAT_FP32 == -1
...
TMN_DEVI void load_dy(const Params& p,uint8_t* sm,uint64_t* bar,int row){
 if(threadIdx.x)return;mbar_arrive_expect_tx(bar,16384);cached_load<(B1_TMA_PRIORITY&4)?-1:0>(sm,&p.dy,bar,0,row);cached_load<(B1_TMA_PRIORITY&4)?-1:0>(sm+8192,&p.dy,bar,64,row);
}
#ifndef GATE_PHASE
#define GATE_PHASE 0
#endif
...
#if LOWREG
  lowreg_dgrad(p,sm,bars,tile*64,slot,wp0,wp1);allsync();
...
TMN_DEVI void load_gate_operands(const Params& p,uint8_t* sm,uint64_t* bars,int slot,int row){
 ...
 cached_load<(B1_GATE_DEMOTE&1)?-1:0>(dst,&p.dgmap,bars+slot,0,row);cached_load<(B1_GATE_DEMOTE&1)?-1:0>(dst+8192,&p.dgmap,bars+slot,64,row);
 cached_load<(B1_GATE_DEMOTE&2)?-1:0>(dst+16384,&p.x,bars+slot,0,row);cached_load<(B1_GATE_DEMOTE&2)?-1:0>(dst+24576,&p.x,bars+slot,64,row);
}
...
#if !B1_LOCAL_GATE
 if(threadIdx.x==0){atomicAdd(p.counts+2,1u);while(atomicAdd(p.counts+2,0u)!=UCOUNT)__nanosleep(32);}allsync();
#endif
 // Phase B consumes exactly this CTA's own Phase A rows. No other CTA's
 // dGate is read. All local TMA stores have completed before this point.
