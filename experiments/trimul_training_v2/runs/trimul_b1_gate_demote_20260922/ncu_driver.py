...
for mode in ('baseline','optimized'):
 stem=R/('ncu-L%d-%s'%(n,mode))
 cmd=['ncu','--profile-from-start','off','--section','SourceCounters','--section','SpeedOfLight','--import-source','yes','--metrics',metrics,'--cache-control','none','--clock-control','none','--target-processes','all','--kernel-name','regex:^b1_fused$','--launch-count','1','--force-overwrite','-o',str(stem),'env','PYTHONPATH='+os.environ.get('PYTHONPATH',''),sys.executable,'-B',str(R/'profile_one.py'),'--length',str(n),'--variant',mode]
 ...
 with (R/('source-L%d-%s.csv'%(n,mode))).open('w') as f:subprocess.run(['ncu','--import',str(stem)+'.ncu-rep','--page','source','--print-source','cuda,sass','--csv'],env=env,stdout=f,stderr=subprocess.STDOUT,check=True,timeout=120)
