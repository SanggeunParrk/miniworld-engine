...
TMN_DEVI void prepare_shared(const Params& p,uint8_t* sm,uint64_t* bars,int slot,int row,int phase,const FragmentMask& mask,int mi){
 ...
 if(wi==1){
#if STREAM_LN
  LnStats st=stream_ln(x+16384,norm,ps+256,ps+512);
#else
...
TMN_DEVI void load_producer(const Params& p,uint8_t* sm,uint64_t* bars){
 ...
 for(int tile=blockIdx.x;tile<p.tiles;tile+=UCOUNT,++round){int slot=round&1;
  ...
  tma_load_2d(dst,&p.x,bars+slot,0,tile*64);tma_load_2d(dst+8192,&p.x,bars+slot,64,tile*64);tma_load_2d(dst+16384,&p.tri,bars+slot,tile*64,0);
...
extern "C" __global__ __launch_bounds__(THREADS,1) void b1_fused(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){for(int b=0;b<6;++b)mbar_init(bars+b,1);fence_barrier_init();mbar_arrive_expect_tx(bars+3,98304);
...
#if PRODUCER
 if(threadIdx.x>=256){setmaxnreg_dec<PROD_REGS>();load_producer(p,sm,bars);}
 else {setmaxnreg_inc<CONS_REGS>();shared_role(p,sm,bars);}
 named_bar_sync(15,THREADS);
#else
