...
TMN_DEVI void stage_tick(const Params& p,unsigned long long& last,int st){
...
TMN_DEVI void shared_role(const Params& p,uint8_t* sm,uint64_t* bars){
 int wi=threadIdx.x/128,round=0,mi=0;unsigned long long last=clock64();
 ...
 for(int tile=blockIdx.x;tile<p.tiles;tile+=UCOUNT,++round){int slot=round&1;
  ...
  #endif
#endif
  stage_tick(p,last,0);prepare_shared(p,sm,bars,slot,tile*64,round&1,mask,mi);stage_tick(p,last,1);
  uint8_t* x=sm+16384+slot*49152;
  uint8_t* norm=sm+16384+(1-slot)*49152;
#if !GATE_PHASE
...
#endif
  if(threadIdx.x==0)tma_store_wait_all();allsync();stage_tick(p,last,2);
#if !B1_DIRECT_DP
...
#if LOWREG
  lowreg_dgrad(p,sm,bars,tile*64,slot);allsync();stage_tick(p,last,3);
...
extern "C" __global__ __launch_bounds__(THREADS,1) void b1_fused(__grid_constant__ const Params p){
 ...
 unsigned long long mainlast=clock64();shared_role(p,sm,bars);stage_tick(p,mainlast,4);
#if GATE_PHASE
 ...
 stage_tick(p,mainlast,5);gate_phase(p,sm,bars);stage_tick(p,mainlast,6);
#endif
#if PART_ONLY==2
 __threadfence();allsync();if(threadIdx.x==0){atomicAdd(p.counts,1u);while(atomicAdd(p.counts,0u)!=UCOUNT)__nanosleep(32);}allsync();
 stage_tick(p,mainlast,7);for(int i=blockIdx.x*256+threadIdx.x;i<49664;i+=UCOUNT*256)reduce_at(p,i);stage_tick(p,mainlast,8);
