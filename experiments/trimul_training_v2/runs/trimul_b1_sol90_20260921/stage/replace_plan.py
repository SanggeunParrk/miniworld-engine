...
OLD=R.parent.parent/'trimul_split_bwd_20260921'
...
class Plan(SP.B1):
 def __init__(self,d,dy,xhat,rstd,count=132,part=2,defines=None):
  if (defines or {}).get('B1_SPLIT_DN') and ((defines or {}).get('B1_PREFETCH_DY') or not (defines or {}).get('B1_DIRECT_DP')):raise ValueError('split dNorm requires direct dProj and disables dy prefetch')
  ...
  self.counts=torch.zeros(512+132*32,device=x.device,dtype=torch.int32);self.bind(dy,xhat,rstd)
