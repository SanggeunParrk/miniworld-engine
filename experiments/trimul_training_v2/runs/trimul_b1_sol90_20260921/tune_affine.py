...
spec=importlib.util.spec_from_file_location('b1_sol90_plan',R/'affine/replace_plan.py');RP=importlib.util.module_from_spec(spec);spec.loader.exec_module(RP)
Q=B.B.BASE.OLD.Q
...
with torch.no_grad():
 ...
 base=json.loads((R/('selected-L%d.json'%n)).read_text())['config']
 candidates=[]
 for both,unroll,pair in itertools.product((0,1),(4,8,2,1),(0,1)):
  cfg=json.loads(json.dumps(base));cfg['defines'].update(B1_AFFINE_BOTH=both,B1_STREAM_AFFINE_UNROLL=unroll,PAIR_WP=pair);candidates.append(cfg)
 for cfg in candidates:
  ...
  records.append(row);(R/('affine-tune-L%d.json'%n)).write_text(json.dumps(records,indent=2));print('TUNE',row,flush=True)
 valid=[r for r in records if r['valid']];best=min(valid,key=lambda r:r['ratio']);(R/('affine-selected-L%d.json'%n)).write_text(json.dumps(best,indent=2));print('BEST',best,flush=True)
