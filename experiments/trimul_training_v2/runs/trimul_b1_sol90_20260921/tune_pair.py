...
spec=importlib.util.spec_from_file_location('b1_sol90_plan',R/'pair/replace_plan.py');RP=importlib.util.module_from_spec(spec);spec.loader.exec_module(RP)
Q=B.B.BASE.OLD.Q
...
with torch.no_grad():
 ...
 base=json.loads((R/('count-selected-L%d.json'%n)).read_text())['config']
 candidates=[]
 for pair,unroll in itertools.product((0,1),(2,4,8)):
  cfg=json.loads(json.dumps(base));cfg['defines'].update(B1_DN_PAIR=pair,B1_STREAM_AFFINE_UNROLL=unroll);candidates.append(cfg)
 for cfg in candidates:
  ...
  records.append(row);(R/('pair-tune-L%d.json'%n)).write_text(json.dumps(records,indent=2));print('TUNE',row,flush=True)
 valid=[r for r in records if r['valid']];best=min(valid,key=lambda r:r['ratio']);(R/('pair-selected-L%d.json'%n)).write_text(json.dumps(best,indent=2));print('BEST',best,flush=True)
