...
with torch.no_grad():
 ...
 base=json.loads((R/('affine-selected-L%d.json'%n)).read_text())['config']
 candidates=[]
 for store in (16,32,64,128):
  cfg=json.loads(json.dumps(base));cfg['defines'].update(B1_DTRI_STORE_C=store);candidates.append(cfg)
 for cfg in candidates:
  ...
  records.append(row);(R/('store-tune-L%d.json'%n)).write_text(json.dumps(records,indent=2));print('TUNE',row,flush=True)
 valid=[r for r in records if r['valid']];best=min(valid,key=lambda r:r['ratio']);(R/('store-selected-L%d.json'%n)).write_text(json.dumps(best,indent=2));print('BEST',best,flush=True)
