...
from sol_policy import Training,B
R=Path(__file__).resolve().parent
BASE=B.B.BASE;OLD=BASE.OLD;Q=OLD.Q;LN=OLD.LN
...
def main():
 ...
 with torch.no_grad():
  ...
  result['cubins']={k:dict(b1=m.p1.k.unit.cubin_path) for k,m in models.items()};result['source_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [R/'bench.py',R/'sol_policy.py',R/'replace_plan.py',R/('selected-L%d.json'%n),*R.glob('*.cu'),*R.glob('*.cuh'),*R.glob('*.inc')]};path.write_text(json.dumps(result,indent=2));print('DONE',n,flush=True)
