...
sp=importlib.util.spec_from_file_location('late_only_policy',R.parent/'trimul_b1_weight_store_late_20260922/policy.py');LP=importlib.util.module_from_spec(sp);sp.loader.exec_module(LP)
def clone(x):return x[0].clone(),tuple(t.clone() for t in x[1])
def errors(x,y):return OLD.H.errors(x,y)
def passed(e):return all(v['bit_exact'] and v['finite'] for v in e.values())
def mutate(a):
...
def main():
 ...
 with torch.no_grad():
  a=Q.setup(n);d=a['d'];models={'baseline':B.Training(a),'late_only':LP.Training(a),'optimized':Training(a)}
  ...
  for scope in ('forward','b1','backward','forward_backward'):
   ...
   blocks=[Q.paired(gs,iterations=200) for _ in range(5)];result['blocks'][scope]=blocks;result['times'][scope]=Q.pool(blocks);print('TIME',scope,{k:v['median_us'] for k,v in result['times'][scope].items()},flush=True)
   path.write_text(json.dumps(result,indent=2));del gs;gc.collect()
  result['cubins']={k:dict(b1=m.p1.k.unit.cubin_path) for k,m in models.items()};result['source_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [R/'bench.py',R/'policy.py',*R.glob('*.cu'),*R.glob('*.cuh'),*R.glob('*.inc')]};path.write_text(json.dumps(result,indent=2));print('DONE',n,flush=True)
