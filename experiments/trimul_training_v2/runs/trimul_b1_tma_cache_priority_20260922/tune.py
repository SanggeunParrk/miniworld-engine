...
Q=B.BASE.OLD.Q
ap=argparse.ArgumentParser();ap.add_argument('--length',type=int,required=True);ap.add_argument('--levels',default='0,1,2,3,4,7,8,11,15');ap.add_argument('--rounds',type=int,default=3);ap.add_argument('--iterations',type=int,default=150);args=ap.parse_args();n=args.length
records=[]
with torch.no_grad():
 ...
 for level in map(int,args.levels.split(',')):
  cfg=json.loads(json.dumps(base));cfg['defines']['B1_TMA_PRIORITY']=level;cfg['defines']['B1_LATE_WEIGHT']=3
  row=dict(config=cfg,host=platform.node(),checks=[]);snap=[];tensors=[]
  try:
