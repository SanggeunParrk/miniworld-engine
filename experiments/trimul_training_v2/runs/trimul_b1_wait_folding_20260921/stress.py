...
p=D/'lowreg_stats.inc';s=p.read_text();needle2=' if(tid==0)tma_store_wait_all();';assert needle2 in s
s=s.replace(needle2,' if(wi==1 && tid==0 && blockIdx.x%17==0){for(int k=0;k<5;++k)__nanosleep(50000);}'+'\n'+needle2);p.write_text(s)
...
with torch.no_grad():
 for n in (384,768):
  ...
  for seed in (19,47,83):
   ...
   records.append(dict(L=n,seed=seed,bit_exact=exact,replays=20,delayed_ctas='blockIdx % 17 == 0, 20 x nanosleep(50000ns) before Phase B plus WG1 delayed before dTri wait'))
