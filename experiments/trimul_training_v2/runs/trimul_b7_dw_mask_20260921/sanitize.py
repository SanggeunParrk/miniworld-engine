...
for tool in ('memcheck','racecheck'):
 log=R/('%s-L%d.log'%(tool,n));cmd=['compute-sanitizer','--tool',tool,'--error-exitcode','99','--kernel-name','kns=front_b7b12_dw',sys.executable,'-B',str(R/'stress_dw.py'),'--length',str(n)]
