...
T=P.T
@lru_cache(None)
def build(role,saved,splits,pc=2,dxctas=132,prod=40,cons=232,resident=1,slices=2,skip_restore=0,overlap=0,ln_mode=0,xn_early=0,mask_hoist=0):
...
class Plan:
 def __init__(self,d,dy,dl,dr,dg,xn=None,split=False,splits=4,pc=2,dxctas=132,prod=40,cons=232,resident=1,slices=2,skip_restore=0,overlap=0,ln_mode=0,xn_early=0,mask_hoist=0):
  self.d=d;self.split=split;self.xn=xn;self.cfg=dict(saved=xn is not None,splits=splits,pc=pc,dxctas=dxctas,prod=prod,cons=cons,resident=resident,slices=slices,skip_restore=skip_restore,overlap=overlap,ln_mode=ln_mode,xn_early=xn_early,mask_hoist=mask_hoist)
