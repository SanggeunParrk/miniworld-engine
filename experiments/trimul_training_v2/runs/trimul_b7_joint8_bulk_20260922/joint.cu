...
// Tree reduction uses bulk DSMEM copies, not serial remote float loads.
...
TMN_DEVI void cluster_dx_reduce(uint8_t* sm,uint64_t* bar,int round){
 int rank=blockIdx.x%8;
 static_for<3>([&](auto ss){constexpr int step=decltype(ss)::value,dist=1<<step;
  bool recv=rank%(dist*2)==0,send=rank%(dist*2)==dist;
  if(recv&&threadIdx.x==0)mbar_arrive_expect_tx(bar+6+step,32768);
  fence_proxy_async();csync();
  if(send&&threadIdx.x==0){uint32_t dst=remote_addr(sm+32768,rank-dist),b=remote_addr(bar+6+step,rank-dist);
   asm volatile("cp.async.bulk.shared::cluster.shared::cta.mbarrier::complete_tx::bytes [%0],[%1],%2,[%3];"::"r"(dst),"r"(smem_u32(sm+114688)),"r"(32768),"r"(b):"memory");
  }
  if(recv){mbar_wait(bar+6+step,round&1);float* out=reinterpret_cast<float*>(sm+114688);const float* in=reinterpret_cast<float*>(sm+32768);
   for(int i=threadIdx.x;i<8192;i+=THREADS)out[i]+=in[i];
  }csync();
 });
}
TMN_DEVI void producer_loop(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 for(int tile=cid;tile<p.tiles;tile+=clusters,++round){int row=tile*64;
  ...
  csync();cluster_dx_reduce(sm,bar,round);
...
TMN_DEVI void dw_loop(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 for(int tile=cid;tile<p.tiles;tile+=clusters,++round){csync();mbar_wait(bar+3,round&1);dw_step(dwg,dwp,sm,half,round>0);csync();cluster_dx_reduce(sm,bar,round);csync();}
...
TMN_DEVI void compute_loop(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 ...
 for(int tile=cid;tile<p.tiles;tile+=clusters,++round){int row=tile*64;csync();mbar_wait(bar,round&1);float acc[64]={};
  ...
  csync();cluster_dx_reduce(sm,bar,round);
  if(rank==0){
   const float* local=reinterpret_cast<float*>(sm+114688);
   static_for<16>([&](auto qq){constexpr int q=decltype(qq)::value;int r=w*16+lane/4,c=q*8+2*(lane%4);
    acc[q*4]=local[r*128+c];acc[q*4+1]=local[r*128+c+1];acc[q*4+2]=local[(r+8)*128+c];acc[q*4+3]=local[(r+8)*128+c+1];});
...
void b7_joint(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar[9];__shared__ float gamma[128],beta[128];
 ...
 if(threadIdx.x==0){for(int i=0;i<9;++i)mbar_init(bar+i,1);fence_barrier_init();}
