...
TMN_DEVI void producer_loop(const Params& p,uint8_t* sm,uint64_t* bar){
 int rank=blockIdx.x%4,cid=blockIdx.x/4,clusters=gridDim.x/4,round=0;
 for(int tile=cid;tile<p.tiles;tile+=clusters,++round){int row=tile*64;
  if(threadIdx.x==0)mbar_arrive_expect_tx(bar,32768);csync();
...
TMN_DEVI void dw_loop(const Params& p,uint8_t* sm,uint64_t* bar){
 int rank=blockIdx.x%4,cid=blockIdx.x/4,clusters=gridDim.x/4,half=threadIdx.x/128-2,round=0;float dwg[64]={},dwp[64]={};
...
TMN_DEVI void compute_loop(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 ...
 for(int tile=cid;tile<p.tiles;tile+=clusters,++round){int row=tile*64;csync();mbar_wait(bar,round&1);float acc[64]={};
  ...
  if(rank==0){
   ...
   for(int other=1;other<4;++other){const float* remote=cooperative_groups::this_cluster().map_shared_rank(reinterpret_cast<float*>(sm+114688),other);
...
void b7_joint(__grid_constant__ const Params p){
 ...
 if(wi==0){setmaxnreg_dec<24>();producer_loop(p,sm,bar);}
 else if(wi==1){setmaxnreg_inc<200>();compute_loop(p,sm,bar,gamma,beta);}
 else{setmaxnreg_inc<144>();dw_loop(p,sm,bar);}
