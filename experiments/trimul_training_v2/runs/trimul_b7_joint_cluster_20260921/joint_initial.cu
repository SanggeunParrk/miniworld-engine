...
constexpr int THREADS=512,SMEM=180224;
...
TMN_DEVI void joint_dx(float (&acc)[64],uint8_t* sm){
 ...
 static_for<2>([&](auto cc){constexpr int chunk=decltype(cc)::value;static_for<4>([&](auto kk){constexpr int k=decltype(kk)::value;
...
TMN_DEVI void store_dw(const Params& p,float (&v)[64],int cluster,int rank,int half,int kind){
 int lane=threadIdx.x%32,w=(threadIdx.x%128)/32;float* dst=p.partw+((((cluster*4+rank)*2+half)*2+kind)*8192);
...
void b7_joint(__grid_constant__ const Params p){
 ...
 auto cl=cooperative_groups::this_cluster();int rank=blockIdx.x%4,cid=blockIdx.x/4,clusters=gridDim.x/4;
 ...
 if(wi==0)setmaxnreg_dec<24>();else if(wi==1)setmaxnreg_inc<200>();else setmaxnreg_inc<144>();
 if(threadIdx.x==0){mbar_arrive_expect_tx(bar+1,rank==0?98304:65536);
  for(int half=0;half<4;++half)for(int k=0;k<2;++k)tma_load_2d(sm+half*16384+k*8192,&p.wp,bar+1,k*64,rank*256+half*64);
 ...
 float dwg[64],dwp[64];float run_g=0,run_b=0;int round=0;
 for(int tile=cid;tile<p.tiles;tile+=clusters,++round){int row=tile*64;
  if(threadIdx.x==0)mbar_arrive_expect_tx(bar,32768);cl.sync();
  if(threadIdx.x==0){if(rank==0)for(int k=0;k<2;++k)multicast_xn(sm+65536+k*8192,&p.xn,bar,k*64,row);
   for(int h=0;h<4;++h)tma_load_2d(sm+81920+h*4096,rank<2?&p.dl:&p.dr,bar,row,(rank%2)*128+h*32);
  }
  float acc[64];
  if(wi==1){mbar_wait(bar,round&1);uint32_t xn[8][4];load_frag_bf16<8,8192>(xn,smem_u32(sm+65536),w*16,lane);allsync();
   uint32_t ma=__bfloat16_as_ushort(p.mask[row+ra]),mb=__bfloat16_as_ushort(p.mask[row+rb]);ma|=ma<<16;mb|=mb<<16;
   for(int h=0;h<2;++h)single_derivatives(p,xn,sm+h*32768,sm+81920+h*8192,sm+81920+h*8192,sm+98304+h*8192,row,ma,mb);
  ...
  }else if(wi>=2){mbar_wait(bar+3,round&1);dw_step(dwg,dwp,sm,wi-2,round>0);}
  __syncthreads();cl.sync();
  ...
  if(rank==0&&wi==1){
   for(int other=1;other<4;++other){const float* remote=cl.map_shared_rank(reinterpret_cast<float*>(sm+114688),other);
   ...
   });wgmma_commit();wgmma_wait<0>();fence_regs(gate);static_for<64>([&](auto jj){constexpr int j=decltype(jj)::value;acc[j]=math::round_bf16(acc[j]+math::round_bf16(gate[j]));});}
  uint8_t* lnsm=sm+65536;
 ...
 if(wi>=2){store_dw(p,dwg,cid,rank,wi-2,1);store_dw(p,dwp,cid,rank,wi-2,0);}
 if(rank==0&&wi==1){p.partln[cid*256+tid]=run_g;p.partln[cid*256+128+tid]=run_b;}
 ...
 for(int i=blockIdx.x*THREADS+threadIdx.x;i<131328;i+=gridDim.x*THREADS){float v=0;
  if(i<131072){int kind=i/32768,c=(i/256)%128,h=i%256,rk=(kind/2)*2+h/128,half=(h%128)/64,j=(h%64)*128+c;
   for(int a=0;a<clusters;++a)v+=reinterpret_cast<volatile float*>(p.partw)[((((a*4+rk)*2+half)*2+(kind&1))*8192+j)];p.dw[i]=__float2bfloat16_rn(v);
