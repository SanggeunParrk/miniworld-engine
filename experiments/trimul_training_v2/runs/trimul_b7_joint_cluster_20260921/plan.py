...
T=P.T
class Plan:
 def __init__(self,d,dy,dl,dr,dg,xn,clusters=32):
  self.d=d;self.xn=xn;self.clusters=clusters;self.count=clusters*4
  ...
  try:
   cfg=drv.drv.d.CUlaunchConfig();cfg.gridDimX=self.count;cfg.gridDimY=1;cfg.gridDimZ=1;cfg.blockDimX=512;cfg.blockDimY=1;cfg.blockDimZ=1;cfg.sharedMemBytes=180224
   ...
   self.clusters=min(clusters,int(result));self.count=self.clusters*4
 ...
 def __call__(self):
  L=T._launch_module();drv=self.k.unit.drv;args=L._Packed([self.params]);drv._unwrap('cuLaunchCooperativeKernel',drv.d.cuLaunchCooperativeKernel(drv.d.CUfunction(int(self.k.handle)),self.count,1,1,512,1,1,180224,drv.d.CUstream(int(torch.cuda.current_stream().cuda_stream)),ctypes.addressof(args.array)));return self.outputs
