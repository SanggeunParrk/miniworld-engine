"""Experimental B7 saved-xn shared-store elision; selected B1 unchanged."""
...
class Training(B.Training):
 def __init__(self,a):
  super().__init__(a)
  self();old=self.p7
  self.p7=RP.Plan(self.d,old.inputs[3],old.inputs[0],old.inputs[1],old.inputs[2],xn=old.xn,split=True,skip_restore=1,overlap=0,ln_mode=2,**{k:v for k,v in old.cfg.items() if k!='saved'})
  self.p7.mask=a['mask']
