...
T=P.T
@lru_cache(None)
def build(role,saved,splits,pc=2,dxctas=132,prod=40,cons=232,resident=1,slices=2,skip_restore=0,overlap=0,ln_mode=0):
 ...
 defs=dict(SPLIT_ROLE=role,USE_SAVED_XN=int(saved),DW_SPLITS=splits,PIPE_CONSUMERS=pc,DX_CTAS=dxctas,DW_PROD_REGS=prod,DW_CONS_REGS=cons,UCOUNT=count,DX_RESIDENT=resident,WGRAD_SLICES=slices,B7_SKIP_XN_RESTORE=skip_restore,DW_OVERLAP=overlap,B7_LN_MODE=ln_mode)
...
class Plan:
 def __init__(self,d,dy,dl,dr,dg,xn=None,split=False,splits=4,pc=2,dxctas=132,prod=40,cons=232,resident=1,slices=2,skip_restore=0,overlap=0,ln_mode=0):
  self.d=d;self.split=split;self.xn=xn;self.cfg=dict(saved=xn is not None,splits=splits,pc=pc,dxctas=dxctas,prod=prod,cons=cons,resident=resident,slices=slices,skip_restore=skip_restore,overlap=overlap,ln_mode=ln_mode)
