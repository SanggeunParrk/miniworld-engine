...
with torch.no_grad():
 ...
 for mode in (0,1,2):
  skip,overlap=1,0
  row=dict(skip_restore=skip,overlap=overlap,ln_mode=mode)
  try:
   p=RP.Plan(m.d,*[m.p7.inputs[i] for i in (3,0,1,2)],xn=m.p7.xn,split=True,skip_restore=skip,overlap=overlap,ln_mode=mode,**cfg);p.mask=m.p7.mask;p.bind(*m.p7.inputs[:4],xn=m.p7.xn)
