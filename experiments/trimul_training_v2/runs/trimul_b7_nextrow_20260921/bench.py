...
import argparse,gc,hashlib,json,platform,sys,torch,importlib.util
from gate_policy import Training,B,BASE
R=Path(__file__).resolve().parent
sp=importlib.util.spec_from_file_location('validated_b7_v62',R.parent/'trimul_b7_weight_prefetch_20260921/gate_policy.py');S62=importlib.util.module_from_spec(sp);sp.loader.exec_module(S62)
...
def passed(e):return all(v['finite'] and (v['relative_l2']<=5e-6 if k in ('dgamma_in','dbeta_in') else v['bit_exact']) for k,v in e.items())
def mutate(a):
...
def main():
 ...
 with torch.no_grad():
  a=Q.setup(n);d=a['d'];models={'baseline':B.Training(a),'v62':S62.Training(a),'optimized':Training(a)}
