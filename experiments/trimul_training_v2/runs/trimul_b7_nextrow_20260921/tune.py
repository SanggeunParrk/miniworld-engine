...
with torch.no_grad():
 ...
 for nextrow in (0,1,2):
  ...
  cfg["dxctas"]=dxctas
  dw_mask_early=1
  xn_early,mask_hoist=0,1
  mode=2
  skip,overlap=1,0
  row=dict(nextrow=nextrow,dw_pair=dw_pair,dx_pc=dx_pc,dxctas=dxctas,skip_restore=skip,overlap=overlap,ln_mode=mode,xn_early=xn_early,mask_hoist=mask_hoist,dw_mask_early=dw_mask_early,weight_prefetch=weight_prefetch)
  try:
   p=RP.Plan(m.d,*[m.p7.inputs[i] for i in (3,0,1,2)],xn=m.p7.xn,split=True,skip_restore=skip,overlap=overlap,ln_mode=mode,xn_early=xn_early,mask_hoist=mask_hoist,dw_mask_early=dw_mask_early,weight_prefetch=weight_prefetch,dx_pc=dx_pc,dw_pair=dw_pair,nextrow=nextrow,**cfg);p.mask=m.p7.mask;p.bind(*m.p7.inputs[:4],xn=m.p7.xn)
   out=p();torch.cuda.synchronize();e=[H.rel(x,y) for x,y in zip(out,ref)];exact=[torch.equal(x,y) for x,y in zip(out,ref)];assert all(exact[:5]) and all(v<=5e-6 for v in e[5:]),(e,exact)
