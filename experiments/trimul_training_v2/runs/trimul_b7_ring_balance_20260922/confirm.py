...
p=argparse.ArgumentParser();p.add_argument('--configs',default='10:10:32,10:10:40,10:10:48,10:10:56,10:10:64');p.add_argument('--cases',type=int,default=3);args=p.parse_args()
tag='confirmation'
...
with torch.no_grad():
 ...
 old=module('audited_ring',R.parent/'trimul_b7_ring_overlap_sync_20260922/plan.py')
 q=old.Plan(d,dy,dl,dr,dg,xn=refplan.xn,clusters=10,mode=10);q.mask=refplan.mask;q.bind(dl,dr,dg,dy,xn=refplan.xn);plans['ring_baseline']=q
 ancient=module('old400',R.parent/'trimul_b7_joint_cachelast_20260922/plan.py')
 ...
 prev=module('candidate_16k',R.parent/'trimul_b7_ring_chunk_20260922/plan.py')
 q=prev.Plan(d,dy,dl,dr,dg,xn=refplan.xn,clusters=10,mode=4);q.mask=refplan.mask;q.bind(dl,dr,dg,dy,xn=refplan.xn);plans['previous_16k']=q
 for spec in args.configs.split(','):
  groups,cons,regs=map(int,spec.split(':'));os.environ['B7_CONSUMERS']=str(cons);os.environ['B7_PRODUCER_REGS']=str(regs);mode=4
  q=plan.Plan(d,dy,dl,dr,dg,xn=refplan.xn,clusters=groups,mode=mode)
  q.mask=refplan.mask;q.bind(dl,dr,dg,dy,xn=refplan.xn);plans[f'g{groups}u{cons}p{regs}']=q
 ...
 good={n:graphs[n] for n in plans if all(c[n]['valid'] for c in rec['checks'].values())};assert 'split' in good and 'ring_baseline' in good
 assert len(good)==len(plans), 'Incorrect candidate: no timing selection allowed'
 rounds=[];rec['telemetry']=[]
 import subprocess
 for _ in range(5):
  rounds.append(H.Q.paired(good,iterations=160))
  rec['telemetry'].append(subprocess.run(['nvidia-smi','--query-gpu=uuid,clocks.sm,clocks.mem,power.draw,temperature.gpu','--format=csv,noheader'],capture_output=True,text=True).stdout)
