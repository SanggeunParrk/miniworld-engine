...
TMN_DEVI void consumer_producer(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 if(threadIdx.x>=32){int lane=threadIdx.x%32;
  for(int tile=cid+2*rank*groups;tile<p.tiles;tile+=2*CONSUMERS*groups,++round){
   int sequence=2*(rank+round*CONSUMERS),rs0=sequence%RINGS,rs1=(sequence+1)%RINGS;bool second=tile+groups<p.tiles;
   unsigned* flags=p.flags+(cid*RINGS+rs0)*(SOURCES+2);
   if(round>0&&lane==0)mbar_wait(bar+17,(round-1)&1);__syncwarp();
   for(int phase=0;phase<8;++phase){int slot=phase%2,plane=phase/2,chunk_source=(phase%2)*4;
    if(phase>=2&&lane==0)mbar_wait(bar+32+slot,((phase/2)-1)&1);__syncwarp();
    if((plane&1)==0&&lane<4)ring_wait(flags+1+(plane/2)*8+chunk_source+lane,sequence+1);
    __syncwarp();
    if(lane==0){mbar_arrive_expect_tx(bar+24+slot,16384*(second?2:1));
     bulk_load(sm+slot*16384,p.ring+(cid*RINGS+rs0)*131072+phase*16384,16384,bar+24+slot);
     if(second)bulk_load(sm+32768+slot*16384,p.ring+(cid*RINGS+rs1)*131072+phase*16384,16384,bar+24+slot);
    }
   }
   if(lane==0){mbar_wait(bar+25,1);publish(flags+SOURCES+1,sequence+1);if(second)publish(p.flags+(cid*RINGS+rs1)*(SOURCES+2)+SOURCES+1,sequence+2);}
   __syncwarp();
  }return;
 }
 if(threadIdx.x)return;
 for(int tile=cid+2*rank*groups;tile<p.tiles;tile+=2*CONSUMERS*groups,++round){bool second=tile+groups<p.tiles;
  ...
  for(int stage=0;stage<16;++stage){int ws=stage%2,phase=stage/4,h=stage%4;
   ...
   if(B7_PAIR_PREFETCH&&stage==12)for(int k=0;k<2;++k)tma_load_2d(sm+98304+k*8192,&p.x,bar+12,k*64,tile*64);
   mbar_arrive_expect_tx(bar+2+ws,16384);tma_load_2d(sm+65536+ws*16384,p.wt+phase,bar+2+ws,h*64,0);
  }
  mbar_wait(bar+7,1);
  for(int outrow=0;outrow<(second?2:1);++outrow){int row=(tile+outrow*groups)*64;
   mbar_arrive_expect_tx(bar+19,16384);for(int k=0;k<2;++k)tma_load_2d(sm+16384+k*8192,&p.dg,bar+19,k*64,row);
   if(outrow)mbar_arrive_expect_tx(bar+12,32768);
   for(int k=0;k<2;++k){if(outrow||!B7_PAIR_PREFETCH)tma_load_2d(sm+98304+k*8192,&p.x,bar+12,k*64,row);tma_load_2d(sm+k*8192,&p.res,bar+12,k*64,row);if(!outrow)tma_load_2d(sm+65536+k*16384,&p.wgate,bar+12,k*64,0);}
   mbar_wait(bar+18,outrow&1);
  }
...
TMN_DEVI void consumer_compute(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 ...
 for(int tile=cid+2*rank*clusters;tile<p.tiles;tile+=2*CONSUMERS*clusters,++round){bool second=tile+clusters<p.tiles;
  float acc[64]={};
  {float other[64]={};fence_regs(acc);fence_regs(other);wgmma_fence();
   for(int phase=0;phase<8;++phase){int slot=phase%2;mbar_wait(bar+24+slot,(phase/2)&1);
    for(int sub=0;sub<2;++sub){int stage=phase*2+sub,ws=stage%2;
     mbar_wait(bar+2+ws,(stage/2)&1);
     static_for<4>([&](auto kk){constexpr int k=decltype(kk)::value;
      input128(acc,smem_desc(smem_u32(sm+slot*16384+sub*8192+k*2048),16,1024,1),smem_desc(smem_u32(sm+65536+ws*16384+k*32),16,1024,1),stage>0||k>0);
     });wgmma_commit();
     if(second){static_for<4>([&](auto kk){constexpr int k=decltype(kk)::value;
      input128(other,smem_desc(smem_u32(sm+32768+slot*16384+sub*8192+k*2048),16,1024,1),smem_desc(smem_u32(sm+65536+ws*16384+k*32),16,1024,1),stage>0||k>0);
     });wgmma_commit();}
    }
    wgmma_wait<0>();fence_regs(acc);fence_regs(other);allsync();if(tid==0){mbar_arrive(bar+6);mbar_arrive(bar+7);mbar_arrive(bar+32+slot);}
   }
   if(second){float* tmp=reinterpret_cast<float*>(sm+32768);static_for<64>([&](auto jj){constexpr int j=decltype(jj)::value;tmp[j*128+tid]=other[j];});}
  }
  allsync();
  for(int outrow=0;outrow<(second?2:1);++outrow){int row=(tile+outrow*clusters)*64;
   if(outrow){float* tmp=reinterpret_cast<float*>(sm+32768);static_for<64>([&](auto jj){constexpr int j=decltype(jj)::value;acc[j]=tmp[j*128+tid];});}
   mbar_wait(bar+12,outrow&1);
#if B7_GATEFIRST
...
#else
  {float gate[64]={};uint32_t unused[8][4];mbar_wait(bar+19,outrow&1);wide_gate(gate,unused,sm+65536,sm+16384);static_for<64>([&](auto jj){constexpr int j=decltype(jj)::value;acc[j]=math::round_bf16(acc[j]+math::round_bf16(gate[j]));});}
#endif
  uint8_t* lnsm=sm+98304;
  ...
  float* tmp=reinterpret_cast<float*>(sm+16384);
  static_for<8>([&](auto qq){constexpr int q=decltype(qq)::value;
   static_for<2>([&](auto pp){constexpr int pair=decltype(pp)::value;int j=pair*2,c=q*16+2*(lane%4)+8*pair;
    ...
    uint8_t* out=lnsm+(q/4)*8192;uint8_t* res=sm+(q/4)*8192;
...
void b7_joint(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){for(int i=0;i<40;++i)mbar_init(bar+i,1);fence_barrier_init();}__syncthreads();
 if(rank<SOURCES){
