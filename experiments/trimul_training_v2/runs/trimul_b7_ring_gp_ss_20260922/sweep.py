...
p=argparse.ArgumentParser();p.add_argument('--configs',default='10:10:12:32,10:10:12:48,10:10:12:64');p.add_argument('--cases',type=int,default=5);args=p.parse_args()
...
with torch.no_grad():
 ...
 for spec in args.configs.split(','):
  groups,cons,depth,regs=map(int,spec.split(':'));os.environ['B7_CONSUMERS']=str(cons);os.environ['B7_RING_DEPTH']=str(depth);os.environ['B7_PRODUCER_REGS']=str(regs)
  q=plan.Plan(d,dy,dl,dr,dg,xn=refplan.xn,clusters=groups,mode=52)
  q.mask=refplan.mask;q.bind(dl,dr,dg,dy,xn=refplan.xn);plans[f'g{groups}u{cons}r{depth}p{regs}']=q
