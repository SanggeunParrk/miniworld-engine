...
p=argparse.ArgumentParser();p.add_argument('--configs',default='2:10:10:0,2:10:10:1');p.add_argument('--cases',type=int,default=5);args=p.parse_args()
tag='confirmation'
...
with torch.no_grad():
 ...
 for spec in args.configs.split(','):
  cluster,groups,cons,mc=map(int,spec.split(':'));os.environ['B7_HW_CLUSTER']=str(cluster);os.environ['B7_MULTICAST']=str(mc);os.environ['B7_CONSUMERS']=str(cons)
  q=plan.Plan(d,dy,dl,dr,dg,xn=refplan.xn,clusters=groups,mode=52)
  q.mask=refplan.mask;q.bind(dl,dr,dg,dy,xn=refplan.xn);plans[f'c{cluster}g{groups}u{cons}m{mc}']=q
 reuse=module('reuse_comparison',R.parent/'trimul_b7_ring_sourcepair_20260922/plan.py')
 os.environ['B7_CONSUMERS']='5';os.environ['B7_RING_DEPTH']='6';os.environ['B7_PRODUCER_REGS']='32'
 q=reuse.Plan(d,dy,dl,dr,dg,xn=refplan.xn,clusters=20,mode=52);q.mask=refplan.mask;q.bind(dl,dr,dg,dy,xn=refplan.xn);plans['source_reuse']=q
 ...
 rounds=[H.Q.paired(good,iterations=160) for _ in range(5)]
