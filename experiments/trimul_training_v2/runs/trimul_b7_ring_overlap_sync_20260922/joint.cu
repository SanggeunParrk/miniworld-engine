...
#endif
constexpr int SOURCES=16,CONSUMERS=B7_CONSUMERS,GROUP=SOURCES+CONSUMERS,RINGS=8;
#define allsync() named_bar_sync(1+threadIdx.x/128,128)
struct Params{CUtensorMap xn,wp,dl,dr,dg,wgate,x,res,dx,wt[4];const __nv_bfloat16* mask;const float *gamma,*beta;__nv_bfloat16 *dxptr,*dw;float *dgam,*dbeta,*partw,*partln;unsigned int* counts;int M,tiles;uint8_t *ring,*xring;unsigned* flags;};
...
TMN_DEVI void multicast_xn(void* dst,const CUtensorMap* map,uint64_t* bar,int c,int row){asm volatile("cp.async.bulk.tensor.2d.shared::cluster.global.mbarrier::complete_tx::bytes.multicast::cluster [%0],[%1,{%3,%4}],[%2],%5;"::"r"(smem_u32(dst)),"l"(map),"r"(smem_u32(bar)),"r"(c),"r"(row),"h"(uint16_t(15)):"memory");}
...
TMN_DEVI void source_producer(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 if(threadIdx.x==32){int rounds=(p.tiles-1-cid)/groups+1;
  for(int round=0;round<rounds;round+=2){bool second=round+1<rounds;mbar_wait(bar+19,(round/2)&1);
   bulk_store(p.ring+(cid*RINGS+round%RINGS)*131072+rank*8192,sm+98304,8192);
   if(second)bulk_store(p.ring+(cid*RINGS+(round+1)%RINGS)*131072+rank*8192,sm+106496,8192);
...
TMN_DEVI void store_pair_dw(const Params& p,float (&v)[64],int part,int rank){
...
TMN_DEVI void consumer_producer(const Params& p,uint8_t* sm,uint64_t* bar){
 int rank=blockIdx.x%GROUP-SOURCES,cid=blockIdx.x/GROUP,groups=gridDim.x/GROUP,round=0;if(threadIdx.x>=32)return;
 for(int tile=cid+rank*groups;tile<p.tiles;tile+=CONSUMERS*groups,++round){int row=tile*64,sequence=rank+round*CONSUMERS,rs=sequence%RINGS;unsigned* flags=p.flags+(cid*RINGS+rs)*(SOURCES+2);
  if(threadIdx.x==0){mbar_arrive_expect_tx(bar+19,16384);for(int k=0;k<2;++k)tma_load_2d(sm+98304+k*8192,&p.dg,bar+19,k*64,row);mbar_arrive_expect_tx(bar+12,65536);}
  for(int side=0;side<2;++side){
#if B7_PREFETCH == 1 || B7_PREFETCH == 3
   // Weight data is independent of ring readiness; prime both free stages now.
   if(threadIdx.x==0)for(int stage=0;stage<2;++stage){
    mbar_arrive_expect_tx(bar+2+stage,16384);
    tma_load_2d(sm+65536+stage*16384,p.wt+side*2,bar+2+stage,stage*64,0);
   }
#endif
#if B7_PREFETCH == 3
   if(threadIdx.x==0)mbar_arrive_expect_tx(bar,65536);
   __syncwarp();
   // Issue each 8 KiB source slice as soon as its release flag is observed.
   bool done=threadIdx.x>=8;
   while(__any_sync(0xffffffff,!done)){
    if(!done){unsigned got;const unsigned* fp=flags-(sequence&1)*(SOURCES+2)+1+side*8+threadIdx.x;
     asm volatile("ld.acquire.gpu.global.u32 %0,[%1];":"=r"(got):"l"(fp):"memory");
     if(got>=unsigned((sequence&~1)+1)){
      bulk_load(sm+threadIdx.x*8192,p.ring+(cid*RINGS+rs)*131072+side*65536+threadIdx.x*8192,8192,bar);done=true;
     }
    }
   }
   __syncwarp();if(threadIdx.x==0){
#else
   if(threadIdx.x<8)ring_wait(flags-(sequence&1)*(SOURCES+2)+1+side*8+threadIdx.x,(sequence&~1)+1);
   __syncwarp();if(threadIdx.x==0){
    mbar_arrive_expect_tx(bar,65536);bulk_load(sm,p.ring+(cid*RINGS+rs)*131072+side*65536,65536,bar);
#endif
#if B7_PREFETCH == 2
...
#endif
   mbar_wait(bar,side);if(side==1)publish(flags+SOURCES+1,sequence+1);
   for(int stage=(B7_PREFETCH?2:0);stage<8;++stage){int ws=stage%2;
    if(stage>=2)mbar_wait(bar+6+ws,((stage/2)-1)&1);
#if B7_LNPREFETCH
    if(side==1&&stage==(B7_ROLLING?7:6))for(int k=0;k<2;++k){
     tma_load_2d(sm+LN+k*8192,&p.x,bar+12,k*64,row);
     tma_load_2d(sm+LN+16384+k*8192,&p.res,bar+12,k*64,row);
    }
#endif
    mbar_arrive_expect_tx(bar+2+ws,16384);
    tma_load_2d(sm+65536+ws*16384,p.wt+side*2+stage/4,bar+2+ws,(stage%4)*64,0);
   }
   mbar_wait(bar+14,side);}
   __syncwarp();
  }
  if(threadIdx.x==0){
#if !B7_LNPREFETCH
  for(int k=0;k<2;++k){tma_load_2d(sm+LN+k*8192,&p.x,bar+12,k*64,row);tma_load_2d(sm+LN+16384+k*8192,&p.res,bar+12,k*64,row);}
#endif
...
TMN_DEVI void consumer_compute(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 ...
 for(int tile=cid+rank*clusters;tile<p.tiles;tile+=CONSUMERS*clusters,++round){int row=tile*64;
  ...
  for(int side=0;side<2;++side){mbar_wait(bar,side);
#if B7_ROLLING
   for(int stage=0;stage<8;++stage){int kind=stage/4,h=stage%4,ws=stage%2;
    mbar_wait(bar+2+ws,(stage/2)&1);uint8_t* ds=sm+h*16384+kind*4096;
    static_for<4>([&](auto kk){constexpr int k=decltype(kk)::value;
     input128(acc,smem_desc(smem_u32(ds+(k/2)*8192+(k%2)*2048),16,1024,1),smem_desc(smem_u32(sm+65536+ws*16384+k*32),16,1024,1),side>0||stage>0||k>0);
    });wgmma_commit();
    if(stage>0){wgmma_wait<1>();allsync();if(tid==0)mbar_arrive(bar+6+(stage-1)%2);}
   }
   wgmma_wait<0>();fence_regs(acc);allsync();if(tid==0)mbar_arrive(bar+7);
#else
   for(int batch=0;batch<4;++batch){for(int sub=0;sub<2;++sub){int stage=batch*2+sub,kind=stage/4,h=stage%4,ws=stage%2;
    mbar_wait(bar+2+ws,(stage/2)&1);uint8_t* ds=sm+h*16384+kind*4096;
    static_for<4>([&](auto kk){constexpr int k=decltype(kk)::value;
     input128(acc,smem_desc(smem_u32(ds+(k/2)*8192+(k%2)*2048),16,1024,1),smem_desc(smem_u32(sm+65536+ws*16384+k*32),16,1024,1),side>0||stage>0||k>0);
    });wgmma_commit();}
    wgmma_wait<0>();fence_regs(acc);allsync();if(tid==0)for(int ws=0;ws<2;++ws)mbar_arrive(bar+6+ws);
   }
#endif
   if(tid==0)mbar_arrive(bar+14);
...
void b7_joint(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar[24];__shared__ float gamma[128],beta[128];
 ...
 if(threadIdx.x==0){for(int i=0;i<24;++i)mbar_init(bar+i,1);fence_barrier_init();}__syncthreads();
 if(rank<SOURCES){
  ...
  if(wi==0){setmaxnreg_dec<48>();source_producer(p,sm,bar);}else{setmaxnreg_inc<208>();source_compute(p,sm,bar);}
 }else{
  if(wi==0){setmaxnreg_dec<48>();consumer_producer(p,sm,bar);}else{setmaxnreg_inc<208>();consumer_compute(p,sm,bar,gamma,beta);}
 }
 cooperative_groups::this_grid().sync();
 for(int i=blockIdx.x*THREADS+threadIdx.x;i<131328;i+=gridDim.x*THREADS){float v=0;
  if(i<131072){int kind=i/32768,c=(i/256)%128,h=i%256,rk=(kind/2)*8+h/32,j=((kind&1)?h%32:32+h%32)*128+c;
   for(int a=0;a<groups;++a)v+=reinterpret_cast<volatile float*>(p.partw)[(a*SOURCES+rk)*8192+j];p.dw[i]=__float2bfloat16_rn(v);
