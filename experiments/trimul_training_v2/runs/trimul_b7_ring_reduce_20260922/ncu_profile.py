...
cmd=['ncu','--profile-from-start','off','--section','MemoryWorkloadAnalysis','--section','SourceCounters','--section','SchedulerStats','--metrics',metrics,'--cache-control','none','--clock-control','none','--target-processes','all','--kernel-name','regex:^b7_joint$','--launch-count','1','--force-overwrite','-o',str(stem),'env','PYTHONPATH='+os.environ.get('PYTHONPATH',''),sys.executable,'-B',str(R/'probe.py')]
