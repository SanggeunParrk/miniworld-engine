...
TMN_DEVI void source_compute(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 for(int round=0,tile=cid;tile<p.tiles;tile+=groups,++round){int slot=round%2;uint8_t* src=sm+slot*24576;uint8_t* sg=sm+81920+slot*16384;
  ...
  {float a[32]={};pair_gp(a,xn,smem_desc(smem_u32(sm+49152),16,1024,1));wgmma_wait<0>();fence_regs(a);
#if B7_SOURCE_PIPE & 2
   if(round>0){fence_regs(dw1);allsync();if(tid==0)mbar_arrive(bar+4+(round-1)%2);}
#endif
pair_glu(a,src,sg,ma,mb);}
...
#if !(B7_SOURCE_PIPE & 1)
  wgmma_wait<0>();fence_regs(dw0);
#endif
  {float a[32]={};pair_gp(a,xn,smem_desc(smem_u32(sm+65536),16,1024,1));wgmma_wait<0>();fence_regs(dw0);fence_regs(a);pair_glu(a,src+4096,sg+8192,ma,mb);}
  fence_proxy_async();allsync();if(tid==0)mbar_arrive(bar+19+slot);
  fence_regs(dw1);wgmma_fence();
  static_for<4>([&](auto kk){constexpr int k=decltype(kk)::value;mma_weight128(dw1,smem_desc(smem_u32(sg+8192+k*32),16,1024,1),smem_desc(smem_u32(src+k*2048),8192,1024,1),round>0||k>0);});wgmma_commit();
#if B7_SOURCE_PIPE & 2
  if(tile+groups>=p.tiles){
#endif
  wgmma_wait<0>();fence_regs(dw1);allsync();if(tid==0)mbar_arrive(bar+4+slot);
#if B7_SOURCE_PIPE & 2
  }
#endif
 }
