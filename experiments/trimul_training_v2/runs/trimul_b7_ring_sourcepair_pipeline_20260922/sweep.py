...
p=argparse.ArgumentParser();p.add_argument('--pipes',default='1,2,3');p.add_argument('--cases',type=int,default=5);args=p.parse_args()
tag='pipes-'+args.pipes.replace(',','_')
...
with torch.no_grad():
 ...
 prev=module('source_reuse',R.parent/'trimul_b7_ring_sourcepair_20260922/plan.py')
 os.environ['B7_CONSUMERS']='5';os.environ['B7_RING_DEPTH']='6';os.environ['B7_PRODUCER_REGS']='32'
 q=prev.Plan(d,dy,dl,dr,dg,xn=refplan.xn,clusters=20,mode=52);q.mask=refplan.mask;q.bind(dl,dr,dg,dy,xn=refplan.xn);plans['sourcepair_base']=q
 for pipe in args.pipes.split(','):
  os.environ['B7_SOURCE_PIPE']=pipe
  q=plan.Plan(d,dy,dl,dr,dg,xn=refplan.xn,clusters=20,mode=52)
  q.mask=refplan.mask;q.bind(dl,dr,dg,dy,xn=refplan.xn);plans[f'pipe{pipe}']=q
