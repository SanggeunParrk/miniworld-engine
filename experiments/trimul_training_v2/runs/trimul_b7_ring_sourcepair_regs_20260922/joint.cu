...
TMN_DEVI void source_compute(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 for(int round=0,tile=cid;tile<p.tiles;tile+=groups,++round){int slot=round%2;uint8_t* src=sm+slot*24576;uint8_t* sg=sm+81920+slot*16384;
  mbar_wait(bar+slot,(round/2)&1);uint32_t xn[8][4];load_frag_bf16<8,8192>(xn,smem_u32(src),w*16,lane);
  ...
  {float a[32]={};pair_gp(a,xn,smem_desc(smem_u32(sm+49152),16,1024,1));wgmma_wait<0>();fence_regs(a);pair_glu(a,src,sg,ma,mb);}
  fence_proxy_async();allsync();fence_regs(dw0);wgmma_fence();
  static_for<4>([&](auto kk){constexpr int k=decltype(kk)::value;mma_weight128(dw0,smem_desc(smem_u32(sg+k*32),16,1024,1),smem_desc(smem_u32(src+k*2048),8192,1024,1),round>0||k>0);});wgmma_commit();wgmma_wait<0>();fence_regs(dw0);
  {float a[32]={};pair_gp(a,xn,smem_desc(smem_u32(sm+65536),16,1024,1));wgmma_wait<0>();fence_regs(a);pair_glu(a,src+4096,sg+8192,ma,mb);}
  fence_proxy_async();allsync();if(tid==0)mbar_arrive(bar+19+slot);
  fence_regs(dw1);wgmma_fence();
  static_for<4>([&](auto kk){constexpr int k=decltype(kk)::value;mma_weight128(dw1,smem_desc(smem_u32(sg+8192+k*32),16,1024,1),smem_desc(smem_u32(src+k*2048),8192,1024,1),round>0||k>0);});wgmma_commit();wgmma_wait<0>();fence_regs(dw1);
