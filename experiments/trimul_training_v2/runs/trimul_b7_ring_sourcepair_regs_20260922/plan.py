...
class Plan:
 def __init__(self,d,dy,dl,dr,dg,xn,clusters=10,mode=0):
  self.d=d;self.xn=xn;self.rings=int(os.environ.get("B7_RING_DEPTH","8"));self.clusters=clusters;self.consumers=int(os.environ.get("B7_CONSUMERS","8"));self.reuse_dp=1;self.count=clusters*(8+self.consumers)
  inc=T._upstream()/'csrc';flags=['-DB7_DX_PRODUCER_REGS='+os.environ.get('B7_DX_PRODUCER_REGS','64'),'-DB7_RING_DEPTH='+str(self.rings),'-DB7_TRANSPOSE_PART='+str(bool(mode&16)*1),'-DB7_REDUCE_NATIVE='+str(bool(mode&32)*1),'-DB7_PRODUCER_REGS='+os.environ.get('B7_PRODUCER_REGS','48'),'-DB7_RING_CHUNK='+str(8192 if mode&8 else 16384 if mode&4 else 32768),'-DB7_GATEFIRST='+str(bool(mode&2)*1),'-DB7_RING_TENSOR='+str(mode&1),'-DB7_ROLLING='+str(bool(mode&4)*1),'-DB7_LNPREFETCH='+str(bool(mode&8)*1),'-DB7_PREFETCH='+str(mode&3),'-DB7_CONSUMERS='+str(self.consumers),'-DB7_REUSE_DP='+str(self.reuse_dp),'-std=c++17','-O3','-arch=sm_90a','--cubin','-lineinfo','-Xptxas=-v','-I'+str(inc),'-I'+str(R.parent/'anthropic_b7b12_fusion_20260920'),'-I'+str(R.parent/'trimul_b7_nextrow_20260921')]
