...
for mode in (0,):
 ...
 cmd=['ncu','--profile-from-start','off','--section','SourceCounters','--section','SpeedOfLight','--import-source','yes','--metrics',metrics,'--cache-control','none','--clock-control','none','--target-processes','all','--kernel-name','regex:^front_b7b12$','--launch-count','2','--force-overwrite','-o',str(stem),'env','PYTHONPATH='+os.environ.get('PYTHONPATH',''),sys.executable,'-B',str(R/'baseline.py'),'--length',str(n),'--profile']
