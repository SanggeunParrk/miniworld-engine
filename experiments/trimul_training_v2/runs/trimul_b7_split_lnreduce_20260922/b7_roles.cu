...
#elif SPLIT_ROLE == 2
 int c=blockIdx.x*32+(threadIdx.x/32)*4+threadIdx.x%4;
 if(c<256){float v=0.f;int bl=(threadIdx.x%32)/4;
  for(int b=bl;b<DXCOUNT;b+=8)v+=reinterpret_cast<volatile float*>(p.partln)[b*256+c];
  for(int sh=4;sh<32;sh*=2)v+=__shfl_xor_sync(0xffffffff,v,sh);
  if(bl==0)(c<128?p.dgam:p.dbeta)[c%128]=v;
 }
#else
