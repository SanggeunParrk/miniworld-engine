...
template<int G> TMN_DEVI void consumer_compute(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 constexpr int g=G;int rank=2*(blockIdx.x%GROUP-SOURCES)+g,cid=blockIdx.x/GROUP,clusters=gridDim.x/GROUP,round=0,tid=threadIdx.x%128,lane=tid%32,w=tid/32,ra=w*16+lane/4,rb=ra+8;float run_g=0,run_b=0;
...
#undef B7_ACC
extern "C" __global__ __launch_bounds__(384,2) __cluster_dims__(B7_HW_CLUSTER,1,1)
void b7_joint(__grid_constant__ const Params p){
 ...
 if(rank<SOURCES){
  ...
  if(wi==0){setmaxnreg_dec<32>();pair_producer(p,sm,bar);}else if(wi==1){setmaxnreg_inc<104>();consumer_compute<0>(p,sm,bar,gamma,beta);}else{setmaxnreg_inc<104>();consumer_compute<1>(p,sm,bar,gamma,beta);}
 }
