...
stem=R/('ncu-L384'+os.environ.get('PROFILE_TAG',''))
...
cmd=['ncu','--profile-from-start','off','--section','MemoryWorkloadAnalysis','--section','SourceCounters','--section','SchedulerStats','--metrics',metrics,'--cache-control','none','--clock-control','none','--target-processes','all','--kernel-name','regex:^front_b7b12_(dw|dx)$','--launch-count','2','--force-overwrite','-o',str(stem),'env','PYTHONPATH='+os.environ.get('PYTHONPATH',''),sys.executable,'-B',str(R/'probe.py')]
