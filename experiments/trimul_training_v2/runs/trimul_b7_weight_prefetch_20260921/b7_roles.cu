...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  ...
  for(int side=0;side<2;++side){
   if(threadIdx.x==0 && (!B7_WEIGHT_PREFETCH || side==0)){mbar_arrive_expect_tx(bar+7,131072);for(int h=0;h<4;++h)for(int wg=0;wg<2;++wg)for(int k=0;k<2;++k)tma_load_2d(sm+h*32768+wg*16384+k*8192,&p.pre,bar+7,k*64,(side*4+h)*128+wg*64);}
...
#if B7_WEIGHT_PREFETCH
   if(side==0){
    if(threadIdx.x==0)mbar_arrive_expect_tx(bar+7,131072);
    auto next_weight=[&](int h){if(threadIdx.x)return;for(int wg=0;wg<2;++wg)for(int k=0;k<2;++k)tma_load_2d(sm+h*32768+wg*16384+k*8192,&p.pre,bar+7,k*64,(4+h)*128+wg*64);};
    fence_regs(acc);wgmma_fence();
    static_for<4>([&](auto hh){constexpr int h=decltype(hh)::value;
     ...
     if constexpr(h>0){wgmma_wait<1>();allsync();next_weight(h-1);}
    });wgmma_wait<0>();fence_regs(acc);allsync();next_weight(3);allsync();
   }else
#endif
   {
   fence_regs(acc);wgmma_fence();
