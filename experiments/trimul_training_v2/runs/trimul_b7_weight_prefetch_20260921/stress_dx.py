...
with torch.no_grad():
 ...
 p=RP.Plan(m.d,dy,dl,dr,dg,xn=p0.xn,split=True,skip_restore=1,overlap=0,ln_mode=2,xn_early=1,mask_hoist=1,dw_mask_early=1,weight_prefetch=1,**cfg);p.mask=p0.mask;p.bind(dl,dr,dg,dy,xn=p0.xn)
 ...
 for case in range(3):
  if case:
   dl.mul_(.93);dr.mul_(1.03);p.xn.mul_(.96);a['mask'].copy_(1-a['mask']);a['d']['w1'].mul_(1.002);a['d']['x'].mul_(.99);a['d']['gi'].mul_(1.01);dg.mul_(.94);dy.mul_(1.02)
  ...
  records.append(dict(case=case,replays=2,dX_LN_bit_exact=True,counters_zero=True));print('STRESS_DX_PASS',n,case,flush=True)
