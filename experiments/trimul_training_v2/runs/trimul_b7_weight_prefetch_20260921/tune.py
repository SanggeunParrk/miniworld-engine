...
with torch.no_grad():
 ...
 for weight_prefetch in (0,1):
  dw_mask_early=1
  xn_early,mask_hoist=1,1
  mode=2
  skip,overlap=1,0
  row=dict(skip_restore=skip,overlap=overlap,ln_mode=mode,xn_early=xn_early,mask_hoist=mask_hoist,dw_mask_early=dw_mask_early,weight_prefetch=weight_prefetch)
  try:
   p=RP.Plan(m.d,*[m.p7.inputs[i] for i in (3,0,1,2)],xn=m.p7.xn,split=True,skip_restore=skip,overlap=overlap,ln_mode=mode,xn_early=xn_early,mask_hoist=mask_hoist,dw_mask_early=dw_mask_early,weight_prefetch=weight_prefetch,**cfg);p.mask=m.p7.mask;p.bind(*m.p7.inputs[:4],xn=m.p7.xn)
