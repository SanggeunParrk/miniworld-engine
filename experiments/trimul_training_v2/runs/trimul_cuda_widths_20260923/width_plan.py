...
def build(D):
 ...
 for k in ks.values():k.set_max_dynamic_smem(33024 if D==64 else 49280)
 return ks,str(out)
def launch(k,params,grid=132,cooperative=True,D=64):
 threads=128 if D==64 else 256;smem=33024 if D==64 else 49280
 L=T._launch_module()
...
class Training:
 def __init__(self,x,wl,wlg,wr,wrg,wg,wp,gi,bi,go,bo,mask,ds,dy):
  ...
  self.D,self.n,self.M=D,n,M;self.weights=(wl,wlg,wr,wrg);self.x,self.dy,self.ds=x,dy,ds;self.mask=mask.float().contiguous();self.ks,self.path=build(D);self.grid=132*min(4,min(int(k.unit.drv._unwrap('cuOccupancyMaxActiveBlocksPerMultiprocessor',k.unit.drv.d.cuOccupancyMaxActiveBlocksPerMultiprocessor(k.unit.drv.d.CUfunction(int(k.handle)),(128 if D==64 else 256),(33024 if D==64 else 49280)))) for k in (self.ks['forward'],self.ks['b1'],self.ks['b7'])))
