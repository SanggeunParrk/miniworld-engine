...
constexpr int D=WIDTH,H=2*D,SPLITS=WEIGHT_SPLITS;
...
template<int TA,int TB> TMN_DEVI void gemm(float (&acc)[32],const CUtensorMap* A,const CUtensorMap* B,int row,int col,int begin,int end,uint8_t* sm,uint64_t* bar,int& phase,bool add=false){
 ...
 if(threadIdx.x==0){mbar_arrive_expect_tx(bar,16384);tma_load_2d(sm,A,bar,TA?row:begin,TA?begin:row);tma_load_2d(sm+8192,B,bar,TB?col:begin,TB?begin:col);}
 for(int k=begin,iter=0;k<end;k+=64,++iter){
  int slot=iter%2;uint8_t* buf=sm+slot*16384;
  if(k+64<end && threadIdx.x==0){int nx=k+64;uint8_t* nxt=sm+(1-slot)*16384;mbar_arrive_expect_tx(bar+1-slot,16384);tma_load_2d(nxt,A,bar+1-slot,TA?row:nx,TA?nx:row);tma_load_2d(nxt+8192,B,bar+1-slot,TB?col:nx,TB?nx:col);}
  mbar_wait(bar+slot,(phase>>slot)&1);phase^=1<<slot;__syncthreads();fence_regs(acc);wgmma_fence();
  #pragma unroll
  for(int q=0;q<4;++q)mma<TA,TB>(acc,smem_desc(smem_u32(buf+(TA?q*2048:q*32)),16,1024,1),smem_desc(smem_u32(buf+8192+(TB?q*2048:q*32)),16,1024,1),add||k>begin||q>0);
...
TMN_DEVI void norm(const Params& p,uint8_t* sm){
 ...
 for(int first=blockIdx.x*16;first<p.M;first+=gridDim.x*16){
  for(int i=threadIdx.x;i<16*H;i+=128){int r=i%16,c=i/16;tile[si(r,c)]=p.t[1][size_t(c)*p.M+first+r];}__syncthreads();
  for(int r=warp;r<16;r+=4){int row=first+r;float sum=0;for(int c=lane;c<H;c+=32)sum+=rd(tile,si(r,c));float mu=wsum(sum)/H;
...
TMN_DEVI void output_ln_bwd(const Params& p,uint8_t* sm){
 ...
 for(int first=blockIdx.x*16;first<p.M;first+=gridDim.x*16){
  for(int i=threadIdx.x;i<16*H;i+=128){int r=i%16,c=i/16;tile[si(r,c)]=p.t[1][size_t(c)*p.M+first+r];}__syncthreads();
  for(int r=warp;r<16;r+=4){int row=first+r;float mu=p.f[5][row],rs=p.f[6][row],s0=0,s1=0;
  ...
  for(int i=threadIdx.x;i<16*H;i+=128){int r=i%16,c=i/16;p.t[12][size_t(c)*p.M+first+r]=tile[si(r,c)];}__syncthreads();
 }
...
TMN_DEVI void front_gp(const Params& p,uint8_t* sm,uint64_t* bar,int& phase){
 for(int t=blockIdx.x;t<(p.M/64)*(H/64)*2;t+=gridDim.x){int side=t/((p.M/64)*(H/64)),u=t%((p.M/64)*(H/64)),row=(u/(H/64))*64,col=(u%(H/64))*64;float proj[32],gate[32];
  ...
  if(threadIdx.x==0){mbar_arrive_expect_tx(bar,8192);tma_load_2d(sm,&p.map[14+side],bar,row,col);}mbar_wait(bar,phase&1);phase^=1;__syncthreads();
  // t[22] and t[23] are channel-first dleft/dright in B7; dWgate uses t[22] only in B1.
  for(int j=0;j<32;++j){int r=row+rr(j),c=col+cc(j);float da=rn(rd(reinterpret_cast<bf*>(sm),swz128(cc(j),rr(j)*2)/2)*p.f[4][r]),g=math::sigmoid(rn(gate[j])),v=rn(proj[j]);p.t[13+side*2][size_t(r)*H+c]=cv(da*g);p.t[14+side*2][size_t(r)*H+c]=cv(((da*v)*g)*(1-g));}__syncthreads();
 }
}
...
extern "C" __global__ __launch_bounds__(128,2) void width_forward(__grid_constant__ const Params p){
 extern __shared__ __align__(128) uint8_t sm[];auto bar=reinterpret_cast<uint64_t*>(sm+32768);if(threadIdx.x==0){mbar_init(bar,1);mbar_init(bar+1,1);}__syncthreads();int phase=0;auto grid=cooperative_groups::this_grid();stamp(p,0);norm(p,sm);stamp(p,1);__threadfence();asm volatile("fence.proxy.async.global;":::"memory");grid.sync();out_gp<true>(p,sm,bar,phase);
}
extern "C" __global__ __launch_bounds__(128,2) void width_b1(__grid_constant__ const Params p){
 extern __shared__ __align__(128) uint8_t sm[];auto bar=reinterpret_cast<uint64_t*>(sm+32768);if(threadIdx.x==0){mbar_init(bar,1);mbar_init(bar+1,1);}__syncthreads();int phase=0;auto grid=cooperative_groups::this_grid();
 for(int c=blockIdx.x*128+threadIdx.x;c<H;c+=gridDim.x*128){p.f[10][c]=0;p.f[11][c]=0;}stamp(p,0);norm(p,sm);stamp(p,1);__threadfence();asm volatile("fence.proxy.async.global;":::"memory");grid.sync();out_gp<false>(p,sm,bar,phase);stamp(p,2);__threadfence();asm volatile("fence.proxy.async.global;":::"memory");grid.sync();
...
extern "C" __global__ __launch_bounds__(128,2) void width_b7(__grid_constant__ const Params p){
 extern __shared__ __align__(128) uint8_t sm[];auto bar=reinterpret_cast<uint64_t*>(sm+32768);if(threadIdx.x==0){mbar_init(bar,1);mbar_init(bar+1,1);}__syncthreads();int phase=0;auto grid=cooperative_groups::this_grid();
 for(int c=blockIdx.x*128+threadIdx.x;c<D;c+=gridDim.x*128){p.f[8][c]=0;p.f[9][c]=0;}stamp(p,8);front_gp(p,sm,bar,phase);stamp(p,9);__threadfence();asm volatile("fence.proxy.async.global;":::"memory");grid.sync();front_dx(p,sm,bar,phase);stamp(p,10);
...
extern "C" __global__ __launch_bounds__(128,2) void width_probe(__grid_constant__ const Params p){
 extern __shared__ __align__(128) uint8_t sm[];auto bar=reinterpret_cast<uint64_t*>(sm+32768);if(threadIdx.x==0){mbar_init(bar,1);mbar_init(bar+1,1);}__syncthreads();int phase=0;float v[32];int ta=p.L/2,tb=p.L%2;
