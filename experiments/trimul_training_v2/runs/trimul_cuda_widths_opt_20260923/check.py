...
with torch.cuda.stream(S),torch.no_grad():
 ...
 record['route']=dict(gp_shared_a=int(os.environ.get('GP_SHARED_A','1' if D>=384 else '0')),fused=m.fused,gp_config=m.gp_native.cfg,grid=m.grid,grid7=m.grid7,b7_cubin=m.ks['b7'].unit.cubin_path);record['complete']=True;record['cubin']=m.path;record['cubin_sha256']=hashlib.sha256(Path(m.path).read_bytes()).hexdigest();save();print('DONE',D,N,flush=True)
