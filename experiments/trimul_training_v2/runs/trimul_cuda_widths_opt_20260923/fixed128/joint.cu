...
#endif
constexpr int RING_CHUNK=B7_RING_CHUNK,RING_SLOTS=32768/RING_CHUNK,RING_PHASES=131072/RING_CHUNK,RING_H=RING_CHUNK/8192;
...
TMN_DEVI void store_pair_dw(const Params& p,float (&v)[64],int part,int rank,uint8_t* sm){
 int tid=threadIdx.x%128,lane=tid%32,w=tid/32;float* dst=p.partw+(part*SOURCES+rank)*8192;
 static_for<16>([&](auto qq){constexpr int q=decltype(qq)::value;int row=w*16+lane/4,c=q*8+2*(lane%4);
#if B7_TRANSPOSE_PART
 dst[(row/32)*4096+c*32+row%32]=v[q*4];dst[(row/32)*4096+(c+1)*32+row%32]=v[q*4+1];
 dst[(row/32)*4096+c*32+row%32+8]=v[q*4+2];dst[(row/32)*4096+(c+1)*32+row%32+8]=v[q*4+3];
#else
 stg64f(dst+row*128+c,v[q*4],v[q*4+1]);stg64f(dst+(row+8)*128+c,v[q*4+2],v[q*4+3]);
#endif
 });
}
TMN_DEVI void source_compute(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 for(int round=0;round<rounds;round+=2){int slot=round%4,tile=cid+round*groups;bool second=round+1<rounds;
  ...
  static_for<4>([&](auto kk){constexpr int k=decltype(kk)::value;mma_weight128(dw,smem_desc(smem_u32(sg0+k*32),16,1024,1),smem_desc(smem_u32(s0+k*2048),8192,1024,1),(round>0&&round!=splitpoint)||k>0);});
  ...
  if(round+2==splitpoint)store_pair_dw(p,dw,cid,rank,sm);
...
TMN_DEVI void consumer_compute(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 ...
 for(int tile=cid+rank*clusters;tile<p.tiles;tile+=CONSUMERS*clusters,++round){int row=tile*64;
  ...
  for(int batch=0;batch<8;++batch){int slot=batch%2,ws=batch%2;
...
#endif
#endif
   for(int a=0;a<groups*2;++a)v+=reinterpret_cast<volatile float*>(p.partw)[(a*SOURCES+rk)*8192+j];p.dw[out]=__float2bfloat16_rn(v);
