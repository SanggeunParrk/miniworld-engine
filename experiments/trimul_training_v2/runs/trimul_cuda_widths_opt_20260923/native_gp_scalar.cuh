...
struct GPParams {K1Params base; const __nv_bfloat16* dl; const __nv_bfloat16* dr; __nv_bfloat16* gp[4];};
...
#pragma unroll
      ...
      const __nv_bfloat16* da=side?ext.dr:ext.dl;
      __nv_bfloat16* dp=ext.gp[2*side];__nv_bfloat16* dg=ext.gp[2*side+1];
#pragma unroll
      for(int q=0;q<4;++q){
#pragma unroll
       for(int j=0;j<4;++j){
        int row=(j<2?iA*p.N+jA:iB*p.N+jB),c=cb+8*q+2*(lane&3)+(j&1);
        float mask=j<2?mA:mB;
        float d=__bfloat162float(__float2bfloat16_rn(__bfloat162float(da[(size_t)c*plane+row])*mask));
        ...
        dp[(size_t)row*(2*CZ)+c]=__float2bfloat16_rn(d*g);
        dg[(size_t)row*(2*CZ)+c]=__float2bfloat16_rn(((d*proj)*g)*(1-g));
       }
      }
    };
...
#pragma unroll
  ...
  if (false && st_elect) tma_store_wait_all();           // every bulk store of this CTA complete before its shared memory is released
}
