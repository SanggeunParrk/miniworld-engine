...
import torch,json,hashlib,subprocess,ctypes,sys
...
@lru_cache(None)
def build(D):
 inc=T._upstream()/'csrc';src=R/'widths.cu';flags=['-std=c++17','-O3','-arch=sm_90a','--cubin','-lineinfo','-Xptxas=-v',f'-DWIDTH={D}','-I'+str(inc)]
 ...
 for k in ks.values():k.set_max_dynamic_smem(16512)
 return ks,str(out)
def launch(k,params,grid=132,cooperative=True):
 L=T._launch_module()
 if not cooperative:k.launch((grid,1,1),(128,1,1),[params],16512);return
 drv=k.unit.drv;args=L._Packed([params]);drv._unwrap('cuLaunchCooperativeKernel',drv.d.cuLaunchCooperativeKernel(drv.d.CUfunction(int(k.handle)),grid,1,1,128,1,1,16512,drv.d.CUstream(int(torch.cuda.current_stream().cuda_stream)),ctypes.addressof(args.array)))
def tm(t):
...
class Training:
 def __init__(self,x,wl,wlg,wr,wrg,wg,wp,gi,bi,go,bo,mask,ds,dy):
  ...
  self.D,self.n,self.M=D,n,M;self.weights=(wl,wlg,wr,wrg);self.x,self.dy,self.ds=x,dy,ds;self.mask=mask.float().contiguous();self.ks,self.path=build(D);self.grid=132
  ...
  self.floats=[gi,bi,go,bo,self.mask,*[torch.empty(M,device=x.device) for _ in range(2)],torch.empty((8,11*D*D),device=x.device),*[torch.empty(c,device=x.device) for c in (D,D,H,H)]]
  ...
  L=T._launch_module();self.params=L.Struct([*maps,*self.tensors,*self.floats,M,n]);t7=self.tensors.copy();t7[22:24]=[self.dl,self.dr];self.params7=L.Struct([*maps,*t7,*self.floats,M,n]);self.maps=maps
