...
rows=[]
with torch.no_grad():
 for group in ([1] if D==64 else [1,2] if D in (256,512) else [1]):
  for splits in [32,64,128]:
   os.environ['WIDTH_GROUPS']=str(group);os.environ['WIDTH_SPLITS']=str(splits);build.cache_clear()
   ...
   torch.cuda.synchronize();t=statistics.median(a.elapsed_time(b)*1000 for a,b in events);row=dict(group=group,splits=splits,grid=m.grid,us=t);rows.append(row);print('TUNE',D,N,row,flush=True);(R/f'tune-D{D}-L{N}.json').write_text(json.dumps(rows,indent=2));del m,g,out;gc.collect()
