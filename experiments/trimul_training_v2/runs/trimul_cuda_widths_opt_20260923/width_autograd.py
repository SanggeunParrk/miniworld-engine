...
def training128(a):
 F=fixed_policy()
 if a['d']['n']==384:return F.Fixed(a)
 m=F.P.B1.Training(a);old=m.p7;dl,dr,dg,dy=old.inputs[:4]
 C=F.load('width128_accuracy_current',R/'fixed128/plan.py');env=F.P.SELECTION['environment'];saved={k:os.environ.get(k) for k in env}
 try:
  os.environ.update(env);p=C.Plan(m.d,dy,dl,dr,dg,xn=old.xn,**F.P.SELECTION['kwargs'])
 finally:
  for k,v in saved.items():
   if v is None:os.environ.pop(k,None)
   else:os.environ[k]=v
 p.mask=old.mask;p.bind(dl,dr,dg,dy,xn=old.xn);m.p7=p;return m
class D128:
 def __init__(self,leaves,mask,ds):
  ...
  self.model=training128(a);self.a=a;self.kept=None
