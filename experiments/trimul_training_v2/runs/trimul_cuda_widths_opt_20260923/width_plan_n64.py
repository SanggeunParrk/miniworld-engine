...
def tuning(D):
 g=int(os.environ.get('WIDTH_GROUPS',1 if D==64 else 2));spl=int(os.environ.get('WIDTH_SPLITS',128 if D==64 else 32));return g,spl,2*8192*(1+g)+128
@lru_cache(None)
def build(D):
 inc=T._upstream()/'csrc';src=R/'widths.cu';flags=['-DWIDTH_GROUPS='+str(tuning(D)[0]),'-DEXTERNAL_GP='+os.environ.get('EXTERNAL_GP','1'),'-DPROFILE_STAGE='+os.environ.get('WIDTH_PROFILE','0'),'-std=c++17','-O3','-arch=sm_90a','--cubin','-lineinfo','-Xptxas=-v',f'-DWIDTH={D}',f'-DWEIGHT_SPLITS={tuning(D)[1]}','-I'+str(inc)]
