...
def tuning(D):
 g=int(os.environ.get('WIDTH_GROUPS',1));spl=int(os.environ.get('WIDTH_SPLITS',128 if D==64 else 32));return g,spl,2*8192*(1+g*(1 if D==64 else 2))+128
@lru_cache(None)
def build(D):
 inc=T._upstream()/'csrc';src=R/'widths.cu';flags=['-DWIDTH_N='+str(64 if D==64 else 128),'-DWIDTH_GROUPS='+str(tuning(D)[0]),'-DEXTERNAL_GP='+os.environ.get('EXTERNAL_GP','1'),'-DPROFILE_STAGE='+os.environ.get('WIDTH_PROFILE','0'),'-std=c++17','-O3','-arch=sm_90a','--cubin','-lineinfo','-Xptxas=-v',f'-DWIDTH={D}',f'-DWEIGHT_SPLITS={tuning(D)[1]}','-I'+str(inc)]
 ...
 for k in ks.values():k.set_max_dynamic_smem(tuning(D)[2])
 return ks,str(out)
def launch(k,params,grid=132,cooperative=True,D=64):
 threads=128*tuning(D)[0];smem=tuning(D)[2]
 L=T._launch_module()
 if not cooperative:k.launch((grid,1,1),(threads,1,1),[params],smem);return
 drv=k.unit.drv;args=L._Packed([params]);drv._unwrap('cuLaunchCooperativeKernel',drv.d.cuLaunchCooperativeKernel(drv.d.CUfunction(int(k.handle)),grid,1,1,threads,1,1,smem,drv.d.CUstream(int(torch.cuda.current_stream().cuda_stream)),ctypes.addressof(args.array)))
def tm(t):
...
class Training:
 def __init__(self,x,wl,wlg,wr,wrg,wg,wp,gi,bi,go,bo,mask,ds,dy):
  ...
  self.D,self.n,self.M=D,n,M;self.weights=(wl,wlg,wr,wrg);self.x,self.dy,self.ds=x,dy,ds;self.mask=mask.float().contiguous();self.ks,self.path=build(D);self.grid=132*min(4,min(int(k.unit.drv._unwrap('cuOccupancyMaxActiveBlocksPerMultiprocessor',k.unit.drv.d.cuOccupancyMaxActiveBlocksPerMultiprocessor(k.unit.drv.d.CUfunction(int(k.handle)),(128*tuning(D)[0]),(tuning(D)[2])))) for k in (self.ks['forward'],self.ks['b1'],self.ks['b7'])))
  ...
  self.gp_native=GP(self) if os.environ.get('EXTERNAL_GP','1')=='1' else None
 ...
 def backward(self,dy=None):
  ...
  if self.gp_native is not None:self.gp_native()
  launch(self.ks['b7'],self.params7,self.grid,D=self.D);return self.outputs
