...
constexpr int D=WIDTH,H=2*D,SPLITS=WEIGHT_SPLITS,GROUPS=WIDTH_GROUPS,THREADS=128*GROUPS,NT=64*GROUPS,STAGE=8192*(1+GROUPS);
...
TMN_DEVI void weight(const Params& p,int amap,int bmap,int N,int K,int offset,uint8_t* sm,uint64_t* bar,int& phase){
 ...
 for(int t=blockIdx.x;t<tiles*SPLITS;t+=gridDim.x){int s=t/tiles,u=t%tiles,row=(u/kt)*64,col=(u%kt)*NT+(threadIdx.x/128)*64;float v[32];if(amap>=6)gemm<0,1>(v,&p.map[amap],&p.map[bmap],row,col,s*step,(s+1)*step,sm,bar,phase);else gemm<1,1>(v,&p.map[amap],&p.map[bmap],row,col,s*step,(s+1)*step,sm,bar,phase);
