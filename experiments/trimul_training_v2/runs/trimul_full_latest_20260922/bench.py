...
s=importlib.util.spec_from_file_location('full_latest_local_policy',R/'policy.py');P=importlib.util.module_from_spec(s);s.loader.exec_module(P)
Q=P.Q;H=P.H
...
with torch.no_grad():
 a=Q.setup(384);d=a['d'];models={'historical_1048':P.Historical(a),'regression_1185':P.Regression(a),'latest_combined':P.Latest(a)}
 ...
 assert record['traces']['latest_combined'].get('b7_joint')==1
 assert record['traces']['latest_combined'].get('b1_fused')==1
 record['source_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [R/'bench.py',R/'policy.py',R.parent/'trimul_b1_gate_demote_20260922/policy.py',R.parent/'trimul_b7_weight_batch128_20260922/plan.py']};record['complete']=True;save();print('DONE',flush=True)
