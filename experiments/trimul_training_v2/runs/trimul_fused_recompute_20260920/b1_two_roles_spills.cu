...
#define PART_ONLY 2
#endif
constexpr int DWCOUNT=2*DW_SPLITS,DXCOUNT=UCOUNT-DWCOUNT;
...
template<bool GATE> TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* full){
 int group=GATE?0:1,split=blockIdx.x/2,wi=threadIdx.x/128,lane=threadIdx.x%32,w=(threadIdx.x/32)%4;
 constexpr int NT=GATE?1:2;
 float acc[NT][64]={};MaskCycle mask=mask_cycle<DW_SPLITS>(p,split);int mi=0,round=0;
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++round){
  ...
  gate_backward<GATE>(p,sm,tile*64,mask,mi);
  static_for<NT>([&](auto ii){constexpr int it=decltype(ii)::value;int t=GATE?wi:2+2*it+wi;
  uint8_t* sa=GATE?sm+49152+t*8192:sm+it*8192;
  uint8_t* sb=GATE?sm+16384:sm+65536+wi*16384;
  fence_regs(acc[it]);wgmma_fence();
  static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;
   mma_ss128(acc[it],smem_desc(smem_u32(sa+k*2048),16,1024,1),smem_desc(smem_u32(sb+k*2048),8192,1024,1),round>0||k>0);});
  wgmma_commit();wgmma_wait<0>();fence_regs(acc[it]);});allsync();
  if(++mi==mask.period)mi=0;
 }
 static_for<NT>([&](auto ii){constexpr int it=decltype(ii)::value;int t=GATE?wi:2+2*it+wi;
 float* out=p.partw+((GATE?0:1+it)*DW_SPLITS+split)*16384;
#pragma unroll
 for(int q=0;q<16;++q){int rr=w*16+lane/4+(t<2?t*64:0),c=q*8+2*(lane%4)+(t<2?0:((t-2)%2)*128),stride=t<2?128:256;
  stg64f(out+rr*stride+c,acc[it][4*q],acc[it][4*q+1]);stg64f(out+(rr+8)*stride+c,acc[it][4*q+2],acc[it][4*q+3]);}
 });
}
...
void b1_fused(__grid_constant__ const Params p){
 ...
 if(blockIdx.x<DWCOUNT){if(blockIdx.x%2==0)weight_role<true>(p,sm,full+1);else weight_role<false>(p,sm,full+1);}else input_role(p,sm,full+1);
#if PART_ONLY==2
