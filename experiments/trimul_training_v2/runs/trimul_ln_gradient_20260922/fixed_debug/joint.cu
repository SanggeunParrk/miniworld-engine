...
struct Params{CUtensorMap xn,wp,dl,dr,dg,wgate,x,res,dx,wt[4],ringstore;const __nv_bfloat16* mask;const float *gamma,*beta;__nv_bfloat16 *dxptr,*dw;float *dgam,*dbeta,*partw,*partln;unsigned int* counts;int M,tiles;uint8_t *ring,*xring;unsigned* flags;float *debug_dxn,*debug_xhat;};
 ...
 for(int tile=cid+rank*clusters;tile<p.tiles;tile+=CONSUMERS*clusters,++round){int row=tile*64;
  ...
  static_for<8>([&](auto qq){constexpr int q=decltype(qq)::value;
   static_for<2>([&](auto pp){constexpr int pair=decltype(pp)::value;int j=pair*2,c=q*16+2*(lane%4)+8*pair;
    ...
    p.debug_dxn[(row+ra)*128+c]=da;p.debug_dxn[(row+ra)*128+c+1]=db;
    p.debug_dxn[(row+rb)*128+c]=dc;p.debug_dxn[(row+rb)*128+c+1]=dd;
    p.debug_xhat[(row+ra)*128+c]=xaa;p.debug_xhat[(row+ra)*128+c+1]=xab;
    p.debug_xhat[(row+rb)*128+c]=xba;p.debug_xhat[(row+rb)*128+c+1]=xbb;
