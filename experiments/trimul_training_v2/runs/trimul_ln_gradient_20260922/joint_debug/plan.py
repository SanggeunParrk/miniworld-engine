...
sys.path.insert(0,str(R.parent.parent/'trimul_recompute_next_20260921'));import plans as P
T=P.T
class Plan:
 def __init__(self,d,dy,dl,dr,dg,xn,clusters=10,mode=0):
  self.d=d;self.xn=xn;self.hwcluster=int(os.environ.get("B7_HW_CLUSTER","2"));self.multicast=int(os.environ.get("B7_MULTICAST","1"));self.rings=int(os.environ.get("B7_RING_DEPTH","8"));self.clusters=clusters;self.consumers=int(os.environ.get("B7_CONSUMERS","8"));self.reuse_dp=1;self.count=clusters*(16+self.consumers)
  inc=T._upstream()/'csrc';flags=['-DB7_HW_CLUSTER='+str(self.hwcluster),'-DB7_MULTICAST='+str(self.multicast),'-DB7_RING_DEPTH='+str(self.rings),'-DB7_TRANSPOSE_PART='+str(bool(mode&16)*1),'-DB7_REDUCE_NATIVE='+str(bool(mode&32)*1),'-DB7_PRODUCER_REGS='+os.environ.get('B7_PRODUCER_REGS','48'),'-DB7_RING_CHUNK='+str(8192 if mode&8 else 16384 if mode&4 else 32768),'-DB7_GATEFIRST='+str(bool(mode&2)*1),'-DB7_RING_TENSOR='+str(mode&1),'-DB7_ROLLING='+str(bool(mode&4)*1),'-DB7_LNPREFETCH='+str(bool(mode&8)*1),'-DB7_PREFETCH='+str(mode&3),'-DB7_CONSUMERS='+str(self.consumers),'-DB7_REUSE_DP='+str(self.reuse_dp),'-std=c++17','-O3','-arch=sm_90a','--cubin','-lineinfo','-Xptxas=-v','-I'+str(inc),'-I'+str(R.parent.parent/'anthropic_b7b12_fusion_20260920'),'-I'+str(R.parent.parent/'trimul_b7_nextrow_20260921')]
  ...
  assert self.clusters>0 and (16+self.consumers)%self.hwcluster==0
  dd=drv.drv.d;cfg=dd.CUlaunchConfig();cfg.gridDimX=self.count;cfg.gridDimY=1;cfg.gridDimZ=1;cfg.blockDimX=256;cfg.blockDimY=1;cfg.blockDimZ=1;cfg.sharedMemBytes=114688;cfg.hStream=dd.CUstream(int(torch.cuda.current_stream().cuda_stream))
  ...
  self.partw=torch.zeros((self.clusters*2,16,64,128),device=x.device);self.partln=torch.empty((self.clusters*self.consumers,256),device=x.device);self.counts=torch.zeros(2,device=x.device,dtype=torch.int32);self.mask=d['mask'].bfloat16().reshape(-1);self.ring=torch.empty((self.clusters,self.rings,131072),device=x.device,dtype=torch.uint8);self.xring=torch.empty((self.clusters,self.rings,16384),device=x.device,dtype=torch.uint8);self.flags=torch.zeros((self.clusters,self.rings,18),device=x.device,dtype=torch.int32);self.debug_dxn=torch.empty((m,128),device=x.device);self.debug_xhat=torch.empty_like(self.debug_dxn);self.outputs=(self.dx,*self.dw.unbind(),self.dgam,self.dbeta);self.bind(dl,dr,dg,dy,xn)
