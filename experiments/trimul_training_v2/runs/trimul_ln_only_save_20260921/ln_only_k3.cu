...
struct RecomputeParams { K3Params base; const __nv_bfloat16* dropscale; CUtensorMap tm_lnin,tm_lnout; __nv_bfloat16 *lnin,*lnout; };
...
#pragma unroll                                           // dependency); the proxy fence orders these generic-proxy reads before the async-proxy refill
    ...
    else if (LNM == 4) ln_fragment<KSP, true, math::TX>(fx, sGout, sBout, lane, p.eps); else ln_fragment<KSP, MW_SERIAL>(fx, sGout, sBout, lane, p.eps);
#endif
#if MW_SAVE_OUT
...
#ifndef TMN_DEV_NOLN
    if (MW_FUSED && !ZF32 && !PRENORM) { if (LNM == 2 || LNM == 3) ln_stock<0>(fz, sGin, sBin, lane, p.eps); else if (LNM == 4) ln_fragment<KSG, true, math::TX>(fz, sGin, sBin, lane, p.eps); else ln_fragment<KSG, MW_SERIAL>(fz, sGin, sBin, lane, p.eps); }
#endif
#if MW_SAVE_IN
...
void infer_k3(__grid_constant__ const tmn::sm90::RecomputeParams p){tmn::sm90::infer_k3_body<tmn::sm90::Cfg,1>(p);}
