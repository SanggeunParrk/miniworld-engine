...
def output(d,tri,ln=1,stats=0,pg=False,method=0,bufs=None):
 ...
 if bufs is None:
  ...
  saves=dict(xn=e(128) if ln&1 else None,xnout=tri if method==-1 else torch.empty((256,m),device=x.device,dtype=torch.float32 if method==1 else torch.float16 if method>=2 else x.dtype) if ln&2 else None,mi=f() if stats&1 else None,ri=f() if stats&1 else None,mo=None,ro=torch.empty(m*(2 if method in (-1,3) else 1),device=x.device,dtype=torch.float32) if stats&2 else None,proj=e(128) if pg else None,gate=e(128) if pg else None);y=torch.empty_like(x)
 else:y,saves=bufs
