...
class Replacement:
 def __init__(self,a,fp32):
  ...
  cfg=json.loads((PREV/('selected-L%d.json'%self.d['n'])).read_text())['config'];cfg=json.loads((R/('selected-L%d.json'%self.d['n'])).read_text())['config'] if fp32==-1 else cfg;cfg['defines']['XHAT_FP32']=int(fp32)
...
def main():
 ...
 path=R/(('check-final-L%d.json' if args.check_only else 'final-L%d.json')%n)
 with torch.no_grad():
  a=Q.setup(n);d=a['d'];models={'baseline':BASE.Training(a),'stats_only':Replacement(a,-1),'xhat_fp32':Replacement(a,1)}
  result=dict(L=n,C=128,H=256,dropout=.25,checks={},mutated={},poison={},times={},blocks={},meta=dict(host=platform.node(),gpu=torch.cuda.get_device_name(),raw_tri_in_backward={'stats_only':True,'xhat_fp32':False,'xhat_fp16':False,'xhat_fp16_recover':False},includes=['both directions','mask','dropout','residual','live packing','all eleven gradients'],excludes=['optimizer','RNG generation','compile','CPU dispatch']),production_ready=False)
  ...
  for name,m in models.items():
   ...
   if name not in ('baseline','stats_only'):
  ...
  result['cubins']={k:dict(b1=m.p1.k.unit.cubin_path) for k,m in models.items()};result['source_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [R/'final.py',R/'replace_plan.py',R/'replace_core.py',*R.glob('*.cu'),*R.glob('*.cuh'),*R.glob('*.inc')]};path.write_text(json.dumps(result,indent=2));print('DONE',n,flush=True)
