...
def run(cmd,name):
 ...
 with (R/name).open('w') as f:p=subprocess.run(cmd,stdout=f,stderr=subprocess.STDOUT,timeout=480)
...
for variant in ('baseline','stats_only','xhat_fp32'):
...
for tool in (('memcheck','racecheck') if n==384 else ('memcheck',)):
 ...
 run([*cmd,sys.executable,'-B',str(R/'final.py'),'--length',str(n),'--check-only'],'%s-L%d.log'%(tool,n))
