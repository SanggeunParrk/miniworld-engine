...
TMN_DEVI void glu_small(const Params& p,uint8_t* s,uint8_t* gout,uint8_t* pout,int row){
 ...
 for(unsigned q=0;q<8;++q){unsigned i=tid+q*256,c=i/32,r=(i%32)*2;
  uint32_t dy=pair_get(s+16384,c,r),gl=pair_get(s,c,r),pr=pair_get(s+8192,c,r);
...
TMN_DEVI void recompute_pre_to(uint8_t* dstg,uint8_t* dstp,uint8_t* xn,uint8_t* wg,uint8_t* wp){
...
#pragma unroll
 for(int q=0;q<4;++q){int mat=lane/8,chq=lane%8+8*(mat>>1);
  stsm_x4_t(smem_u32(wi==0?dstg:dstp)+swz128(q*16+chq,(w*16+8*(mat&1))*2),
   pack_bf16(acc[q*8],acc[q*8+1]),pack_bf16(acc[q*8+2],acc[q*8+3]),pack_bf16(acc[q*8+4],acc[q*8+5]),pack_bf16(acc[q*8+6],acc[q*8+7]));
 }
 allsync();
}
TMN_DEVI void recompute_pre(uint8_t* pre,uint8_t* xn,uint8_t* wg,uint8_t* wp){recompute_pre_to(pre,pre+8192,xn,wg,wp);}
TMN_DEVI void glu_held(const Params& p,uint8_t* sg,uint8_t* sp,uint32_t (&dy)[8],int row){
 unsigned tid=threadIdx.x;uint32_t mask=*reinterpret_cast<const uint32_t*>(p.mask+row+(tid%32)*2);
 #pragma unroll
 for(unsigned q=0;q<8;++q){unsigned i=tid+q*256,c=i/32,r=(i%32)*2;
  uint32_t gl=pair_get(sg,c,r),pr=pair_get(sp,c,r),masked;
  asm("mul.rn.bf16x2 %0,%1,%2;":"=r"(masked):"r"(dy[q]),"r"(mask));float ga=math::sigmoid(bf16lo(gl)),gb=math::sigmoid(bf16hi(gl));
  uint32_t g=pack_bf16(((bf16lo(masked)*bf16lo(pr))*ga)*(1.f-ga),((bf16hi(masked)*bf16hi(pr))*gb)*(1.f-gb));
  uint32_t v=pack_bf16(bf16lo(masked)*ga,bf16hi(masked)*gb);
  *reinterpret_cast<uint32_t*>(sg+swz128(c,r*2))=g;*reinterpret_cast<uint32_t*>(sp+swz128(c,r*2))=v;
 }fence_proxy_async();allsync();
}
...
TMN_DEVI void weight_role(const Params& p,uint8_t* sm,uint64_t* b,const float* gamma,const float* beta){
 ...
 int rounds=(p.tiles-split+DW_SPLITS-1)/DW_SPLITS,segmentRounds=(rounds+WGRAD_SLICES-1)/WGRAD_SLICES,r=0;float acc[64]={};
 if(split<p.tiles)load_dw(p,sm,b,0,split*64,group);if(split+DW_SPLITS<p.tiles)load_dw(p,sm,b,1,(split+DW_SPLITS)*64,group);
 for(int tile=split;tile<p.tiles;tile+=DW_SPLITS,++r){int slot=r&1;uint8_t* s=sm+slot*DW_SLOT;mbar_wait(b+slot,(r/2)&1);recompute_dw(p,s,gamma,beta);glu_small(p,s,s+40960,s+49152,tile*64);
  ...
  if((r+1)%segmentRounds==0||tile+DW_SPLITS>=p.tiles){float* out=p.partw+((group*DW_SPLITS+split)*WGRAD_SLICES+r/segmentRounds)*16384+wi*8192;
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  ...
  allsync();
  if(threadIdx.x==0){mbar_arrive_expect_tx(bar+4,16384);for(int c=0;c<2;++c)tma_load_2d(sm+90112+c*8192,&p.x,bar+4,c*64,row);}
  mbar_wait(bar+4,round&1);
  if(threadIdx.x<128)normalize_tile<8>(sm+90112,sm+90112,gamma,beta);
  fence_proxy_async();allsync();
  ...
  for(int side=0;side<2;++side){
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);
    uint32_t dy[8];
    #pragma unroll
    for(int q=0;q<8;++q){int i=threadIdx.x+q*256;dy[q]=pair_get(s+16384,i/32,(i%32)*2);}
    allsync();
    recompute_pre_to(s+16384,sm+81920,sm+90112,s+24576,s);
    glu_held(p,s+16384,sm+81920,dy,row);
    fence_regs(acc);wgmma_fence();
    static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;
     mma_input64(acc,smem_desc(smem_u32(s+16384+k*2048),16,1024,1),smem_desc(smem_u32(s+24576+wi*8192+k*32),16,1024,1),side>0||h>0||k>0);});
    static_for<4>([&](auto ki){constexpr int k=decltype(ki)::value;
     mma_input64(acc,smem_desc(smem_u32(sm+81920+k*2048),16,1024,1),smem_desc(smem_u32(s+wi*8192+k*32),16,1024,1),1);});
    wgmma_commit();wgmma_wait<0>();fence_regs(acc);allsync();
    if(h<2)load_g(p,sm,bar,row,side,h+2);
   }
  }
  load_ln_next(p,sm+65536,bar+3,row);
...
TMN_DEVI void reduce_at(const Params& p,int i){
 if(i<131072){int group=i/16384,j=i%16384,kind=j/8192,z=j%8192;float v=0;
  for(int b=0;b<DW_SPLITS*WGRAD_SLICES;++b)v+=reinterpret_cast<volatile float*>(p.partw)[(group*DW_SPLITS*WGRAD_SLICES+b)*16384+j];
...
void front_b7b12(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar[5];__shared__ float gamma[128],beta[128];if(threadIdx.x<128){gamma[threadIdx.x]=p.gamma[threadIdx.x];beta[threadIdx.x]=p.beta[threadIdx.x];}
 if(threadIdx.x==0){for(int i=0;i<5;++i)mbar_init(bar+i,1);fence_barrier_init();}allsync();
 if(blockIdx.x<DWCOUNT)weight_role(p,sm,bar,gamma,beta);else input_role(p,sm,bar,gamma,beta);
#if PART_ONLY == 2
