...
#else
  for(int b=0;b<DW_SPLITS*2*WGRAD_SLICES;++b)v+=reinterpret_cast<volatile float*>(p.partw)[(group*DW_SPLITS*2*WGRAD_SLICES+b)*8192+j];
#endif
...
void front_b7b12(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){for(int i=0;i<10;++i)mbar_init(bar+i,1);fence_barrier_init();}named_bar_sync(0,384);

#if PROFILE_ROLES
 unsigned long long role_begin=clock64();
#endif
 if(blockIdx.x<DWCOUNT){
  if(threadIdx.x<128){setmaxnreg_dec<40>();if(threadIdx.x==0){mbar_arrive_expect_tx(bar+8,16384*(PIPE_HIDDEN64?2:1));for(int c=0;c<(PIPE_HIDDEN64?2:1);++c)for(int k=0;k<2;++k)tma_load_2d(sm+98304+c*16384+k*8192,&p.pre,bar+8,k*64,((blockIdx.x%PIPE_GROUPS)*(PIPE_HIDDEN64?2:1)+c)*64);}pipe_producer(p,sm,bar);}
  else {setmaxnreg_inc<232>();pipe_consumer(p,sm,bar,gamma,beta);}
 }else {
  if(threadIdx.x>=256)setmaxnreg_dec<32>();
  else {setmaxnreg_inc<232>();input_role(p,sm,bar,gamma,beta);}
 }
 named_bar_sync(15,384);
#if PROFILE_ROLES
 if(threadIdx.x==0)p.counts[2+blockIdx.x]=(unsigned)(clock64()-role_begin);
#endif

#if PART_ONLY == 2
 __threadfence();named_bar_sync(15,384);
 if(threadIdx.x==0){atomicAdd(p.counts,1u);while(atomicAdd(p.counts,0u)!=UCOUNT)__nanosleep(32);}named_bar_sync(15,384);
#if DW_REDUCE_TMA
 reduce_tile(p,sm);
#else
