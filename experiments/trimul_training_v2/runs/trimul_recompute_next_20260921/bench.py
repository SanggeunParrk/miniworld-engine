...
spec1=importlib.util.spec_from_file_location('accepted_plans',R.parent/'trimul_recompute_optimized_20260920/plans.py')
P1=importlib.util.module_from_spec(spec1);spec1.loader.exec_module(P1)
...
def main():
    ...
    cfg1=json.loads((P1.R/('selected-L%d.json'%n)).read_text())
    with torch.no_grad():
        a=Q.setup(n);d=a['d'];old=P.S.A.Training(a,k3);new=Training(a,k3,cfg['b1'],cfg['b7']);previous=Training(a,k3,cfg0['b1'],cfg0['b7'],plans=P0);accepted=Training(a,k3,cfg1['b1'],cfg1['b7'],plans=P1)
        y,ss=old.forward_saved();_,kept=new.forward()
        ref=(y,P.S.C.backward(d,ss,a['dy']))
        out=new();torch.cuda.synchronize();checks={'new':assert_full(out,ref),'saved':assert_full(old.saved(),ref),'initial':assert_full(previous(),ref),'previous':assert_full(accepted(),ref)}
        ...
        result=dict(L=n,C=128,dropout=.25,checks=checks,configs=cfg,initial_configs=cfg0,previous_configs=cfg1,blocks={},times={},traces={},replay_checks={},
        ...
        scopes={'forward':{'saved':old.forward_saved,'initial_recompute':previous.forward,'previous_recompute':accepted.forward,'optimized_recompute':new.forward},
                'backward':{'saved':lambda:old.backward(ss),'initial_recompute':lambda:previous.backward(kept),'previous_recompute':lambda:accepted.backward(kept),'optimized_recompute':lambda:new.backward(kept)},
                'forward_backward':{'saved':old.saved,'initial_recompute':previous,'previous_recompute':accepted,'optimized_recompute':new}}
