...
#endif
constexpr int PIPE_THREADS=(PIPE_CONSUMERS+1)*128,PIPE_SG_BASE=PIPE_CONSUMERS*40960,PIPE_W_BASE=PIPE_SG_BASE+PIPE_CONSUMERS*8192;
...
#endif
constexpr int DWCOUNT=PIPE_GROUPS*DW_SPLITS,DXCOUNT=UCOUNT-DWCOUNT;
...
TMN_DEVI void mma_packed64(float (&d)[32],uint64_t a,uint64_t b,int accumulate){
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  ...
  allsync();
  if(threadIdx.x==0){mbar_arrive_expect_tx(bar+4,16384);for(int c=0;c<2;++c)tma_load_2d(sm+81920+c*8192,&p.x,bar+4,c*64,row);}
  mbar_wait(bar+4,round&1);
  uint32_t xn_frag[8][4];load_frag_bf16<8,8192>(xn_frag,smem_u32(sm+81920),w*16,lane);
#if B7_WEIGHT_EARLY
  allsync();
  if(threadIdx.x==0){mbar_arrive_expect_tx(bar+7,131072);for(int h=0;h<4;++h)for(int wg=0;wg<2;++wg)for(int k=0;k<2;++k)tma_load_2d(sm+h*32768+wg*16384+k*8192,&p.pre,bar+7,k*64,h*128+wg*64);}
#endif
  LnStats input_stats=ln_recompute_fragment<8,B7_LN_SERIAL>(xn_frag,gamma,beta,lane,1e-5f);
#if B7_REUSE_STATS
  if(wi==0 && lane%4==0){float* st=reinterpret_cast<float*>(sm+218112);st[ra]=input_stats.mA;st[rb]=input_stats.mB;st[64+ra]=input_stats.rA;st[64+rb]=input_stats.rB;}
#endif
#if B7_HOIST_MASK
  ...
  for(int side=0;side<2;++side){
   if(threadIdx.x==0
    ...
    && side!=0
#endif
   ){mbar_arrive_expect_tx(bar+7,131072);for(int h=0;h<4;++h)for(int wg=0;wg<2;++wg)for(int k=0;k<2;++k)tma_load_2d(sm+h*32768+wg*16384+k*8192,&p.pre,bar+7,k*64,(side*4+h)*128+wg*64);}
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* wb=sm+h*32768;uint8_t* dy=sm+131072+slot*8192;mbar_wait(bar+5+slot,h/2);packed_derivatives(p,xn_frag,wb,dy,dy,sm+147456+h*8192,row
#if B7_HOIST_MASK
    ,ma,mb
#endif
   );
  ...
  for(int side=0;side<2;++side){
   ...
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);packed_derivatives(p,xn_frag,s,s+32768,s+32768,sm+81920+h*8192,row);
...
#if B7_REUSE_STATS
  float* saved_stats=reinterpret_cast<float*>(sm+218112);LnStats recstats={saved_stats[ra],saved_stats[64+ra],saved_stats[rb],saved_stats[64+rb]};
#else
...
TMN_DEVI void reduce_tile(const Params& p,uint8_t* sm){
 ...
 if(blockIdx.x<128){
  ...
  fence_proxy_async();named_bar_sync(15,PIPE_THREADS);if(threadIdx.x==0){store2d(&p.xn,sm,h0,kind*128+c0);tma_store_commit();tma_store_wait_all();}
 }
 if(blockIdx.x==0&&threadIdx.x<256)reduce_at(p,131072+threadIdx.x);
}
#endif
extern "C" __global__ __launch_bounds__(PIPE_THREADS,1)
void front_b7b12(__grid_constant__ const Params p){
 extern __shared__ __align__(1024) uint8_t sm[];__shared__ uint64_t bar[16];__shared__ float gamma[128],beta[128];
 if(threadIdx.x<128){gamma[threadIdx.x]=p.gamma[threadIdx.x];beta[threadIdx.x]=p.beta[threadIdx.x];}
 if(threadIdx.x==0){for(int i=0;i<16;++i)mbar_init(bar+i,1);fence_barrier_init();}named_bar_sync(0,PIPE_THREADS);

#if PROFILE_ROLES
 unsigned long long role_begin=clock64();
#endif
 if(blockIdx.x<DWCOUNT){
  if(threadIdx.x<128){setmaxnreg_dec<(PIPE_CONSUMERS==2?40:32)>();if(threadIdx.x==0){mbar_arrive_expect_tx(bar+4*PIPE_CONSUMERS,16384*(PIPE_HIDDEN64?2:1));for(int c=0;c<(PIPE_HIDDEN64?2:1);++c)for(int k=0;k<2;++k)tma_load_2d(sm+PIPE_W_BASE+c*16384+k*8192,&p.pre,bar+4*PIPE_CONSUMERS,k*64,((blockIdx.x%PIPE_GROUPS)*(PIPE_HIDDEN64?2:1)+c)*64);}pipe_producer(p,sm,bar);}
  else {setmaxnreg_inc<(PIPE_CONSUMERS==2?232:160)>();pipe_consumer(p,sm,bar,gamma,beta);}
 }else {
  if(threadIdx.x>=256)setmaxnreg_dec<(PIPE_CONSUMERS==2?32:24)>();
  else {setmaxnreg_inc<232>();input_role(p,sm,bar,gamma,beta);}
 }
 named_bar_sync(15,PIPE_THREADS);
#if PROFILE_ROLES
 if(threadIdx.x==0)p.counts[2+blockIdx.x]=(unsigned)(clock64()-role_begin);
#endif

#if PART_ONLY == 2
 __threadfence();named_bar_sync(15,PIPE_THREADS);
 if(threadIdx.x==0){atomicAdd(p.counts,1u);while(atomicAdd(p.counts,0u)!=UCOUNT)__nanosleep(32);}named_bar_sync(15,PIPE_THREADS);
#if DW_REDUCE_TMA
 reduce_tile(p,sm);
#else
 for(int i=blockIdx.x*PIPE_THREADS+threadIdx.x;i<131328;i+=UCOUNT*PIPE_THREADS)reduce_at(p,i);
#endif
 __threadfence();named_bar_sync(15,PIPE_THREADS);
