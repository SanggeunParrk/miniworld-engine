...
class B7(Base):
    def __init__(self,d,dy,dl,dr,dg,count=132,splits=6,part=2,slices=2,defines=None):
        ...
        self.pipe=bool(dict(self.defines).get('PIPE_DW'));self.consumers=dict(self.defines).get('PIPE_CONSUMERS',2);self.threads=128*(1+self.consumers) if self.pipe else 256
        ...
        self.partw=torch.zeros((groups,splits*slices,8192*self.consumers if self.pipe else 16384),device=x.device);self.partln=torch.empty((count-groups*splits,256),device=x.device)
