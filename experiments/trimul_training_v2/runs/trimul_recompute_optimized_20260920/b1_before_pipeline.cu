...
template<bool DW> TMN_DEVI void recreate(const Params& p,uint8_t* sm,int group){
 ...
 if(wi==0)normalize_tile<8>(xn,xn,params,params+128);
  ...
  LnStats st=normalize_tile<16,true,B1_LN_SERIAL,DW>(tri,DW?sm+65536:nullptr,params+256,params+512);
...
#if PROFILE_ROLES
 if(threadIdx.x==0)p.counts[2+blockIdx.x]=clock();allsync();
#endif
 if(blockIdx.x<DWCOUNT)weight_role(p,sm,full+1);else input_role(p,sm,full+1);
#if PROFILE_ROLES
 allsync();if(threadIdx.x==0)p.counts[2+blockIdx.x]=clock()-p.counts[2+blockIdx.x];
#endif
#if PART_ONLY==2
