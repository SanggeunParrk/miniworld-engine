...
TMN_DEVI void recompute_pre_regs(uint8_t* pre,uint32_t (&xn)[8][4],uint8_t* wg,uint8_t* wp){
 int wi=threadIdx.x/128,lane=threadIdx.x%32,w=(threadIdx.x/32)%4;
 float acc[32];gemm_recompute_regs(acc,xn,wi==0?wg:wp);
 // All weight reads retire before their shared memory can become preactivation.
 allsync();
#pragma unroll
 for(int q=0;q<4;++q){int mat=lane/8,chq=lane%8+8*(mat>>1);
...
TMN_DEVI void input_role(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 ...
 if(split<p.tiles)issue_gate(p,sm,bar+2,split*64);int round=0;for(int tile=split;tile<p.tiles;tile+=DXCOUNT,++round){
  ...
  ln_recompute_fragment<8,true>(xn_frag,gamma,beta,lane,1e-5f);
  allsync();
  if(threadIdx.x==0&&round>0)tma_store_wait_all();allsync();
  for(int side=0;side<2;++side){
   load_g(p,sm,bar,row,side,0);load_g(p,sm,bar,row,side,1);
   for(int h=0;h<4;++h){int slot=h&1;uint8_t* s=sm+slot*DX_SLOT;mbar_wait(bar+slot,h/2);recompute_pre_regs(s,xn_frag,s+24576,s);glu_small(p,s,s+16384,sm+81920+h*8192,row);
