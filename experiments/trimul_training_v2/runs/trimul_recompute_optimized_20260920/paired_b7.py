...
configs={
 'warp':dict(splits=4,defines=dict(PIPE_DW=1,SMEM_BYTES=114688)),
 'resident':dict(splits=4,defines=dict(PIPE_DW=1,SMEM_BYTES=221184,DX_RESIDENT=1)),
 'resident_reduce':dict(splits=4,defines=dict(PIPE_DW=1,SMEM_BYTES=221184,DX_RESIDENT=1,DW_REDUCE_TMA=1)),
 'reduce':dict(splits=4,defines=dict(PIPE_DW=1,SMEM_BYTES=114688,DW_REDUCE_TMA=1)),
}
with torch.no_grad():
