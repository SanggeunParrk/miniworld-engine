...
def kernel(kind,ln=0,stats=0,pg=False,method=0):
 ...
 if kind=='k1':smem=I.k1_smem(K1)+(32768 if pg and method==0 else 0);threads=384
  ...
  bi,bj,slots,acc,regs,serial=K3;defs.update(MW_SAVE_IN=ln&1,MW_SAVE_OUT=0,MW_STORE_METHOD=0,MW_SAVE_STATS_IN=stats&1,MW_SAVE_STATS_OUT=(stats>>1)&1,MW_BI=bi,MW_BJ=bj,MW_SLOT=slots,MW_ACC=acc,TMN_K3_REGS_24_240=regs,MW_SERIAL=serial,MW_FUSED=1);smem=I.k3_smem(K3)+(32768 if pg and method==0 else 0);threads=384
 assert smem<=232448
...
def output(d,tri,ln=1,stats=0,pg=False,method=0,bufs=None):
 n=d['n'];x=d['x'];m=n*n;bi,bj=K3[:2];L=T._launch_module();k,smem=kernel('k3',ln,stats,pg,method)
 if bufs is None:
  e=lambda c:torch.empty((m,c),device=x.device,dtype=x.dtype)
  f=lambda:torch.empty(m,device=x.device,dtype=torch.float32)
  saves=dict(xn=e(128) if ln&1 else None,xnout=torch.empty((256,m),device=x.device,dtype=torch.float32 if method else x.dtype) if ln&2 else None,mi=f() if stats&1 else None,ri=f() if stats&1 else None,mo=None,ro=f() if stats&2 else None,proj=e(128) if pg else None,gate=e(128) if pg else None);y=torch.empty_like(x)
 else:y,saves=bufs
 ...
 p=L.Struct([base,d['ds'],om('xn',128),maps[-1],saves['xn'],saves['xnout'],saves['mi'],saves['ri'],saves['mo'],saves['ro'],om('proj',128),om('gate',128),saves['proj'],saves['gate']]);k.launch((min(tiles,torch.cuda.get_device_properties(0).multi_processor_count),1,1),(384,1,1),[p],smem);return y,saves
