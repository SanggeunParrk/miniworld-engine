...
static_assert(sizeof(TrainParams) == 1344, "training ABI");
...
TMN_DEVI void k3_training_body(const TrainParams& tp) {
  ...
  if (tid == 0) {
    ...
    tma_prefetch_desc(&tp.tm_norm); tma_prefetch_desc(&tp.tm_proj); tma_prefetch_desc(&tp.tm_gate); tma_prefetch_desc(&tp.tm_residual);
  }
  __syncthreads();
...
#pragma unroll                                           // dependency); the proxy fence orders these generic-proxy reads before the async-proxy refill
    ...
    const LnStats stats = ln_fragment<KSP, MWK3_LNSERIAL>(fx, sGout, sBout, lane, p.eps);
    // Save normalized WGMMA operands through the same swizzled/TMA path as
