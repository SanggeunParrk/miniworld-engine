...
// MiniWorld saved-policy variant: both operands already normalized by existing LN kernels.
...
struct SavedOutputParams {
  K3Params base;
  CUtensorMap tm_norm, tm_proj, tm_gate, tm_residual;
  __nv_bfloat16 *norm, *proj, *gate;
  float *mean, *rstd;
  const __nv_bfloat16 *dropscale;
};
static_assert(sizeof(K3Params) == 768, "upstream ABI");
static_assert(sizeof(SavedOutputParams) == 1344, "training ABI");
struct SavedOutputCfg : K3Cfg<MWK3_CZ, MWK3_CH, 0, MWK3_BI, MWK3_BJ, MWK3_NSLOT, MWK3_NACC> {
...
template <class G>
TMN_DEVI void saved_output_body(const SavedOutputParams& tp) {
  ...
  constexpr bool R240 = MWK3_REGS == 240;
  if (tid == 0) {
    ...
    tma_prefetch_desc(&p.tm_out);
    tma_prefetch_desc(&tp.tm_proj); tma_prefetch_desc(&tp.tm_gate); tma_prefetch_desc(&tp.tm_residual);
  }
  __syncthreads();
...
#pragma unroll 1
        for (int kc = 0; kc < NKCZ; ++kc) {
          if (t_local > 0) mbar_wait(barZ_empty + kc, 1);
...
#define PF(i) do { } while (0)
  ...
  const uint32_t sZ_u = smem_u32(sZ), sX_u = smem_u32(sX), sW_u = smem_u32(sW);
  const uint32_t stg_u = smem_u32(sOut) + (uint32_t)((4 * cw + wiw) * OB);   // this warp's staging slice [16 tok][32 ch]
  const uint32_t proj_u = smem_u32(sProj) + (uint32_t)((4*cw+wiw)*OB);
  ...
  for (int t_local = 0; t_local < n_iter; ++t_local) {
    ...
    PF(1);                                               // 1: X wait
    {
      load_frag_bf16<KSP, BI*BJ*128>(fx,sX_u,rho0,lane);
...
#pragma unroll                                           // dependency); the proxy fence orders these generic-proxy reads before the async-proxy refill
    ...
    // F4 already produced/stored normalized X and mean/rstd. No LN here.
    PF(3);                                               // 3: LN_out
    // ---- gate operand: z rows -> release the z stage -> LN_in
    uint32_t fz[KSG][4];
    for (int kc = 0; kc < NKCZ; ++kc) mbar_wait(barZ_full + kc, 0);
    PF(4);                                               // 4: z wait
...
#pragma unroll
    ...
    // Split-N: both warpgroups released phase 0, but only the owner reads
    // each residual chunk. The other warpgroup still owes its phase-1 arrivals.
    if (SPLITN && lane == 0) {
#pragma unroll
      for (int kc=0;kc<NKCZ;++kc) {
        if ((kc & 1) != cw) {
          mbar_wait(barZ_full+kc,1);
          mbar_arrive(barZ_empty+kc);
        }
      }
    }
...
#pragma unroll
    ...
    // Training rounding contract matches engine F567: BF16 projection/logit,
...
#pragma unroll
        for (int r = 0; r < 2; ++r) {
          ...
          fp[j>>1][2*(j&1)+r]=pack_bf16(p0,p1);
...
#pragma unroll
    ...
    auto store_pass = [&](int b0) {
      ...
      if (lane == 0) {
        ...
        tma_store_commit();
        mbar_arrive_dep(barZ_empty + (b0 >> 1), d);
      }
      zdep = 0;
    };
...
void mw_saved_output(__grid_constant__ const tmn::sm90::SavedOutputParams p) {
  tmn::sm90::saved_output_body<tmn::sm90::SavedOutputCfg>(p);
}
