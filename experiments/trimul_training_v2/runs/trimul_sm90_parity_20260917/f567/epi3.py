...
class ParityF567Sm90:
    ...
    def kernel(
        ...
        sP = storage.sP.get_tensor(sO_layout.outer, swizzle=sO_layout.inner)
        sG = storage.sG.get_tensor(sO_layout.outer, swizzle=sO_layout.inner)
        ...
        pfrag = cute.make_fragment_like(acc_G, BFloat16)
        gfrag = cute.make_fragment_like(acc_G, BFloat16)
        pfrag.store(pv)
        gfrag.store(gv.to(BFloat16))
        cute.copy(store_op,copyC.retile(out_frag),copyC.partition_D(sO))
        cute.copy(store_op,copyC.retile(pfrag),copyC.partition_D(sP))
        cute.copy(store_op,copyC.retile(gfrag),copyC.partition_D(sG))
        cute.arch.fence_view_async_shared()
        cute.arch.barrier()
        if warp_idx == 0:
            for which in cutlass.range_constexpr(3):
                if cutlass.const_expr(which == 0):
                    target=tO;atom=atomO;smem_src=sO
                elif cutlass.const_expr(which == 1):
                    target=tP;atom=atomP;smem_src=sP
                else:
                    target=tG;atom=atomG;smem_src=sG
                dst=cute.local_tile(target,(TILE_M,TILE_N),(m_block,n_block))
                so,go=cpasync.tma_partition(atom,0,cute.make_layout(1),cute.group_modes(smem_src,0,cute.rank(smem_src)),cute.group_modes(dst,0,cute.rank(dst)))
    ...
    def __call__(
        ...
        sX1_layout = cute.tile_to_shape(atom, (TILE_M, TILE_K, min(STAGES, G_LOOP)), order=(0, 1, 2))
        sX2_layout = cute.tile_to_shape(atom, (TILE_M, TILE_K, min(STAGES, P_LOOP)), order=(0, 1, 2))
        wg_layout = (
...
def feasibility(config, smem_limit=232448, kp=None, kg=None):
    ...
    if 2 * (bm + bn) * bk * (ps + gs) + 6 * bm * bn + 1024 > smem_limit:
...
def output_f567_impl(norm, x, wp, wg, residual, dropscale, seq_len, config=None):
    ...
    if config is None:
        ...
        config = resolve(
            ...
            run=lambda c: output_f567_impl(*tensors, seq_len, c),
        )
    reason = feasibility(config, limit, kp, kg)
    if reason:
        raise ValueError(reason)
    outputs = _fake(norm, x, wp, wg, residual, dropscale, seq_len)
    # Transpose is a metadata view; no transpose kernel or allocation.
    wg_nk = wg.t()
    args = [
    ...
    key = (
        str(norm.device),
        tensor_key(*tensors, extra=(seq_len,)),
...
@opaque(fake=_fake, name="trimul_parity_f567_sm90_epi3_experiment")
