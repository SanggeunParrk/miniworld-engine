...
class ParityF567Sm90:
    ...
    def kernel(
        ...
        for k in cutlass.range_constexpr(G_LOOP):
            ...
            if cutlass.const_expr(k + STAGES < G_LOOP or k == G_LOOP - 1):
    ...
    def __call__(
        ...
        sX1_layout = cute.tile_to_shape(atom, (TILE_M, TILE_K, STAGES), order=(0, 1, 2))
        sX2_layout = cute.tile_to_shape(atom, (TILE_M, TILE_K, STAGES), order=(0, 1, 2))
        wg_layout = (
...
def feasibility(config, smem_limit=232448):
    ...
    if nw != bm // 16:
        return "Each 64-row WGMMA group requires exactly four participating warps"
    if (
    ...
    if 4 * (bm + bn) * bk * ns + 2 * bm * bn + 1024 > smem_limit:
