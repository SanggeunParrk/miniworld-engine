...
class DualBackwardSm90:
    ...
    def kernel(
        ...
        if warp == 0:
            with cute.arch.elect_one():
                ...
                for stage in cutlass.range_constexpr(3 * self.stages):
        ...
        if warp < 4:
            ...
            thrg, thrf = mg.get_slice(tidx % 128), mf.get_slice(tidx % 128)
            ...
            if cutlass.const_expr(self.n % 8 == 0):
                ...
                copy_c = cute.make_tiled_copy_C(store_op, mg).get_slice(tidx % 128)
    ...
    def __call__(self, g, f, w, v, y, stream: cuda.CUstream):
        ...
        @cute.struct
        class Storage:
            bar: cute.struct.MemRange[cutlass.Int64, 3 * self.stages]
            sg: sg_type
            sw: sw_type
...
def feasibility(config, smem_limit=232448):
    ...
    if config["num_warps"] != 8 or bm != 64 or bn > 128:
...
@opaque(fake=_fake, name="trimul_round2_specialized2cta_sm90")
