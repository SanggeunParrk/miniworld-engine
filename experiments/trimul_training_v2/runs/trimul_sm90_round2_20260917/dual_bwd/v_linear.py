...
class DualBackwardSm90:
    ...
    def __call__(self, g, f, w, v, y, stream: cuda.CUstream):
        ...
        lw = layout(self.bn, True)
        lv = cute.make_composed_layout(
            cute.make_swizzle(0, 4, 3), 0,
            cute.make_layout((self.bn, self.bk, self.stages), stride=(1, self.bn, self.bn * self.bk)),
        )
...
@opaque(fake=_fake, name="trimul_round2_v_linear_sm90")
