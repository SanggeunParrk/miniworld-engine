...
from miniworld_engine.kernels.trimul_inproj.cute import parity_f567 as base
...
for L in (384,768):
 ...
 for c in (cfg(gm=4),cfg(128,128,gm=4)):
  try:
  ...
  rows.append(row);print(L,json.dumps(row),flush=True);(root/'dropout_tma_wide.json').write_text(json.dumps(result,indent=2))
