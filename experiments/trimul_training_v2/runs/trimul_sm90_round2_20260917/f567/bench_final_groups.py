...
for L in (384,768):
 ...
 for c in (cfg(gm=1),cfg(gm=2),cfg(gm=4),cfg(gm=8)):
  try:
   ...
   fns={'candidate':lambda:mod.output_f567_impl(*args,c),'checkpoint':lambda:mod.output_f567_impl(*args,cfg(gm=4)),'triton':lambda:tri(tc)};samples={k:[] for k in fns}
   for rnd in range(5):
    for k in list(fns)[::1 if rnd%2==0 else -1]:samples[k].append(triton.testing.do_bench_cudagraph(fns[k],rep=60))
  ...
  rows.append(row);print(L,json.dumps(row),flush=True);(root/'final_groups.json').write_text(json.dumps(result,indent=2))
