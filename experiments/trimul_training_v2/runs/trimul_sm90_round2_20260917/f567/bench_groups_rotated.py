...
for L in (384,768):
 ...
 configs={f'group{g}':cfg(gm=g) for g in (1,2,4,8)}
 fns={name:(lambda c=c:mod.output_f567_impl(*args,c)) for name,c in configs.items()}
 fns['triton']=lambda:tri(tc)
 samples={k:[] for k in fns}
 for name,c in configs.items():
  got=mod.output_f567_impl(*args,c);torch.cuda.synchronize()
  assert all(torch.equal(u,v) for u,v in zip(got,(y,p,g)))
 for rnd in range(5):
  keys=list(fns);keys=keys[rnd%len(keys):]+keys[:rnd%len(keys)]
  if rnd%2:keys=keys[::-1]
  for name in keys:samples[name].append(triton.testing.do_bench_cudagraph(fns[name],rep=60))
 row={'configs':configs,'samples_ms':samples,'medians_ms':{k:statistics.median(v) for k,v in samples.items()}}
 result[str(L)]=row;print(L,json.dumps(row),flush=True);(root/'final_groups_rotated.json').write_text(json.dumps(result,indent=2))
