...
import torch,triton
import projection_prefetch_staged as mod
...
for L in (384,768):
 ...
 for c in (cfg(gm=4),cfg(128,gm=4)):
  ...
  rows.append(row);print(L,json.dumps(row),flush=True);(root/'projection_prefetch_staged.json').write_text(json.dumps(result,indent=2))
