...
for L in (384,768):
 ...
 fns={'cute':lambda:mod.output_f567_impl(*args,c),'cute_previous':lambda:base.output_f567_impl(*args,cb),'triton_bm128':lambda:tri(tc),'triton_old':lambda:tri(cfg(s=3,gm=8)),'triton_m64':lambda:tri(cfg(s=3)),'triton_matched':lambda:tri(c)}
 ...
 compiled=list(mod._COMPILE_CACHE.values())[-1];folder=root/f'dropout-tma-final-L{L}';folder.mkdir(exist_ok=True);binary=compiled.__cubin__;binary=Path(binary).read_bytes() if isinstance(binary,str) else binary;(folder/'kernel.cubin').write_bytes(binary)
 ...
 row={'cute_config':c,'triton_config':tconfigs[tkey],'triton_winner':tkey,'samples_ms':samples,'medians_ms':medians,'speedup':medians[tkey]/medians['cute'],'relative_l2':errors,'resources':resources,'cubin_sha256':hashlib.sha256(binary).hexdigest(),'has_hgmma':'HGMMA' in sass,'has_tma':'UTMALDG' in sass or 'UTMASTG' in sass};result[str(L)]=row;print(L,json.dumps(row),flush=True);(root/'dropout-tma-final-results.json').write_text(json.dumps(result,indent=2))
