...
for seed,L in ((s,l) for s in (123,345,678) for l in (384,768)):
 ...
 c=cfg();cb=cfg() if L==384 else cfg(128)
 ...
 fns={'cute':lambda:mod.output_f567_impl(*args,c),'dropout_only':lambda:dropout.output_f567_impl(*args,c),'cute_previous':lambda:base.output_f567_impl(*args,cb),'triton_bm128':lambda:tri(tc),'triton_old':lambda:tri(cfg(s=3,gm=8)),'triton_m64':lambda:tri(cfg(s=3)),'triton_matched':lambda:tri(c)}
 ...
 compiled=list(mod._COMPILE_CACHE.values())[-1];folder=root/f'prefetch-final-{seed}-L{L}';folder.mkdir(exist_ok=True);binary=compiled.__cubin__;binary=Path(binary).read_bytes() if isinstance(binary,str) else binary;(folder/'kernel.cubin').write_bytes(binary)
 ...
 row={'cute_config':c,'triton_config':tconfigs[tkey],'triton_winner':tkey,'samples_ms':samples,'medians_ms':medians,'speedup':medians[tkey]/medians['cute'],'relative_l2':errors,'resources':resources,'cubin_sha256':hashlib.sha256(binary).hexdigest(),'has_hgmma':'HGMMA' in sass,'has_tma':'UTMALDG' in sass or 'UTMASTG' in sass};result[f'{seed}-{L}']=row;print(L,json.dumps(row),flush=True);(root/'prefetch-final-results.json').write_text(json.dumps(result,indent=2))
