...
cute.compile=keep
from miniworld_engine.kernels.trimul_inproj.cute import parity_f567 as mod
...
args=(a,x,wp,wg,r,ds,L);cfg=dict(BLOCK_M1=64,BLOCK_N=64 if L==384 else 128,BLOCK_K=64,GROUP_M=4,num_warps=4,num_stages=2)
...
Path(f'current-L{L}.cubin').write_bytes(binary)
Path(f'current-L{L}.resources.txt').write_text(subprocess.check_output(['/usr/local/cuda/bin/cuobjdump','--dump-resource-usage',f'current-L{L}.cubin'],text=True))
Path(f'current-L{L}.sass').write_text(subprocess.check_output(['/usr/local/cuda/bin/cuobjdump','--dump-sass',f'current-L{L}.cubin'],text=True))
Path(f'current-L{L}.json').write_text(json.dumps({'L':L,'config':cfg,'sha256':hashlib.sha256(binary).hexdigest()},indent=2))
