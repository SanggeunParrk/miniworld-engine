...
class ParityF567Sm90:
    ...
    def kernel(
        ...
        sP_base = storage.sX1.get_tensor(sO_layout.outer, swizzle=sO_layout.inner)
        ...
        for iteration in cutlass.range(min(self.group_m, nm - first_m), unroll=1):
            ...
            for k in cutlass.range_constexpr(G_LOOP):
                stage = k % STAGES
                cute.arch.mbarrier_wait(mbar_full_ptr + stage, Int32((iteration * (cute.ceil_div(G_LOOP - stage, STAGES) + (1 if has_ds and stage == 0 else 0)) + k // STAGES) % 2))
                warpgroup.fence()
            ...
            for k in cutlass.range_constexpr(P_LOOP):
                ...
                cute.arch.mbarrier_wait(
                    mbar_full_ptr + STAGES + stage, Int32((iteration * cute.ceil_div(P_LOOP - stage, STAGES) + k // STAGES) % 2)
                )
                warpgroup.fence()
            ...
            cute.arch.mbarrier_wait(mbar_full_ptr + 2 * STAGES, Int32(iteration % 2))
            ...
            if cutlass.const_expr(
                ...
                a_tiles = max(cute.cosize(sX1_layout), cute.cosize(sX2_layout)) // cute.cosize(sO_layout)
                ...
                if cutlass.const_expr(use_tma_ds):
                    cute.arch.mbarrier_wait(mbar_full_ptr, Int32((iteration * (cute.ceil_div(G_LOOP, STAGES) + 1) + cute.ceil_div(G_LOOP, STAGES)) % 2))
    ...
    def __call__(
        ...
        M = mX1.shape[0]
        m_blocks = cute.ceil_div(cute.ceil_div(M, TILE_M), self.group_m) * cute.ceil_div(self.N, TILE_N)
        ...
        atomD, tD_tma = cpasync.make_tiled_tma_atom(cpasync.CopyBulkTensorTileG2SOp(), mD, sO_layout, (TILE_M, TILE_N))
...
@opaque(fake=_fake, name="trimul_round2_small_ring")
