...
from tri_policy import Training,BASE,S,STATS,NATIVE,normalized_comparison
R=Path(__file__).resolve().parent
OLD=BASE.OLD;Q=OLD.Q;LN=OLD.LN
N=normalized_comparison()
...
def main():
 ...
 with torch.no_grad():
  a=Q.setup(n);d=a['d'];models={'baseline':BASE.Training(a),'xhat_fp32':N.Replacement(a,1),'tri_stats':Training(a)}
  result=dict(L=n,C=128,H=256,dropout=.25,checks={},mutated={},poison={},times={},blocks={},meta=dict(host=platform.node(),gpu=torch.cuda.get_device_name(),raw_tri_in_backward={'baseline':True,'tri_stats':True,'xhat_fp32':False},includes=['both directions','mask','dropout','residual','live packing','all eleven gradients'],excludes=['optimizer','RNG generation','compile','CPU dispatch']),production_ready=False)
  ...
  m=models['tri_stats'];m.audit=True;y,k=m.forward();ab,tri,packed,stats,xn=k
  ...
  result['cubins']={k:dict(b1=m.p1.k.unit.cubin_path) for k,m in models.items()};result['source_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [R/'bench.py',R/'tri_policy.py',R/'training.py',STATS/'final.py',STATS/'replace_plan.py',STATS/'replace_core.py',STATS/('selected-L%d.json'%n),NATIVE/('fwd-selected-L%d.json'%n),NATIVE/('bwd-selected-L%d.json'%n),*STATS.glob('*.cu'),*STATS.glob('*.cuh'),*STATS.glob('*.inc')]};path.write_text(json.dumps(result,indent=2));print('DONE',n,flush=True)
