...
assert D==128 and a.no_save_xn and not a.separate_ln
settings.configure(engine_backend='triton',trimul_sm90_kernels=(),autotune_miss_cap=24)
record=dict(D=D,job=os.environ.get('SLURM_JOB_ID'),source='Anthropic v5 K1/K3, K1 occupancy widened; shared-memory-limited K3 uses existing Triton',results={},candidates=[(2,64,8,2,1)], tuning="Original D128 configs retained; no retuning")
