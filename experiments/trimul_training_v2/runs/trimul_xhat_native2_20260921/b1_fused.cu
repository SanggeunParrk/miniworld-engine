...
struct Params {
 ...
 const __nv_bfloat16* ds;const float *gi,*bi,*gamma,*bo,*saved_rs,*xhat_ptr;
 __nv_bfloat16 *dg,*dwg,*dwp,*dtri_ptr;float *dgam,*dbeta,*partw,*partln;
...
TMN_DEVI void load_raw(const Params& p,uint8_t* sm,uint64_t* bar,int slot,int row){
 if(threadIdx.x)return;uint8_t* dst=sm+16384+slot*49152;mbar_arrive_expect_tx(bar+slot,(XHAT_FP32==1?16384:49152)+(XHAT_FP32==-1?512:0));
 tma_load_2d(dst,&p.x,bar+slot,0,row);tma_load_2d(dst+8192,&p.x,bar+slot,64,row);
#if XHAT_FP32 == 1
 // Gate weights stay resident. xhat loads after the input gate consumes x_n.
#else
 tma_load_2d(dst+16384,&p.xhat,bar+slot,row,0);
#if XHAT_FP32 == -1
 tma_load_2d(sm+225280,&p.statsmap,bar+slot,row,1);tma_load_2d(sm+225536,&p.statsmap,bar+slot,row,0);
#endif
#endif
...
TMN_DEVI void prepare_shared(const Params& p,uint8_t* sm,uint64_t* bars,int slot,int row,int phase,const FragmentMask& mask,int mi){
...
#endif
#if XHAT_FP32 == -1
 int r8=lane%8;float* mus=reinterpret_cast<float*>(sm+225280);float ma=mus[ra],mb=mus[rb],rA=mus[64+ra],rB=mus[64+rb];
 constexpr int affine_unroll = B1_STREAM_AFFINE_UNROLL;
 #pragma unroll affine_unroll
 for(int k=0;k<16;++k){uint32_t f[4];ldsm_x4_t(f,smem_u32(x+16384)+swz128(k*16+r8+8*(mat>>1),(w*16+8*(mat&1))*2));
...
#else
  constexpr int native_affine_unroll = B1_STREAM_AFFINE_UNROLL;
  #pragma unroll native_affine_unroll
  for(int k=0;k<16;++k){uint32_t f[4];int c=16*k+2*(lane&3);float h[8];xhat_frag(h,sm,slot,w,lane,k);
   #pragma unroll
   for(int j=0;j<4;++j){int r=(j&1)?rb:ra,cc=c+8*(j>>1);float a=h[j*2],b=h[j*2+1];f[j]=pack_bf16(__fmaf_rn(a,ps[256+cc],ps[512+cc]),__fmaf_rn(b,ps[257+cc],ps[513+cc]));}
...
#if XHAT_FP32 == 1
 // Wg has finished: x_n and dy staging are free; keep Wg resident.
 if(threadIdx.x==0){mbar_arrive_expect_tx(bars+4,65536);for(int q=0;q<4;++q){void* dst=q<3?x+q*16384:sm;const float* src=p.xhat_ptr+(size_t)(row/64)*16384+q*4096;
 ...
 for(int q=0;q<4;++q){uint32_t dy[4],dg[4],dp[4];
  ...
  ldsm_x4(dy,smem_u32((XHAT_FP32==1?norm+32768:sm)+wi*8192)+off);
  #pragma unroll
 ...
 fence_proxy_async();allsync();
 if(XHAT_FP32!=1 && threadIdx.x==0){dg_store(&p.dgmap,sm,0,row);dg_store(&p.dgmap,sm+8192,64,row);tma_store_commit();}
}
...
TMN_DEVI void shared_role(const Params& p,uint8_t* sm,uint64_t* bars){
 ...
 for(int tile=blockIdx.x;tile<p.tiles;tile+=UCOUNT,++round){int slot=round&1;
...
#if !PRODUCER
  load_dy(p,XHAT_FP32==1?sm+16384+(1-slot)*49152+32768:sm,bars+2,tile*64);
#endif
  ...
  if(XHAT_FP32!=1)for(int j=threadIdx.x;j<4096;j+=256)reinterpret_cast<uint32_t*>(sm)[j]=reinterpret_cast<uint32_t*>(norm+32768)[j];
  fence_proxy_async();allsync();
