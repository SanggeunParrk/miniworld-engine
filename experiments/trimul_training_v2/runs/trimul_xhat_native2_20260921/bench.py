...
class Replacement:
 def __init__(self,a,fp32):
  self.a=a;self.d=a['d'];self.fp32=fp32;self.audit=False;self.fcfg=tuple(json.loads((R/('fwd-selected-L%d.json'%self.d['n'])).read_text())['config']) if (R/('fwd-selected-L%d.json'%self.d['n'])).exists() else None
  _,k=self.forward();ab,xhat,packed,rs,xn=k
  cfg=json.loads((PREV/('selected-L%d.json'%self.d['n'])).read_text())['config'];cfg=json.loads((R/('bwd-selected-L%d.json'%self.d['n'])).read_text())['config'] if (R/('bwd-selected-L%d.json'%self.d['n'])).exists() else cfg;cfg['defines']['XHAT_FP32']=int(fp32)
 ...
 def forward(self):
  ...
  y,s=RC.output(d,tri,ln=3,stats=2,method=int(self.fp32),cfg=self.fcfg)
...
def main():
 ...
 with torch.no_grad():
  a=Q.setup(n);d=a['d'];models={'baseline':BASE.Training(a),'prior_xhat':OLDN.Replacement(a,1),'xhat_fp32':Replacement(a,1)}
