...
def build(count,length,part,defines=()):
 ...
 L=T._launch_module();drv=L.BlockDriver(device=0);mod=drv.load(out.read_bytes());u=L.Unit('shared_b1','sm_90a',0,drv.drv,mod,{},str(out));k=u.kernel('b1_fused');k.set_max_dynamic_smem(227840)
 return k,u.kernel('b1_reduce'),227840
class Plan(SP.B1):
 ...
 def bind(self,dy,xhat,rstd):
  ...
  maps=[row(dy,128),row(d['x'],128),L.tensor_map(xhat,[16,256],dims=[m,256],strides_bytes=[m*es],swizzle='64B',l2='128B') if es==4 else tm(xhat,[64,256],[m,256],[m*es]),tm(self.wpt,[64,64],[128,256],[256]),tm(self.wgt,[64,64],[128,128],[256]),tm(dt,[64,16,1],[m,256,1],[m*2,m*512]),row(dg,128)]
  ...
  self.p=L.Struct([*maps,d['ds'],d['gi'],d['bi'],d['go'],d['bo'],rstd,xhat,dg,dwg,dwp,dt,dgo,dbo,self.partw,self.partln,self.counts,m,n,m//64])
