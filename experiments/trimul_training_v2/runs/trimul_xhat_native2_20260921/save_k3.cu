...
TMN_DEVI LnStats ln_save_xhat(uint32_t (&fa)[KS][4], const float* sGamma, const float* sBeta, int lane, float eps, void* output, int row, int M, const CUtensorMap* outmap, uint8_t* stage) {
...
#pragma unroll
  for (int ks = 0; ks < KS; ++ks) {
    ...
    // Native tile layout: [tile64, ks16, warp4, channel_half2, lane32, float4].
    // Each lane writes two aligned vectors with all normalization bits intact.
    uint32_t delay=zero_dep(__float_as_uint(g0.x)^__float_as_uint(g1.x));
    float z[8];
    #pragma unroll
    for(int j=0;j<4;++j){float mu=(j&1)?meanB:meanA,rs=(j&1)?rB:rA;
     z[j*2]=__fmul_rn(__fsub_rn(bf16lo(fa[ks][j]+delay),mu),rs);z[j*2+1]=__fmul_rn(__fsub_rn(bf16hi(fa[ks][j]+delay),mu),rs);
    }
    size_t offset=((size_t)(row/64)*16+ks)*1024+((row%64)/16)*256+lane*4;
    if(output)stg128(reinterpret_cast<float*>(output)+offset,make_uint4(__float_as_uint(z[0]),__float_as_uint(z[1]),__float_as_uint(z[2]),__float_as_uint(z[3])));
    if(output)stg128(reinterpret_cast<float*>(output)+offset+128,make_uint4(__float_as_uint(z[4]),__float_as_uint(z[5]),__float_as_uint(z[6]),__float_as_uint(z[7])));
    if (CLS == math::TX) {                             // y = x * (r g) + (b - mean r g)
      ...
      fa[ks][0] = pack_bf16(__fmaf_rn(z[0],g0.x,b0.x), __fmaf_rn(z[1],g0.y,b0.y));
      fa[ks][1] = pack_bf16(__fmaf_rn(z[2],g0.x,b0.x), __fmaf_rn(z[3],g0.y,b0.y));
      fa[ks][2] = pack_bf16(__fmaf_rn(z[4],g1.x,b1.x), __fmaf_rn(z[5],g1.y,b1.y));
      fa[ks][3] = pack_bf16(__fmaf_rn(z[6],g1.x,b1.x), __fmaf_rn(z[7],g1.y,b1.y));
    }
...
#pragma unroll                                           // dependency); the proxy fence orders these generic-proxy reads before the async-proxy refill
    ...
    LnStats stats_out=ln_save_xhat<KSP,MW_SERIAL>(fx,sGout,sBout,lane,p.eps,(SPLITN&&cw!=0)?nullptr:tp.lnout,iw*p.N+jw+16*wiw+(lane>>2),p.N*p.N,&tp.tm_lnout,sOut+(4*cw+wiw)*OB);
#if MW_SAVE_STATS_OUT
    if((!SPLITN||cw==0)&&(lane&3)==0 && iw<p.N){int r=jw+16*wiw+(lane>>2);if(r<p.N){tp.rs_out[(size_t)iw*p.N+r]=stats_out.rA;
#if XHAT_FP32 == -1 || XHAT_FP32 == 3
 tp.mean_out[(size_t)iw*p.N+r]=stats_out.mA;
#endif
}if(r+8<p.N){tp.rs_out[(size_t)iw*p.N+r+8]=stats_out.rB;
#if XHAT_FP32 == -1 || XHAT_FP32 == 3
