...
@opaque(fake=_persist_fwd_fake, name="layernorm_persistent_fwd")
def _persist_fwd(
    x_2d: torch.Tensor,
    weight: torch.Tensor,
    bias: torch.Tensor,
    ...
    Split out of ``TritonLayerNormPersistentFunction.forward`` so the reshape and
...
@opaque(fake=_persist_bwd_fake, name="layernorm_persistent_bwd")
def _persist_bwd(
    dy_2d: torch.Tensor,
    x: torch.Tensor,
    weight: torch.Tensor,
    ...
    The persistent grid width comes from the device's SM count, so it is resolved in here rather
    than by the caller: it is a launch detail, and a fake must not depend on which card runs.
    """
    m, n = x.shape
