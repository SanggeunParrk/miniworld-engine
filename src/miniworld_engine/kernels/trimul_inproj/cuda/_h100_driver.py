...
class DriverError(RuntimeError):
    ...
    def __init__(self, call, code, name, detail=""):
        RuntimeError.__init__(
            self,
            "%s -> %s (%d)%s" % (call, name, code, (": " + detail) if detail else ""),
        )
        self.call, self.code, self.name = call, int(code), name


# ---------------------------------------------------------------------------------------------------------------- the ctypes form
class _CUlaunchAttribute(ctypes.Structure):
    _fields_ = [
        ("id", ctypes.c_int),
        ("pad", ctypes.c_int),
        ("value", ctypes.c_ubyte * 64),
    ]  # CUlaunchAttribute: id + 64-byte value union


class _CUlaunchConfig(ctypes.Structure):
    _fields_ = [
        ...
        ("attrs", ctypes.POINTER(_CUlaunchAttribute)),
...
_SIGS = {  # entry point -> argtypes; restype is CUresult (c_int) everywhere
    "cuLaunchKernelEx": [
        ctypes.POINTER(_CUlaunchConfig),
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,
    ],
...
class _CtypesDriver(object):
    ...
    def __init__(self):
        try:
            ...
            raise Unresolvable(
                "libcuda.so.1 not loadable (%s)" % str(e).split("\n")[0][:120]
            )
        self._missing = set()
        for name, argtypes in list(_SIGS.items()) + list(_OPTIONAL_SIGS.items()):
            fn = None
            for cand in (
                (name + "_v2", name) if name in ("cuCtxGetCurrent",) else (name,)
            ):
    ...
    def device_attribute(self, attr, dev):
        ...
        self._check(
            ...
            self._cuDeviceGetAttribute(ctypes.byref(v), int(attr), int(dev)),
        )
        return int(v.value)
    ...
    def primary_ctx_retain(self, dev):
        ...
        self._check(
            ...
            self._cuDevicePrimaryCtxRetain(ctypes.byref(c), int(dev)),
        )
        return int(c.value or 0)
    ...
    def module_load_data(self, image):
        ...
        buf = ctypes.create_string_buffer(
            image, len(image)
        )  # keeps a NUL-safe copy alive for the call
        m = ctypes.c_void_p(0)
    ...
    def module_get_function(self, module, name):
        ...
        self._check(
            ...
            self._cuModuleGetFunction(
                ctypes.byref(f), ctypes.c_void_p(int(module)), nm
            ),
            nm.decode(),
        )
        return int(f.value)
    ...
    def module_unload(self, module):
        self._check(
            "cuModuleUnload", self._cuModuleUnload(ctypes.c_void_p(int(module)))
        )
    ...
    def func_get_attribute(self, attr, func):
        ...
        self._check(
            "cuFuncGetAttribute",
            self._cuFuncGetAttribute(
                ctypes.byref(v), int(attr), ctypes.c_void_p(int(func))
            ),
        )
        return int(v.value)
    ...
    def func_set_attribute(self, func, attr, value):
        self._check(
            "cuFuncSetAttribute",
            self._cuFuncSetAttribute(ctypes.c_void_p(int(func)), int(attr), int(value)),
        )

    def launch_kernel_ex(
        self, func, grid, block, smem_bytes, stream, kernel_params, pdl=False
    ):
        """cuLaunchKernelEx with the programmatic-stream-serialisation attribute (CU_LAUNCH_ATTRIBUTE_PROGRAMMATIC_STREAM_SERIALIZATION = 3)."""
        cfg = _CUlaunchConfig(
            int(grid[0]),
            int(grid[1]),
            int(grid[2]),
        ...
        attrs = (_CUlaunchAttribute * 1)()
        if pdl:
            attrs[0].id = 3
            ctypes.memmove(attrs[0].value, ctypes.byref(ctypes.c_int(1)), 4)
            cfg.attrs = ctypes.cast(attrs, ctypes.POINTER(_CUlaunchAttribute))
            cfg.numAttrs = 1
        kp = (
        ...
        self._check(
            "cuLaunchKernelEx",
            self._cuLaunchKernelEx(
                ctypes.byref(cfg), ctypes.c_void_p(int(func)), kp, ctypes.c_void_p(0)
            ),
        )
    ...
    def launch_kernel(self, func, grid, block, smem_bytes, stream, kernel_params):
        ...
        rc = self._cuLaunchKernel(
            ctypes.c_void_p(int(func)),
            int(grid[0]),
            int(grid[1]),
            int(grid[2]),
            int(block[0]),
    ...
    def stream_is_capturing(self, stream):
        ...
        self._check(
            "cuStreamIsCapturing",
            self._cuStreamIsCapturing(ctypes.c_void_p(int(stream)), ctypes.byref(v)),
        )
    ...
    def tensor_map_encode_tiled(
        ...
        if not self.has_tensor_maps():
            raise DriverError(
                ...
                "libcuda.so.1 lacks the tensor-map API (driver older than CUDA 12.0)",
            )
        rank = int(rank)
        gd = (ctypes.c_uint64 * rank)(*[int(v) for v in global_dim])
        gs = (ctypes.c_uint64 * max(rank - 1, 1))(
            *([int(v) for v in global_strides][: rank - 1] or [0])
        )
        ...
        rc = self._cuTensorMapEncodeTiled(
        ...
        self._check(
            "cuTensorMapEncodeTiled",
            rc,
            "dtype=%d rank=%d dim=%s strides=%s box=%s"
            % (int(dtype), rank, list(global_dim), list(global_strides), list(box_dim)),
        )
...
class _WheelDriver(object):
    ...
    def __init__(self):
        try:
            ...
            raise Unresolvable(
                "cuda.bindings not importable (%s)" % str(e).split("\n")[0][:120]
            )
        for need in (
            "cuTensorMapEncodeTiled",
            "cuLaunchKernel",
    ...
    def device_name(self, dev):
        r = self._unwrap(
            "cuDeviceGetName", self.d.cuDeviceGetName(256, self.d.CUdevice(int(dev)))
        )
    ...
    def device_attribute(self, attr, dev):
        return int(
            self._unwrap(
                "cuDeviceGetAttribute",
                self.d.cuDeviceGetAttribute(
                    self.d.CUdevice_attribute(int(attr)), self.d.CUdevice(int(dev))
                ),
            )
        )
    ...
    def primary_ctx_retain(self, dev):
        return int(
            self._unwrap(
                "cuDevicePrimaryCtxRetain",
                self.d.cuDevicePrimaryCtxRetain(self.d.CUdevice(int(dev))),
            )
            or 0
        )
    ...
    def ctx_set_current(self, ctx):
        self._unwrap(
            "cuCtxSetCurrent", self.d.cuCtxSetCurrent(self.d.CUcontext(int(ctx)))
        )
    ...
    def module_get_function(self, module, name):
        ...
        return int(
            self._unwrap(
                "cuModuleGetFunction",
                self.d.cuModuleGetFunction(self.d.CUmodule(int(module)), nm),
                nm.decode(),
            )
        )
    ...
    def module_unload(self, module):
        self._unwrap(
            "cuModuleUnload", self.d.cuModuleUnload(self.d.CUmodule(int(module)))
        )
    ...
    def func_get_attribute(self, attr, func):
        return int(
            self._unwrap(
                "cuFuncGetAttribute",
                self.d.cuFuncGetAttribute(
                    self.d.CUfunction_attribute(int(attr)), self.d.CUfunction(int(func))
                ),
            )
        )
    ...
    def func_set_attribute(self, func, attr, value):
        self._unwrap(
            "cuFuncSetAttribute",
            self.d.cuFuncSetAttribute(
                self.d.CUfunction(int(func)),
                self.d.CUfunction_attribute(int(attr)),
                int(value),
            ),
        )
    ...
    def launch_kernel_ex(
        ...
        cfg.sharedMemBytes = int(smem_bytes)
        cfg.hStream = d.CUstream(int(stream))
        attrs = []
        if pdl:
            at = d.CUlaunchAttribute()
            at.id = d.CUlaunchAttributeID.CU_LAUNCH_ATTRIBUTE_PROGRAMMATIC_STREAM_SERIALIZATION
        ...
        self._unwrap(
            "cuLaunchKernelEx", d.cuLaunchKernelEx(cfg, d.CUfunction(int(func)), kp, 0)
        )
    ...
    def stream_is_capturing(self, stream):
        v = self._unwrap(
            "cuStreamIsCapturing",
            self.d.cuStreamIsCapturing(self.d.CUstream(int(stream))),
        )
    ...
    def tensor_map_encode_tiled(
        ...
        tm = self._unwrap(
            "cuTensorMapEncodeTiled",
            d.cuTensorMapEncodeTiled(
                d.CUtensorMapDataType(int(dtype)),
                rank,
                int(global_address),
                [d.cuuint64_t(int(v)) for v in global_dim],
                [d.cuuint64_t(int(v)) for v in list(global_strides)[: rank - 1]],
                [d.cuuint32_t(int(v)) for v in box_dim],
                [d.cuuint32_t(int(v)) for v in element_strides],
                d.CUtensorMapInterleave(int(interleave)),
                d.CUtensorMapSwizzle(int(swizzle)),
                d.CUtensorMapL2promotion(int(l2_promotion)),
                d.CUtensorMapFloatOOBfill(int(oob_fill)),
            ),
            "dtype=%d rank=%d dim=%s strides=%s box=%s"
            % (int(dtype), rank, list(global_dim), list(global_strides), list(box_dim)),
        )
        ...
        if len(words) != 16:
            raise DriverError(
                ...
                999,
                "CUDA_ERROR_UNKNOWN",
                "unexpected CUtensorMap layout in cuda.bindings (%d words)"
                % len(words),
            )
...
def get(binding=None):
    ...
    nothing resolves."""
    want = (
        (binding or os.environ.get("TRIMUL_NATIVE_DRIVER", "auto") or "auto")
        .strip()
        .lower()
        .replace(".", "_")
        .replace("-", "_")
    ...
    if want not in ("auto", "ctypes", "cuda_bindings", "wheel"):
        raise ValueError(
            "binding must be auto | ctypes | cuda_bindings (got %r)" % (binding,)
        )
    if want == "wheel":
        want = "cuda_bindings"
    with _LOCK:
        drv = _STATE["drv"]
        if drv is not None:
            if want == "auto" or drv.word.startswith(
                "ctypes" if want == "ctypes" else "cuda.bindings"
            ):
                return drv
            raise Unresolvable(
                "binding %s already resolved in this process (asked for %s)"
                % (drv.word, want)
            )
        errs = []
...
def driver(binding=None, device=None):
    ...
    from miniworld_engine.kernels.trimul_inproj.cuda import _h100_launch as launch
