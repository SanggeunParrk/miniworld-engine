...
PDL = os.environ.get("TRIMUL_NATIVE_PDL", "1").strip().lower() not in (
    "",
    "0",
    "off",
    "false",
)  # programmatic dependent launch of K1 / K3 (TMN_PDL builds only)
MASK_CODE = {
    torch.float32: 1,
    torch.bfloat16: 2,
    torch.bool: 3,
    torch.uint8: 3,
}  # K1 name field m: 0 no mask | 1 fp32 | 2 bf16 | 3 bool/uint8 (TMN_MASK_TEMPLATE)
...
TILE_TABLE = {
    ("sm_90a", 256, 256, "b"): dict(
        k1=(2, 64, 4, 4),
        k3=(2, 64, 4, 1),
        k3_u_late=1,  # update-only K3 releasing z pair by pair: -18..-24 % K3 at N 800-2048 (per-kernel same-process A/B); 64 / 128 keep the eager release (measured faster there)   # K1 4 x 32 KB ring: -1..-3 % op vs (2,64,8,2) at N 1200-2048 (same-process A/B); K3 (1,128,4,1) at
        k1_variants=[(2, 64, 4, 4), (2, 64, 8, 2), (1, 128, 8, 2)],
        k3_variants=[(2, 64, 4, 1), (1, 128, 4, 1), (1, 64, 4, 1)],
    ),  # N > 1200: -0.6 % without / +1..3 % with residual -> not tabled
    ("sm_90a", 256, 256, "f"): dict(
        k1=(2, 64, 4, 2),
        k3=(2, 64, 4, 1),
        k3_mode="c",
        k1_variants=[(2, 64, 4, 2)],
        k3_variants=[(2, 64, 4, 1)],
        k3_variants_f=[(1, 64, 4, 1)],
    ),  # fp32 z at 256: route "c" = one bf16 cast pass + the bf16 K1 + K3 mode c (fp32 residual/out); "p" = fp32 K1 emitting bf16 LN rows + K3 mode p; "f" = fp32 tiles (split-N K3)
    ("sm_90a", 128, 128, "b"): dict(
        k1=(6, 32, 8, 2),
        k3=(3, 64, 8, 1),
        by_n={
            1536: dict(k3=(3, 64, 8, 1)),
            4096: dict(k3=(2, 64, 8, 1)),
        },  # experiment: 192-token K3 (3 consumer WGs) as the default up to N 1536   # measured: 192-token K1 tile -3..-5 % op vs (2,64,8,2) at every N (same-process A/B); K3 two accumulator sets N <= 1536
        k1_variants=[
            (6, 32, 8, 2),
            (3, 64, 8, 2),
            (2, 64, 8, 2),
    ...
    ("sm_90a", 128, 128, "f"): dict(
        k1=(2, 64, 8, 2),
        k3=(2, 64, 8, 1),
        k1_variants=[(2, 64, 8, 2)],
        k3_variants=[(2, 64, 8, 1), (1, 64, 8, 1)],
    ),
    ("sm_90a", 64, 64, "b"): dict(
        k1=(3, 64, 4, 1),
        k3=(2, 64, 4, 1),
        by_n={
            400: dict(k1=(6, 32, 4, 1), k3=(2, 64, 4, 2)),
            1536: dict(k1=(6, 32, 4, 1)),
            4096: dict(),
        },  # measured: 192-token K1 tiles (3 consumer warpgroups): (6,32) N<=1536, (3,64) above (-7..-16 % K1 vs the 128-token tiles); K3 a2 N<=400
        k1_variants=[
            (3, 64, 4, 1),
            (6, 32, 4, 1),
            (1, 192, 4, 1),
        ...
        k3_variants=[(2, 64, 4, 1), (2, 64, 4, 2), (1, 128, 4, 1)],
    ),
    ("sm_90a", 64, 64, "f"): dict(
        k1=(3, 64, 4, 1),
        k3=(2, 64, 4, 1),
        k1_variants=[(3, 64, 4, 1), (2, 64, 4, 1)],
        k3_variants=[(2, 64, 4, 1)],
    ),
    ("sm_90a", 64, 256, "b"): dict(
        k1=(2, 64, 16, 1),
        k3=(2, 64, 4, 1),
        k1_variants=[(2, 64, 16, 1)],
        k3_variants=[(2, 64, 4, 1)],
    ),
    ("sm_90a", 64, 256, "f"): dict(
        k1=(2, 64, 16, 1),
        k3=(2, 64, 4, 1),
        k1_variants=[(2, 64, 16, 1)],
        k3_variants=[(2, 64, 4, 1)],
    ),
    ("sm_90a", 128, 64, "b"): dict(
        k1=(2, 64, 4, 2),
        k3=(2, 64, 4, 1),
        k1_variants=[(2, 64, 4, 2)],
        k3_variants=[(2, 64, 4, 1), (2, 64, 8, 2)],
    ),
    ("sm_90a", 128, 64, "f"): dict(
        k1=(2, 64, 4, 2),
        k3=(2, 64, 8, 1),
        k1_variants=[(2, 64, 4, 2)],
        k3_variants=[(2, 64, 8, 1)],
    ),
    ("sm_90a", 128, 256, "b"): dict(
        k1=(3, 64, 8, 2),
        k3=(
            2,
            64,
            8,
            1,
        ),  # measured (bidirectional shared-LN composition, L384/768, interleaved): 192-token K1 tile -18.7 % K1 vs (2,64,8,2);
        k1_variants=[
            (3, 64, 8, 2),
            (6, 32, 8, 2),
            (2, 64, 8, 2),
            (1, 128, 8, 2),
            (2, 64, 4, 2),
        ],  # K3 8-slot ring (only reachable with the packed ring) -6.2 / -4.0 % K3 vs the 4-slot one, 6 slots less;
        k3_variants=[
            (2, 64, 8, 1),
            (2, 64, 6, 1),
            (2, 64, 4, 1),
    ...
    ),  # (3,64,4,1) fits now but loses by 11 %: 512 threads cap it at 128 registers and its epilogue spills 444/408.
    ...
    ("sm_90a", 128, 256, "f"): dict(
        k1=(2, 64, 8, 2),
        k3=(1, 64, 4, 1),
        k1_variants=[(2, 64, 8, 2)],
        k3_variants=[(1, 64, 4, 1)],
    ),
    ("sm_90a", 256, 64, "b"): dict(
        k1=(2, 64, 8, 2),
        k3=(2, 64, 4, 1),
        k1_variants=[(2, 64, 8, 2)],
        k3_variants=[(2, 64, 4, 1), (2, 64, 6, 1)],
    ),
    ("sm_90a", 256, 64, "f"): dict(
        k1=(2, 64, 4, 2),
        k3=(1, 64, 4, 1),
        k1_variants=[(2, 64, 4, 2)],
        k3_variants=[(1, 64, 4, 1)],
    ),
    ("sm_90a", 256, 128, "b"): dict(
        k1=(2, 64, 8, 2),
        k3=(2, 64, 4, 1),
        k1_variants=[(2, 64, 8, 2)],
        k3_variants=[(2, 64, 4, 1)],
    ),
    ("sm_90a", 256, 128, "f"): dict(
        k1=(2, 64, 4, 2),
        k3=(1, 64, 4, 1),
        k1_variants=[(2, 64, 4, 2)],
        k3_variants=[(1, 64, 4, 1)],
    ),
    ("sm_90a", 64, 128, "b"): dict(
        k1=(3, 64, 8, 1),
        k3=(3, 64, 4, 1),
        by_n={
            800: dict(k1=(6, 32, 8, 1)),
            1536: dict(),
            4096: dict(k3=(2, 64, 4, 2)),
        },  # measured: 192-token K1 tiles (3 consumer warpgroups): (6,32) N<=800, (3,64) above (-9..-16 % K1 vs the 128-token tiles); K3 a2 N>=2048
    ...
    ("sm_90a", 64, 128, "f"): dict(
        k1=(3, 64, 8, 1),
        k3=(2, 64, 4, 1),
        k1_variants=[(3, 64, 8, 1), (2, 64, 8, 1)],
        k3_variants=[(2, 64, 4, 1)],
    ),
    # wide-K pairs (c_z/16 + c_hidden/16 > 24): K3 = the wide member (tmn_k3_wide.cuh), marked by a trailing "w" in the K3 config
    ("sm_90a", 384, 384, "b"): dict(
        k1=(2, 64, 6, 2),
        k3=(1, 64, 4, 1, "w"),
        k1_variants=[(2, 64, 6, 2), (1, 128, 6, 2), (2, 64, 4, 3)],
        k3_variants=[(1, 64, 4, 1, "w")],
    ),
    ("sm_90a", 128, 384, "b"): dict(
        k1=(2, 64, 8, 2),
        k3=(2, 64, 4, 1, "w"),
        k1_variants=[(2, 64, 8, 2)],
        k3_variants=[(2, 64, 4, 1, "w"), (2, 64, 4, 2, "w")],
    ),
    ("sm_90a", 384, 128, "b"): dict(
        k1=(2, 64, 6, 2),
        k3=(2, 64, 4, 1, "w"),
        k1_variants=[(2, 64, 6, 2)],
        k3_variants=[(2, 64, 4, 1, "w")],
    ),
    ("sm_90a", 256, 384, "b"): dict(
        k1=(2, 64, 8, 2),
        k3=(1, 64, 4, 1, "w"),
        k1_variants=[(2, 64, 8, 2)],
        k3_variants=[(1, 64, 4, 1, "w")],
    ),
    ("sm_90a", 384, 256, "b"): dict(
        k1=(2, 64, 6, 2),
        k3=(1, 64, 4, 1, "w"),
        k1_variants=[(2, 64, 6, 2)],
        k3_variants=[(1, 64, 4, 1, "w")],
    ),
    ("sm_90a", 64, 384, "b"): dict(
        k1=(2, 64, 8, 1),
        k3=(2, 64, 4, 2, "w"),
        k1_variants=[(2, 64, 8, 1)],
        k3_variants=[(2, 64, 4, 2, "w"), (2, 64, 4, 1, "w")],
    ),
    ("sm_90a", 384, 64, "b"): dict(
        k1=(2, 64, 6, 2),
        k3=(2, 64, 4, 1, "w"),
        k1_variants=[(2, 64, 6, 2)],
        k3_variants=[(2, 64, 4, 1, "w")],
...
def k3_smem(cz, ch, mode, bi, bj, nslot, nacc=1, packed=False):
    ...
    esz = (
        4 if mode in ("f", "g") else 2
    )  # g: fp32 tile (staging is sized in the tile dtype), bf16 update out
    bmt = bi * bj
    nkcz = cz // (128 // esz)
    ...
    smem_w = (
        (nslot // 2) * (cz + ch) * 64
        if packed and cz != ch and nslot % 2 == 0
        else nslot * max(cz, ch) * 64
    )
    ob = 16 * 64 * esz
...
def k1_name(cz, ch, zf32, bi, bj, nslot, skch, mask, lnm, save, emitx=False):
    return "tmn_k1_z%d_h%d_%s_t%dx%d_s%dk%d_m%d_l%d_v%d%s" % (
        cz,
        ch,
        "f" if zf32 else "b",
        bi,
...
def k3_name(cz, ch, mode, bi, bj, nslot, nacc, *rest):
    ...
    if isinstance(mode, bool):
        mode = "f" if mode else "b"
    assert len(rest) in (1, 2) and (len(rest) == 1 or rest[0] == "w"), (
        "k3_name(cz, ch, mode, BI, BJ, NSLOT, NACC[, 'w'], lnm)"
    )
    return "tmn_k3%s_z%d_h%d_%s_t%dx%d_s%da%d_l%d" % (
        "w" if len(rest) == 2 else "",
        cz,
        ch,
        mode,
        bi,
...
def lookup(arch, cz, ch, form, N=None, direction=None, residual=None, exact=False):
    ...
    key = (
        ...
        ch,
        direction,
        bool(residual) if residual is not None else None,
        None,
    )
    out = dict(
        key=key,
        k1=tuple(ent["k1"]),
        k3=tuple(ent["k3"]),
        k3_mode=ent.get("k3_mode", "f" if form == "f" else "b"),
        covered=True,
        token=None,
    )
    tbl = (ent.get("by_call") or {}).get(
        (direction, bool(residual) if residual is not None else None, cls)
    )  # this call form's own buckets only
    if tbl is None:
    ...
    if tbl and N is not None:
        ...
        out["key"] = key[:-1] + (sel,)
        out["k1"] = tuple(tbl[sel].get("k1", out["k1"]))
        out["k3"] = tuple(tbl[sel].get("k3", out["k3"]))
...
def serve_names(arch, cz, ch, form, exact=False):
    ...
    if form == "b":
        ...
        for k3c in _cfgs(tb, "k3"):
            ...
            if not k3_is_wide(_k3cfg(k3c)):
                names.append(
                    k3_name(cz, ch, "b", *_k3cfg(k3c), 1) + "_u"
                )  # + its update-only specialisation (TMN_K3_SET_BF16 emits both)
    ...
    if mode == "c":
        ...
        for k1c in _cfgs(tf, "k1"):
            names += [
                k1_name(cz, ch, True, *k1c, m, 1, False, True) for m in (False, True)
            ]
        for k3c in _cfgs(tf, "k3"):
        ...
        for k3c in _cfgs(tf, "k3"):
            names.append(
                k3_name(cz, ch, "f", *_k3cfg(k3c), 1)
            )  # residual=True: z + update in fp32
            if not k3_is_wide(k3c):
                names.append(
                    k3_name(cz, ch, "g", *_k3cfg(k3c), 1)
                )  # residual=False: the update in bf16 (the compute dtype)
    return list(dict.fromkeys(names))
...
class Kernels:
    ...
    def __init__(self, device=None, build_dir=None, verify=True):
        self.device_index = (
            ...
            if device is None
            else (device.index if isinstance(device, torch.device) else int(device))
        )
        ...
        self.build_dir = (
            build_dir
            or os.environ.get("TRIMUL_NATIVE_BUILD_DIR")
            or L.DEFAULT_BUILD_DIR
        )
    ...
    def has_unit(self, cz, ch):
        ...
        if r is None:
            r = bool(self.arch) and os.path.isfile(
                os.path.join(
                    self.build_dir, self.arch, unit_name(self.arch, cz, ch) + ".cubin"
                )
            )
            self._has[(cz, ch)] = r
        return r
    ...
    def packed_ring(self, cz, ch):
        ...
        if r is None:
            ...
            r = any(
                f.startswith("-DTMN_K3_PACKED_RING=")
                and f.rsplit("=", 1)[1] not in ("0", "")
                for f in flags
            )
            self._has[key] = r
        return r
    ...
    def kernel_names(self, cz, ch):
        ...
        u = self.unit(cz, ch)
        return set((getattr(u, "entry", None) or {}).get("kernels") or {})
    ...
    def k1(
        ...
        *,
        ...
        if mcode > 1 and k1_name(
            ...
            stats is not None,
            xz is not None,
        ) not in self.kernel_names(cz, ch):
            mask = mask.to(torch.float32)
            mcode = 1  # no native-mask instantiation of this configuration: the fp32 contract (one cast pass)
        name = k1_name(
            cz,
            ch,
            zf32,
        ...
        if ent is None:
            ...
            grid = (
                min(num_tiles, ctas)
                if not grid_limit
                else min(num_tiles, ctas, grid_limit),
                1,
                1,
            )
            wsig = (
        ...
        mp, abp, stp = (
            (mask.data_ptr() if mask is not None else 0),
            ab.data_ptr(),
            (stats.data_ptr() if stats is not None else 0),
        )
        if ent[4] is not tm_z:
            pack.set_tensor_map(0, tm_z, field=0)
            ent[4] = tm_z
        if ent[5] != mp:
            pack.set_ptr(0, mp, field=3)
            ent[5] = mp
        if (
            ent[6] != abp
        ):  # planes pointer + the planes store descriptor (dims [Np (j)][Np (i)][2 ch], box [64][1][32]: one channel row of a 64-token run)
            # BJ == 32 tiles: [32 tok][2 pair rows][32 ch] box (a warpgroup's 64 tokens span two pair rows); other tiles: one 64-token row.
            ab_box = [32, 2, 32] if bj == 32 else [64, 1, 32]
            tm_ab = self.tmap(
                cache,
                ("ab1", Np, ch, bj),
                ab,
                ...
                [Np, Np, 2 * ch],
                [Np * 2, Np * Np * 2],
                "64B" if bj == 32 else "128B",
                "128B",
            )
            pack.set_tensor_map(0, tm_ab, field=2)
            pack.set_ptr(0, abp, field=6)
            ent[6] = abp
        if ent[7] != stp:
            pack.set_ptr(0, stp, field=7)
            ent[7] = stp
        xp = xz.data_ptr() if xz is not None else 0
        if ent[8] != xp:
            pack.set_ptr(0, xp, field=8)
            ent[8] = xp
        k.launch_packed(
            grid,
            block,
            pack,
            smem,
            self.stream_handle() if stream is None else stream,
            pdl=PDL,
        )
        return ab
    ...
    def k3(
        ...
        *,
        ...
        mode = (
            ("g" if out.dtype == torch.bfloat16 else "f")
            if z.dtype == torch.float32
            else ("b" if zres is None else ("p" if prenorm else "c"))
        )
        if mode in ("p", "c"):
            assert zres.dtype == torch.float32 and out.dtype == torch.float32
        if mode == "g":
            assert not residual, (
                "mode g (fp32 z tile, bf16 update) never adds a residual: the fp32 sum is mode f"
            )
        zf32 = mode in ("f", "g")
        esz = 4 if zf32 else 2
        ...
        upd = False
        if (
            not residual and not k3_is_wide(cfg)
        ):  # the update-only specialisation (name + "_u": residual add compiled out) when the unit carries it
            nu = name + "_u"
        ...
        if ent is None:
            smem = (
                k3w_smem(cz, ch, mode, bi, bj, nslot, nacc)
                if k3_is_wide(cfg)
                else k3_smem(
                    cz, ch, mode, bi, bj, nslot, nacc, packed=self.packed_ring(cz, ch)
                )
            )
            k = self.kernel(cz, ch, name, smem)
            ...
            num_tiles = tiles_i * tiles_j
            tm_wg = self.tmap(
                cache, ("wg", cz), w["wg"], [64, 32], [cz, cz], [cz * 2], "128B", "256B"
            )
            tm_wo = self.tmap(
                cache,
                ("wo", cz, ch),
            ...
            )  # field 12: K1-saved LN_in statistics (experiment)
            pack = L.ArgPack([st])
            assert pack.sizes[0] == K3_PARAMS_SIZE, pack.sizes[0]
            grid = (
                min(num_tiles, self.sms)
                if not grid_limit
                else min(num_tiles, self.sms, grid_limit),
                1,
                1,
            )
            wsig = tuple(
                w[n].data_ptr()
                for n in ("wg", "wo", "ln_in_w", "ln_in_b", "ln_out_w", "ln_out_b")
            )
            ent = [
                pack,
                k,
                ...
                wsig,
                None,
            ]  # + last-patched (z map, x map, residual flag, prof ptr, weight signature, out map)
        ...
        if (
            ent[8] != wsig
        ):  # another weight set through the same cache: re-point W_og / W_o streams and the four LN affines
            tm_wg = self.tmap(
                cache, ("wg", cz), w["wg"], [64, 32], [cz, cz], [cz * 2], "128B", "256B"
            )
            tm_wo = self.tmap(
            ...
            pack.set_tensor_map(0, tm_wg, field=2)
            pack.set_tensor_map(0, tm_wo, field=3)
        ...
        if ent[4] is not tm_z:
            pack.set_tensor_map(0, tm_z, field=0)
            pack.set_ptr(0, z if zres is None else zres, field=9)
            ent[4] = tm_z
        elif zres is not None:
            pack.set_ptr(0, zres, field=9)
        if ent[5] is not tm_x:
            pack.set_tensor_map(0, tm_x, field=1)
            ent[5] = tm_x
        pack.set_ptr(0, out, field=10)
        pack.set_ptr(0, (stats.data_ptr() if stats is not None else 0), field=12)
        if (
            mode == "b" and not k3_is_wide(cfg)
        ):  # bf16 output store descriptor: dims [cz (c)][N (j)][N (i)], box [64][16][1] = one warp's 16-token x 64-channel slice
            tm_o = self.tmap(
                cache,
                ("o3", N, cz),
                out,
            ...
            if ent[9] is not tm_o:
                pack.set_tensor_map(0, tm_o, field=4)
                ent[9] = tm_o
        ...
        pp, rf = (
            (prof.data_ptr() if prof is not None else 0),
            (int(ent_tt.get("k3_u_late", 0)) if upd else int(bool(residual))),
        )
        if ent[7] != pp:
            pack.set_ptr(0, pp, field=11)
            ent[7] = pp
        if ent[6] != rf:
            pack.set_i32(0, rf, field=17)
            ent[6] = rf  # residual flag: field 17 after the stats pointer (experiment)
        k3_block = (
            128 * (3 if bi * bj == 64 else bi * bj // 64 + 1),
            1,
            1,
        )  # == K3Cfg::NTHR (experiment: 192-token tiles launch 512 threads)
        k.launch_packed(
            grid,
            k3_block,
            pack,
