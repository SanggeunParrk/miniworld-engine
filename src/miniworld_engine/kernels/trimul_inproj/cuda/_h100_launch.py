...
ROOT = os.path.abspath(
    os.path.join(HERE, "..", "..")
)  # <tree>/python/trimul_native/ -> <tree>/ (source tree and sealed package alike)
DEFAULT_BUILD_DIR = os.environ.get("TRIMUL_NATIVE_BUILD_DIR") or os.path.join(
    ROOT, "build"
)  # env override: load cubins built elsewhere (same manifest layout)
ARCH_OF_CC = {
    (9, 0): "sm_90a",
    (8, 0): "sm_80",
    (8, 6): "sm_80",
    (8, 7): "sm_80",
    (8, 9): "sm_80",
}  # sm_80 SASS is binary-compatible with every 8.x device
...
def f32(v):
    return _Scalar("f", float(v))


def f64(v):
    return _Scalar("d", float(v))
...
class ArgPack(object):
    ...
    def __init__(self, args):
        ...
        for (b, al), a in zip(blobs, args):
            ...
            self.field_offsets.append(
                _struct_field_offsets(a) if isinstance(a, Struct) else None
            )
            total += len(b)
        ...
        self.array = (ctypes.c_void_p * max(len(blobs), 1))(
            *[self.base + off for off in self.offsets]
        )
        self.keep = [a for a in args if isinstance(a, TensorMap)] + [
            f
            for a in args
            if isinstance(a, Struct)
            for f in a.fields
            if isinstance(f, TensorMap)
        ]
    ...
    def set_ptr(self, slot, tensor_or_int, field=None):
        v = (
            tensor_or_int
            if isinstance(tensor_or_int, int)
            else tensor_or_int.data_ptr()
        )
...
def _torch():
    try:
        ...
        raise LoadError(
            "driver_unavailable:torch_not_importable", str(e).split("\n")[0][:120]
        )
...
class Kernel(object):
    ...
    def attrs(self):
        return {
            ...
            "max_dynamic_smem": self.max_dynamic_smem,
        }

    def set_max_dynamic_smem(self, nbytes):
        self.unit.drv.func_set_attribute(
            self.handle, _driver.FUNC_ATTR["MAX_DYNAMIC_SHARED_SIZE_BYTES"], int(nbytes)
        )
    ...
    def launch(self, grid, block, args, smem=0, stream=None):
        ...
        if self.max_dynamic_smem is not None and smem > self.max_dynamic_smem:
            raise ValueError(
                "%s: dynamic smem %d B > the opted-in maximum %d B"
                % (self.name, smem, self.max_dynamic_smem)
            )
        packed = _Packed(args)
        st = (
            current_stream_handle(self.unit.device_index)
            if stream is None
            else int(stream)
        )
    ...
    def launch_packed(self, grid, block, pack, smem=0, stream=None, pdl=False):
        """Launch with a prebuilt ``ArgPack`` (slots patched by the caller beforehand): one driver call, no packing, no allocation.
        ``grid`` / ``block``: 3-tuples of ints (no normalisation here -- this is the per-call fast path).  ``pdl``: programmatic stream
        serialisation attribute (cuLaunchKernelEx) -- only for kernels compiled with griddepcontrol.wait before their dependent accesses."""
        st = current_stream_handle(self.unit.device_index) if stream is None else stream
        if pdl:
            self.unit.drv.launch_kernel_ex(
                self.handle, grid, block, smem, st, pack.array, pdl=True
            )
        else:
...
class Unit(object):
    ...
    def __init__(self, name, arch, device_index, drv, module, entry, cubin_path):
        (
            ...
            self.entry,
            self.cubin_path,
        ) = name, arch, device_index, drv, module, entry, cubin_path
        self._kernels = {}
        ...
        with self._lock:
            ...
            if k is None:
                try:
                    ...
                    raise LoadError(
                        "no_kernel:%s" % fname, "%s in %s" % (e.name, self.cubin_path)
                    )
    ...
    def loadcheck(self):
        ...
        for fname, want in sorted((self.entry.get("kernels") or {}).items()):
            k = self.kernel(fname)
            ok = want.get("regs") in (None, k.regs) and (k.local_bytes == 0) == (
                int(want.get("spill_stores", 0) or 0) == 0
                and int(want.get("stack", 0) or 0) == 0
            )
            out[fname] = {
                "regs": k.regs,
                "local_bytes": k.local_bytes,
                "static_smem": k.static_smem,
                "want_regs": want.get("regs"),
                "want_smem": want.get("smem"),
                "ok": bool(ok),
            }
        return out
...
def load_unit(unit, arch=None, device=None, build_dir=None, verify=True):
    ...
    try:
        entry = (
            _manifest.verify_unit(
                build_dir, unit, arch, check_sources=True, source_root=ROOT
            )
            if verify  # sources resolve against THIS tree's csrc/
    ...
    with _UNITS_LOCK:
        ...
        try:
            ...
            raise LoadError(
                "load:%s" % e.name,
                "%s (%s, driver %d)" % (cubin_path, arch, drv.driver_version()),
            )
...
def tensor_map_cached(cache, tensor, box, **kw):
    ...
    if tm is None:
        ...
        tm = TensorMap(
            tm.raw, keep=None, spec=tm.spec
        )  # the cache must not pin the tensor: the key already names its address
        cache[key] = tm
    return tm
...
class BlockDriver(object):
    ...
    def load_unit(self, name, build_dir=None, verify=True):
        return load_unit(
            name, device=self.device_index, build_dir=build_dir, verify=verify
        ).module
    ...
    def function(self, module, name, smem_bytes=0):
        ...
        if int(smem_bytes) > 48 * 1024:
            self.drv.func_set_attribute(
                fn, _driver.FUNC_ATTR["MAX_DYNAMIC_SHARED_SIZE_BYTES"], int(smem_bytes)
            )
        return fn
    ...
    def attrs(self, fn):
        A = _driver.FUNC_ATTR
        return {
            "regs": self.drv.func_get_attribute(A["NUM_REGS"], fn),
            "local_bytes": self.drv.func_get_attribute(A["LOCAL_SIZE_BYTES"], fn),
    ...
    def launch(self, fn, grid, block, smem_bytes, stream, param_bytes):
        ...
        self.drv.launch_kernel(
            fn,
            (int(grid[0]), int(grid[1]), int(grid[2])),
            (int(block[0]), int(block[1]), int(block[2])),
            int(smem_bytes),
            int(stream) if not isinstance(stream, int) else stream,
            arr,
        )
    ...
    def encode_tiled(
        ...
        rank = len(dims)
        return self.drv.tensor_map_encode_tiled(
            _driver.TMAP_DTYPE[self._DT.get(dtype, dtype)],
            rank,
            int(base_ptr),
            [int(v) for v in dims],
            [int(v) for v in strides_bytes],
            [int(v) for v in box],
            [1] * rank,
            _driver.TMAP_INTERLEAVE["none"],
            _driver.TMAP_SWIZZLE[int(swizzle_bytes)],
            _driver.TMAP_L2[self._L2[int(l2promo_bytes)]],
            _driver.TMAP_OOB["none"],
        )
