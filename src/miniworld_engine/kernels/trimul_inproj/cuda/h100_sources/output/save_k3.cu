...
#if XHAT_FP32 == 1
     int tr=lane/4+8*(j&1),c0=cc%32;
     *reinterpret_cast<float*>(stage+c0*64+((tr*4)^(((c0>>1)&3)*16)))=a;
     *reinterpret_cast<float*>(stage+(c0+1)*64+((tr*4)^((((c0+1)>>1)&3)*16)))=b;
#elif XHAT_FP32 >= 2
...
#ifndef MW_HIDDEN
#define MW_HIDDEN 256
#endif
using Cfg=K3Cfg<128,MW_HIDDEN,0,MW_BI,MW_BJ,MW_SLOT,MW_ACC>;
...
#pragma unroll                                           // dependency); the proxy fence orders these generic-proxy reads before the async-proxy refill
    ...
    LnStats stats_out=ln_save_xhat<KSP,MW_SERIAL>(fx,sGout,sBout,lane,p.eps,tp.lnout,iw*p.N+jw+16*wiw+(lane>>2),p.N*p.N,&tp.tm_lnout,sOut+(4*cw+wiw)*OB);
