...
#endif
constexpr int RING_CHUNK=B7_RING_CHUNK,RING_SLOTS=32768/RING_CHUNK,RING_PHASES=65536/RING_CHUNK,RING_H=RING_CHUNK/8192;
...
TMN_DEVI void pair_glu(float (&a)[32],uint8_t* s,uint8_t* sg,uint32_t ma,uint32_t mb){
 ...
 for(int q=0;q<2;++q){uint32_t dy[4],dg[4],dp[4];ldsm_x4_t(dy,smem_u32(s+16384)+swz128(q*16+lane%8+8*(mat>>1),(w*16+8*(mat&1))*2));
  ...
  for(int j=0;j<4;++j){uint32_t masked,m=(j&1)?mb:ma;asm("mul.rn.bf16x2 %0,%1,%2;":"=r"(masked):"r"(dy[j]),"r"(m));
   ...
   float dpa=bf16lo(masked)*ga,dpb=bf16hi(masked)*gb;dg[j]=pack_bf16(((bf16lo(masked)*pa)*ga)*(1.f-ga),((bf16hi(masked)*pb)*gb)*(1.f-gb));dp[j]=pack_bf16(dpa,dpb);
  }
...
// Single-direction derivative planes: left dg/dp, right dg/dp (16 KiB each).
static_assert(B7_RING_TENSOR == 0, "single-direction ring uses linear bulk stores");
...
#else
 uint8_t* dst=p.ring+(cid*RINGS+rs)*65536+(rank/4)*32768+(rank%4)*4096;
 bulk_store(dst,src,4096);bulk_store(dst+16384,src+4096,4096);
#endif
}
...
#endif
  tma_load_2d(dst+16384,rank<4?&p.dl:&p.dr,bar+slot,tile*64,(rank%4)*32);
 }
}
...
TMN_DEVI void consumer_producer(const Params& p,uint8_t* sm,uint64_t* bar){
 ...
 if(threadIdx.x>=32){int lane=threadIdx.x%32;
  for(int tile=cid+rank*groups;tile<p.tiles;tile+=CONSUMERS*groups,++round){
   ...
   for(int phase=0;phase<RING_PHASES;++phase){int slot=phase%RING_SLOTS,plane=(phase*RING_CHUNK)/16384,chunk_source=((phase*RING_CHUNK)%16384)/4096;
    if(phase>=RING_SLOTS&&lane==0)mbar_wait(bar+32+slot,((phase/RING_SLOTS)-1)&1);
    __syncwarp();
    if((plane&1)==0&&lane<RING_CHUNK/4096)ring_wait(flags-(sequence&1)*(SOURCES+2)+1+(plane/2)*4+chunk_source+lane,(sequence&~1)+1);
    __syncwarp();
 ...
 for(int tile=cid+rank*groups;tile<p.tiles;tile+=CONSUMERS*groups,++round){int row=tile*64;
  ...
  for(int batch=0;batch<4;++batch){int ws=batch%2;
   if(batch>=2)mbar_wait(bar+6+ws,((batch/2)-1)&1);
   mbar_arrive_expect_tx(bar+2+ws,32768);
   for(int sub=0;sub<2;++sub){int stage=batch*2+sub;tma_load_2d(sm+32768+ws*32768+sub*16384,p.wt+stage/2,bar+2+ws,(stage%2)*64,0);}
  }
...
TMN_DEVI void consumer_compute(const Params& p,uint8_t* sm,uint64_t* bar,const float* gamma,const float* beta){
 ...
 for(int tile=cid+rank*clusters;tile<p.tiles;tile+=CONSUMERS*clusters,++round){int row=tile*64;
  ...
  for(int batch=0;batch<4;++batch){int slot=batch%2,ws=batch%2;
...
#endif
#if B7_DEBUG_DXN
  static_for<16>([&](auto qq){constexpr int q=decltype(qq)::value;int c=q*8+2*(lane%4);
   auto out=reinterpret_cast<__nv_bfloat16*>(p.xring);
   stg32(out+size_t(row+ra)*128+c,pack_bf16(acc[q*4],acc[q*4+1]));
   stg32(out+size_t(row+rb)*128+c,pack_bf16(acc[q*4+2],acc[q*4+3]));
  });
#endif
  uint8_t* lnsm=sm+LN;
...
void b7_joint(__grid_constant__ const Params p){
 ...
 if(threadIdx.x==0){for(int i=0;i<40;++i)mbar_init(bar+i,(B7_MULTICAST&&i>=32&&i<36&&rank<SOURCES)?B7_HW_CLUSTER:1);fence_barrier_init();}__syncthreads();
#if B7_MULTICAST
 ...
 for(int i=blockIdx.x*THREADS+threadIdx.x;i<65792;i+=gridDim.x*THREADS){float v=0;
  if(i<65536){
#if B7_REDUCE_NATIVE
...
#endif
   int kind=(rk/4)*2+(half==0),out=kind*16384+c*128+(rk%4)*32+h;
#else
   int kind=i/16384,c=(i/128)%128,h=i%128,rk=(kind/2)*4+h/32,out=i;
#if B7_TRANSPOSE_PART
