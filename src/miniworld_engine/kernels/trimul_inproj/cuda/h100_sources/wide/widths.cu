...
template<bool FWD> TMN_DEVI void out_gp(const Params& p,uint8_t* sm,uint64_t* bar,int& phase){
 for(int tile=blockIdx.x;tile<(p.M/64)*((D+NT-1)/NT);tile+=gridDim.x){
  int row=(tile/((D+NT-1)/NT))*64,col=(tile%((D+NT-1)/NT))*NT+(threadIdx.x/128)*NN;float proj[ACC],gate[ACC];
  gemm<0,0>(proj,&p.map[3],&p.map[1],row,col,0,H,sm,bar,phase);
  gemm<0,0>(gate,&p.map[0],&p.map[2],row,col,0,D,sm,bar,phase);
  for(int j=0;j<ACC;++j){int r=row+rr(j),c=col+cc(j);if(c>=D)continue;size_t ix=size_t(r)*D+c;float g=math::sigmoid(rn(gate[j])),v=rn(proj[j]),ds=rd(p.t[3],(r%p.L)*D+c);
...
extern "C" __global__ __launch_bounds__(THREADS,B7_MINB) void width_b7(__grid_constant__ const Params p
#if FUSED_GP
