...
TMN_DEVI void front_gp(const Params& p,uint8_t* sm,uint64_t* bar,int& phase){
 for(int t=blockIdx.x;t<(p.M/64)*(H/NT)*2;t+=gridDim.x){int side=t/((p.M/64)*(H/NT)),u=t%((p.M/64)*(H/NT)),row=(u/(H/NT))*64,col=(u%(H/NT))*NT+(threadIdx.x/128)*64;float proj[32],gate[32];
  ...
  for(int j=0;j<32;++j){int r=row+rr(j),c=col+cc(j);float da=rn(rd(reinterpret_cast<bf*>(sm+(threadIdx.x/128)*8192),swz128(cc(j),rr(j)*2)/2)*p.f[4][r]),g=math::sigmoid(rn(gate[j])),v=rn(proj[j]);p.t[13+side*2][size_t(r)*H+c]=cv(da*g);p.t[14+side*2][size_t(r)*H+c]=cv(((da*v)*g)*(1-g));}__syncthreads();
 }
}
TMN_DEVI void front_dx(const Params& p,uint8_t* sm,uint64_t* bar,int& phase){
 for(int t=blockIdx.x;t<(p.M/64)*(D/NT);t+=gridDim.x){int row=(t/(D/NT))*64,col=(t%(D/NT))*NT+(threadIdx.x/128)*64;float v[32];
...
extern "C" __global__ __launch_bounds__(THREADS,2) void width_forward(__grid_constant__ const Params p){
...
extern "C" __global__ __launch_bounds__(THREADS,2) void width_b1(__grid_constant__ const Params p){
 ...
 for(int t=blockIdx.x;t<(p.M/64)*(H/NT);t+=gridDim.x){int row=(t/(H/NT))*64,col=(t%(H/NT))*NT+(threadIdx.x/128)*64;float v[32];gemm<0,1>(v,&p.map[4],&p.map[1],row,col,0,D,sm,bar,phase);store(p.t[9],v,row,col,H);}
 stamp(p,3);weight(p,4,3,D,H,0,sm,bar,phase);stamp(p,4);weight(p,5,0,D,D,H*D,sm,bar,phase);stamp(p,5);__threadfence();asm volatile("fence.proxy.async.global;":::"memory");grid.sync();output_ln_bwd(p,sm);stamp(p,6);reduce_w(p,0,D*H,p.t[21]);reduce_w(p,H*D,D*D,p.t[22]);stamp(p,7);
}
extern "C" __global__ __launch_bounds__(THREADS,2) void width_b7(__grid_constant__ const Params p){
 extern __shared__ __align__(128) uint8_t sm[];auto bar=reinterpret_cast<uint64_t*>(sm+2*STAGE);if(threadIdx.x==0){mbar_init(bar,1);mbar_init(bar+1,1);fence_barrier_init();}__syncthreads();int phase=0;auto grid=cooperative_groups::this_grid();
 for(int c=blockIdx.x*THREADS+threadIdx.x;c<D;c+=gridDim.x*THREADS){p.f[8][c]=0;p.f[9][c]=0;}stamp(p,8);front_gp(p,sm,bar,phase);stamp(p,9);__threadfence();asm volatile("fence.proxy.async.global;":::"memory");grid.sync();front_dx(p,sm,bar,phase);stamp(p,10);
 for(int i=0;i<4;++i)weight(p,6+i,0,H,D,(H+D+i*H)*D,sm,bar,phase);stamp(p,11);__threadfence();asm volatile("fence.proxy.async.global;":::"memory");grid.sync();ln_bwd<true>(p);stamp(p,12);for(int i=0;i<4;++i)reduce_w(p,(H+D+i*H)*D,H*D,p.t[17+i]);stamp(p,13);
}
...
extern "C" __global__ __launch_bounds__(THREADS,2) void width_probe(__grid_constant__ const Params p){
 extern __shared__ __align__(128) uint8_t sm[];auto bar=reinterpret_cast<uint64_t*>(sm+2*STAGE);if(threadIdx.x==0){mbar_init(bar,1);mbar_init(bar+1,1);fence_barrier_init();}__syncthreads();int phase=0;float v[32];int ta=p.L/2,tb=p.L%2;
