...
def front_config_rejection(config, *, m=None, k=None, h2=None):
    ...
    one four-warp WGMMA consumer and one four-warp producer group (8 total).
    ...
    if config["num_warps"] != 8:
        return "This warp-specialized mainloop has one producer and one consumer group:8 physical CTA warps"
...
def _compile_front(dtype, a_major, b_major, save_preact, has_mask, bm, bh, bk, warps, stages, device):
    ...
    epi = ParityFrontSm90.EpilogueArguments(out, gate_fn_map["glu"], mColVecBroadcast=mask)
...
def launch_front(a, w, packed, preact, pair_mask, config):
    ...
    sched = make_scheduler_args(get_max_active_clusters(1, device_capacity=device), 1, None)
