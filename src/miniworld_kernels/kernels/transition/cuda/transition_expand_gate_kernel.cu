...
struct ExpandGateSmem {
    using WeightPipeline = cutlass::PipelineTmaAsync<STAGES>;
    static constexpr int kXnElems = CTA_M * K;
    static constexpr int kExpandWeightElemsPerStage = K == KT ? (K * BN + K * BN) : (KT * BN + KT * BN);
...
template <
    ...
    class TmaWa,
    class TmaWb>
__global__ __launch_bounds__(256, 1) void transition_expand_gate_rs_wgmma_kernel(
    ...
    using Smem = ExpandGateSmem<CTA_M, WG_M, K, BN, STAGES, KT>;
    ...
    static_assert(BN % 8 == 0, "BN must be divisible by 8 for vectorized h stores");
    ...
    auto store_h_tile = [&](auto& h_acc, int nd_base) {
        BF* pHShuffle = pWa + wg_id * (WG_M * BN);
        static_assert(kWarpgroups * WG_M * BN <= STAGES * cosize_v<decltype(lW)>,
                      "h shuffle tiles must fit in the Wa smem staging area");
        union OutPair {
            uint32_t u32;
            BF bf[2];
        };
        ...
        for (int i = 0; i < size(h_acc); i += 2) {
            ...
            if (m_next == m && n_next == n + 1 && (n % 2) == 0) {
                OutPair pair;
                pair.bf[0] = static_cast<BF>(h_acc(i));
                pair.bf[1] = static_cast<BF>(h_acc(i + 1));
                smem_store_u32(pHShuffle + output_shuffle_idx<BN>(m, n), pair.u32);
            } else {
                pHShuffle[output_shuffle_idx<BN>(m, n)] = static_cast<BF>(h_acc(i));
                pHShuffle[output_shuffle_idx<BN>(m_next, n_next)] = static_cast<BF>(h_acc(i + 1));
            }
        }
        cutlass::arch::NamedBarrier::arrive_and_wait(kWarpgroupThreads, wg_barrier_id);
        ...
        for (int vec = wg_tid; vec < WG_M * kOutVecsPerRow; vec += kWarpgroupThreads) {
            ...
            const int64_t row = static_cast<int64_t>(wg_row0 + m);
            const bool valid = row < M;
            uint4 out_vec = smem_load_u128(pHShuffle + output_shuffle_idx<BN>(m, n));
            __nv_bfloat16* dst = valid ? h_raw + row * ND + nd_base + n : h_raw;
...
void check_transition_expand_gate_inputs(
    ...
    TORCH_CHECK(K == 128 || K == 256 || K == 512,
                "transition_expand_gate CUDA path supports K=128, K=256, or K=512");
    ...
    TORCH_CHECK(ND == 4 * K, "transition_expand_gate CUDA path requires n=4, i.e. ND == 4*K");
}

template <int K, int ND, int CTA_M, int WG_M, int BN, int STAGES, int KT>
void launch_transition_expand_gate_kernel(
    const torch::Tensor& x,
    const torch::Tensor& rstd,
    const torch::Tensor& c1,
    const torch::Tensor& g,
    ...
    auto tma_wa = make_tma_copy(SM90_TMA_LOAD{}, mWaTma, lW,
                                make_shape(Int<BN>{}, Int<KT>{}), Int<1>{});
    auto tma_wb = make_tma_copy(SM90_TMA_LOAD{}, mWbTma, lW,
                                make_shape(Int<BN>{}, Int<KT>{}), Int<1>{});

    auto* kernel = transition_expand_gate_rs_wgmma_kernel<
    ...
    check_cuda(
        ...
        "transition_expand_gate_rs_wgmma_kernel dynamic shared-memory attribute failed"
    );
    check_cuda(
        cudaFuncSetAttribute(kernel, cudaFuncAttributePreferredSharedMemoryCarveout, 100),
        "transition_expand_gate_rs_wgmma_kernel shared-memory carveout attribute failed"
    );
    ...
    check_cuda(cudaGetLastError(), "transition_expand_gate_rs_wgmma_kernel launch failed");
}

}  // namespace expand_gate_detail

using namespace expand_gate_detail;

torch::Tensor transition_expand_gate_fwd(
    const torch::Tensor& x,
    const torch::Tensor& rstd,
    const torch::Tensor& c1,
    const torch::Tensor& g,
    ...
    check_transition_expand_gate_inputs(x, rstd, c1, g, beta, wa, wb);

    c10::cuda::CUDAGuard device_guard(x.device());
    const int64_t K = x.size(1);
    const int64_t ND = wa.size(0);
    auto h = torch::empty({x.size(0), ND}, x.options());
    const int64_t M = x.size(0);
    if (M == 0) {
        return h;
    }
    TORCH_CHECK(M % kBlockM == 0,
                "transition_expand_gate CUDA path requires M to be divisible by 128");

    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    if (K == 128 && ND == 512) {
        launch_transition_expand_gate_kernel<128, 512, kBlockM, kWarpgroupM, 128, 3, 128>(
            x, rstd, c1, g, beta, wa, wb, h, M, stream
        );
    } else if (K == 256 && ND == 1024) {
        launch_transition_expand_gate_kernel<256, 1024, kBlockM, kWarpgroupM, 128, 2, 128>(
            x, rstd, c1, g, beta, wa, wb, h, M, stream
        );
    } else if (K == 512 && ND == 2048) {
        launch_transition_expand_gate_kernel<512, 2048, kBlockM, kWarpgroupM, 64, 2, 64>(
            x, rstd, c1, g, beta, wa, wb, h, M, stream
        );
    } else {
        TORCH_CHECK(false, "unsupported transition_expand_gate CUDA shape");
    }
    return h;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("transition_expand_gate_fwd", &transition_expand_gate_fwd,
          "Transition LN + expand SwiGLU h forward (CUDA)");
}
