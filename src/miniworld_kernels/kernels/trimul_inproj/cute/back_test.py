"""Fused-back (gate computed IN the back) — verify + bench vs v2 / v3p.

v4 = quack front (left+right only) -> bmm -> Triton fused back
     (LN_out(tri)@Wp + sigmoid(x_n@Wg) + mul, ONE kernel, gate not materialized).
v2 = ... -> layer_norm_transpose + tm2          (gate in tm2/back, cuequiv)
v3p= ... -> layernorm_linear + torch gate mul   (gate in front, materialized)
...
def main():
    ...
    print(f"fused-back (gate in back) test (K={K}) on {torch.cuda.get_device_name(0)}", flush=True)
    _bdll_patch.apply()
    ...
    Wp_t, Wg_t = mod.to_out.weight.T, mod.to_gate.weight.T  # (D,D) for x@W form
    b_lr = prepack_lr_operand(WL, WLg, WR, WRg)

    # --- correctness: triton fused back vs fp32 reference ---
    L0, M0 = 256, 256 * 256
    tri = torch.randn(1, D, L0, L0, device="cuda", dtype=dtype)
    xn = torch.randn(1, L0, L0, D, device="cuda", dtype=dtype)
    with torch.no_grad():
        y = trimul_back_triton(tri, xn, Wp_t, Wg_t, gln_w, gln_b, eps)
        tri_md = tri.reshape(D, M0).t().float()       # (M,D) channel vec per row
        proj = F.linear(F.layer_norm(tri_md, (D,), gln_w.float(), gln_b.float(), eps),
                        mod.to_out.weight.float())
        gate = torch.sigmoid(F.linear(xn.reshape(M0, D).float(), mod.to_gate.weight.float()))
        yref = (gate * proj).view(1, L0, L0, D)
    ...
    def v3p_one(pair):
        ...
        proj = layernorm_linear_cute_fused(view, gln_w, gln_b, mod.to_out.weight, None, eps=eps)
        return (proj * gate.reshape(b * l1 * l2, d)).view(b, l1, l2, d)

    def v4_one(pair):  # gate computed IN the fused triton back
        b, l1, l2, d = pair.shape
        xn = _ln_in(pair)
        left, right, _ = front_lr(xn)
        tri = torch.einsum("bdik,bdjk->bdij", left, right)
    ...
    print(f"\n{'L':>5} | {'v2':>7} | {'v3p':>7} | {'v4(fused-back)':>14} | "
          f"{'v4/v2':>6} | {'v4/v3p':>6}  (ms/layer)")
    print("-" * 70)
    for L in (512, 1024):
        torch.manual_seed(L)
        pair = torch.randn(1, L, L, D, device="cuda", dtype=dtype)
        t = {}
        for name, one in (("v2", v2_one), ("v3p", v3p_one), ("v4", v4_one)):
            with torch.no_grad():
                torch._dynamo.reset()
        ...
        print(f"{L:>5} | {t['v2']:>7.3f} | {t['v3p']:>7.3f} | {t['v4']:>14.3f} | "
              f"{t['v4']/t['v2']:>5.2f}x | {t['v4']/t['v3p']:>5.2f}x", flush=True)
