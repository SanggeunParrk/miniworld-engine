...
def test_sm80_primitives_never_import_or_calibrate_quack(monkeypatch, op, enabled):
    ...
    monkeypatch.setattr(settings, "_ACTIVE", settings.Settings(trimul_cute_dispatch=enabled))
...
def test_warm_cute_winner_is_filtered_before_cache_lookup(monkeypatch):
    ...
    monkeypatch.setattr(settings, "_ACTIVE", settings.Settings(trimul_cute_dispatch=True))
    calls = []
