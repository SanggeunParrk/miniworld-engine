...
# NOT `"triton" not in sys.modules`: importing any modules/<op> package pulls triton in on
# this tree -- modules.transition does it too -- so that tested the module layer's import graph
# rather than this family's laziness. What the family owes is narrower and still true.
assert "miniworld_engine.kernels.mpnn_edge_dropout.triton.main" not in sys.modules
"""
...
def test_mpnn_edge_dropout_rejects_double_backward() -> None:
    ...
    with pytest.raises(RuntimeError, match=r"differentiate twice|once_differentiable"):
