...
def legacy_window(q, k, v, cu_seqlens, seqused, max_seqlen, valid, n, s, scale, half_window):
    ...
    from flash_attn.bert_padding import (  # ty: ignore[unresolved-import]  # optional FlashAttention backend
        index_first_axis,
        pad_input,
        unpad_input,
    )

    # Loaded only in GPU tests; the CPU CI environment omits this optional backend.
    flash_attn_varlen_func = import_module("flash_attn.flash_attn_interface").flash_attn_varlen_func
