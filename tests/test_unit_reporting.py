...
`build_all` decomposes work into OpUnits, so `--op` is the path a real sweep runs -- and it was
the path missing `probe_log_summary`. 219,680 probed configs were timed and written to disk while
the per-axis breakdown that explains them was printed for exactly zero units. The `--case` path
had the mirror-image hole: no `record_errors`, so a capture failing silently stayed silent.
"""
...
def test_the_reporter_prints_every_section(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(capture, "probe_log_summary", lambda top=0: "  [probe-log] y")
    monkeypatch.setattr(capture, "summary", lambda: "  [bench-budget] z")
    ...
    assert n == 7
    for want in ("[precompile]", "[probe-log]", "[bench-budget]"):
...
def test_capture_failures_are_never_swallowed(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(capture, "probe_log_summary", lambda top=0: "")
...
def test_an_empty_probe_log_prints_nothing(monkeypatch, capsys, tmp_path):
    """A single-config build has no probes; it must not print an empty header."""
    monkeypatch.setattr(capture, "precompile_summary", lambda: "  [precompile] x")
    monkeypatch.setattr(capture, "probe_log_summary", lambda top=0: "")
    monkeypatch.setattr(capture, "summary", lambda: "  [bench-budget] z")
    monkeypatch.setattr(capture, "dump_shard", lambda p: 1)
    monkeypatch.setattr(capture, "record_errors", lambda: "")
    builder._report_unit(str(tmp_path / "s.json"))
    assert "[probe-log]" not in capsys.readouterr().out
