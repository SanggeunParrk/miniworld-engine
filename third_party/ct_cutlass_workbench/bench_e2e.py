"""Decisive e2e: CUTLASS fused fwd+bwd vs cond_transition_train (current champion) vs
torch.compile(reduce-overhead), CUDA graph, atom + token. Mirrors _ct_tmp/bench_train_vc.py.

CUTLASS path forward: fused_h (SwiGLU-fused expand EVT) + plain GEMMs + sigmoid-gate elem.
CUTLASS path backward: input-fused fused_da/fused_db (dh & dab never materialize) + plain GEMMs.
"""
import sys
...
from ct_autograd import cond_transition_train_cutlass
...
def main():
    ...
    # correctness: CUTLASS path 7 grads + cos_y vs fp64-ish reference (throwaway tensors)
    for s, d, M in shapes:
        tc = make(M, d)
        yr = ref(*tc); gr = torch.autograd.grad(yr, tc, torch.ones_like(yr))
        yc = cond_transition_train_cutlass(*tc)
    ...
    cutlass = {}; ours = {}
    for s, d, M in shapes:
        cutlass[(s, d, M)] = gbench(lambda: make(M, d), cond_transition_train_cutlass)
    ...
    print("\n=== TRAINING fwd+bwd: CUTLASS vs cond_transition_train vs compile (CUDA graph) ===")
    hdr = (f"{'stream':>6} {'M':>6} {'d':>4} | {'cos_min':>8} | {'cutlass':>8} {'ours':>8} "
