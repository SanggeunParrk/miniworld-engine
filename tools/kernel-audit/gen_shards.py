...
from gen_grid import grid_for, op_axes                    # noqa: E402
    ...
    for i, sh in enumerate(shards):
        ...
        for op, cfgs in grid.items():
            if op in sh:
                ...
                # FILLER: one config, taken from this op's own grid so the union over all shards
                # stays exactly equal to the full grid (see module docstring).
                rows = cfgs[:1]
            _write_csv(d / f"{op}.csv", axes[op], rows)
