...
def outer_product_mean(module, msa: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    return OpmTrainFn.apply(msa.contiguous(), mask, module.ln_msa.weight, module.ln_msa.bias, module.to_left.weight, module.to_right.weight,
...
def _forward_op(args:list[torch.Tensor],eps:float,save_o:bool)->list[torch.Tensor]:
    ctx=_SavedContext();out=_OpmMath.forward(ctx,*args,eps,save_o);sv=ctx.saved_tensors
...
def _backward_op(args:list[torch.Tensor],kept:list[torch.Tensor],dz:torch.Tensor,eps:float,save_o:bool)->list[torch.Tensor]:
    ...
    ctx.saved_tensors=(args[0][0],*kept[:5],*args[2:],*((kept[5],) if save_o else ()))
...
class OpmTrainFn(torch.autograd.Function):
    ...
    def forward(ctx,*args):
        tensors=list(args[:8]);ctx.eps,ctx.save_o=args[8:]
    ...
    def backward(ctx,dz):
        ...
        grads=_backward_op(list(vals[:8]),list(vals[8:]),dz,ctx.eps,ctx.save_o)
        return (grads[0],None,*grads[1:],None,None)
