...
for length in (384,768):
 for outgoing in (True,False):
    ...
    legacy=TriangleMultiplication(128,outgoing=outgoing,implementation='cute',p_drop=.25).cuda().bfloat16()
    legacy.load_state_dict(m.state_dict());legacy._make_drop_row_scale=lambda pair,p:ds
    xl=x.detach().clone().requires_grad_()
    ...
    with torch.cuda.stream(stream):
     graphs={}; keep=[]
     for label,model,inp in [('cuda',m,x),('triton',ref,xr),('legacy_cuda',legacy,xl)]:
    ...
    Path('verdicts/trimul-single-20260923/legacy-bench.json').write_text(json.dumps(records,indent=2))
